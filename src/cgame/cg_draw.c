...
==============
*/
static void CG_DrawFieldPadded(int x, int y, int width, int cw, int ch,
			       int value)
{
	char num[16], *ptr;
	int l, orgL;
	...
	while (*ptr && l) {
		if (width > orgL) {
			CG_DrawPic(x, y, charWidth, charHeight,
			       ...
			       float progress)
{
	...
	if (align == ITEM_ALIGN_RIGHT) {
		...
		CG_DrawPic(rect->x + rimWidth, rect->y, leftWidth, rimWidth,
			   cgs.media.whiteShader);
		...
		CG_DrawPic(rect->x, rect->y, rimWidth + doneWidth, rect->h,
			   cgs.media.whiteShader);
		CG_DrawPic(rimWidth + rect->x + doneWidth, rect->y, leftWidth,
			   rimWidth, cgs.media.whiteShader);
		CG_DrawPic(rimWidth + rect->x + doneWidth,
			   rect->y + rect->h - rimWidth, leftWidth, rimWidth,
			   cgs.media.whiteShader);
		CG_DrawPic(rect->x + rect->w - rimWidth, rect->y, rimWidth,
	...
	//draw text
	if (scale > 0.0) {
		Com_sprintf(textBuffer, sizeof(textBuffer), "%d%%",
			    (int)(progress * 100));
		...
		CG_Text_Paint(tx, ty, scale, color, textBuffer, 0, 0,
			      textStyle);
	}
}
				      ...
				      qboolean padding)
{
	...
	if (value > -1) {
		if (cg.predictedPlayerState.stats[STAT_PTEAM] == PTE_ALIENS) {
			if (!CG_AtHighestClass()
			    && cg.time - cg.lastEvolveAttempt <=
			    NO_CREDITS_TIME) {
				if (((cg.time -
				      cg.lastEvolveAttempt) / 300) % 2)
					color[3] = 0.0f;
			}
		}
		    ...
		    && cg.predictedPlayerState.stats[STAT_PTEAM] == PTE_ALIENS)
		{
			...
			if (cg.predictedPlayerState.stats[STAT_PTEAM] ==
			    PTE_ALIENS)
				value /= EVO_TO_CREDS_RATE;

			if (padding)
				CG_DrawFieldPadded(rect->x, rect->y, 4,
						   rect->w / 4, rect->h, value);
			else
				CG_DrawField(rect->x, rect->y, 1, rect->w,
					     rect->h, value);
		}
				   ...
				   qboolean padding)
{
	...
	if (value > -1) {
		...
		if (padding)
			CG_DrawFieldPadded(rect->x, rect->y, 4, rect->w / 4,
					   rect->h, value);
		else
...
==============
*/
static void CG_DrawPlayerStamina1(rectDef_t * rect, vec4_t color,
				  qhandle_t shader)
{
...
==============
*/
static void CG_DrawPlayerStamina2(rectDef_t * rect, vec4_t color,
				  qhandle_t shader)
{
	playerState_t *ps = &cg.snap->ps;
	float stamina = ps->stats[STAT_STAMINA];
...
==============
*/
static void CG_DrawPlayerStamina3(rectDef_t * rect, vec4_t color,
				  qhandle_t shader)
{
	playerState_t *ps = &cg.snap->ps;
	float stamina = ps->stats[STAT_STAMINA];
...
==============
*/
static void CG_DrawPlayerStamina4(rectDef_t * rect, vec4_t color,
				  qhandle_t shader)
{
	playerState_t *ps = &cg.snap->ps;
	float stamina = ps->stats[STAT_STAMINA];
...
==============
*/
static void CG_DrawPlayerStaminaBolt(rectDef_t * rect, vec4_t color,
				     qhandle_t shader)
{
	playerState_t *ps = &cg.snap->ps;
	float stamina = ps->stats[STAT_STAMINA];
...
==============
*/
static void CG_DrawPlayerClipsRing(rectDef_t * rect, vec4_t color,
				   qhandle_t shader)
{
	playerState_t *ps = &cg.snap->ps;
	centity_t *cent;
	...
	case WP_HBUILD2:
		maxDelay =
		    (float)BG_FindBuildDelayForWeapon(cent->currentState.
						      weapon);
	...
	default:
		if (ps->weaponstate == WEAPON_RELOADING) {
			maxDelay =
			    (float)BG_FindReloadTimeForWeapon(cent->
							      currentState.
							      weapon);
			progress =
			    (maxDelay - (float)ps->weaponTime) / maxDelay;
...
==============
*/
static void CG_DrawPlayerBuildTimerRing(rectDef_t * rect, vec4_t color,
					qhandle_t shader)
{
	playerState_t *ps = &cg.snap->ps;
	centity_t *cent;
...
==============
*/
static void CG_DrawPlayerBoosted(rectDef_t * rect, vec4_t color,
				 qhandle_t shader)
{
	playerState_t *ps = &cg.snap->ps;
...
==============
*/
static void CG_DrawPlayerBoosterBolt(rectDef_t * rect, vec4_t color,
				     qhandle_t shader)
{
	playerState_t *ps = &cg.snap->ps;
	qboolean boosted = ps->stats[STAT_STATE] & SS_BOOSTED;
...
==============
*/
static void CG_DrawPlayerPoisonBarbs(rectDef_t * rect, vec4_t color,
				     qhandle_t shader)
{
	playerState_t *ps = &cg.snap->ps;
	int x = rect->x;
...
==============
*/
static void CG_DrawPlayerWallclimbing(rectDef_t * rect, vec4_t color,
				      qhandle_t shader)
{
	playerState_t *ps = &cg.snap->ps;
				 ...
				 int align, int textStyle, int special)
{
	...
	float progress =
	    ((float)stamina + (float)MAX_STAMINA) / ((float)MAX_STAMINA * 2.0f);

	CG_DrawProgressBar(rect, color, scale, align, textStyle, special,
			   progress);
}
...
==============
*/
static void CG_DrawUsableBuildable(rectDef_t * rect, qhandle_t shader,
				   vec4_t color)
{
	vec3_t view, point;
	trace_t trace;
	...
	if (es->eType == ET_BUILDABLE
	    && BG_FindUsableForBuildable(es->modelindex)
	    && cg.predictedPlayerState.stats[STAT_PTEAM] ==
...
static void CG_DrawPlayerBuildTimer(rectDef_t * rect, vec4_t color)
{
	...
	if (cent->currentState.weapon) {
		...
		case WP_ABUILD:
			progress =
			    (float)ps->stats[STAT_MISC] /
			    (float)ABUILDER_BASE_DELAY;
			break;

		case WP_ABUILD2:
			progress =
			    (float)ps->stats[STAT_MISC] /
			    (float)ABUILDER_ADV_DELAY;
			break;

		case WP_HBUILD:
			progress =
			    (float)ps->stats[STAT_MISC] / (float)HBUILD_DELAY;
			break;

		case WP_HBUILD2:
			progress =
			    (float)ps->stats[STAT_MISC] / (float)HBUILD2_DELAY;
			break;
...
static void CG_DrawPlayerClipsValue(rectDef_t * rect, vec4_t color)
{
	...
	if (cent->currentState.weapon) {
		...
		default:
			...
			if (value > -1) {
				trap_R_SetColor(color);
				CG_DrawField(rect->x, rect->y, 4, rect->w / 4,
					     rect->h, value);
				trap_R_SetColor(NULL);
			}
			break;
				   ...
				   int align, int textStyle, int special)
{
	...
	total =
	    ((float)ps->stats[STAT_HEALTH] / (float)ps->stats[STAT_MAX_HEALTH]);
	CG_DrawProgressBar(rect, color, scale, align, textStyle, special,
			   total);
}
...
==============
*/
static void CG_DrawPlayerHealthCross(rectDef_t * rect, vec4_t color,
				     qhandle_t shader)
{
	playerState_t *ps = &cg.snap->ps;
...
static void CG_DrawProgressLabel(rectDef_t * rect, float text_x, float text_y,
				 ...
				 const char *s, float fraction)
{
	...
	if (fraction < 1.0f)
		CG_Text_Paint(rect->x + text_x + tx, rect->y + text_y, scale,
			      white, s, 0, 0, ITEM_TEXTSTYLE_NORMAL);
	else
		CG_Text_Paint(rect->x + text_x + tx, rect->y + text_y, scale,
			      color, s, 0, 0, ITEM_TEXTSTYLE_NEON);
}
...
static void CG_DrawMediaProgressLabel(rectDef_t * rect, float text_x,
				      float text_y, vec4_t color, float scale,
				      int align)
{
	CG_DrawProgressLabel(rect, text_x, text_y, color, scale, align,
...
static void CG_DrawBuildablesProgressLabel(rectDef_t * rect, float text_x,
					   float text_y, vec4_t color,
...
static void CG_DrawCharModelProgressLabel(rectDef_t * rect, float text_x,
					  float text_y, vec4_t color,
					  float scale, int align)
{
	CG_DrawProgressLabel(rect, text_x, text_y, color, scale, align,
				   ...
				   int align, int textStyle, int special)
{
	...
	total =
	    (cg.charModelFraction + cg.buildablesFraction +
	     cg.mediaFraction) / 3.0f;
	CG_DrawProgressBar(rect, color, scale, align, textStyle, special,
			   total);
}
...
static void CG_DrawLoadingString(rectDef_t * rect, float text_x, float text_y,
				 vec4_t color, float scale, int align,
				 int textStyle, const char *s)
{
	float tw, th, tx;
	int pos, i;
	char buffer[1024];
	...
	while (pos < strlen(s)) {
		...
		CG_Text_Paint(tx + text_x, rect->y + text_y + i * (th + 3),
			      scale, color, buffer, 0, 0, textStyle);
			     ...
			     int textStyle)
{
	...
	CG_DrawLoadingString(rect, text_x, text_y, color, scale, align,
			     textStyle, s);
}
...
==============
*/
static void CG_DrawDemoPlayback(rectDef_t * rect, vec4_t color,
				qhandle_t shader)
{
...
==============
*/
static void CG_DrawDemoRecording(rectDef_t * rect, vec4_t color,
				 qhandle_t shader)
{
	if (!cg_drawDemoState.integer)
		return;
			  ...
			  qhandle_t shader, int textStyle)
{
	...
	if (cg.killerName[0]) {
		...
		CG_Text_Paint(x -
			      CG_Text_Width(CG_GetKillerText(), scale, 0) / 2,
			      rect->y + rect->h, scale, color,
...
static void CG_DrawTeamSpectators(rectDef_t * rect, float scale, vec4_t color,
				  qhandle_t shader)
{
	if (cg.spectatorLen) {
		float maxX;
		...
		if (cg.time > cg.spectatorTime) {
			...
			if (cg.spectatorPaintX <= rect->x + 2) {
				if (cg.spectatorOffset < cg.spectatorLen) {
					...
					if (Q_IsColorString
					    (&cg.
					     spectatorList[cg.spectatorOffset]))
		...
		CG_Text_Paint_Limit(&maxX, cg.spectatorPaintX,
				    rect->y + rect->h - 3, scale, color,
				    &cg.spectatorList[cg.spectatorOffset], 0,
				    0);

		if (cg.spectatorPaintX2 >= 0) {
			float maxX2 = rect->x + rect->w - 2;
			CG_Text_Paint_Limit(&maxX2, cg.spectatorPaintX2,
					    rect->y + rect->h - 3, scale, color,
					    cg.spectatorList, 0,
					    cg.spectatorOffset);
		}
			       ...
			       int textStyle)
{
	...
	if (cg.intermissionStarted) {
		...
		if (cgs.alienNextStageThreshold < 0)
			Com_sprintf(s, MAX_TOKEN_CHARS, "Stage %d",
				    cgs.alienStage + 1);
		else if (kills == 1)
			Com_sprintf(s, MAX_TOKEN_CHARS,
				    "Stage %d, %d kill for next stage",
				    cgs.alienStage + 1, kills);
		else
			Com_sprintf(s, MAX_TOKEN_CHARS,
				    "Stage %d, %d kills for next stage",
				    cgs.alienStage + 1, kills);
	...
	CG_Text_Paint(text_x + tx, rect->y + text_y, scale, color, s, 0, 0,
		      textStyle);
}
		       ...
		       qboolean scalableText)
{
	...
	if (index > FPS_FRAMES) {
		...
		totalWidth =
		    CG_Text_Width(FPS_STRING, scale, 0) + w * strLength;
		...
		if (scalableText) {
			for (i = 0; i < strLength; i++) {
				...
				CG_Text_Paint(text_x + tx + i * w,
					      rect->y + text_y, scale, color, c,
					      0, 0, textStyle);
			}
		} else {
			trap_R_SetColor(color);
			CG_DrawField(rect->x, rect->y, 3, rect->w / 3, rect->h,
				     fps);
			trap_R_SetColor(NULL);
		}

		if (scalableText)
			CG_Text_Paint(text_x + tx + i * w, rect->y + text_y,
				      scale, color, FPS_STRING, 0, 0,
				      textStyle);
	}
}
			 ...
			 float scale, vec4_t color, int align, int textStyle)
{
	...
	for (i = 0; i < strLength; i++) {
		...
		CG_Text_Paint(text_x + tx + i * w, rect->y + text_y, scale,
			      color, c, 0, 0, textStyle);
	}
}
			 ...
			 float scale, vec4_t color, int align, int textStyle)
{
	...
	if (cg_drawClock.integer == 2) {
		...
		s = va("%d%s%02d%s", h, (qt.tm_sec % 2) ? ":" : " ", qt.tm_min,
		       pm);
	}
	w = CG_Text_Width("0", scale, 0);
	strLength = CG_DrawStrlen(s);
...
void CG_AddLagometerSnapshotInfo(snapshot_t * snap)
{
	...
	// dropped packet
	if (!snap) {
		lagometer.snapshotSamples[lagometer.
					  snapshotCount & (LAG_SAMPLES - 1)] =
		    -1;
...
static void CG_DrawDisconnect(void)
{
	...
	CG_Text_Paint(320 - w / 2, 100, 0.7f, color, s, 0, 0,
			     ...
			     float scale, vec4_t textColor)
{
	...
	for (a = 0; a < aw; a++) {
		...
		if (v > 0) {
			...
			trap_R_DrawStretchPic(ax + aw - a, mid, 1, v, 0, 0, 0,
					      0, cgs.media.whiteShader);
		}
	}
	...
	if (cg_nopredict.integer || cg_synchronousClients.integer)
		CG_Text_Paint(ax, ay, 0.5, white, "snc", 0, 0,
		...
		ax = rect->x + (rect->w / 2.0f) -
		    (CG_Text_Width(s, scale, 0) / 2.0f) + text_x;
		ay = rect->y + (rect->h / 2.0f) +
		    (CG_Text_Height(s, scale, 0) / 2.0f) + text_y;

		Vector4Copy(textColor, adjustedColor);
		adjustedColor[3] = 0.5f;
		CG_Text_Paint(ax, ay, scale, adjustedColor, s, 0, 0,
			      ITEM_TEXTSTYLE_NORMAL);
	}
...
==============
*/
static void CG_DrawTextBlock(rectDef_t * rect, float text_x, float text_y,
			     vec4_t color, float scale, int align,
...
===================
*/
static void CG_DrawConsole(rectDef_t * rect, float text_x, float text_y,
			   vec4_t color, float scale, int align, int textStyle)
{
...
===================
*/
static void CG_DrawTutorial(rectDef_t * rect, float text_x, float text_y,
			    vec4_t color, float scale, int align, int textStyle)
{
	static menuDef_t dummyParent;
	static itemDef_t textItem;
...
void CG_DrawWeaponIcon(rectDef_t * rect, vec4_t color)
{
	...
	if (clips == 0
	    && !BG_FindInfinteAmmoForWeapon(cent->currentState.weapon)) {
...
static void CG_DrawCrosshair(void)
{
	...
	if (hShader != 0) {
		trap_R_DrawStretchPic(x + cg.refdef.x +
				      0.5 * (cg.refdef.width - w),
				      y + cg.refdef.y +
				      0.5 * (cg.refdef.height - h), w, h, 0, 0,
				      1, 1, hShader);
	}
}
...
static void CG_DrawCrosshairNames(rectDef_t * rect, float scale, int textStyle)
{
	...
	x = rect->x + rect->w / 2;
	CG_Text_Paint(x - w / 2, rect->y + rect->h, scale, color, name, 0, 0,
		      textStyle);
	trap_R_SetColor(NULL);
}
		  ...
		  qhandle_t shader, int textStyle)
{
	...
	case CG_STAGE_REPORT_TEXT:
		CG_DrawStageReport(&rect, text_x, text_y, color, scale, align,
				   textStyle);
		break;
	...
	case CG_LOAD_MEDIA_LABEL:
		CG_DrawMediaProgressLabel(&rect, text_x, text_y, color, scale,
					  align);
		break;
	...
	case CG_LOAD_BUILDABLES_LABEL:
		CG_DrawBuildablesProgressLabel(&rect, text_x, text_y, color,
					       scale, align);
		break;
	...
	case CG_LOAD_CHARMODEL_LABEL:
		CG_DrawCharModelProgressLabel(&rect, text_x, text_y, color,
					      scale, align);
		break;
	...
	case CG_LOAD_LEVELNAME:
		CG_DrawLevelName(&rect, text_x, text_y, color, scale, align,
				 textStyle);
		break;
	case CG_LOAD_MOTD:
		CG_DrawMOTD(&rect, text_x, text_y, color, scale, align,
			    textStyle);
		break;
	case CG_LOAD_HOSTNAME:
		CG_DrawHostname(&rect, text_x, text_y, color, scale, align,
				textStyle);
		break;

	case CG_FPS:
		CG_DrawFPS(&rect, text_x, text_y, scale, color, align,
			   textStyle, qtrue);
		break;
	case CG_FPS_FIXED:
		CG_DrawFPS(&rect, text_x, text_y, scale, color, align,
		...
		break;
	case CG_TIMER:
		CG_DrawTimer(&rect, text_x, text_y, scale, color, align,
			     textStyle);
		break;
	case CG_CLOCK:
		CG_DrawClock(&rect, text_x, text_y, scale, color, align,
			     textStyle);
		break;
	case CG_TIMER_MINS:
	...
	case CG_TIMER_SECS:
		...
		break;
	case CG_SNAPSHOT:
		CG_DrawSnapshot(&rect, text_x, text_y, scale, color, align,
				textStyle);
		break;
	case CG_LAGOMETER:
	...
	case CG_CONSOLE:
		CG_DrawConsole(&rect, text_x, text_y, color, scale, align,
			       textStyle);
		break;

	case CG_TUTORIAL:
		CG_DrawTutorial(&rect, text_x, text_y, color, scale, align,
				textStyle);
		break;
...
void CG_KeyEvent(int key, qboolean down)
{
	...
	if (cgs.capturedItem)
		...
		if (key == K_MOUSE2 && down)
			cgs.capturedItem =
			    Display_CaptureItem(cgs.cursorX, cgs.cursorY);
	}
}
...
static void CG_DrawLighting(void)
{
	...
	if ((cg.snap->ps.stats[STAT_STAMINA] < -800) &&
		...
		black[3] =
		    1.0 -
		    ((float)(cg.snap->ps.stats[STAT_STAMINA] + 1000) / 200.0f);
...
static void CG_DrawSpeed(void)
{
	...
	if (cg_drawSpeed.integer & 4) {
		...
		dot =
		    DotProduct(cg.predictedPlayerState.velocity,
			       cg.predictedPlayerState.grapplePoint);
		VectorMA(cg.predictedPlayerState.velocity, -dot,
			 cg.predictedPlayerState.grapplePoint, xyvelocity);
...
static void CG_DrawVote(void)
{
	...
	s2 = va
	    ("^9Status ^7- ^7[^2%s^7]^ZYes: %i ^7- ^7[^1%s^7]^ANo: %i^7 - ^9%i^7 seconds remaining,",
	     yeskey, cgs.voteYes, nokey, cgs.voteNo, sec);
...
static void CG_DrawTeamVote(void)
{
	...
	s2 = va
	    ("^9Status ^7- ^7[^2%s^7]^ZYes: %i ^7- ^7[^1%s^7]^ANo: %i^7 - ^9%i^7 seconds remaining,",
	     yeskey, cgs.teamVoteYes[cs_offset], nokey,
	     cgs.teamVoteNo[cs_offset], sec);
...
static qboolean CG_DrawFollow(void)
{
	...
	CG_Text_Paint(320 - w / 2, 400, 0.7f, color, buffer, 0, 0,
		      ITEM_TEXTSTYLE_SHADOWED);
...
static qboolean CG_DrawQueue(void)
{
	...
	Com_sprintf(buffer, MAX_STRING_CHARS,
		    "You are in position %d of the spawn queue.",
		    cg.snap->ps.persistant[PERS_QUEUEPOS] + 1);

	w = CG_Text_Width(buffer, 0.7f, 0);
	CG_Text_Paint(320 - w / 2, 360, 0.7f, color, buffer, 0, 0,
		      ITEM_TEXTSTYLE_SHADOWED);
...
static void CG_Draw2D(void)
{
	...
	if (cg.snap->ps.persistant[PERS_TEAM] == TEAM_SPECTATOR) {
		...
		CG_Text_Paint(320 - w / 2, 440, 0.7f, color, SPECTATOR_STRING,
			      0, 0, ITEM_TEXTSTYLE_SHADOWED);
	} else
		menu =
		    Menus_FindByName(BG_FindHudNameForClass
