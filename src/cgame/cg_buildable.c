...
static void CG_Creep(centity_t * cent)
{
	...
	vec3_t temp, origin;
	int scaleUpTime =
	    BG_FindBuildTimeForBuildable(cent->currentState.modelindex);
	int time;
	...
	CG_Trace(&tr, cent->lerpOrigin, NULL, NULL, temp,
	...
	if (size > 0.0f)
		CG_ImpactMark(cgs.media.creepShader, origin,
			      cent->currentState.origin2, 0.0f, 1.0f, 1.0f,
			      1.0f, 1.0f, qfalse, size, qtrue);
}
...
======================
*/
static qboolean CG_ParseBuildableAnimationFile(const char *filename,
...
======================
*/
static qboolean CG_ParseBuildableSoundFile(const char *filename,
					   buildable_t buildable)
{
	char *text_p;
	int len;
...
void CG_InitBuildables(void)
{
	...
	for (i = BA_NONE + 1; i < BA_NUM_BUILDABLES; i++) {
		...
		//animation.cfg
		Com_sprintf(filename, sizeof(filename),
			    "models/buildables/%s/animation.cfg",
			    buildableName);
		if (!CG_ParseBuildableAnimationFile(filename, i))
			Com_Printf(S_COLOR_YELLOW
				   "WARNING: failed to load animation file %s\n",
				   filename);
		...
		if (!CG_ParseBuildableSoundFile(filename, i))
			Com_Printf(S_COLOR_YELLOW
				   "WARNING: failed to load sound file %s\n",
				   filename);

		//models
		for (j = 0; j <= 3; j++) {
			if ((modelFile = BG_FindModelsForBuildable(i, j)))
				cg_buildables[i].models[j] =
				    trap_R_RegisterModel(modelFile);
		}
		...
		for (j = BANIM_NONE + 1; j < MAX_BUILDABLE_ANIMATIONS; j++) {
			strcpy(soundfile, cg_buildableSoundNames[j - 1]);
			Com_sprintf(filename, sizeof(filename),
				    "sound/buildables/%s/%s", buildableName,
				    soundfile);

			if (cg_buildables[i].sounds[j].enabled) {
				if (trap_FS_FOpenFile(filename, &f, FS_READ) >
				    0) {
					//file exists so close it
					trap_FS_FCloseFile(f);

					cg_buildables[i].sounds[j].sound =
					    trap_S_RegisterSound(filename,
								 qfalse);
				} else {
					//file doesn't exist - use default
					if (BG_FindTeamForBuildable(i) ==
					    BIT_ALIENS)
						cg_buildables[i].sounds[j].
						    sound =
						    defaultAlienSounds[j];
					else
						cg_buildables[i].sounds[j].
						    sound =
						    defaultHumanSounds[j];
				}
			}
		}

		cg.buildablesFraction =
		    (float)i / (float)(BA_NUM_BUILDABLES - 1);
		trap_UpdateScreen();
	}

	cgs.media.teslaZapTS =
	    CG_RegisterTrailSystem("models/buildables/tesla/zap");
}
...
===============
*/
static void CG_SetBuildableLerpFrameAnimation(buildable_t buildable,
					      lerpFrame_t * lf,
					      int newAnimation)
{
	animation_t *anim;
...
static void CG_RunBuildableLerpFrame(centity_t * cent)
{
	...
	animation_t *anim;
	buildableAnimNumber_t newAnimation =
	...
	if (newAnimation != lf->animationNumber || !lf->animation) {
		...
		if (!cg_buildables[buildable].sounds[newAnimation].looped &&
			...
			trap_S_StartSound(cent->lerpOrigin,
					  cent->currentState.number, CHAN_AUTO,
					  cg_buildables[buildable].
					  sounds[newAnimation].sound);
		}
	}
	...
	if (cg_buildables[buildable].sounds[lf->animationNumber].looped &&
		...
		trap_S_AddLoopingSound(cent->currentState.number,
				       ...
				       cg_buildables[buildable].sounds[lf->
								       animationNumber].
				       sound);
...
===============
*/
static void CG_BuildableAnimation(centity_t * cent, int *old, int *now,
				  float *backLerp)
{
		...
		if ((cent->oldBuildableAnim ^ es->legsAnim) & ANIM_TOGGLEBIT) {
			if (cg_debugAnim.integer)
				CG_Printf("%d->%d l:%d t:%d %s(%d)\n",
					  ...
					  cent->buildableAnim, es->legsAnim,
					  es->torsoAnim,
					  BG_FindHumanNameForBuildable(es->
								       modelindex),
					  es->number);

			if (cent->buildableAnim == es->torsoAnim
			    || es->legsAnim & ANIM_FORCEBIT)
				cent->buildableAnim = cent->oldBuildableAnim =
				    es->legsAnim;
			else
				cent->buildableAnim = cent->oldBuildableAnim =
				    es->torsoAnim;
...
===============
*/
static void CG_PositionAndOrientateBuildable(const vec3_t angles,
					     const vec3_t inOrigin,
					     const vec3_t normal,
...
void CG_GhostBuildable(buildable_t buildable)
{
	...
	BG_PositionBuildableRelativeToPlayer(ps, mins, maxs, CG_Trace,
	...
	if (team == BIT_HUMANS) {
		...
		if (healthFrac < 0.33f
			...
			if (CG_IsParticleSystemValid(&cent->buildablePS)) {
				CG_SetAttachmentCent(&cent->buildablePS->
						     attachment, cent);
				CG_SetParticleSystemNormal(cent->buildablePS,
							   es->origin2);
				CG_AttachToCent(&cent->buildablePS->attachment);
			}
		} else if (healthFrac >= 0.33f
...
void CG_BuildableStatusParse(const char *filename, buildStat_t * bs)
{
	...
	while (1) {
		...
		if (!Q_stricmp(token.string, "frameShader")) {
			...
			Com_Printf
			    ("CG_BuildableStatusParse: unknown token %s in %s\n",
...
static void CG_BuildableStatusDisplay(centity_t * cent)
{
	...
	for (j = 0; j < 3; j++) {
		...
		for (i = 0; i < 3; i++) {
			CG_Trace(&tr, trOrigin, NULL, NULL, origin, entNum,
				 MASK_SHOT);
			...
			if (tr.entityNum < MAX_CLIENTS
			    || (hit->eType == ET_BUILDABLE
				&& (!(es->generic1 & B_SPAWNED_TOGGLEBIT)
				    || BG_FindTransparentTestForBuildable(hit->
									  modelindex))))
			{
				entNum = tr.entityNum;
	...
	if (CG_WorldToScreen(origin, &x, &y)) {
		...
		if (health > 0) {
			...
			hW = picW * healthScale -
			    (bs->healthPadding * 2.0f * scale);

			if (healthScale == 1.0f)
				Vector4Copy(bs->healthLowColor, healthColor);
			else if (healthScale >= 0.75f)
				Vector4Copy(bs->healthGuardedColor,
					    healthColor);
			else if (healthScale >= 0.50f)
				Vector4Copy(bs->healthElevatedColor,
					    healthColor);
	...
	case WP_HBUILD2:
		for (i = 0; i < cg.snap->numEntities; i++) {
			...
			if (es->eType == ET_BUILDABLE &&
			    ...
			    BG_FindTeamForWeapon(cg.predictedPlayerState.
						 weapon))
		...
		for (i = 0; i < buildables; i++)
			CG_BuildableStatusDisplay(&cg_entities
						  [buildableList[i]]);
		break;
...
void CG_Buildable(centity_t * cent)
{
	...
	if (!(es->generic1 & B_SPAWNED_TOGGLEBIT)) {
		...
		trap_S_AddLoopingSound(es->number, cent->lerpOrigin,
				       vec3_origin, prebuildSound);
	}
	...
	if (cg_buildables[es->modelindex].models[1]) {
		...
		CG_PositionEntityOnTag(&turretBarrel, &ent, ent.hModel,
				       "tag_turret");
		VectorCopy(cent->lerpOrigin, turretBarrel.lightingOrigin);
		AnglesToAxis(es->angles2, flatAxis);

		RotatePointAroundVector(turretBarrel.axis[0], xNormal,
					flatAxis[0], -rotAngle);
		RotatePointAroundVector(turretBarrel.axis[1], xNormal,
					flatAxis[1], -rotAngle);
		RotatePointAroundVector(turretBarrel.axis[2], xNormal,
					flatAxis[2], -rotAngle);
		...
		if (scale != 1.0f) {
			VectorScale(turretBarrel.axis[0], scale,
				    turretBarrel.axis[0]);
			VectorScale(turretBarrel.axis[1], scale,
				    turretBarrel.axis[1]);
			VectorScale(turretBarrel.axis[2], scale,
				    turretBarrel.axis[2]);
	...
	if (cg_buildables[es->modelindex].models[2]) {
		...
		CG_PositionRotatedEntityOnTag(&turretTop, &ent, ent.hModel,
					      "tag_turret");
		...
		if (scale != 1.0f) {
			VectorScale(turretTop.axis[0], scale,
				    turretTop.axis[0]);
			VectorScale(turretTop.axis[1], scale,
				    turretTop.axis[1]);
			VectorScale(turretTop.axis[2], scale,
				    turretTop.axis[2]);
	...
	if (healthScale < cent->lastBuildableHealthScale
	    && (es->generic1 & B_SPAWNED_TOGGLEBIT)) {
		if (cent->lastBuildableDamageSoundTime +
		    BUILDABLE_SOUND_PERIOD < cg.time) {
			if (team == BIT_HUMANS) {
				int i = rand() % 4;
				trap_S_StartSound(NULL, es->number, CHAN_BODY,
						  cgs.media.
						  humanBuildableDamage[i]);
			} else if (team == BIT_ALIENS)
				trap_S_StartSound(NULL, es->number, CHAN_BODY,
						  cgs.media.
						  alienBuildableDamage);
