...
static void CG_ParseScores(void)
{
	...
	for (i = 0; i < cg.numScores; i++) {
		...
		if (cg.scores[i].client < 0
		    || cg.scores[i].client >= MAX_CLIENTS)
...
void CG_SetConfigValues(void)
{
	sscanf(CG_ConfigString(CS_BUILDPOINTS),
	       ...
	       &cgs.humanBuildPointsTotal, &cgs.humanBuildPointsPowered);

	sscanf(CG_ConfigString(CS_STAGES), "%d %d %d %d %d %d", &cgs.alienStage,
	       &cgs.humanStage, &cgs.alienKills, &cgs.humanKills,
	       &cgs.alienNextStageThreshold, &cgs.humanNextStageThreshold);
	sscanf(CG_ConfigString(CS_SPAWNS), "%d %d", &cgs.numAlienSpawns,
	       &cgs.numHumanSpawns);
...
static void CG_AnnounceHumanStageTransistion(stage_t from, stage_t to)
{
	...
	CG_CenterPrint("Reinforcements have arrived!", 200,
		       GIANTCHAR_WIDTH * 4);
}
...
void CG_ConfigStringModified(int num)
{
	...
	if (num == CS_MUSIC)
		...
		if (cgs.alienStage != oldAlienStage)
			CG_AnnounceAlienStageTransistion(oldAlienStage,
							 cgs.alienStage);

		if (cgs.humanStage != oldHumanStage)
			CG_AnnounceHumanStageTransistion(oldHumanStage,
							 cgs.humanStage);
		...
		Q_strncpyz(cgs.teamVoteString[num - CS_TEAMVOTE_STRING], str,
			   sizeof(cgs.
				  teamVoteString[num - CS_TEAMVOTE_STRING]));
		...
		cgs.gameParticleSystems[num - CS_PARTICLE_SYSTEMS] =
		    CG_RegisterParticleSystem((char *)str);
...
void CG_Menu(int menu)
{
	...
	case MN_A_TEAMFULL:
		longMsg =
		    "The alien team has too many players. Please wait until slots "
	...
	case MN_H_TEAMFULL:
		longMsg =
		    "The human team has too many players. Please wait until slots "
	...
	case MN_A_TEAMCHANGEBUILDTIMER:
		longMsg =
		    "You cannot leave the Alien team until your build timer "
		    "has expired.";
		shortMsg =
		    "You cannot change teams until your build timer expires.\n";
		cmd = "menu tremulous_alien_dialog\n";
		break;

	case MN_H_TEAMCHANGEBUILDTIMER:
		longMsg =
		    "You cannot leave the Human team until your build timer "
		    "has expired.";
		shortMsg =
		    "You cannot change teams until your build timer expires.\n";
		cmd = "menu tremulous_human_dialog\n";
	...
	case MN_H_NOROOM:
		longMsg =
		    "There is no room to build here. Move until the buildable turns "
	...
	case MN_H_NOPOWER:
		longMsg =
		    "There is no power remaining. Free up power by destroying "
	...
	case MN_H_NOTPOWERED:
		longMsg =
		    "This buildable is not powered. Build a Reactor and/or Repeater "
	...
	case MN_H_NORMAL:
		longMsg =
		    "Cannot build on this surface. The surface is too steep or "
	...
	case MN_H_REACTOR:
		longMsg =
		    "There can only be one Reactor. Destroy the existing one if you "
		    "wish to move it.";
	...
	case MN_H_REPEATER:
		longMsg =
		    "There is no power here. If available, a Repeater may be used to "
	...
	case MN_H_NODCC:
		longMsg =
		    "There is no Defense Computer. A Defense Computer is needed to "
		    "build this.";
	...
	case MN_H_TNODEWARN:
		longMsg =
		    "WARNING: This Telenode will not be powered. Build near a power "
	...
	case MN_H_RPTWARN:
		longMsg =
		    "WARNING: This Repeater will not be powered as there is no parent "
	...
	case MN_H_RPTWARN2:
		longMsg =
		    "This area already has power. A Repeater is not required here.";
	...
	case MN_H_NOSLOTS:
		longMsg =
		    "You have no room to carry this. Please sell any conflicting "
	...
	case MN_H_NOFUNDS:
		longMsg =
		    "Insufficient funds. You do not have enough credits to perform "
		    "this action.";
	...
	case MN_H_ITEMHELD:
		longMsg =
		    "You already hold this item. It is not possible to carry multiple "
	...
	case MN_H_NOARMOURYHERE:
		longMsg =
		    "You must be near a powered Armoury in order to purchase "
	...
	case MN_H_NOENERGYAMMOHERE:
		longMsg =
		    "You must be near an Armoury, Reactor or Repeater in order "
	...
	case MN_H_NOROOMBSUITON:
		longMsg =
		    "There is not enough room here to put on a Battle Suit. "
	...
	case MN_H_NOROOMBSUITOFF:
		longMsg =
		    "There is not enough room here to take off your Battle Suit. "
		    "Make sure you have enough head room to climb out.";
		shortMsg =
		    "Not enough room here to take off your Battle Suit\n";
		cmd = "menu tremulous_human_dialog\n";
		break;

	case MN_H_ARMOURYBUILDTIMER:
		longMsg =
		    "You are not allowed to buy or sell weapons until your "
		    "build timer has expired.";
		shortMsg =
		    "You can not buy or sell weapos until your build timer "
		    "expires\n";
		cmd = "menu tremulous_human_dialog\n";
		break;
	...
	case MN_A_NOROOM:
		longMsg =
		    "There is no room to build here. Move until the structure turns "
		    "translucent green indicating a valid build location.";
		shortMsg = "There is no room to build here\n";
		cmd = "menu tremulous_alien_dialog\n";
		break;
	...
	case MN_A_NOCREEP:
		longMsg =
		    "There is no creep here. You must build near existing Eggs or "
	...
	case MN_A_NOOVMND:
		longMsg =
		    "There is no Overmind. An Overmind must be built to control "
	...
	case MN_A_OVERMIND:
		longMsg =
		    "There can only be one Overmind. Destroy the existing one if you "
		    "wish to move it.";
	...
	case MN_A_HOVEL:
		longMsg =
		    "There can only be one Hovel. Destroy the existing one if you "
		    "wish to move it.";
	...
	case MN_A_NOASSERT:
		longMsg =
		    "The Overmind cannot control any more structures. Destroy existing "
	...
	case MN_A_SPWNWARN:
		longMsg =
		    "WARNING: This spawn will not be controlled by an Overmind. "
	...
	case MN_A_NORMAL:
		longMsg =
		    "Cannot build on this surface. This surface is too steep or "
		    "unsuitable to build on. Please choose another site for this "
		    "structure.";
		shortMsg = "Cannot build on this surface\n";
		cmd = "menu tremulous_alien_dialog\n";
	...
	case MN_A_NOEROOM:
		longMsg =
		    "There is no room to evolve here. Move away from walls or other "
	...
	case MN_A_TOOCLOSE:
		longMsg =
		    "This location is too close to the enemy to evolve. Move away "
	...
	case MN_A_NOOVMND_EVOLVE:
		longMsg =
		    "There is no Overmind. An Overmind must be built to allow "
		    "you to upgrade.";
		shortMsg = "There is no Overmind\n";
		cmd = "menu tremulous_alien_dialog\n";
		break;
	...
	case MN_A_EVOLVEBUILDTIMER:
		longMsg =
		    "You cannot Evolve until your build timer has expired.";
	...
	case MN_A_HOVEL_OCCUPIED:
		...
		shortMsg =
		    "This Hovel is already occupied by another builder\n";
		cmd = "menu tremulous_alien_dialog\n";
		break;

	case MN_A_HOVEL_BLOCKED:
		longMsg =
		    "The exit to this Hovel is currently blocked. Please wait until it "
	...
	case MN_A_HOVEL_EXIT:
		longMsg =
		    "The exit to this Hovel would always be blocked. Please choose "
	...
	case MN_A_INFEST:
		trap_Cvar_Set("ui_currentClass",
			      va("%d %d", cg.snap->ps.stats[STAT_PCLASS],
...
static void CG_ServerCommand(void)
{
	...
	if (!strcmp(cmd, "tchat")) {
		...
		if (Q_stricmpn(text, "[skipnotify]", 12)) {
			if (cg.snap->ps.stats[STAT_PTEAM] == PTE_ALIENS)
				trap_S_StartLocalSound(cgs.media.alienTalkSound,
						       CHAN_LOCAL_SOUND);
			else if (cg.snap->ps.stats[STAT_PTEAM] == PTE_HUMANS)
				trap_S_StartLocalSound(cgs.media.humanTalkSound,
						       CHAN_LOCAL_SOUND);
			else
				trap_S_StartLocalSound(cgs.media.talkSound,
						       CHAN_LOCAL_SOUND);
	...
	if (!strcmp(cmd, "poisoncloud")) {
		...
		if (CG_IsParticleSystemValid(&cg.poisonCloudPS)) {
			cg.poisonCloudPS =
			    CG_SpawnNewParticleSystem(cgs.media.poisonCloudPS);
			CG_SetAttachmentCent(&cg.poisonCloudPS->attachment,
