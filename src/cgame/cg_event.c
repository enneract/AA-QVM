...
static void CG_Obituary(entityState_t * ent)
{
	...
	if (attacker != ENTITYNUM_WORLD) {
		...
		if (message) {
			CG_Printf("%s %s %s%s%s\n",
				  targetName, message,
				  (teamKill) ? S_COLOR_RED "TEAMMATE "
				  S_COLOR_WHITE : "", attackerName, message2);
			if (teamKill && attacker == cg.clientNum) {
				CG_CenterPrint(va
					       ("You killed " S_COLOR_RED
						"TEAMMATE " S_COLOR_WHITE "%s",
						targetName),
					       SCREEN_HEIGHT * 0.30,
					       BIGCHAR_WIDTH);
			}
...
void CG_EntityEvent(centity_t * cent, vec3_t position)
{
	...
	if (cg.snap->ps.persistant[PERS_TEAM] == TEAM_SPECTATOR)
		...
		steptime =
		    BG_FindSteptimeForClass(cg.snap->ps.stats[STAT_PCLASS]);
		...
		if (cg_footsteps.integer && ci->footsteps != FOOTSTEP_NONE) {
			if (ci->footsteps == FOOTSTEP_CUSTOM)
				trap_S_StartSound(NULL, es->number, CHAN_BODY,
						  ci->
						  customFootsteps[rand() & 3]);
			else
				trap_S_StartSound(NULL, es->number, CHAN_BODY,
						  cgs.media.footsteps[ci->
								      footsteps]
						  [rand() & 3]);
		}
		break;
	...
	case EV_FOOTSTEP_METAL:
		...
		if (cg_footsteps.integer && ci->footsteps != FOOTSTEP_NONE) {
			if (ci->footsteps == FOOTSTEP_CUSTOM)
				trap_S_StartSound(NULL, es->number, CHAN_BODY,
						  ci->
						  customMetalFootsteps[rand() &
								       3]);
			else
				trap_S_StartSound(NULL, es->number, CHAN_BODY,
						  cgs.media.
						  footsteps[FOOTSTEP_METAL][rand
									    () &
									    3]);
		}
		break;
	...
	case EV_FOOTSPLASH:
		...
		if (cg_footsteps.integer && ci->footsteps != FOOTSTEP_NONE) {
			trap_S_StartSound(NULL, es->number, CHAN_BODY,
					  cgs.media.
					  footsteps[FOOTSTEP_SPLASH][rand() &
								     3]);
		}
		break;
	...
	case EV_FALLING:
		...
		trap_S_StartSound(NULL, es->number, CHAN_AUTO,
				  CG_CustomSound(es->number, "*falling1.wav"));
		break;
	...
	case EV_JUMP:
		...
		if (BG_ClassHasAbility
		    (cg.predictedPlayerState.stats[STAT_PCLASS],
		     SCA_WALLJUMPER)) {
	...
	case EV_MISSILE_HIT:
		...
		CG_MissileHitPlayer(es->weapon, es->generic1, position, dir,
				    es->otherEntityNum);
		break;
	...
	case EV_MISSILE_MISS:
		...
		ByteToDir(es->eventParm, dir);
		CG_MissileHitWall(es->weapon, es->generic1, 0, position, dir,
				  IMPACTSOUND_DEFAULT);
		break;
	...
	case EV_TESLATRAIL:
		...
		cent->currentState.weapon = WP_TESLAGEN;
		{
			...
			if (!CG_IsTrailSystemValid(&source->muzzleTS)) {
				source->muzzleTS =
				    CG_SpawnNewTrailSystem(cgs.media.
							   teslaZapTS);

				if (CG_IsTrailSystemValid(&source->muzzleTS)) {
					CG_SetAttachmentCent(&source->muzzleTS->
							     frontAttachment,
							     source);
					CG_SetAttachmentCent(&source->muzzleTS->
							     backAttachment,
							     target);
					CG_AttachToCent(&source->muzzleTS->
							frontAttachment);
					CG_AttachToCent(&source->muzzleTS->
							backAttachment);
					CG_SetAttachmentOffset(&source->
							       muzzleTS->
							       frontAttachment,
							       sourceOffset);

					source->muzzleTSDeathTime =
					    cg.time + cg_teslaTrailTime.integer;
				}
			}
		}
		break;
	...
	case EV_BUILD_DELAY:
		...
		if (clientNum == cg.predictedPlayerState.clientNum) {
			trap_S_StartLocalSound(cgs.media.buildableRepairedSound,
					       CHAN_LOCAL_SOUND);
	...
	case EV_OVERMIND_ATTACK:
		...
		if (cg.predictedPlayerState.stats[STAT_PTEAM] == PTE_ALIENS) {
			trap_S_StartLocalSound(cgs.media.alienOvermindAttack,
					       CHAN_ANNOUNCER);
			CG_CenterPrint("The Overmind is under attack!", 200,
				       GIANTCHAR_WIDTH * 4);
		}
		break;
	...
	case EV_OVERMIND_DYING:
		...
		if (cg.predictedPlayerState.stats[STAT_PTEAM] == PTE_ALIENS) {
			trap_S_StartLocalSound(cgs.media.alienOvermindDying,
					       CHAN_ANNOUNCER);
			CG_CenterPrint("The Overmind is dying!", 200,
				       GIANTCHAR_WIDTH * 4);
		}
		break;
	...
	case EV_DCC_ATTACK:
		...
		if (cg.predictedPlayerState.stats[STAT_PTEAM] == PTE_HUMANS) {
			...
			CG_CenterPrint("Our base is under attack!", 200,
				       GIANTCHAR_WIDTH * 4);
		}
		break;
	...
	case EV_OVERMIND_SPAWNS:
		...
		if (cg.predictedPlayerState.stats[STAT_PTEAM] == PTE_ALIENS) {
			trap_S_StartLocalSound(cgs.media.alienOvermindSpawns,
					       CHAN_ANNOUNCER);
			CG_CenterPrint("The Overmind needs spawns!", 200,
				       GIANTCHAR_WIDTH * 4);
		}
		break;
	...
	case EV_ALIEN_EVOLVE:
		...
		{
			particleSystem_t *ps =
			    CG_SpawnNewParticleSystem(cgs.media.alienEvolvePS);
