...
static void CG_CalculateBeamNodeProperties(trailBeam_t * tb)
{
	...
	TCRange = tb->class->backTextureCoord - tb->class->frontTextureCoord;
	widthRange = tb->class->backWidth - tb->class->frontWidth;
	alphaRange = tb->class->backAlpha - tb->class->frontAlpha;
	VectorSubtract(tb->class->backColor, tb->class->frontColor, colorRange);
	...
	for (j = 0, i = tb->nodes; i; i = i->next, j++) {
		if (tb->class->textureType == TBTT_STRETCH) {
			...
			if (tb->class->clampToBack)
				...
				i->textureCoord =
				    position / tb->class->repeatLength;
		}

		i->halfWidth = (tb->class->frontWidth +
				((position / totalDistance) * widthRange)) /
		    2.0f;
		i->alpha =
		    (byte) ((float)0xFF *
			    (tb->class->frontAlpha +
			     ((position / totalDistance) * alphaRange)) *
			    fadeAlpha);
...
static void CG_RenderBeam(trailBeam_t * tb)
{
	...
	if (bts->thirdPersonOnly &&
	    (CG_AttachmentCentNum(&ts->frontAttachment) == cg.snap->ps.clientNum
	     || CG_AttachmentCentNum(&ts->backAttachment) ==
	...
	do {
		...
		if (prev && next) {
			...
			GetPerpendicularViewVector(cg.refdef.vieworg,
						   next->position,
						   prev->position, up);
		} else if (!prev && next) {
			//this is the front
			GetPerpendicularViewVector(cg.refdef.vieworg,
						   next->position, i->position,
						   up);
		} else if (prev && !next) {
			//this is the back
			GetPerpendicularViewVector(cg.refdef.vieworg,
		...
		if (prev) {
			VectorMA(i->position, i->halfWidth, up,
				 verts[numVerts].xyz);
			verts[numVerts].st[0] = i->textureCoord;
			verts[numVerts].st[1] = 1.0f;

			if (btb->realLight)
				CG_LightVertex(verts[numVerts].xyz, i->alpha,
					       verts[numVerts].modulate);
			else {
	...
	for (j = 0; j < btb->numJitters; j++) {
		if (tb->nextJitterTimes[j] <= cg.time) {
			for (i = tb->nodes; i; i = i->next) {
				i->jitters[j][0] =
				    (crandom() * btb->jitters[j].magnitude);
				i->jitters[j][1] =
				    (crandom() * btb->jitters[j].magnitude);
			}

			tb->nextJitterTimes[j] =
			    cg.time + btb->jitters[j].period;
		}
	}
...
static void CG_UpdateBeam(trailBeam_t * tb)
{
	...
	if (ts->destroyTime <= 0) {
		...
		while (nodesToAdd--) {
			...
			if (!tb->nodes->next
			    && CG_Attached(&ts->frontAttachment)) {
				// this is the first node to be added
				if (!CG_AttachmentPoint
				    (&ts->frontAttachment, i->refPosition))
					CG_DestroyTrailSystem(&ts);
			} else
				VectorCopy(i->prev->refPosition,
					   i->refPosition);
		}
	}
	...
	if (CG_Attached(&ts->frontAttachment)
		...
		if (i && i->timeLeft >= 0) {
			...
			if (i->timeLeft < 0) {
				...
				VectorSubtract(i->refPosition,
					       i->prev->refPosition, dir);
				length =
				    VectorNormalize(dir) * ((float)i->timeLeft /
							    (float)tb->class->
							    segmentTime);

				VectorMA(i->prev->refPosition, length, dir,
					 i->position);
			}
		}
		...
		if (tb->nodes) {
			if (!CG_AttachmentPoint
			    (&ts->frontAttachment, tb->nodes->refPosition))
				CG_DestroyTrailSystem(&ts);
		...
		if (!Q_stricmp(token, "segments")) {
			...
			if (btb->numSegments >= MAX_TRAIL_BEAM_NODES) {
				...
				CG_Printf(S_COLOR_YELLOW
					  "WARNING: too many segments in trail beam\n");
			}
			continue;
			...
			if (!Q_stricmp(token, "{")) {
				if (!CG_ParseTrailBeamColor
				    (btb->frontColor, text_p))
					break;
			...
			if (!Q_stricmp(token, "stretch")) {
				...
				btb->frontTextureCoord =
				    atof_neg(token, qfalse);
				...
				if (!Q_stricmp(token, "front"))
					...
					CG_Printf(S_COLOR_RED
						  "ERROR: unknown textureType clamp \"%s\"\n",
						  token);
					break;
				}
			...
			btb->jitters[btb->numJitters].magnitude =
			    atof_neg(token, qfalse);
			...
			btb->jitters[btb->numJitters].period =
			    atoi_neg(token, qfalse);
			...
			CG_Printf(S_COLOR_RED
				  "ERROR: unknown token '%s' in trail beam\n",
				  token);
			return qfalse;
		}
	}
...
===============
*/
static qboolean CG_ParseTrailSystem(baseTrailSystem_t * bts, char **text_p,
				    const char *name)
{
	char *token;
	...
	while (1) {
		...
		if (!Q_stricmp(token, "{")) {
			CG_InitialiseBaseTrailBeam(&baseTrailBeams
						   [numBaseTrailBeams]);

			if (!CG_ParseTrailBeam
			    (&baseTrailBeams[numBaseTrailBeams], text_p)) {
				CG_Printf(S_COLOR_RED
					  "ERROR: failed to parse trail beam\n");
				return qfalse;
			}

			if (bts->numBeams == MAX_BEAMS_PER_SYSTEM) {
				CG_Printf(S_COLOR_RED
					  "ERROR: trail system has > %d beams\n",
					  MAX_BEAMS_PER_SYSTEM);
				return qfalse;
			} else if (numBaseTrailBeams == MAX_BASETRAIL_BEAMS) {
				CG_Printf(S_COLOR_RED
					  "ERROR: maximum number of trail beams (%d) reached\n",
					  MAX_BASETRAIL_BEAMS);
				return qfalse;
			} else {
				//start parsing beams again
				bts->beams[bts->numBeams] =
				    &baseTrailBeams[numBaseTrailBeams];
				bts->numBeams++;
				numBaseTrailBeams++;
			}
			continue;
			...
			CG_Printf(S_COLOR_RED
				  "ERROR: unknown token '%s' in trail system %s\n",
				  token, bts->name);
			return qfalse;
		}
	}
...
static qboolean CG_ParseTrailFile(const char *fileName)
{
	...
	while (1) {
		...
		if (!Q_stricmp(token, "{")) {
			if (tsNameSet) {
				...
				for (i = 0; i < numBaseTrailSystems; i++) {
					if (!Q_stricmp
					    (baseTrailSystems[i].name,
					     tsName)) {
						CG_Printf(S_COLOR_RED
							  "ERROR: a trail system is already named %s\n",
							  tsName);
						return qfalse;
					}
				}

				Q_strncpyz(baseTrailSystems
					   [numBaseTrailSystems].name, tsName,
					   MAX_QPATH);

				if (!CG_ParseTrailSystem
				    (&baseTrailSystems[numBaseTrailSystems],
				     &text_p, tsName)) {
					CG_Printf(S_COLOR_RED
						  "ERROR: %s: failed to parse trail system %s\n",
						  fileName, tsName);
					return qfalse;
				}
				...
				if (numBaseTrailSystems ==
				    MAX_BASETRAIL_SYSTEMS) {
					CG_Printf(S_COLOR_RED
						  "ERROR: maximum number of trail systems (%d) reached\n",
						  MAX_BASETRAIL_SYSTEMS);
					return qfalse;
				} else
					numBaseTrailSystems++;
		...
		if (!tsNameSet) {
			...
			CG_Printf(S_COLOR_RED
				  "ERROR: trail system already named\n");
			return qfalse;
		}
	}
	...
	for (i = 0; i < MAX_BASETRAIL_SYSTEMS; i++) {
		...
		if (!Q_stricmp(bts->name, name)) {
			...
			for (j = 0; j < bts->numBeams; j++) {
				...
				btb->shader =
				    trap_R_RegisterShader(btb->shaderName);
			}
...
trailSystem_t *CG_SpawnNewTrailSystem(qhandle_t psHandle)
{
	...
	if (!bts->registered) {
		CG_Printf(S_COLOR_RED
			  "ERROR: a trail system has not been registered yet\n");
		return NULL;
	}
...
void CG_TestTS_f(void)
{
	...
	if (testTSHandle) {
		...
		if (CG_IsTrailSystemValid(&testTS)) {
			CG_SetAttachmentCent(&testTS->frontAttachment,
					     &cg_entities[0]);
