...
void CG_TestModel_f(void)
{
	...
	cg.testModelBarrelEntity.hModel =
	    trap_R_RegisterModel(cg.testModelBarrelName);
...
static void CG_AddTestModel(void)
{
	...
	if (cg.testModelBarrelEntity.hModel) {
		CG_PositionEntityOnTag(&cg.testModelBarrelEntity,
				       &cg.testModelEntity,
...
static void CG_OffsetThirdPersonView(void)
{
	...
	if (cg.predictedPlayerState.stats[STAT_STATE] & SS_WALLCLIMBING) {
		if (cg.predictedPlayerState.
			...
			VectorCopy(cg.predictedPlayerState.grapplePoint,
				   surfNormal);
	} else
		VectorSet(surfNormal, 0.0f, 0.0f, 1.0f);

	VectorMA(cg.refdef.vieworg, cg.predictedPlayerState.viewheight,
		 surfNormal, cg.refdef.vieworg);
	...
	VectorMA(view, -cg_thirdPersonRange.value * forwardScale, forward,
		 view);
	...
	cg.refdefViewAngles[PITCH] =
	    -180 / M_PI * atan2(focusPoint[2], focusDist);
...
static void CG_StepOffset(void)
{
	...
	if (timeDelta < steptime) {
		...
		if (ps->stats[STAT_STATE] & SS_WALLCLIMBING)
			VectorMA(cg.refdef.vieworg, -stepChange, normal,
				 cg.refdef.vieworg);
		else
...
static void CG_OffsetFirstPersonView(void)
{
	...
	if (cg.damageTime) {
		...
		if (ratio < DAMAGE_DEFLECT_TIME) {
			...
			ratio =
			    1.0 - (ratio -
				   DAMAGE_DEFLECT_TIME) / DAMAGE_RETURN_TIME;
			if (ratio > 0) {
				angles[PITCH] += ratio * cg.v_dmg_pitch;
				angles[ROLL] += ratio * cg.v_dmg_roll;
			}
	...
	if (cg.snap->ps.persistant[PERS_TEAM] == TEAM_SPECTATOR)
		...
		bob2 =
		    BG_FindBobForClass(cg.predictedPlayerState.
				       stats[STAT_PCLASS]);
...
#define LEVEL3_FEEDBACK  20.0f
	...
	if (cg.predictedPlayerState.weapon == WP_ALEVEL3 ||
		...
		if (cg.predictedPlayerState.stats[STAT_MISC] > 0) {
			...
			VectorMA(origin, LEVEL3_FEEDBACK * fraction2, forward,
				 origin);
		}
	}
...
#define STRUGGLE_TIME 250
	...
	if (cg.predictedPlayerState.pm_type == PM_GRABBED) {
		...
		fFraction =
		    (float)(cg.time - cg.forwardMoveTime) / STRUGGLE_TIME;
		...
		if (cmd.forwardmove > 0)
			VectorMA(origin, STRUGGLE_DIST * fFraction, forward,
				 origin);
		...
		if (cmd.rightmove > 0)
			VectorMA(origin, STRUGGLE_DIST * rFraction, right,
				 origin);
		...
		if (cmd.upmove > 0)
			...
			VectorMA(origin, -STRUGGLE_DIST * uFraction, up,
				 origin);
		else
	...
	if (cg.predictedPlayerState.stats[STAT_STATE] & SS_POISONCLOUDED &&
		...
		float fraction =
		    sin(((float)cg.time / 1000.0f) * M_PI * 2 *
			PCLOUD_ROLL_FREQUENCY);
		float pitchFraction =
		    sin(((float)cg.time / 1000.0f) * M_PI * 5 *
			PCLOUD_ROLL_FREQUENCY);

		fraction *=
		    1.0f -
		    ((cg.time - cg.poisonedTime) / (float)LEVEL1_PCLOUD_TIME);
		pitchFraction *=
		    1.0f -
		    ((cg.time - cg.poisonedTime) / (float)LEVEL1_PCLOUD_TIME);
	...
	if (cg.predictedPlayerState.stats[STAT_PTEAM] == PTE_HUMANS) {
		...
		if (cg.predictedPlayerState.stats[STAT_STAMINA] < 0) {
			float deltaBreath =
			    (float)(cg.predictedPlayerState.
				    stats[STAT_STAMINA] <
				    0 ? -cg.predictedPlayerState.
				    stats[STAT_STAMINA] : cg.
				    predictedPlayerState.stats[STAT_STAMINA]) /
			    200.0;
			float deltaAngle =
			    cos((float)cg.time / 150.0) * deltaBreath;

			deltaAngle +=
			    (deltaAngle < 0 ? -deltaAngle : deltaAngle) * 0.5;
...
static int CG_CalcFov(void)
{
	...
	if (cg.predictedPlayerState.pm_type == PM_INTERMISSION ||
		...
		attribFov =
		    BG_FindFovForClass(cg.predictedPlayerState.
				       stats[STAT_PCLASS]);
		fov_x = attribFov;
		...
		if (cg.spawnTime > (cg.time - FOVWARPTIME) &&
		    BG_ClassHasAbility(cg.predictedPlayerState.
				       stats[STAT_PCLASS], SCA_FOVWARPS)) {
			float temp, temp2;
		...
		// account for zooms
		zoomFov =
		    BG_FindZoomFovForWeapon(cg.predictedPlayerState.weapon);
		if (zoomFov < 1)
			zoomFov = 1;
...
===============
*/
static void CG_smoothWWTransitions(playerState_t * ps, const vec3_t in,
				   vec3_t out)
{
	...
	for (i = MAXSMOOTHS - 1; i >= 0; i--) {
		smoothTime =
		    (int)(cg_wwSmoothTime.integer * cg.sList[i].timeMod);

		//if this op has time remaining, perform it
		if (cg.time < cg.sList[i].time + smoothTime) {
			stLocal =
			    1.0f -
			    (((cg.sList[i].time + smoothTime) -
			      cg.time) / smoothTime);
...
===============
*/
static void CG_smoothWJTransitions(playerState_t * ps, const vec3_t in,
				   vec3_t out)
{
	int i;
	...
	for (i = MAXSMOOTHS - 1; i >= 0; i--) {
		...
		if (cg.time < cg.sList[i].time + cg_wwSmoothTime.integer) {
			stLocal =
			    ((cg.sList[i].time + cg_wwSmoothTime.integer) -
			     cg.time) / cg_wwSmoothTime.integer;
			sFraction =
			    1.0f - ((cos(stLocal * M_PI * 2.0f) + 1.0f) / 2.0f);
...
static void CG_PlayBufferedSounds(void)
{
	if (cg.soundTime < cg.time) {
		if (cg.soundBufferOut != cg.soundBufferIn
			...
			trap_S_StartLocalSound(cg.
					       soundBuffer[cg.soundBufferOut],
					       CHAN_ANNOUNCER);
			...
			qboolean demoPlayback)
{
	...
	memcpy(cg.refdef.areamask, cg.snap->areamask,
	       sizeof(cg.refdef.areamask));

	//remove expired console lines
	if (cg.consoleLines[0].time + cg_consoleLatency.integer < cg.time
	    && cg_consoleLatency.integer > 0)
		CG_RemoveNotifyLine();
	...
	if (cg_timescale.value != cg_timescaleFadeEnd.value) {
		...
		if (cg_timescaleFadeSpeed.value)
			trap_Cvar_Set("timescale",
				      va("%f", cg_timescale.value));
	}
