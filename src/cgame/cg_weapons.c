		...
		if (!Q_stricmp(token, "missileModel")) {
			...
			if (!wim->missileModel)
				CG_Printf(S_COLOR_RED
					  "ERROR: missile model not found %s\n",
					  token);
			...
			if (!wim->missileSprite)
				CG_Printf(S_COLOR_RED
					  "ERROR: missile sprite not found %s\n",
					  token);
			...
			wim->missileParticleSystem =
			    CG_RegisterParticleSystem(token);

			if (!wim->missileParticleSystem)
				CG_Printf(S_COLOR_RED
					  "ERROR: missile particle system not found %s\n",
					  token);
			...
			if (!wim->missileTrailSystem)
				CG_Printf(S_COLOR_RED
					  "ERROR: missile trail system not found %s\n",
					  token);
			...
			wim->muzzleParticleSystem =
			    CG_RegisterParticleSystem(token);

			if (!wim->muzzleParticleSystem)
				CG_Printf(S_COLOR_RED
					  "ERROR: muzzle particle system not found %s\n",
					  token);
			...
			if (!wim->impactParticleSystem)
				CG_Printf(S_COLOR_RED
					  "ERROR: impact particle system not found %s\n",
					  token);
			...
			if (!wim->impactMark)
				CG_Printf(S_COLOR_RED
					  "ERROR: impact mark shader not found %s\n",
					  token);
			...
			wim->impactSound[index] =
			    trap_S_RegisterSound(token, qfalse);
			...
			CG_Printf(S_COLOR_RED
				  "ERROR: unknown token '%s' in weapon section\n",
				  token);
			return qfalse;
		}
	}
	...
	while (1) {
		...
		if (!Q_stricmp(token, "{")) {
			if (weaponMode == WPM_NONE) {
				CG_Printf(S_COLOR_RED
					  "ERROR: weapon mode section started without a declaration\n");
				return qfalse;
			} else
			    if (!CG_ParseWeaponModeSection
				(&wi->wim[weaponMode], &text_p)) {
				CG_Printf(S_COLOR_RED
					  "ERROR: failed to parse weapon mode section\n");
				return qfalse;
			}
			...
			if (!wi->weaponModel)
				CG_Printf(S_COLOR_RED
					  "ERROR: weapon model not found %s\n",
					  token);
			...
			wi->weaponIcon = wi->ammoIcon =
			    trap_R_RegisterShader(token);

			if (!wi->weaponIcon)
				CG_Printf(S_COLOR_RED
					  "ERROR: weapon icon not found %s\n",
					  token);
			...
			if (!wi->crossHair)
				CG_Printf(S_COLOR_RED
					  "ERROR: weapon crosshair not found %s\n",
					  token);
...
void CG_RegisterWeapon(int weaponNum)
{
	...
	Com_sprintf(path, MAX_QPATH, "models/weapons/%s/weapon.cfg",
		    BG_FindNameForWeapon(weaponNum));
	...
	for (i = 0; i < 3; i++)
		weaponInfo->weaponMidpoint[i] =
		    mins[i] + 0.5 * (maxs[i] - mins[i]);
...
void CG_InitWeapons(void)
{
	...
	cgs.media.level2ZapTS =
	    CG_RegisterTrailSystem("models/weapons/lev2zap/lightning");
}
...
static void CG_CalculateWeaponPosition(vec3_t origin, vec3_t angles)
{
	...
	if (!BG_ClassHasAbility
	    (cg.predictedPlayerState.stats[STAT_PCLASS], SCA_NOWEAPONDRIFT)) {
		delta = cg.time - cg.landTime;
		if (delta < LAND_DEFLECT_TIME)
			origin[2] +=
			    cg.landChange * 0.25 * delta / LAND_DEFLECT_TIME;
...
static float CG_MachinegunSpinAngle(centity_t * cent, qboolean firing)
{
	...
	if (cent->pe.barrelSpinning)
		...
		speed =
		    0.5 * (SPIN_SPEED +
			   (float)(COAST_TIME - delta) / COAST_TIME);
			...
			centity_t * cent)
{
	...
	noGunModel = ((!ps || cg.renderingThirdPerson)
		      && weapon->disableIn3rdPerson) || !gun.hModel;
	...
	if (!noGunModel) {
		...
		if (weapon->barrelModel) {
			...
			VectorCopy(parent->lightingOrigin,
				   barrel.lightingOrigin);
	...
	if (CG_IsParticleSystemValid(&cent->muzzlePS)) {
		if (ps || cg.renderingThirdPerson ||
			...
			if (noGunModel)
				CG_SetAttachmentTag(&cent->muzzlePS->attachment,
						    *parent, parent->hModel,
						    "tag_weapon");
			else
				CG_SetAttachmentTag(&cent->muzzlePS->attachment,
	...
	if (flash.hModel) {
		...
		if (noGunModel)
			CG_PositionRotatedEntityOnTag(&flash, parent,
						      parent->hModel,
						      "tag_weapon");
		else
	...
	if (ps || cg.renderingThirdPerson ||
		...
		if (weapon->wim[weaponMode].muzzleParticleSystem
		    && cent->muzzlePsTrigger) {
			cent->muzzlePS =
			    CG_SpawnNewParticleSystem(weapon->wim[weaponMode].
						      muzzleParticleSystem);

			if (CG_IsParticleSystemValid(&cent->muzzlePS)) {
				if (noGunModel)
					CG_SetAttachmentTag(&cent->muzzlePS->
							    attachment, *parent,
							    parent->hModel,
							    "tag_weapon");
				else
				...
				CG_SetAttachmentCent(&cent->muzzlePS->
						     attachment, cent);
...
void CG_AddViewWeapon(playerState_t * ps)
{
	...
	if ((ps->stats[STAT_BUILDABLE] & ~SB_VALID_TOGGLEBIT) > BA_NONE)
		CG_GhostBuildable(ps->
				  stats[STAT_BUILDABLE] & ~SB_VALID_TOGGLEBIT);

	if (weapon == WP_LUCIFER_CANNON && ps->stats[STAT_MISC] > 0) {
		if (ps->stats[STAT_MISC] >
		    (LCANNON_TOTAL_CHARGE - (LCANNON_TOTAL_CHARGE / 3)))
			trap_S_AddLoopingSound(ps->clientNum, ps->origin,
					       vec3_origin,
	...
	if (!cg_drawGun.integer) {
		...
		if (cent->muzzlePS)
			CG_SetAttachmentPoint(&cent->muzzlePS->attachment,
					      origin);
		...
		if (wi->wim[weaponMode].muzzleParticleSystem
		    && cent->muzzlePsTrigger) {
			cent->muzzlePS =
			    CG_SpawnNewParticleSystem(wi->wim[weaponMode].
						      muzzleParticleSystem);

			if (CG_IsParticleSystemValid(&cent->muzzlePS)) {
				CG_SetAttachmentPoint(&cent->muzzlePS->
						      attachment, origin);
				CG_SetAttachmentCent(&cent->muzzlePS->
						     attachment, cent);
	...
	VectorMA(hand.origin, cg_gun_x.value, cg.refdef.viewaxis[0],
		 hand.origin);
	VectorMA(hand.origin, cg_gun_y.value, cg.refdef.viewaxis[1],
		 hand.origin);
	VectorMA(hand.origin, (cg_gun_z.value + fovOffset),
		 cg.refdef.viewaxis[2], hand.origin);

	if (weapon == WP_LUCIFER_CANNON && ps->stats[STAT_MISC] > 0) {
		float fraction =
		    (float)ps->stats[STAT_MISC] / (float)LCANNON_TOTAL_CHARGE;

		VectorMA(hand.origin, random() * fraction,
	...
	if (cg_gun_frame.integer) {
		...
		hand.oldframe =
		    CG_MapTorsoToWeaponFrame(ci, cent->pe.torso.oldFrame);
...
void CG_DrawItemSelect(rectDef_t * rect, vec4_t color)
{
	...
	if (!(cg.snap->ps.pm_flags & PMF_FOLLOW)) {
		...
		if (cg.weaponSelect <= 32
			...
			CG_NextWeapon_f();
			 ...
			 && !CG_UpgradeSelectable(cg.weaponSelect - 32))
			CG_NextWeapon_f();
	}
	...
	for (i = 0; i < length; i++) {
		...
		if ((item >= 0) && (item < numItems)) {
			...
			if (items[item] <= 32)
				CG_DrawPic(x, y, iconsize, iconsize,
					   cg_weapons[items[item]].weaponIcon);
			else if (items[item] > 32)
				CG_DrawPic(x, y, iconsize, iconsize,
					   cg_upgrades[items[item] -
						       32].upgradeIcon);
...
void CG_DrawItemSelectText(rectDef_t * rect, float scale, int textStyle)
{
	...
	if (cg.weaponSelect <= 32) {
		if (cg_weapons[cg.weaponSelect].registered &&
		    BG_InventoryContainsWeapon(cg.weaponSelect,
					       cg.snap->ps.stats)) {
			if ((name = cg_weapons[cg.weaponSelect].humanName)) {
				w = CG_Text_Width(name, scale, 0);
				x = rect->x + rect->w / 2;
				CG_Text_Paint(x - w / 2, rect->y + rect->h,
					      scale, color, name, 0, 0,
					      textStyle);
			}
		}
	} else if (cg.weaponSelect > 32) {
		if (cg_upgrades[cg.weaponSelect - 32].registered &&
		    BG_InventoryContainsUpgrade(cg.weaponSelect - 32,
						cg.snap->ps.stats)) {
			if ((name =
			     cg_upgrades[cg.weaponSelect - 32].humanName)) {
				w = CG_Text_Width(name, scale, 0);
				x = rect->x + rect->w / 2;
				CG_Text_Paint(x - w / 2, rect->y + rect->h,
					      scale, color, name, 0, 0,
	...
	if (c > 0) {
		...
		if (wi->wim[weaponMode].flashSound[c])
			trap_S_StartSound(NULL, es->number, CHAN_WEAPON,
					  wi->wim[weaponMode].flashSound[c]);
	}
}
...
=================
*/
void CG_MissileHitWall(weapon_t weaponNum, weaponMode_t weaponMode,
		       int clientNum, vec3_t origin, vec3_t dir,
		       impactSound_t soundType)
{
	...
	if (soundType == IMPACTSOUND_FLESH) {
		...
		if (c > 0) {
			c = rand() % c;
			if (weapon->wim[weaponMode].impactFleshSound[c])
				trap_S_StartSound(origin, ENTITYNUM_WORLD,
						  CHAN_AUTO,
						  weapon->wim[weaponMode].
						  impactFleshSound[c]);
		}
			 ...
			 vec3_t origin, vec3_t dir, int entityNum)
{
	...
	if (weapon->wim[weaponMode].alwaysImpact)
		CG_MissileHitWall(weaponNum, weaponMode, 0, origin, dir,
				  IMPACTSOUND_FLESH);
}
	       ...
	       int fleshEntityNum)
{
	...
	if (flesh)
		...
		CG_MissileHitWall(WP_MACHINEGUN, WPM_PRIMARY, 0, end, normal,
				  IMPACTSOUND_DEFAULT);
}
			      ...
			      int otherEntNum)
{
	...
	for (i = 0; i < SHOTGUN_PELLETS; i++) {
		...
		if (!(tr.surfaceFlags & SURF_NOIMPACT)) {
			if (cg_entities[tr.entityNum].currentState.eType ==
			    ET_PLAYER)
				CG_MissileHitPlayer(WP_SHOTGUN, WPM_PRIMARY,
						    tr.endpos, tr.plane.normal,
						    tr.entityNum);
			else if (tr.surfaceFlags & SURF_METALSTEPS)
				CG_MissileHitWall(WP_SHOTGUN, WPM_PRIMARY, 0,
						  tr.endpos, tr.plane.normal,
						  IMPACTSOUND_METAL);
			else
				CG_MissileHitWall(WP_SHOTGUN, WPM_PRIMARY, 0,
