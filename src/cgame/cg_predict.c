				  ...
				  traceType_t collisionType)
{
	...
	for (i = 0; i < j; i++) {
		...
		if (ent->solid == SOLID_BMODEL) {
			...
			if (i == cg_numSolidEntities)
				BG_FindBBoxForClass((ent->misc >> 8) & 0xFF,
						    bmins, bmaxs, NULL, NULL,
						    NULL);
...
int CG_PointContents(const vec3_t point, int passEntityNum)
{
	...
	for (i = 0; i < cg_numSolidEntities; i++) {
		...
		contents |=
		    trap_CM_TransformedPointContents(point, cmodel, ent->origin,
						     ent->angles);
	}
...
static void CG_InterpolatePlayerState(qboolean grabAngles)
{
	...
	*out = cg.snap->ps;
	...
	f = (float)(cg.time - prev->serverTime) / (next->serverTime -
						   prev->serverTime);
	...
	for (i = 0; i < 3; i++) {
		out->origin[i] =
		    prev->ps.origin[i] + f * (next->ps.origin[i] -
					      prev->ps.origin[i]);

		if (!grabAngles)
			out->viewangles[i] =
			    LerpAngle(prev->ps.viewangles[i],
				      next->ps.viewangles[i], f);
...
static int CG_IsUnacceptableError(playerState_t * ps, playerState_t * pps)
{
	...
	if (pps->pm_type != ps->pm_type ||
	    pps->pm_flags != ps->pm_flags || pps->pm_time != ps->pm_time) {
		return 1;
	}
	...
	if (pps->clientNum != ps->clientNum ||
	    pps->weapon != ps->weapon || pps->weaponstate != ps->weaponstate) {
		return 11;
	}
...
void CG_PredictPlayerState(void)
{
	...
	for (cmdNum = current - CMD_BACKUP + 1; cmdNum <= current; cmdNum++) {
			...
			if (cg.thisFrameTeleport) {
				...
				if (len > 0.1) {
					...
					if (cg_errorDecay.integer) {
						...
						f = (cg_errorDecay.value -
						     t) / cg_errorDecay.value;
		...
		if (!cg_optimizePrediction.integer) {
			Pmove(&cg_pmove);
		} else if (cg_optimizePrediction.integer
			   && (cmdNum >= predictCmd
			       || (stateIndex + 1) % NUM_SAVED_STATES ==
			       cg.stateHead)) {
			Pmove(&cg_pmove);
