...
===============
*/
static particle_t *CG_SpawnNewParticle(baseParticle_t * bp,
				       particleEjector_t * parent)
{
	...
	for (i = 0; i < MAX_PARTICLES; i++) {
		...
		if (!p->valid && cg.clientFrame > p->frameWhenInvalidated + 1) {
			...
			p->birthTime = cg.time;
			p->lifeTime =
			    (int)CG_RandomiseValue((float)bp->lifeTime,
						   bp->lifeTimeRandFrac);

			p->radius.delay =
			    (int)CG_RandomiseValue((float)bp->radius.delay,
						   bp->radius.delayRandFrac);
			p->radius.initial =
			    CG_RandomiseValue(bp->radius.initial,
					      bp->radius.initialRandFrac);
			p->radius.final =
			    CG_RandomiseValue(bp->radius.final,
					      bp->radius.finalRandFrac);

			p->alpha.delay =
			    (int)CG_RandomiseValue((float)bp->alpha.delay,
						   bp->alpha.delayRandFrac);
			p->alpha.initial =
			    CG_RandomiseValue(bp->alpha.initial,
					      bp->alpha.initialRandFrac);
			p->alpha.final =
			    CG_RandomiseValue(bp->alpha.final,
					      bp->alpha.finalRandFrac);

			p->rotation.delay =
			    (int)CG_RandomiseValue((float)bp->rotation.delay,
						   bp->rotation.delayRandFrac);
			p->rotation.initial =
			    CG_RandomiseValue(bp->rotation.initial,
					      bp->rotation.initialRandFrac);
			p->rotation.final =
			    CG_RandomiseValue(bp->rotation.final,
					      bp->rotation.finalRandFrac);

			p->dLightRadius.delay =
			    (int)CG_RandomiseValue((float)bp->dLightRadius.
						   delay,
						   bp->dLightRadius.
						   delayRandFrac);
			p->dLightRadius.initial =
			    CG_RandomiseValue(bp->dLightRadius.initial,
					      bp->dLightRadius.initialRandFrac);
			p->dLightRadius.final =
			    CG_RandomiseValue(bp->dLightRadius.final,
					      bp->dLightRadius.finalRandFrac);

			p->colorDelay =
			    CG_RandomiseValue(bp->colorDelay,
					      bp->colorDelayRandFrac);

			p->bounceMarkRadius =
			    CG_RandomiseValue(bp->bounceMarkRadius,
					      bp->bounceMarkRadiusRandFrac);
			p->bounceMarkCount =
			    rint(CG_RandomiseValue
				 ((float)bp->bounceMarkCount,
				  bp->bounceMarkCountRandFrac));
			p->bounceSoundCount =
			    rint(CG_RandomiseValue
				 ((float)bp->bounceSoundCount,
			...
			if (bp->numModels) {
				...
				if (bp->modelAnimation.frameLerp < 0) {
					bp->modelAnimation.frameLerp =
					    p->lifeTime /
					    bp->modelAnimation.numFrames;
					bp->modelAnimation.initialLerp =
					    p->lifeTime /
					    bp->modelAnimation.numFrames;
				}
			}

			if (!CG_AttachmentPoint
			    (&ps->attachment, attachmentPoint))
				return NULL;
			...
			if (CG_AttachmentAxis(&ps->attachment, transform)) {
				...
				VectorMatrixMultiply(bp->displacement,
						     transform,
						     transDisplacement);
				VectorAdd(p->origin, transDisplacement,
					  p->origin);
			} else
				VectorAdd(p->origin, bp->displacement,
					  p->origin);

			for (j = 0; j <= 2; j++)
				p->origin[j] +=
				    (crandom() * bp->randDisplacement);

			switch (bp->velMoveType) {
			case PMT_STATIC:
				if (bp->velMoveValues.dirType == PMD_POINT)
					VectorSubtract(bp->velMoveValues.point,
						       p->origin, p->velocity);
				else if (bp->velMoveValues.dirType ==
					 PMD_LINEAR)
					VectorCopy(bp->velMoveValues.dir,
						   p->velocity);
				break;

			case PMT_STATIC_TRANSFORM:
				if (!CG_AttachmentAxis
				    (&ps->attachment, transform))
					return NULL;

				if (bp->velMoveValues.dirType == PMD_POINT) {
					vec3_t transPoint;

					VectorMatrixMultiply(bp->velMoveValues.
							     point, transform,
							     transPoint);
					VectorSubtract(transPoint, p->origin,
						       p->velocity);
				} else if (bp->velMoveValues.dirType ==
					   PMD_LINEAR)
					VectorMatrixMultiply(bp->velMoveValues.
			...
			case PMT_CENT_ANGLES:
				if (bp->velMoveValues.dirType == PMD_POINT)
					VectorSubtract(attachmentPoint,
						       p->origin, p->velocity);
				else if (bp->velMoveValues.dirType ==
					 PMD_LINEAR) {
					if (!CG_AttachmentDir
					    (&ps->attachment, p->velocity))
						return NULL;
				}
				break;
			...
			case PMT_NORMAL:
				if (!ps->normalValid) {
					CG_Printf(S_COLOR_RED
						  "ERROR: a particle with velocityType "
				...
				VectorMA(p->origin, bp->normalDisplacement,
					 p->velocity, p->origin);
				break;
			}

			VectorNormalize(p->velocity);
			CG_SpreadVector(p->velocity,
					bp->velMoveValues.dirRandAngle);
			VectorScale(p->velocity,
				    CG_RandomiseValue(bp->velMoveValues.mag,
						      bp->velMoveValues.
						      magRandFrac),
				    p->velocity);

			if (CG_AttachmentVelocity
			    (&ps->attachment, attachmentVelocity)) {
				VectorMA(p->velocity,
					 CG_RandomiseValue(bp->velMoveValues.
							   parentVelFrac,
							   bp->velMoveValues.
							   parentVelFracRandFrac),
					 attachmentVelocity, p->velocity);
			}
			...
			if (bp->childSystemName[0] != '\0') {
				particleSystem_t *ps =
				    CG_SpawnNewParticleSystem(bp->
							      childSystemHandle);

				if (CG_IsParticleSystemValid(&ps)) {
					CG_SetAttachmentParticle(&ps->
								 attachment, p);
			...
			if (bp->childTrailSystemName[0] != '\0') {
				trailSystem_t *ts =
				    CG_SpawnNewTrailSystem(bp->
							   childTrailSystemHandle);

				if (CG_IsTrailSystemValid(&ts)) {
					CG_SetAttachmentParticle(&ts->
								 frontAttachment,
								 p);
					CG_AttachToParticle(&ts->
							    frontAttachment);
				}
			}
			...
			if (!ps->lazyRemove) {
				while (pe->nextEjectionTime <= cg.time &&
					...
					lerpFrac =
					    1.0 -
					    ((float)pe->count /
					     (float)pe->totalParticles);
					pe->nextEjectionTime =
					    cg.time +
					    (int)
					    CG_RandomiseValue(CG_LerpValues
							      (pe->ejectPeriod.
							       initial,
							       pe->ejectPeriod.
							       final, lerpFrac),
...
===============
*/
static particleEjector_t *CG_SpawnNewParticleEjector(baseParticleEjector_t *
						     bpe,
	...
	for (i = 0; i < MAX_PARTICLE_EJECTORS; i++) {
		...
		if (!pe->valid) {
			...
			pe->nextEjectionTime = cg.time +
			    (int)CG_RandomiseValue((float)bpe->eject.delay,
						   bpe->eject.delayRandFrac);
			pe->count = pe->totalParticles =
			    (int)
			    rint(CG_RandomiseValue
				 ((float)bpe->totalParticles,
				  bpe->totalParticlesRandFrac));
	...
	for (i = 0; i < MAX_PARTICLE_SYSTEMS; i++) {
		...
		if (!ps->valid) {
			...
			for (j = 0; j < bps->numEjectors; j++)
				CG_SpawnNewParticleEjector(bps->ejectors[j],
							   ps);
			...
			for (j = 0; j < bps->numEjectors; j++) {
				...
				for (l = 0; l < bpe->numParticles; l++) {
					...
					for (k = 0; k < bp->numFrames; k++)
						bp->shaders[k] =
						    trap_R_RegisterShader(bp->
									  shaderNames
									  [k]);

					for (k = 0; k < bp->numModels; k++)
						bp->models[k] =
						    trap_R_RegisterModel(bp->
									 modelNames
									 [k]);
					...
					if (bp->childTrailSystemName[0] != '\0')
						bp->childTrailSystemHandle =
						    CG_RegisterTrailSystem(bp->
									   childTrailSystemName);
				}
			}
				     ...
				     qboolean allowNegative)
{
	...
	if (variancePtr) {
		...
		if (varEndPointer) {
			...
			if (localValue != 0.0f)
				localVariance =
				    atof_neg(variancePtr, qfalse) / localValue;
			else
	...
	while (1) {
		...
		if (!Q_stricmp(token, "bounce")) {
			...
			if (!Q_stricmp(token, "cull")) {
				...
				CG_ParseValueAndVariance(token, &number,
							 &randFrac, qfalse);
			...
			while (*token && bp->numFrames < MAX_PS_SHADER_FRAMES) {
				Q_strncpyz(bp->shaderNames[bp->numFrames++],
					   token, MAX_QPATH);
				token = COM_ParseExt(text_p, qfalse);
			}
			...
			while (*token && bp->numModels < MAX_PS_MODELS) {
				Q_strncpyz(bp->modelNames[bp->numModels++],
					   token, MAX_QPATH);
				token = COM_ParseExt(text_p, qfalse);
			}
		...
		///
			...
			for (i = 0; i <= 2; i++) {
				...
				bp->velMoveValues.dir[i] =
				    atof_neg(token, qtrue);
			}
		...
		///
			...
			if (!Q_stricmp(token, "-")) {
				bp->dLightRadius.final =
				    PARTICLES_SAME_AS_INITIAL;
			...
			CG_Printf(S_COLOR_RED
				  "ERROR: unknown token '%s' in particle\n",
				  token);
			return qfalse;
		}
	}
...
===============
*/
static qboolean CG_ParseParticleEjector(baseParticleEjector_t * bpe,
					char **text_p)
{
	char *token;
	float number, randFrac;
	...
	while (1) {
		...
		if (!Q_stricmp(token, "{")) {
			CG_InitialiseBaseParticle(&baseParticles
						  [numBaseParticles]);

			if (!CG_ParseParticle
			    (&baseParticles[numBaseParticles], text_p)) {
				CG_Printf(S_COLOR_RED
					  "ERROR: failed to parse particle\n");
				return qfalse;
			}

			if (bpe->numParticles == MAX_PARTICLES_PER_EJECTOR) {
				CG_Printf(S_COLOR_RED
					  "ERROR: ejector has > %d particles\n",
					  MAX_PARTICLES_PER_EJECTOR);
				return qfalse;
			} else if (numBaseParticles == MAX_BASEPARTICLES) {
				CG_Printf(S_COLOR_RED
					  "ERROR: maximum number of particles (%d) reached\n",
					  MAX_BASEPARTICLES);
				return qfalse;
			} else {
				//start parsing particles again
				bpe->particles[bpe->numParticles] =
				    &baseParticles[numBaseParticles];
				bpe->numParticles++;
				numBaseParticles++;
			}
			continue;
			...
			CG_ParseValueAndVariance(token, NULL,
						 &bpe->eject.randFrac, qfalse);
			...
			CG_Printf(S_COLOR_RED
				  "ERROR: unknown token '%s' in particle ejector\n",
				  token);
			return qfalse;
		}
	}
...
===============
*/
static qboolean CG_ParseParticleSystem(baseParticleSystem_t * bps,
				       char **text_p, const char *name)
{
	char *token;
	baseParticleEjector_t *bpe;
	...
	while (1) {
		...
		if (!Q_stricmp(token, "{")) {
			if (!CG_ParseParticleEjector
			    (&baseParticleEjectors[numBaseParticleEjectors],
			     text_p)) {
				CG_Printf(S_COLOR_RED
					  "ERROR: failed to parse particle ejector\n");
				return qfalse;
			}
			...
			if (bpe->totalParticles == PARTICLES_INFINITE &&
			    (bpe->eject.initial == 0.0f
			     || bpe->eject.final == 0.0f)) {
				CG_Printf(S_COLOR_RED
			...
			if (bps->numEjectors == MAX_EJECTORS_PER_SYSTEM) {
				CG_Printf(S_COLOR_RED
					  "ERROR: particle system has > %d ejectors\n",
					  MAX_EJECTORS_PER_SYSTEM);
				return qfalse;
			} else if (numBaseParticleEjectors ==
			...
			CG_Printf(S_COLOR_RED
				  "ERROR: unknown token '%s' in particle system %s\n",
				  token, bps->name);
			return qfalse;
		}
	}
...
static qboolean CG_ParseParticleFile(const char *fileName)
{
	...
	while (1) {
		...
		if (!Q_stricmp(token, "{")) {
			if (psNameSet) {
				...
				for (i = 0; i < numBaseParticleSystems; i++) {
					if (!Q_stricmp
					    (baseParticleSystems[i].name,
					     psName)) {
						CG_Printf(S_COLOR_RED
							  "ERROR: a particle system is already named %s\n",
							  psName);
						return qfalse;
					}
				}

				Q_strncpyz(baseParticleSystems
					   [numBaseParticleSystems].name,
					   psName, MAX_QPATH);

				if (!CG_ParseParticleSystem
				    (&baseParticleSystems
				     [numBaseParticleSystems], &text_p,
				     psName)) {
					CG_Printf(S_COLOR_RED
						  "ERROR: %s: failed to parse particle system %s\n",
						  fileName, psName);
					return qfalse;
				}
		...
		if (!psNameSet) {
			...
			CG_Printf(S_COLOR_RED
				  "ERROR: particle system already named\n");
			return qfalse;
		}
	}
	...
	for (i = 0; i < numBaseParticles; i++) {
		...
		if (bp->childSystemName[0]) {
			...
			if (j == numBaseParticleSystems) {
				...
				CG_Printf(S_COLOR_YELLOW
					  "WARNING: failed to find child %s\n",
					  bp->childSystemName);
		...
		if (bp->onDeathSystemName[0]) {
			...
			if (j == numBaseParticleSystems) {
				//couldn't find named particle system
				CG_Printf(S_COLOR_YELLOW
					  "WARNING: failed to find onDeath system %s\n",
...
void CG_SetParticleSystemNormal(particleSystem_t * ps, vec3_t normal)
{
	if (ps == NULL || !ps->valid) {
		CG_Printf(S_COLOR_YELLOW
			  "WARNING: tried to modify a NULL particle system\n");
		return;
	}
...
void CG_DestroyParticleSystem(particleSystem_t ** ps)
{
	...
	if (*ps == NULL || !(*ps)->valid) {
		CG_Printf(S_COLOR_YELLOW
			  "WARNING: tried to destroy a NULL particle system\n");
		return;
	}
...
qboolean CG_IsParticleSystemInfinite(particleSystem_t * ps)
{
	...
	if (ps == NULL) {
		CG_Printf(S_COLOR_YELLOW
			  "WARNING: tried to test a NULL particle system\n");
		return qfalse;
	}
...
static float CG_CalculateTimeFrac(int birth, int life, int delay)
{
	...
	frac =
	    ((float)cg.time - (float)(birth + delay)) / (float)(life - delay);
...
static void CG_EvaluateParticlePhysics(particle_t * p)
{
	...
	case PMT_STATIC:
		if (bp->accMoveValues.dirType == PMD_POINT)
			VectorSubtract(bp->accMoveValues.point, p->origin,
				       acceleration);
	...
	case PMT_STATIC_TRANSFORM:
		...
		if (bp->accMoveValues.dirType == PMD_POINT) {
			...
			VectorMatrixMultiply(bp->accMoveValues.point, transform,
					     transPoint);
			VectorSubtract(transPoint, p->origin, acceleration);
		} else if (bp->accMoveValues.dirType == PMD_LINEAR)
			VectorMatrixMultiply(bp->accMoveValues.dir, transform,
					     acceleration);
		break;
...
#define MAX_ACC_RADIUS 1000.0f
	...
	if (bp->accMoveValues.dirType == PMD_POINT) {
		...
		scale *=
		    CG_RandomiseValue(bp->accMoveValues.mag,
	...
	if (bp->bounceMarkName[0] && p->bounceMarkCount > 0) {
		CG_ImpactMark(bp->bounceMark, trace.endpos, trace.plane.normal,
			      random() * 360, 1, 1, 1, 1, qtrue,
			      bp->bounceMarkRadius, qfalse);
		p->bounceMarkCount--;
	}
...
===============
*/
static void CG_Radix(int bits, int size, particle_t ** source,
		     particle_t ** dest)
{
	int count[256];
	int index[256];
...
static void CG_CompactAndSortParticles(void)
{
	...
	for (i = 0; i < numParticles; i++) {
		VectorSubtract(sortedParticles[i]->origin, cg.refdef.vieworg,
			       delta);
...
static void CG_RenderParticle(particle_t * p)
{
	...
	if (bp->numFrames)	//shader based
	{
		...
		if (bp->realLight) {
			trap_R_LightForPoint(p->origin, alight, dlight,
					     lightdir);
			for (i = 0; i <= 2; i++)
		...
		if (bp->framerate == 0.0f) {
			...
			//looping animation
			index =
			    (int)(bp->framerate * timeFrac * p->lifeTime *
				  0.001) % bp->numFrames;
			re.customShader = bp->shaders[index];
		}
	...
	} else if (bp->numModels)	//model based
	{
		...
		if (p->atRest)
			...
			if (re.axis[0][0] == 0.0f && re.axis[0][1] == 0.0f)
				...
				CrossProduct(re.axis[2], re.axis[0],
					     re.axis[1]);
			}
	...
	if (bp->dynamicLight && !(re.renderfx & RF_THIRD_PERSON)) {
		trap_R_AddLightToScene(p->origin,
				       CG_LerpValues(p->dLightRadius.initial,
						     p->dLightRadius.final,
						     CG_CalculateTimeFrac(p->
									  birthTime,
									  p->
									  lifeTime,
									  p->
...
void CG_ParticleSystemEntity(centity_t * cent)
{
	...
	if (es->eFlags & EF_NODRAW) {
		if (CG_IsParticleSystemValid(&cent->entityPS)
		    && CG_IsParticleSystemInfinite(cent->entityPS))
	...
	if (!CG_IsParticleSystemValid(&cent->entityPS)
	    && !cent->entityPSMissing) {
		cent->entityPS =
		    CG_SpawnNewParticleSystem(cgs.
					      gameParticleSystems[es->
								  modelindex]);

		if (CG_IsParticleSystemValid(&cent->entityPS)) {
			CG_SetAttachmentPoint(&cent->entityPS->attachment,
					      cent->lerpOrigin);
