...
{
	return syscall(CG_CM_TRANSFORMEDPOINTCONTENTS, p, model, origin,
		       angles);
}
...
void trap_CM_TransformedBiSphereTrace(trace_t * results, const vec3_t start,
				      const vec3_t end, float startRad,
				      float endRad, clipHandle_t model,
				      int mask, const vec3_t origin)
{
	syscall(CG_CM_TRANSFORMEDBISPHERETRACE, results, start, end,
		PASSFLOAT(startRad), PASSFLOAT(endRad), model, mask, origin);
}
...
void trap_R_AddLightToScene(const vec3_t org, float intensity, float r, float g,
			    float b)
{
	syscall(CG_R_ADDLIGHTTOSCENE, org, PASSFLOAT(intensity), PASSFLOAT(r),
		PASSFLOAT(g), PASSFLOAT(b));
}

void trap_R_AddAdditiveLightToScene(const vec3_t org, float intensity, float r,
				    float g, float b)
{
	syscall(CG_R_ADDADDITIVELIGHTTOSCENE, org, PASSFLOAT(intensity),
...
void trap_R_DrawStretchPic(float x, float y, float w, float h,
			   ...
			   qhandle_t hShader)
{
	syscall(CG_R_DRAWSTRETCHPIC, PASSFLOAT(x), PASSFLOAT(y), PASSFLOAT(w),
...
int trap_R_LerpTag(orientation_t * tag, clipHandle_t mod, int startFrame,
...
qboolean trap_GetSnapshot(int snapshotNumber,
			  moduleAlternateSnapshot_t * snapshot)
#else
