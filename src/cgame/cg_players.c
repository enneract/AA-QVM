		...
		for (i = 0; i < MAX_PLAYER_ANIMATIONS; i++) {
			...
			if (!*token) {
				if (i >= TORSO_GETFLAG && i <= TORSO_NEGATIVE) {
					animations[i].firstFrame =
					    animations[TORSO_GESTURE].
					    firstFrame;
					animations[i].frameLerp =
					    animations[TORSO_GESTURE].frameLerp;
				      ...
				      const char *skinName)
{
	...
	if (!ci->nonsegmented) {
		...
		Com_sprintf(filename, sizeof(filename),
			    "models/players/%s/head_%s.skin", modelName,
			    skinName);
		...
		Com_sprintf(filename, sizeof(filename),
			    "models/players/%s/nonseg_%s.skin", modelName,
			    skinName);
...
==========================
*/
static qboolean CG_RegisterClientModelname(clientInfo_t * ci,
...
static void CG_LoadClientInfo(clientInfo_t * ci)
{
	...
	if (!CG_RegisterClientModelname(ci, ci->modelName, ci->skinName)) {
		if (cg_buildScript.integer)
			CG_Error("CG_RegisterClientModelname( %s, %s ) failed",
	...
	for (i = 0; i < MAX_CUSTOM_SOUNDS; i++) {
		...
		if (!CG_FileExists(va("sound/player/%s/%s", dir, s + 1))) {
			...
			{
				ci->sounds[i] =
				    trap_S_RegisterSound("sound/null.wav",
							 qfalse);
			} else {
				if (i == 9)	//gasp
	...
	if (ci->footsteps == FOOTSTEP_CUSTOM) {
		for (i = 0; i < 4; i++) {
			ci->customFootsteps[i] =
			    trap_S_RegisterSound(va
						 ("sound/player/%s/step%d.wav",
						  dir, i + 1), qfalse);
			if (!ci->customFootsteps[i])
				ci->customFootsteps[i] =
				    trap_S_RegisterSound(va
							 ("sound/player/footsteps/step%d.wav",
							  i + 1), qfalse);

			ci->customMetalFootsteps[i] =
			    trap_S_RegisterSound(va
						 ("sound/player/%s/clank%d.wav",
						  dir, i + 1), qfalse);
			if (!ci->customMetalFootsteps[i])
				ci->customMetalFootsteps[i] =
				    trap_S_RegisterSound(va
							 ("sound/player/footsteps/clank%d.wav",
							  i + 1), qfalse);
		}
	}
...
void CG_PrecacheClientInfo(pClass_t class, char *model, char *skin)
{
	...
	if (!skin) {
		...
		Q_strncpyz(newInfo.headSkinName, "default",
			   sizeof(newInfo.headSkinName));
	} else {
		Q_strncpyz(newInfo.skinName, skin, sizeof(newInfo.skinName));
		Q_strncpyz(newInfo.headSkinName, skin,
			   sizeof(newInfo.headSkinName));
	}
...
void CG_NewClientInfo(int clientNum)
{
	...
	if (!slash) {
		...
		Q_strncpyz(newInfo.headSkinName, slash + 1,
			   sizeof(newInfo.headSkinName));
		// truncate modelName
		*slash = 0;
	}
...
===============
*/
static void CG_SetLerpFrameAnimation(clientInfo_t * ci, lerpFrame_t * lf,
				     int newAnimation)
{
	animation_t *anim;
...
===============
*/
static void CG_RunPlayerLerpFrame(clientInfo_t * ci, lerpFrame_t * lf,
				  int newAnimation, float speedScale)
{
	int f, numFrames;
	animation_t *anim;
	...
	if (cg.time >= lf->frameTime) {
		...
		if (anim->reversed)
			...
			lf->frame =
			    anim->firstFrame + anim->numFrames - 1 -
			    (f % anim->numFrames);
		else
	...
	if (lf->frameTime == lf->oldFrameTime)
		...
		lf->backlerp =
		    1.0 - (float)(cg.time - lf->oldFrameTime) / (lf->frameTime -
								 lf->
								 oldFrameTime);
}
...
===============
*/
static void CG_ClearLerpFrame(clientInfo_t * ci, lerpFrame_t * lf,
			      int animationNumber)
{
...
static void CG_PlayerAnimation(centity_t * cent, int *legsOld, int *legs,
			       float *legsBackLerp, int *torsoOld, int *torso,
			       float *torsoBackLerp)
{
	clientInfo_t *ci;
	int clientNum;
	...
	if (cent->pe.legs.yawing
		...
		CG_RunPlayerLerpFrame(ci, &cent->pe.legs, LEGS_TURN,
				      speedScale);
	else
		CG_RunPlayerLerpFrame(ci, &cent->pe.legs,
				      cent->currentState.legsAnim, speedScale);

	*legsOld = cent->pe.legs.oldFrame;
	*legs = cent->pe.legs.frame;
	*legsBackLerp = cent->pe.legs.backlerp;

	CG_RunPlayerLerpFrame(ci, &cent->pe.torso, cent->currentState.torsoAnim,
			      speedScale);
				     ...
				     int *nonSeg, float *nonSegBackLerp)
{
	...
	if (cent->pe.nonseg.yawing
		...
		CG_RunPlayerLerpFrame(ci, &cent->pe.nonseg, NSPA_TURN,
				      speedScale);
	else
		CG_RunPlayerLerpFrame(ci, &cent->pe.nonseg,
				      cent->currentState.legsAnim, speedScale);
...
==================
*/
static void CG_SwingAngles(float destination, float swingTolerance,
			    ...
			    vec3_t legs[3], vec3_t torso[3], vec3_t head[3])
{
	...
	CG_SwingAngles(dest, 15, 30, 0.1f, &cent->pe.torso.pitchAngle,
...
static void CG_PlayerWWSmoothing(centity_t * cent, vec3_t in[3], vec3_t out[3])
{
	...
	if (!VectorCompare(surfNormal, cent->pe.lastNormal)) {
		...
		for (i = 0; i < MAXSMOOTHS; i++) {
			...
			if (cent->pe.sList[i].time + MODEL_WWSMOOTHTIME <
			    cg.time) {
	...
	for (i = MAXSMOOTHS - 1; i >= 0; i--) {
		...
		if (cg.time < cent->pe.sList[i].time + MODEL_WWSMOOTHTIME) {
			stLocal =
			    1.0f -
			    (((cent->pe.sList[i].time + MODEL_WWSMOOTHTIME) -
			      cg.time) / MODEL_WWSMOOTHTIME);
			sFraction = -(cos(stLocal * M_PI) + 1.0f) / 2.0f;

			RotatePointAroundVector(outAxis[0],
						cent->pe.sList[i].rotAxis,
						inAxis[0],
						sFraction *
						cent->pe.sList[i].rotAngle);
			RotatePointAroundVector(outAxis[1],
						cent->pe.sList[i].rotAxis,
						inAxis[1],
						sFraction *
						cent->pe.sList[i].rotAngle);
			RotatePointAroundVector(outAxis[2],
						cent->pe.sList[i].rotAxis,
						inAxis[2],
						sFraction *
						cent->pe.sList[i].rotAngle);
...
===============
*/
static void CG_PlayerNonSegAngles(centity_t * cent, vec3_t srcAngles,
				  vec3_t nonSegAxis[3])
{
	vec3_t localAngles;
	vec3_t velocity;
	...
	if (cent->currentState.eFlags & EF_DEAD) {
		CG_SwingAngles(localAngles[YAW], 0, 0, cg_swingSpeed.value,
			       &cent->pe.nonseg.yawAngle,
			       &cent->pe.nonseg.yawing);
	} else {
...
static void CG_PlayerUpgrades(centity_t * cent, refEntity_t * torso)
{
	...
	if (held & (1 << UP_JETPACK)) {
		...
		CG_PositionRotatedEntityOnTag(&jetpack, torso, torso->hModel,
					      "tag_head");
		...
		if (active & (1 << UP_JETPACK)) {
			if (es->pos.trDelta[2] > 10.0f) {
				if (cent->jetPackState != JPS_ASCENDING) {
					if (CG_IsParticleSystemValid
					    (&cent->jetPackPS))
						CG_DestroyParticleSystem(&cent->
									 jetPackPS);

					cent->jetPackPS =
					    CG_SpawnNewParticleSystem(cgs.media.
								      jetPackAscendPS);
					cent->jetPackState = JPS_ASCENDING;
				}

				trap_S_AddLoopingSound(cent->currentState.
						       number, cent->lerpOrigin,
						       vec3_origin,
						       cgs.media.
						       jetpackAscendSound);
			...
			CG_PositionRotatedEntityOnTag(&flash, &jetpack,
						      jetpack.hModel,
						      "tag_flash");
			trap_R_AddRefEntityToScene(&flash);

			if (CG_IsParticleSystemValid(&cent->jetPackPS)) {
				CG_SetAttachmentTag(&cent->jetPackPS->
						    attachment, jetpack,
						    jetpack.hModel,
						    "tag_flash");
				CG_SetAttachmentCent(&cent->jetPackPS->
						     attachment, cent);
	...
	if (held & (1 << UP_BATTPACK)) {
		...
		//FIXME: change to tag_back when it exists
		CG_PositionRotatedEntityOnTag(&battpack, torso, torso->hModel,
					      "tag_head");
	...
	if (es->eFlags & EF_BLOBLOCKED) {
		...
		CG_Trace(&tr, es->pos.trBase, NULL, NULL, temp, es->number,
			 MASK_SOLID);
		...
		if (size > 0.0f)
			CG_ImpactMark(cgs.media.creepShader, origin, up,
				      0.0f, 1.0f, 1.0f, 1.0f, 1.0f, qfalse,
				      size, qtrue);
	}
}
				...
				pClass_t class)
{
	...
	*shadowPlane = 0;
	...
	CG_ImpactMark(cgs.media.shadowMarkShader, trace.endpos,
		      trace.plane.normal, cent->pe.legs.yawAngle, 0.0f, 0.0f,
		      0.0f, alpha, qfalse,
...
static void CG_PlayerSplash(centity_t * cent, pClass_t class)
{
	...
	CG_ImpactMark(cgs.media.wakeMarkShader, trace.endpos,
		      trace.plane.normal, cent->pe.legs.yawAngle, 1.0f, 1.0f,
		      1.0f, 1.0f, qfalse,
...
void CG_Player(centity_t * cent)
{
	...
	if (es->eFlags & EF_WALLCLIMB &&
	    BG_RotateAxis(es->angles2, tempAxis, tempAxis2, qtrue,
	...
	if (es->eFlags & EF_WALLCLIMB &&
	    BG_RotateAxis(es->angles2, legs.axis, tempAxis, qfalse,
			  es->eFlags & EF_WALLCLIMBCEILING))
	...
	if (!ci->nonsegmented)
		CG_PlayerAnimation(cent, &legs.oldframe, &legs.frame,
				   &legs.backlerp, &torso.oldframe,
				   &torso.frame, &torso.backlerp);
	else
		CG_PlayerNonSegAnimation(cent, &legs.oldframe, &legs.frame,
					 &legs.backlerp);
	...
	if (es->eFlags & EF_WALLCLIMB && !(es->eFlags & EF_DEAD)
		...
		CG_CapTrace(&tr, start, mins, maxs, end, es->number,
			    MASK_PLAYERSOLID);

		//if the trace misses completely then just use legs.origin
		//apparently capsule traces are "smaller" than box traces
		if (tr.fraction != 1.0f)
			VectorMA(legs.origin, tr.fraction * -TRACE_DEPTH,
				 surfNormal, legs.origin);
	...
	VectorMA(legs.origin, BG_FindZOffsetForClass(class), surfNormal,
		 legs.origin);
	VectorCopy(legs.origin, legs.lightingOrigin);
	VectorCopy(legs.origin, legs.oldorigin);	// don't positionally lerp at all
	...
	if (!ci->nonsegmented) {
		...
		CG_PositionRotatedEntityOnTag(&torso, &legs, ci->legsModel,
					      "tag_torso");
		...
		CG_PositionRotatedEntityOnTag(&head, &torso, ci->torsoModel,
					      "tag_head");
...
void CG_Corpse(centity_t * cent)
{
	...
	// get the rotation information
	if (!ci->nonsegmented)
		CG_PlayerAngles(cent, cent->lerpAngles, legs.axis, torso.axis,
				head.axis);
	else
