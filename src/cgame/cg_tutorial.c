			...
			if (bindings[i].keys[0] != K_NONE) {
				...
				if (bindings[i].keys[1] != K_NONE) {
					Q_strcat(buffer, MAX_STRING_CHARS,
						 " or ");
					trap_Key_KeynumToStringBuf(bindings[i].
								   keys[1],
								   buffer +
								   strlen
								   (buffer),
					...
					for (j = firstKeyLength + 4;
					     j < strlen(buffer); j++)
						buffer[j] = toupper(buffer[j]);
				}
			} else {
				Q_strncpyz(buffer,
					   va("\"%s\"", bindings[i].humanName),
					   MAX_STRING_CHARS);
				Q_strcat(buffer, MAX_STRING_CHARS,
					 " (unbound)");
			}
	...
	if (buildable > BA_NONE) {
		...
		if ((es = CG_BuildableInRange(ps, NULL))) {
			if (cgs.markDeconstruct) {
				...
				Q_strcat(text, MAX_TUTORIAL_TEXT,
					 va
					 ("Press %s to destroy this structure\n",
	...
	if (buildable > BA_NONE) {
		...
		if (CG_BuildableInRange(ps, &health)) {
			if (health < 1.0f) {
				Q_strcat(text, MAX_TUTORIAL_TEXT,
					 va
					 ("Hold %s to repair this structure\n",
					  CG_KeyNameForCommand("+button5")));
			}
...
static void CG_HumanText(char *text, playerState_t * ps)
{
	...
	if (!ammo && !clips && !BG_FindInfinteAmmoForWeapon(ps->weapon)) {
		...
		case WP_FLAMER:
			Q_strcat(text, MAX_TUTORIAL_TEXT,
				 va
				 ("Find an Armoury and press %s for more ammo\n",
		...
		case WP_LUCIFER_CANNON:
			Q_strcat(text, MAX_TUTORIAL_TEXT,
				 va
				 ("Find a Reactor or Repeater and press %s for more ammo\n",
				  CG_KeyNameForCommand("buy ammo")));
			break;
		...
		case WP_LUCIFER_CANNON:
			Q_strcat(text, MAX_TUTORIAL_TEXT,
				 va
				 ("Hold and release %s to fire a charged shot\n",
				  CG_KeyNameForCommand("+attack")));
	...
	Q_strcat(text, MAX_TUTORIAL_TEXT,
		 va("Press %s and ", CG_KeyNameForCommand("weapprev")));
	Q_strcat(text, MAX_TUTORIAL_TEXT,
	...
	if (upgrade == UP_NONE ||
		...
		Q_strcat(text, MAX_TUTORIAL_TEXT,
			 va("Press %s to use the %s\n",
			    CG_KeyNameForCommand("+button2"), name));
	}
