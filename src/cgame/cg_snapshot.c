...
static void CG_TransitionSnapshot(void)
{
	...
	BG_PlayerStateToEntityState(&cg.snap->ps,
...
static void CG_SetNextSnap(snapshot_t * snap)
{
	...
	BG_PlayerStateToEntityState(&snap->ps,
				    &cg_entities[snap->ps.clientNum].nextState,
				    qfalse);
	...
	for (num = 0; num < snap->numEntities; num++) {
		...
		if (!cent->currentValid
		    || ((cent->currentState.eFlags ^ es->eFlags) &
			EF_TELEPORT_BIT))
			cent->interpolate = qfalse;
		else
	...
	if (cg.snap
	    && ((snap->ps.eFlags ^ cg.snap->ps.eFlags) & EF_TELEPORT_BIT))
	...
	if ((cg.nextSnap->snapFlags ^ cg.snap->
	     snapFlags) & SNAPFLAG_SERVERCOUNT)
		cg.nextFrameTeleport = qtrue;
...
static qboolean CG_GetModuleAlternateSnapshot(int snapshotNumber,
					      snapshot_t * snap)
{
...
#define PSFO(x) ((size_t)&(((playerState_t*)0)->x))
		memcpy(&snap->ps.commandTime, &alt->ps.commandTime,
		       PSFO(tauntTimer) - PSFO(commandTime));
		memcpy(&snap->ps.movementDir, &alt->ps.movementDir,
		       PSFO(ammo) - PSFO(movementDir));
		memcpy(&snap->ps.generic1, &alt->ps.generic1,
		       PSFO(entityEventSequence) - PSFO(generic1));
		...
		for (i = 0; i < alt->numEntities; ++i) {
			...
			const moduleAlternateEntityState_t *a =
			    &alt->entities[i];

#define ESFO(x) ((size_t)&(((entityState_t*)0)->x))
			memcpy(&s->number, &a->number,
			       ESFO(weaponAnim) - ESFO(number));
...
#ifdef MODULE_INTERFACE_11
		r = CG_GetModuleAlternateSnapshot(cgs.processedSnapshotNum,
						  dest);
#else
