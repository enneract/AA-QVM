...
static void CG_EntityEffects(centity_t * cent)
{
	...
	if (CG_IsTrailSystemValid(&cent->muzzleTS)) {
		...
		if (cent->currentState.eType == ET_BUILDABLE) {
			...
			CG_AttachmentPoint(&cent->muzzleTS->frontAttachment,
					   front);
			CG_AttachmentPoint(&cent->muzzleTS->backAttachment,
					   back);
		...
		if (cg.time > cent->muzzleTSDeathTime
		    && CG_IsTrailSystemValid(&cent->muzzleTS))
			CG_DestroyTrailSystem(&cent->muzzleTS);
	}
}
...
static void CG_Speaker(centity_t * cent)
{
	...
	cent->miscTime =
	    cg.time + cent->currentState.frame * 100 +
...
static void CG_LaunchMissile(centity_t * cent)
{
	...
	if (wi->wim[weaponMode].missileParticleSystem) {
		ps = CG_SpawnNewParticleSystem(wi->wim[weaponMode].
	...
	if (wi->wim[weaponMode].missileTrailSystem) {
		ts = CG_SpawnNewTrailSystem(wi->wim[weaponMode].
					    missileTrailSystem);
...
static void CG_LightFlare(centity_t * cent)
{
	...
	if (!(cg.clientFrame % 2)) {
		...
		if (maxAngle > 0.0f) {
			float radiusMod =
			    1.0f - (180.0f -
				    RAD2DEG(acos(DotProduct(delta, forward)))) /
			    maxAngle;
		...
		if (cg_lightFlare.integer == FLARE_REALFADE) {
			...
			} else if ((tr.fraction == 1.0f && !tr.startsolid)
				   && !cent->lfs.status) {
	...
	if (ratio < 1.0f) {
		flare.radius *= ratio;
		flare.shaderRGBA[3] =
		    (byte) ((float)flare.shaderRGBA[3] * ratio);
	}
...
static void CG_Lev2ZapChain(centity_t * cent)
{
	...
	for (i = 0; i <= 2; i++) {
		...
		if (!CG_IsTrailSystemValid(&cent->level2ZapTS[i]))
			cent->level2ZapTS[i] =
			    CG_SpawnNewTrailSystem(cgs.media.level2ZapTS);

		if (CG_IsTrailSystemValid(&cent->level2ZapTS[i])) {
			CG_SetAttachmentCent(&cent->level2ZapTS[i]->
					     frontAttachment, source);
			CG_SetAttachmentCent(&cent->level2ZapTS[i]->
					     backAttachment, target);
...
static void CG_CalcEntityLerpPositions(centity_t * cent)
{
	...
	if (timeshift) {
		...
		CG_Trace(&tr, lastOrigin, vec3_origin, vec3_origin,
			 cent->lerpOrigin, cent->currentState.number,
			 MASK_SHOT);

		// don't let the projectile go through the floor
		if (tr.fraction < 1.0f)
			VectorLerp(tr.fraction, lastOrigin, cent->lerpOrigin,
				   cent->lerpOrigin);
	}
	...
	if (cg_drawBBOX.integer) {
		for (num = 0; num < cg.snap->numEntities; num++) {
			...
			case ET_CORPSE:
				...
				CG_DrawBoundingBox(cent->lerpOrigin, mins,
						   maxs);
				break;
