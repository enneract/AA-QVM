...
void CG_TileClear(void)
{
	...
	CG_TileClearBox(0, top, left, bottom - top + 1,
...
qboolean CG_WorldToScreen(vec3_t point, float *x, float *y)
{
	...
	if (x)
		*x = xc - DotProduct(trans,
				     cg.refdef.viewaxis[1]) * xc / (z * px);

	if (y)
		*y = yc - DotProduct(trans,
				     cg.refdef.viewaxis[2]) * yc / (z * py);
