...
static cvarTable_t cvarTable[] = {
	...
	{ &cg_teamOverlayUserinfo, "teamoverlay", "0",
	...
	{ &cg_disableWarningDialogs, "cg_disableWarningDialogs", "0",
	 CVAR_ARCHIVE },
	{ &cg_disableScannerPlane, "cg_disableScannerPlane", "0",
	 CVAR_ARCHIVE },
	...
	{ &cg_smoothClients, "cg_smoothClients", "0",
...
static void CG_SetPVars(void)
{
	...
	case PTE_ALIENS:
		...
		trap_Cvar_Set("player_kns",
			      va("%d",
				 ((cgs.alienStage ==
				   2) ? 0 : abs(cgs.alienNextStageThreshold -
						cgs.alienKills))));
		trap_Cvar_Set("player_stage", va("%d", cgs.alienStage + 1));
		trap_Cvar_Set("player_bp", va("%d", cgs.alienBuildPoints));
		trap_Cvar_Set("player_maxbp",
			      va("%d", cgs.alienBuildPointsTotal));
	...
	case PTE_HUMANS:
		...
		trap_Cvar_Set("player_kns",
			      va("%d",
				 ((cgs.humanStage ==
				   2) ? 0 : abs(cgs.humanNextStageThreshold -
						cgs.humanKills))));
		trap_Cvar_Set("player_bp", va("%d", cgs.humanBuildPoints));
		trap_Cvar_Set("player_maxbp",
			      va("%d", cgs.humanBuildPointsTotal));
		trap_Cvar_Set("player_credits",
			      va("%d", ps->persistant[PERS_CREDIT]));
		break;
	}
	...
	if (CG_LastAttacker() != -1) {
		trap_Cvar_Set("player_attacker",
			      cgs.clientinfo[CG_LastAttacker()].name);
		trap_Cvar_Set("player_attacker_hp",
			      va("%d",
				 cgs.clientinfo[CG_LastAttacker()].health));
	} else {
		trap_Cvar_Set("player_attacker", "");
		trap_Cvar_Set("player_attacker_hp", "");
	}
	...
	if (CG_CrosshairPlayer() != -1) {
		trap_Cvar_Set("player_crosshair",
			      cgs.clientinfo[CG_CrosshairPlayer()].name);
		//XXX hax required
	...
	if (cgs.alienStage == 2)
		...
		trap_Cvar_Set("alien_kns",
			      va("%d",
				 abs(cgs.alienNextStageThreshold -
				     cgs.alienKills)));
	...
	if (cgs.humanStage == 2)
		...
		trap_Cvar_Set("human_kns",
			      va("%d",
				 abs(cgs.humanNextStageThreshold -
				     cgs.humanKills)));
...
static void CG_RegisterSounds(void)
{
	...
	cgs.media.alienStageTransition =
	    trap_S_RegisterSound("sound/announcements/overmindevolved.wav",
				 qtrue);
	cgs.media.humanStageTransition =
	    trap_S_RegisterSound("sound/announcements/reinforcement.wav",
				 qtrue);

	cgs.media.alienOvermindAttack =
	    trap_S_RegisterSound("sound/announcements/overmindattack.wav",
				 qtrue);
	cgs.media.alienOvermindDying =
	    trap_S_RegisterSound("sound/announcements/overminddying.wav",
				 qtrue);
	cgs.media.alienOvermindSpawns =
	    trap_S_RegisterSound("sound/announcements/overmindspawns.wav",
				 qtrue);

	cgs.media.alienL1Grab =
	    trap_S_RegisterSound("sound/player/level1/grab.wav", qtrue);
	cgs.media.alienL4ChargePrepare =
	    trap_S_RegisterSound("sound/player/level4/charge_prepare.wav",
				 qtrue);
	cgs.media.alienL4ChargeStart =
	    trap_S_RegisterSound("sound/player/level4/charge_start.wav", qtrue);

	cgs.media.tracerSound =
	    trap_S_RegisterSound("sound/weapons/tracer.wav", qfalse);
	cgs.media.selectSound =
	    trap_S_RegisterSound("sound/weapons/change.wav", qfalse);

	cgs.media.weaponZoomIn =
	    trap_S_RegisterSound("sound/weapons/zoom_in.wav", qfalse);
	cgs.media.weaponZoomOut =
	    trap_S_RegisterSound("sound/weapons/zoom_out.wav", qfalse);

	cgs.media.talkSound =
	    trap_S_RegisterSound("sound/misc/talk.wav", qfalse);
	cgs.media.alienTalkSound =
	    trap_S_RegisterSound("sound/misc/alien_talk.wav", qfalse);
	cgs.media.humanTalkSound =
	    trap_S_RegisterSound("sound/misc/human_talk.wav", qfalse);
	cgs.media.landSound =
	    trap_S_RegisterSound("sound/player/land1.wav", qfalse);

	cgs.media.watrInSound =
	    trap_S_RegisterSound("sound/player/watr_in.wav", qfalse);
	cgs.media.watrOutSound =
	    trap_S_RegisterSound("sound/player/watr_out.wav", qfalse);
	cgs.media.watrUnSound =
	    trap_S_RegisterSound("sound/player/watr_un.wav", qfalse);

	cgs.media.disconnectSound =
	    trap_S_RegisterSound("sound/misc/disconnect.wav", qfalse);

	cgs.media.voteNow =
	    trap_S_RegisterSound("sound/bunker/votebing.wav", qtrue);

	for (i = 0; i < 4; i++) {
		Com_sprintf(name, sizeof(name),
			    "sound/player/footsteps/step%i.wav", i + 1);
		cgs.media.footsteps[FOOTSTEP_NORMAL][i] =
		    trap_S_RegisterSound(name, qfalse);

		Com_sprintf(name, sizeof(name),
			    "sound/player/footsteps/flesh%i.wav", i + 1);
		cgs.media.footsteps[FOOTSTEP_FLESH][i] =
		    trap_S_RegisterSound(name, qfalse);

		Com_sprintf(name, sizeof(name),
			    "sound/player/footsteps/splash%i.wav", i + 1);
		cgs.media.footsteps[FOOTSTEP_SPLASH][i] =
		    trap_S_RegisterSound(name, qfalse);

		Com_sprintf(name, sizeof(name),
			    "sound/player/footsteps/clank%i.wav", i + 1);
		cgs.media.footsteps[FOOTSTEP_METAL][i] =
		    trap_S_RegisterSound(name, qfalse);
	}
	...
	cgs.media.jetpackDescendSound =
	    trap_S_RegisterSound("sound/upgrades/jetpack/low.wav", qfalse);
	cgs.media.jetpackIdleSound =
	    trap_S_RegisterSound("sound/upgrades/jetpack/idle.wav", qfalse);
	cgs.media.jetpackAscendSound =
	    trap_S_RegisterSound("sound/upgrades/jetpack/hi.wav", qfalse);

	cgs.media.medkitUseSound =
	    trap_S_RegisterSound("sound/upgrades/medkit/medkit.wav", qfalse);
	...
	cgs.media.alienBuildableDamage =
	    trap_S_RegisterSound("sound/buildables/alien/damage.wav", qfalse);
	cgs.media.alienBuildablePrebuild =
	    trap_S_RegisterSound("sound/buildables/alien/prebuild.wav", qfalse);

	cgs.media.humanBuildableExplosion =
	    trap_S_RegisterSound("sound/buildables/human/explosion.wav",
				 qfalse);
	cgs.media.humanBuildablePrebuild =
	    trap_S_RegisterSound("sound/buildables/human/prebuild.wav", qfalse);

	for (i = 0; i < 4; i++)
		cgs.media.humanBuildableDamage[i] =
		    trap_S_RegisterSound(va
					 ("sound/buildables/human/damage%d.wav",
					  i), qfalse);

	cgs.media.hardBounceSound1 =
	    trap_S_RegisterSound("sound/misc/hard_bounce1.wav", qfalse);
	cgs.media.hardBounceSound2 =
	    trap_S_RegisterSound("sound/misc/hard_bounce2.wav", qfalse);

	cgs.media.repeaterUseSound =
	    trap_S_RegisterSound("sound/buildables/repeater/use.wav", qfalse);

	cgs.media.buildableRepairSound =
	    trap_S_RegisterSound("sound/buildables/human/repair.wav", qfalse);
	cgs.media.buildableRepairedSound =
	    trap_S_RegisterSound("sound/buildables/human/repaired.wav", qfalse);
...
static void CG_RegisterGraphics(void)
{
	...
	cgs.media.viewBloodShader =
	    trap_R_RegisterShader("gfx/damage/fullscreen_painblend");
	...
	cgs.media.greenBuildShader =
	    trap_R_RegisterShader("gfx/misc/greenbuild");
	cgs.media.redBuildShader = trap_R_RegisterShader("gfx/misc/redbuild");
	cgs.media.humanSpawningShader =
	    trap_R_RegisterShader("models/buildables/telenode/rep_cyl");

	for (i = 0; i < 8; i++)
		cgs.media.buildWeaponTimerPie[i] =
		    trap_R_RegisterShader(buildWeaponTimerPieShaders[i]);

	cgs.media.upgradeClassIconShader =
	    trap_R_RegisterShader("icons/icona_upgrade.tga");

	cgs.media.balloonShader =
	    trap_R_RegisterShader("gfx/sprites/chatballoon");
	...
	cgs.media.poisonCloudPS =
	    CG_RegisterParticleSystem("firstPersonPoisonCloudPS");
	cgs.media.alienEvolvePS = CG_RegisterParticleSystem("alienEvolvePS");
	cgs.media.alienAcidTubePS =
	    CG_RegisterParticleSystem("alienAcidTubePS");

	cgs.media.jetPackDescendPS =
	    CG_RegisterParticleSystem("jetPackDescendPS");
	cgs.media.jetPackHoverPS = CG_RegisterParticleSystem("jetPackHoverPS");
	cgs.media.jetPackAscendPS =
	    CG_RegisterParticleSystem("jetPackAscendPS");

	cgs.media.humanBuildableDamagedPS =
	    CG_RegisterParticleSystem("humanBuildableDamagedPS");
	cgs.media.alienBuildableDamagedPS =
	    CG_RegisterParticleSystem("alienBuildableDamagedPS");
	cgs.media.humanBuildableDestroyedPS =
	    CG_RegisterParticleSystem("humanBuildableDestroyedPS");
	cgs.media.alienBuildableDestroyedPS =
	    CG_RegisterParticleSystem("alienBuildableDestroyedPS");
	...
	CG_BuildableStatusParse("ui/assets/human/buildstat.cfg",
				&cgs.humanBuildStat);
	CG_BuildableStatusParse("ui/assets/alien/buildstat.cfg",
				&cgs.alienBuildStat);
	...
	for (i = 1; i < MAX_GAME_PARTICLE_SYSTEMS; i++) {
		...
		cgs.gameParticleSystems[i] =
		    CG_RegisterParticleSystem((char *)psName);
	}
}
...
void CG_BuildSpectatorString(void)
{
	...
	for (i = 0; i < MAX_CLIENTS; i++) {
		if (cgs.clientinfo[i].infoValid
		    && cgs.clientinfo[i].team == PTE_NONE)
...
static void CG_RegisterClients(void)
{
	...
	cgs.media.larmourHeadSkin =
	    trap_R_RegisterSkin("models/players/human_base/head_light.skin");
	cgs.media.larmourLegsSkin =
	    trap_R_RegisterSkin("models/players/human_base/lower_light.skin");
	cgs.media.larmourTorsoSkin =
	    trap_R_RegisterSkin("models/players/human_base/upper_light.skin");

	cgs.media.jetpackModel =
	    trap_R_RegisterModel("models/players/human_base/jetpack.md3");
	cgs.media.jetpackFlashModel =
...
qboolean CG_Asset_Parse(int handle)
{
	...
	while (1) {
		...
		if (Q_stricmp(token.string, "cursor") == 0) {
			...
			cgDC.Assets.cursor =
			    trap_R_RegisterShaderNoMip(cgDC.Assets.cursorStr);
			continue;
		}
		...
		if (Q_stricmp(token.string, "shadowColor") == 0) {
			...
			cgDC.Assets.shadowFadeClamp =
			    cgDC.Assets.shadowColor[3];
			continue;
		}
	}
				     ...
				     qhandle_t * handle)
{
	...
	*handle = -1;
	...
	if (info && info->infoValid) {
		...
		case 0:
			if (showIcons) {
				if (sp->weapon != WP_NONE)
					*handle =
					    cg_weapons[sp->weapon].weaponIcon;
			}
			break;

		case 1:
			if (showIcons) {
				if (sp->team == PTE_HUMANS
				    && sp->upgrade != UP_NONE)
					*handle =
					    cg_upgrades[sp->upgrade].
					    upgradeIcon;
		...
		case 2:
			if ((atoi(CG_ConfigString(CS_CLIENTS_READY)) &
			     (1 << sp->client)) && cg.intermissionStarted)
				return "Ready";
			break;
...
static void CG_FeederSelection(float feederID, int index)
{
	int i, count;
	int team =
	    (feederID == FEEDER_ALIENTEAM_LIST) ? PTE_ALIENS : PTE_HUMANS;
	count = 0;
	...
	if (Q_stricmp(voipSendTarget, "team") == 0) {
		...
		for (slen = i = 0; i < cgs.maxclients; i++) {
			if (!cgs.clientinfo[i].infoValid || i == cg.clientNum)
				continue;
			if (cgs.clientinfo[i].team !=
			    cgs.clientinfo[cg.clientNum].team)
				continue;

			Com_sprintf(&voipString[slen],
				    sizeof(voipString) - slen, "%s%d",
				    (slen > 0) ? "," : "", i);
			slen = strlen(voipString);
			if (slen + 1 >= sizeof(voipString)) {
				CG_Printf(S_COLOR_YELLOW
					  "WARNING: voipString overflowed\n");
				break;
			}
		}
