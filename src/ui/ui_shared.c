		  ...
		  float fadeCycle)
{
	...
	if (w->style == WINDOW_STYLE_FILLED) {
		...
		if (w->cinematic == -1) {
			w->cinematic =
			    DC->playCinematic(w->cinematicName, fillRect.x,
					      fillRect.y, fillRect.w,
					      fillRect.h);
			if (w->cinematic == -1) {
				w->cinematic = -2;
			}
		...
		if (w->cinematic >= 0) {
			...
			DC->drawCinematic(w->cinematic, fillRect.x, fillRect.y,
	...
	if (w->border == WINDOW_BORDER_FULL) {
		...
		if (w->style == WINDOW_STYLE_TEAMCOLOR) {
			...
			color[3] = 1;
			DC->drawRect(w->rect.x, w->rect.y, w->rect.w, w->rect.h,
				     w->borderSize, color);
		} else {
			DC->drawRect(w->rect.x, w->rect.y, w->rect.w, w->rect.h,
		...
		// left right
		DC->setColor(w->borderColor);
		DC->drawSides(w->rect.x, w->rect.y, w->rect.w, w->rect.h,
			      w->borderSize);
		DC->setColor(NULL);
...
int Menu_ItemsMatchingGroup(menuDef_t * menu, const char *name)
{
	...
	for (i = 0; i < menu->itemCount; i++) {
		if (Q_stricmp(menu->items[i]->window.name, name) == 0
			...
			&& Q_stricmp(menu->items[i]->window.group,
				     name) == 0)) {
			count++;
		}
	}
...
void Script_SetItemColor(itemDef_t * item, char **args)
{
	...
	if (String_Parse(args, &itemname) && String_Parse(args, &name)) {
		...
		for (j = 0; j < count; j++) {
			item2 =
			    Menu_GetMatchingItemByNumber(item->parent, j,
							 itemname);
			if (item2 != NULL) {
				out = NULL;
				if (Q_stricmp(name, "backcolor") == 0) {
	...
	for (i = 0; i < count; i++) {
		...
		if (item != NULL) {
			if (fadeOut) {
				...
				item->window.flags |=
				    (WINDOW_VISIBLE | WINDOW_FADINGIN);
...
void Script_ConditionalOpen(itemDef_t * item, char **args)
{
	...
	if (String_Parse(args, &cvar) && String_Parse(args, &name1)
	    && String_Parse(args, &name2)) {
			       ...
			       float amt)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (item != NULL) {
			item->window.flags |=
			    (WINDOW_INTRANSITION | WINDOW_VISIBLE);
			...
			item->window.rectEffects2.x =
			    fabs(rectTo.x - rectFrom.x) / amt;
			item->window.rectEffects2.y =
			    fabs(rectTo.y - rectFrom.y) / amt;
			item->window.rectEffects2.w =
			    fabs(rectTo.w - rectFrom.w) / amt;
			item->window.rectEffects2.h =
			    fabs(rectTo.h - rectFrom.h) / amt;
...
void Script_Transition(itemDef_t * item, char **args)
{
	...
	if (String_Parse(args, &name)) {
		if (Rect_Parse(args, &rectFrom) && Rect_Parse(args, &rectTo)
			...
			Menu_TransitionItemByName(item->parent, name, rectFrom,
						  rectTo, time, amt);
		}
	}
}
			  ...
			  float cx, float cy, int time)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (item != NULL) {
			item->window.flags |=
			    (WINDOW_ORBITING | WINDOW_VISIBLE);
			item->window.offsetTime = time;
...
void Script_Orbit(itemDef_t * item, char **args)
{
	...
	if (String_Parse(args, &name)) {
		if (Float_Parse(args, &x) && Float_Parse(args, &y)
		    ...
		    && Int_Parse(args, &time)) {
			Menu_OrbitItemByName(item->parent, name, x, y, cx, cy,
					     time);
		}
	}
}
	...
	if (String_Parse(args, &name)) {
		...
		if (focusItem && !(focusItem->window.flags & WINDOW_DECORATION)) {
			...
			if (focusItem->type == ITEM_TYPE_EDITFIELD
			    || focusItem->type == ITEM_TYPE_SAYFIELD
			...
			if (DC->Assets.itemFocusSound) {
				DC->startLocalSound(DC->Assets.itemFocusSound,
						    CHAN_LOCAL_SOUND);
			}
		}
	}
	...
	if (item && s && s[0]) {
		...
		while (1) {
			...
			for (i = 0; i < scriptCommandCount; i++) {
				if (Q_stricmp(command, commandList[i].name) ==
				    0) {
...
int Item_ListBox_MaxScroll(itemDef_t * item)
{
	...
	if (item->window.flags & WINDOW_HORIZONTAL) {
		...
		max =
		    count - (item->window.rect.h / listPtr->elementHeight) + 1;
	}
	if (max < 0) {
		return 0;
	}
...
int Item_ListBox_OverLB(itemDef_t * item, float x, float y)
{
	...
	if (item->window.flags & WINDOW_HORIZONTAL) {
		...
		r.y =
		    item->window.rect.y + item->window.rect.h - SCROLLBAR_SIZE;
		...
		r.x =
		    item->window.rect.x + item->window.rect.w - SCROLLBAR_SIZE;
		if (Rect_ContainsPoint(&r, x, y)) {
...
void Item_ListBox_MouseEnter(itemDef_t * item, float x, float y)
{
	...
	if (item->window.flags & WINDOW_HORIZONTAL) {
		if (!
		    (item->window.
		     flags & (WINDOW_LB_LEFTARROW | WINDOW_LB_RIGHTARROW |
			      WINDOW_LB_THUMB | WINDOW_LB_PGUP |
			      WINDOW_LB_PGDN))) {
			...
			if (listPtr->elementStyle == LISTBOX_IMAGE) {
				...
				r.w =
				    item->window.rect.w - listPtr->drawPadding;
				if (Rect_ContainsPoint(&r, x, y)) {
					listPtr->cursorPos =
					    (int)((x -
						   r.x) /
		...
		if (Rect_ContainsPoint(&r, x, y)) {
			listPtr->cursorPos =
			    (int)((y - 2 - r.y) / listPtr->elementHeight) +
			    listPtr->startPos;
...
void Item_MouseLeave(itemDef_t * item)
{
	if (item) {
		...
		item->window.flags &=
		    ~(WINDOW_LB_RIGHTARROW | WINDOW_LB_LEFTARROW);
	}
}
	...
	if (force
		...
		if (item->window.flags & WINDOW_HORIZONTAL) {
			...
			if (key == K_LEFTARROW || key == K_KP_LEFTARROW) {
				if (!listPtr->notselectable) {
					...
					DC->feederSelection(item->special,
							    item->cursorPos);
				} else {
					listPtr->startPos--;
		...
		if (key == K_MOUSE1 || key == K_MOUSE2) {
			if (item->window.flags & WINDOW_LB_LEFTARROW) {
				...
				lastListBoxClickTime =
				    DC->realTime + DOUBLE_CLICK_DELAY;
	...
	if (Rect_ContainsPoint(&item->window.rect, DC->cursorx, DC->cursory)
		...
		if (key == K_MOUSE1 || key == K_ENTER || key == K_MOUSE2
			...
			DC->setCVar(item->cvar,
				    va("%i", !DC->getCVarValue(item->cvar)));
			return qtrue;
		}
	}
	...
	if (multiPtr) {
		if (Rect_ContainsPoint
			...
			if (key == K_MOUSE1 || key == K_ENTER || key == K_MOUSE2
			    || key == K_MOUSE3) {
				int current =
				    Item_Multi_FindCvarByValue(item) + 1;
				...
				if (multiPtr->strDef) {
					DC->setCVar(item->cvar,
						    multiPtr->cvarStr[current]);
				} else {
					float value =
					    multiPtr->cvarValue[current];
	...
	if (item->cvar) {
		...
		if (key & K_CHAR_FLAG) {
			...
			if (!DC->getOverstrikeMode()) {
				...
				if (editPtr->maxChars
				    && item->cursorPos >= editPtr->maxChars) {
					return qtrue;
				}
			}
			...
			if (key == K_DEL || key == K_KP_DEL) {
				if (item->cursorPos < len) {
					memmove(buff + item->cursorPos,
						buff + item->cursorPos + 1,
						len - item->cursorPos);
					DC->setCVar(item->cvar, buff);
				}
				return qtrue;
		...
		if (key == K_TAB || key == K_DOWNARROW || key == K_KP_DOWNARROW) {
			...
			if (newItem
			    && (newItem->type == ITEM_TYPE_EDITFIELD
				|| newItem->type == ITEM_TYPE_SAYFIELD
...
static void Scroll_ListBox_ThumbFunc(void *p)
{
	...
	if (si->item->window.flags & WINDOW_HORIZONTAL) {
		...
		r.y =
		    si->item->window.rect.y + si->item->window.rect.h -
		    SCROLLBAR_SIZE - 1;
		r.h = SCROLLBAR_SIZE;
		...
		//
		pos =
		    (DC->cursorx - r.x - SCROLLBAR_SIZE / 2) * max / (r.w -
								      SCROLLBAR_SIZE);
		if (pos < 0) {
			pos = 0;
		} else if (pos > max) {
		...
		r.x =
		    si->item->window.rect.x + si->item->window.rect.w -
		    SCROLLBAR_SIZE - 1;
		...
		//
		pos =
		    (DC->cursory - r.y - SCROLLBAR_SIZE / 2) * max / (r.h -
								      SCROLLBAR_SIZE);
		if (pos < 0) {
			pos = 0;
		} else if (pos > max) {
...
void Item_StartCapture(itemDef_t * item, int key)
{
	...
	case ITEM_TYPE_LISTBOX:
		{
			flags =
			    Item_ListBox_OverLB(item, DC->cursorx, DC->cursory);
			if (flags &
			    (WINDOW_LB_LEFTARROW | WINDOW_LB_RIGHTARROW)) {
				scrollInfo.nextScrollTime =
				    DC->realTime + SCROLL_TIME_START;
				scrollInfo.nextAdjustTime =
				    DC->realTime + SCROLL_TIME_ADJUST;
				scrollInfo.adjustValue = SCROLL_TIME_START;
				scrollInfo.scrollKey = key;
				scrollInfo.scrollDir =
				    (flags & WINDOW_LB_LEFTARROW) ? qtrue :
				    qfalse;
	...
	case ITEM_TYPE_SLIDER:
		{
			flags =
			    Item_Slider_OverSlider(item, DC->cursorx,
						   DC->cursory);
...
qboolean Item_Slider_HandleKey(itemDef_t * item, int key, qboolean down)
{
	...
	if (item->window.flags & WINDOW_HASFOCUS && item->cvar
	    && Rect_ContainsPoint(&item->window.rect, DC->cursorx,
				  DC->cursory)) {
		if (key == K_MOUSE1 || key == K_ENTER || key == K_MOUSE2
		    || key == K_MOUSE3) {
			editFieldDef_t *editDef = item->typeData;
			if (editDef) {
				...
				testRect.w =
				    (SLIDER_WIDTH +
				     (float)SLIDER_THUMB_WIDTH / 2);
				if (Rect_ContainsPoint
				    (&testRect, DC->cursorx, DC->cursory)) {
					work = DC->cursorx - x;
					value = work / width;
					value *=
					    (editDef->maxVal - editDef->minVal);
					// vm fuckage
...
itemDef_t *Menu_SetPrevCursorItem(menuDef_t * menu)
{
	...
	while (menu->cursorItem > -1) {
		...
		if (Item_SetFocus
		    (menu->items[menu->cursorItem], DC->cursorx, DC->cursory)) {
			Menu_HandleMouseMove(menu,
					     menu->items[menu->cursorItem]->
					     window.rect.x + 1,
					     menu->items[menu->cursorItem]->
					     window.rect.y + 1);
...
void Menus_HandleOOBClick(menuDef_t * menu, int key, qboolean down)
{
	if (menu) {
		...
		for (i = 0; i < menuCount; i++) {
			if (Menu_OverActiveItem
			    (&Menus[i], DC->cursorx, DC->cursory)) {
				Menu_RunCloseScript(menu);
				menu->window.flags &=
				    ~(WINDOW_HASFOCUS | WINDOW_VISIBLE);
				Menus_Activate(&Menus[i]);
				Menu_HandleMouseMove(&Menus[i], DC->cursorx,
						     DC->cursory);
...
void Menu_HandleKey(menuDef_t * menu, int key, qboolean down)
{
	...
	if (down && !(menu->window.flags & WINDOW_POPUP)
	    && !Rect_ContainsPoint(&menu->window.rect, DC->cursorx,
				   DC->cursory)) {
	...
	case K_MOUSE2:
		if (item) {
			if (item->type == ITEM_TYPE_TEXT) {
				if (Rect_ContainsPoint
				    (Item_CorrectedTextRect(item), DC->cursorx,
				     DC->cursory)) {
					Item_Action(item);
				}
			 ...
			 const char *text)
{
	...
	*height = item->textRect.h;
	...
	if (*width == 0
		...
		int originalWidth =
		    DC->textWidth(item->text, item->textscale, 0);

		if (item->type == ITEM_TYPE_OWNERDRAW
		    && (item->textalignment == ITEM_ALIGN_CENTER
			|| item->textalignment == ITEM_ALIGN_RIGHT)) {
			originalWidth +=
			    DC->ownerDrawWidth(item->window.ownerDraw,
					       item->textscale);
		} else if (item->type == ITEM_TYPE_EDITFIELD
			   && item->textalignment == ITEM_ALIGN_CENTER
			   && item->cvar) {
		...
		*height = DC->textHeight(textPtr, item->textscale, 0);
		...
		ToWindowCoords(&item->textRect.x, &item->textRect.y,
			       &item->window);
	}
}
...
void Item_TextColor(itemDef_t * item, vec4_t * newColor)
{
	...
	Fade(&item->window.flags, &item->window.foreColor[3], parent->fadeClamp,
	     &item->window.nextTime, parent->fadeCycle, qtrue,
	     parent->fadeAmount);
...
/*    lowLight[0] = 0.8 * parent->focusColor[0];
	...
	if (item->enableCvar != NULL && *item->enableCvar
	    && item->cvarTest != NULL && *item->cvarTest) {
		if (item->cvarFlags & (CVAR_ENABLE | CVAR_DISABLE)
		    && !Item_EnableShowViaCvar(item, CVAR_ENABLE)) {
...
static int checkCache(const char *text, rectDef_t * rect, int width, int height)
{
	...
	for (i = 0; i < MAX_AUTOWRAP_CACHE; i++) {
		...
		if (rect->x != awc[i].rect.x ||
		    rect->y != awc[i].rect.y ||
		    rect->w != awc[i].rect.w || rect->h != awc[i].rect.h)
			continue;
	...
	if (cache >= 0) {
		...
		for (i = 0; i < lineNum; i++) {
			...
			DC->drawText(item->textRect.x, item->textRect.y,
				     item->textscale, color,
				     awc[cache].lines[i], 0, 0,
				     item->textStyle);
		}
	} else {
		y = item->textaligny;
		...
		while (p) {
			...
			if ((newLine && textWidth > item->window.rect.w)
				...
				if (len) {
					...
					item->textRect.y = y;
					ToWindowCoords(&item->textRect.x,
						       &item->textRect.y,
						       &item->window);
					//
					buff[newLine] = '\0';
					...
					if (!skipLines) {
						DC->drawText(item->textRect.x,
							     item->textRect.y,
							     item->textscale,
							     color, buff, 0, 0,
							     item->textStyle);

						strcpy(awc[cacheIndex].
						       lines[lineNum], buff);
						awc[cacheIndex].
						    lineOffsets[lineNum][0] =
						    item->textRect.x;
						awc[cacheIndex].
...
void Item_Text_Wrapped_Paint(itemDef_t * item)
{
	...
	while (p && *p) {
		...
		buff[p - start] = '\0';
		DC->drawText(x, y, item->textscale, color, buff, 0, 0,
			     item->textStyle);
		y += height + 5;
		start += p - start + 1;
		p = strchr(p + 1, '\r');
	}
	DC->drawText(x, y, item->textscale, color, start, 0, 0,
		     item->textStyle);
}
...
/*    lowLight[0] = 0.8 * parent->focusColor[0];
	...
	if (item->window.flags & WINDOW_HASFOCUS && g_editingField) {
		...
		DC->drawTextWithCursor(item->textRect.x + item->textRect.w +
				       offset, item->textRect.y,
				       item->textscale, newColor,
				       buff + editPtr->paintOffset,
				       item->cursorPos - editPtr->paintOffset,
				       cursor, editPtr->maxPaintChars,
				       item->textStyle);
	} else {
...
/*    lowLight[0] = 0.8 * parent->focusColor[0];
	...
	if (item->text) {
		Item_Text_Paint(item);
		DC->drawText(item->textRect.x + item->textRect.w + 8,
			     item->textRect.y, item->textscale, newColor, text,
			     0, 0, item->textStyle);
	} else {
		DC->drawText(item->textRect.x, item->textRect.y,
			     item->textscale, newColor, text, 0, 0,
			     item->textStyle);
	}
}
...
void Controls_SetConfig(qboolean restart)
{
	...
	for (i = 0; i < g_bindCount; i++) {
		...
		if (g_bindings[i].bind1 != -1) {
			DC->setBinding(g_bindings[i].bind1,
				       g_bindings[i].command);

			if (g_bindings[i].bind2 != -1)
				DC->setBinding(g_bindings[i].bind2,
					       g_bindings[i].command);
		}
	}
...
/*    lowLight[0] = 0.8 * parent->focusColor[0];
	...
	DC->drawHandlePic(x - (SLIDER_THUMB_WIDTH / 2), y - 2,
			  SLIDER_THUMB_WIDTH, SLIDER_THUMB_HEIGHT,
...
qboolean Item_Bind_HandleKey(itemDef_t * item, int key, qboolean down)
{
	...
	if (id != -1) {
		...
		} else if (g_bindings[id].bind1 != key
			   && g_bindings[id].bind2 == -1) {
	...
	if (modelPtr->rotationSpeed) {
		if (DC->realTime > item->window.nextTime) {
			item->window.nextTime =
			    DC->realTime + modelPtr->rotationSpeed;
...
void Item_ListBox_Paint(itemDef_t * item)
{
	...
	if (item->window.flags & WINDOW_HORIZONTAL) {
		...
		x = item->window.rect.x + item->window.rect.w - SCROLLBAR_SIZE -
		    1;
		y = item->window.rect.y + 1;
		DC->drawHandlePic(x, y, SCROLLBAR_SIZE, SCROLLBAR_SIZE,
		...
		DC->drawHandlePic(x, y, SCROLLBAR_SIZE, size + 1,
				  DC->Assets.scrollBar);
		y += size - 1;
		DC->drawHandlePic(x, y, SCROLLBAR_SIZE, SCROLLBAR_SIZE,
		...
		if (listPtr->elementStyle == LISTBOX_IMAGE) {
			...
			for (i = listPtr->startPos; i < count; i++) {
				...
				if (listPtr->numColumns > 0) {
					int j;
					for (j = 0; j < listPtr->numColumns;
					     j++) {
						text =
						    DC->feederItemText(item->
								       special,
								       i, j,
								       &optionalImage);
						if (optionalImage >= 0) {
					...
					text =
					    DC->feederItemText(item->special, i,
							       0,
							       &optionalImage);
					if (optionalImage >= 0) {
	...
	if (DC->ownerDrawItem) {
		...
		if (item->numColors > 0 && DC->getValue) {
			...
			for (i = 0; i < item->numColors; i++) {
				if (f >= item->colorRanges[i].low
				    && f <= item->colorRanges[i].high) {
					memcpy(&color,
					       &item->colorRanges[i].color,
					       sizeof(color));
					break;
				}
			}
...
/*      lowLight[0] = 0.8 * parent->focusColor[0];
      ...
      LerpColor(parent->focusColor,lowLight,color,0.5+0.5*sin(DC->realTime / PULSE_DIVISOR));*/
			...
			LerpColor(item->window.foreColor, lowLight, color,
				  0.5 +
				  0.5 * sin(DC->realTime / PULSE_DIVISOR));
		}
		...
		if (item->text) {
			...
			if (item->text[0]) {
				...
				DC->ownerDrawItem(item->textRect.x +
						  item->textRect.w + 8,
						  item->window.rect.y,
						  item->window.rect.w,
						  item->window.rect.h, 0,
						  item->textaligny,
			...
			DC->ownerDrawItem(item->window.rect.x,
					  item->window.rect.y,
					  item->window.rect.w,
					  item->window.rect.h, item->textalignx,
					  item->textaligny,
					  item->window.ownerDraw,
					  item->window.ownerDrawFlags,
					  item->alignment, item->special,
					  item->textscale, color,
					  item->window.background,
					  item->textStyle);
		}
...
void Item_Paint(itemDef_t * item)
{
	...
	if (item->window.flags & WINDOW_ORBITING) {
		if (DC->realTime > item->window.nextTime) {
			...
			item->window.nextTime =
			    DC->realTime + item->window.offsetTime;
			// translate
			w = item->window.rectClient.w / 2;
			h = item->window.rectClient.h / 2;
			rx = item->window.rectClient.x + w -
			    item->window.rectEffects.x;
			ry = item->window.rectClient.y + h -
	...
	if (item->window.flags & WINDOW_INTRANSITION) {
		if (DC->realTime > item->window.nextTime) {
			...
			if (item->window.rectClient.w ==
			    item->window.rectEffects.w) {
				done++;
			} else {
				if (item->window.rectClient.w <
				    item->window.rectEffects.w) {
					item->window.rectClient.w +=
					    item->window.rectEffects2.w;
					if (item->window.rectClient.w >
					    item->window.rectEffects.w) {
						item->window.rectClient.w =
			...
			if (item->window.rectClient.h ==
			    item->window.rectEffects.h) {
				done++;
			} else {
				if (item->window.rectClient.h <
				    item->window.rectEffects.h) {
					item->window.rectClient.h +=
					    item->window.rectEffects2.h;
					if (item->window.rectClient.h >
					    item->window.rectEffects.h) {
						item->window.rectClient.h =
			...
			if (menu->items[i]->special == feeder) {
				if (index == 0) {
					listBoxDef_t *listPtr =
					    (listBoxDef_t *) menu->items[i]->
					    typeData;
					listPtr->cursorPos = 0;
					listPtr->startPos = 0;
				}
				menu->items[i]->cursorPos = index;
				DC->feederSelection(menu->items[i]->special,
						    menu->items[i]->cursorPos);
				return;
			}
		}
	}
		...
		if (Q_stricmp(Menus[i].window.name, p) == 0) {
			...
			for (j = 0; j < m->itemCount; j++)	//TA: reset selection in listboxes when opened
			{
				if (m->items[j]->type == ITEM_TYPE_LISTBOX) {
					listBoxDef_t *listPtr =
					    (listBoxDef_t *) m->items[j]->
					    typeData;
					m->items[j]->cursorPos = 0;
					listPtr->startPos = 0;
					DC->feederSelection(m->items[j]->
							    special, 0);
				}
			}
...
void Menu_HandleMouseMove(menuDef_t * menu, float x, float y)
{
	...
	for (pass = 0; pass < 2; pass++) {
		for (i = 0; i < menu->itemCount; i++) {
			...
			if (!
			    (menu->items[i]->window.
			     flags & (WINDOW_VISIBLE | WINDOW_FORCED))) {
				continue;
			}
			// items can be enabled and disabled based on cvars
			if (menu->items[i]->
			    cvarFlags & (CVAR_ENABLE | CVAR_DISABLE)
			    && !Item_EnableShowViaCvar(menu->items[i],
						       CVAR_ENABLE)) {
				continue;
			}
			...
			} else if (menu->items[i]->window.
				   flags & WINDOW_MOUSEOVER) {
...
void Menu_Paint(menuDef_t * menu, qboolean forcePaint)
{
	...
	if (debugMode) {
		...
		DC->drawRect(menu->window.rect.x, menu->window.rect.y,
			     menu->window.rect.w, menu->window.rect.h, 1,
			     color);
	}
}
	...
	if (item->type == ITEM_TYPE_LISTBOX) {
		...
		if (item->type == ITEM_TYPE_EDITFIELD
			...
			if (!((editFieldDef_t *) item->typeData)->maxPaintChars) {
				((editFieldDef_t *) item->typeData)->
				    maxPaintChars = MAX_EDITFIELD;
			}
		}
...
qboolean ItemParse_cvarStrList(itemDef_t * item, int handle)
{
	...
	while (1) {
		...
		if (pass == 0) {
			...
			multiPtr->cvarStr[multiPtr->count] =
			    String_Alloc(token.string);
			pass = 0;
			multiPtr->count++;
...
qboolean ItemParse_cvarFloatList(itemDef_t * item, int handle)
{
	...
	while (1) {
		...
		multiPtr->cvarList[multiPtr->count] =
		    String_Alloc(token.string);
		if (!PC_Float_Parse
		    (handle, &multiPtr->cvarValue[multiPtr->count])) {
			return qfalse;
		}
...
qboolean ItemParse_addColorRange(itemDef_t * item, int handle)
{
	...
	if (PC_Float_Parse(handle, &color.low) &&
		...
		if (item->numColors < MAX_COLOR_RANGES) {
			memcpy(&item->colorRanges[item->numColors], &color,
			       sizeof(color));
			item->numColors++;
		}
		return qtrue;
...
qboolean Item_Parse(int handle, itemDef_t * item)
{
	...
	while (1) {
		...
		if (!key) {
			PC_SourceError(handle, "unknown menu item keyword %s",
				       token.string);
			continue;
		}
...
qboolean Menu_Parse(int handle, menuDef_t * menu)
{
	...
	while (1) {
		...
		if (!key) {
			PC_SourceError(handle, "unknown menu keyword %s",
				       token.string);
			continue;
		}
		if (!key->func((itemDef_t *) menu, handle)) {
			PC_SourceError(handle, "couldn't parse menu keyword %s",
				       token.string);
			return qfalse;
		}
	}
...
static void Window_CacheContents(windowDef_t * window)
{
	if (window) {
		if (window->cinematicName) {
			int cin =
			    DC->playCinematic(window->cinematicName, 0, 0, 0,
					      0);
	...
	if (menu && menu->window.flags & (WINDOW_VISIBLE | WINDOW_FORCED)) {
		if (Rect_ContainsPoint(&menu->window.rect, x, y)) {
			...
			for (i = 0; i < menu->itemCount; i++) {
				...
				if (menu->items[i]->window.
				    flags & WINDOW_DECORATION) {
					continue;
				}
