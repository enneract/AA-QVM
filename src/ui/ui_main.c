...
int ProcessNewUI(int command, int arg0, int arg1, int arg2, int arg3, int arg4,
		 int arg5, int arg6);
...
void AssetCache(void)
{
	...
	uiInfo.uiDC.Assets.scrollBar =
	    trap_R_RegisterShaderNoMip(ASSET_SCROLLBAR);
	...
	uiInfo.uiDC.Assets.sliderBar =
	    trap_R_RegisterShaderNoMip(ASSET_SLIDER_BAR);
...
void _UI_DrawSides(float x, float y, float w, float h, float size)
{
	...
	trap_R_DrawStretchPic(x, y, size, h, 0, 0, 0, 0,
...
void _UI_DrawTopBottom(float x, float y, float w, float h, float size)
{
	...
	trap_R_DrawStretchPic(x, y, w, size, 0, 0, 0, 0,
			      uiInfo.uiDC.whiteShader);
...
void Text_PaintChar(float x, float y, float width, float height, float scale,
		...
		while (s && *s && count < len) {
			...
			if (Q_IsColorString(s)) {
				...
				if (style == ITEM_TEXTSTYLE_SHADOWED
					...
					inner[0] =
					    newColor[0] * 1.5 >
					    1.0f ? 1.0f : newColor[0] * 1.5;
					inner[1] =
					    newColor[1] * 1.5 >
					    1.0f ? 1.0f : newColor[1] * 1.5;
					inner[2] =
					    newColor[2] * 1.5 >
					    1.0f ? 1.0f : newColor[2] * 1.5;
					inner[3] = newColor[3];
	...
	if (text) {
		...
		while (s && *s && count < len) {
			...
			if (Q_IsColorString(s)) {
				...
				if (count == cursorPos
				    && !((uiInfo.uiDC.realTime / BLINK_DIVISOR)
					 & 1)) {
					Text_PaintChar(x, y - yadj,
						       glyph2->imageWidth,
						       glyph2->imageHeight,
		...
		if (cursorPos == len
			...
			Text_PaintChar(x, y - yadj,
				       ...
				       glyph2->s,
				       glyph2->t,
				       glyph2->s2, glyph2->t2, glyph2->glyph);
...
void _UI_Refresh(int realtime)
{
	...
	if (Menu_Count() > 0 && !trap_Cvar_VariableValue("ui_loading")
	    && !trap_Cvar_VariableValue("ui_hideCursor"))
		UI_DrawHandlePic(uiInfo.uiDC.cursorx - 16,
				 uiInfo.uiDC.cursory - 16, 32, 32,
...
qboolean Asset_Parse(int handle)
{
	...
	while (1) {
		...
		if (Q_stricmp(token.string, "menuEnterSound") == 0) {
			...
			uiInfo.uiDC.Assets.menuEnterSound =
			    trap_S_RegisterSound(tempStr, qfalse);
			continue;
		}
		// exitMenuSound
		if (Q_stricmp(token.string, "menuExitSound") == 0) {
			...
			uiInfo.uiDC.Assets.menuExitSound =
			    trap_S_RegisterSound(tempStr, qfalse);
			continue;
		}
		// itemFocusSound
		if (Q_stricmp(token.string, "itemFocusSound") == 0) {
			...
			uiInfo.uiDC.Assets.itemFocusSound =
			    trap_S_RegisterSound(tempStr, qfalse);
			continue;
		}
		// menuBuzzSound
		if (Q_stricmp(token.string, "menuBuzzSound") == 0) {
			...
			uiInfo.uiDC.Assets.menuBuzzSound =
			    trap_S_RegisterSound(tempStr, qfalse);
			continue;
		}
		...
		if (Q_stricmp(token.string, "cursor") == 0) {
			if (!PC_String_Parse
			    (handle, &uiInfo.uiDC.Assets.cursorStr)) {
				return qfalse;
			}
			uiInfo.uiDC.Assets.cursor =
			    trap_R_RegisterShaderNoMip(uiInfo.uiDC.Assets.
						       cursorStr);
			continue;
		}
		...
		if (Q_stricmp(token.string, "fadeClamp") == 0) {
			if (!PC_Float_Parse
			    (handle, &uiInfo.uiDC.Assets.fadeClamp)) {
				return qfalse;
			}
			continue;
		}
		...
		if (Q_stricmp(token.string, "fadeCycle") == 0) {
			if (!PC_Int_Parse
			    (handle, &uiInfo.uiDC.Assets.fadeCycle)) {
				return qfalse;
			}
			continue;
		}
		...
		if (Q_stricmp(token.string, "fadeAmount") == 0) {
			if (!PC_Float_Parse
			    (handle, &uiInfo.uiDC.Assets.fadeAmount)) {
				return qfalse;
			}
			continue;
		}
		...
		if (Q_stricmp(token.string, "shadowX") == 0) {
			if (!PC_Float_Parse
			    (handle, &uiInfo.uiDC.Assets.shadowX)) {
				return qfalse;
			}
			continue;
		}
		...
		if (Q_stricmp(token.string, "shadowY") == 0) {
			if (!PC_Float_Parse
			    (handle, &uiInfo.uiDC.Assets.shadowY)) {
				return qfalse;
			}
			continue;
		}
		...
		if (Q_stricmp(token.string, "shadowColor") == 0) {
			if (!PC_Color_Parse
			    (handle, &uiInfo.uiDC.Assets.shadowColor)) {
				return qfalse;
			}
			uiInfo.uiDC.Assets.shadowFadeClamp =
			    uiInfo.uiDC.Assets.shadowColor[3];
			continue;
		}
...
tremInfoPane_t *UI_FindInfoPaneByName(const char *name)
{
	...
	strncpy(uiInfo.tremInfoPanes[i].text,
		"Not implemented.\n\nui/infopanes.def\n", MAX_INFOPANE_TEXT);
	Q_strcat(uiInfo.tremInfoPanes[i].text, MAX_INFOPANE_TEXT,
		 String_Alloc(name));
...
qboolean UI_LoadInfoPane(int handle)
{
	...
	while (1) {
		...
		if (!Q_stricmp(token.string, "name")) {
			...
			uiInfo.tremInfoPanes[uiInfo.tremInfoPaneCount].name =
			    String_Alloc(token.string);
			valid = qtrue;
			...
			graphic =
			    &uiInfo.tremInfoPanes[uiInfo.tremInfoPaneCount].
			    numGraphics;

			if (!Q_stricmp(token.string, "top"))
				uiInfo.tremInfoPanes[uiInfo.tremInfoPaneCount].
				    graphics[*graphic].side = INFOPANE_TOP;
			else if (!Q_stricmp(token.string, "bottom"))
				uiInfo.tremInfoPanes[uiInfo.tremInfoPaneCount].
				    graphics[*graphic].side = INFOPANE_BOTTOM;
			else if (!Q_stricmp(token.string, "left"))
				uiInfo.tremInfoPanes[uiInfo.tremInfoPaneCount].
				    graphics[*graphic].side = INFOPANE_LEFT;
			else if (!Q_stricmp(token.string, "right"))
				uiInfo.tremInfoPanes[uiInfo.tremInfoPaneCount].
				    graphics[*graphic].side = INFOPANE_RIGHT;
			else
				break;
			...
			if (!Q_stricmp(token.string, "center"))
				uiInfo.tremInfoPanes[uiInfo.tremInfoPaneCount].
				    graphics[*graphic].offset = -1;
			else
				uiInfo.tremInfoPanes[uiInfo.tremInfoPaneCount].
				    graphics[*graphic].offset = token.intvalue;
			...
			uiInfo.tremInfoPanes[uiInfo.tremInfoPaneCount].
			    graphics[*graphic].width = token.intvalue;
			...
			uiInfo.tremInfoPanes[uiInfo.tremInfoPaneCount].
			    graphics[*graphic].height = token.intvalue;
			...
			Q_strcat(uiInfo.tremInfoPanes[uiInfo.tremInfoPaneCount].
				 text, MAX_INFOPANE_TEXT, token.string);
...
void UI_LoadInfoPanes(const char *file)
{
	...
	if (!handle) {
		trap_Error(va
			   (S_COLOR_YELLOW "infopane file not found: %s\n",
			    file));
		return;
	}
...
static const char *handicapValues[] =
    { "None", "95", "90", "85", "80", "75", "70", "65", "60", "55", "50", "45",
"40", "35", "30", "25", "20", "15", "10", "5", NULL };

static void UI_DrawHandicap(rectDef_t * rect, float scale, vec4_t color,
			    int textStyle)
{
	int i, h;
...
static void UI_DrawClanName(rectDef_t * rect, float scale, vec4_t color,
			    int textStyle)
{
...
{
	Text_Paint(rect->x, rect->y, scale, color,
		   uiInfo.gameTypes[ui_gameType.integer].gameType, 0, 0,
		   textStyle);
}

static void UI_DrawNetGameType(rectDef_t * rect, float scale, vec4_t color,
			       int textStyle)
{
	...
	Text_Paint(rect->x, rect->y, scale, color,
		   uiInfo.gameTypes[ui_netGameType.integer].gameType, 0, 0,
		   textStyle);
}

static void UI_DrawJoinGameType(rectDef_t * rect, float scale, vec4_t color,
				int textStyle)
{
	if (ui_joinGameType.integer < 0
	    || ui_joinGameType.integer > uiInfo.numJoinGameTypes) {
		trap_Cvar_Set("ui_joinGameType", "0");
	}
	Text_Paint(rect->x, rect->y, scale, color,
		   uiInfo.joinGameTypes[ui_joinGameType.integer].gameType, 0, 0,
		   textStyle);
}
	...
	if (i >= 0 && i < uiInfo.teamCount) {
		...
		if (uiInfo.teamList[i].teamIcon == -1) {
			uiInfo.teamList[i].teamIcon =
			    trap_R_RegisterShaderNoMip(uiInfo.teamList[i].
						       imageName);
	...
	if (i >= 0 && i < uiInfo.teamCount) {
		...
		if (uiInfo.teamList[i].cinematic >= -2) {
			...
			if (uiInfo.teamList[i].cinematic >= 0) {
				trap_CIN_RunCinematic(uiInfo.teamList[i].
						      cinematic);
				trap_CIN_SetExtents(uiInfo.teamList[i].
						    cinematic, rect->x, rect->y,
						    rect->w, rect->h);
				trap_CIN_DrawCinematic(uiInfo.teamList[i].
						       cinematic);
			} else {
				trap_R_SetColor(color);
...
===============
*/
static void UI_DrawInfoPane(tremInfoPane_t * pane, rectDef_t * rect,
			    float text_x, float text_y, float scale,
			    vec4_t color, int textStyle)
{
	...
	for (i = 0; i < pane->numGraphics; i++) {
		...
		if (pane->graphics[i].side == INFOPANE_TOP
		    || pane->graphics[i].side == INFOPANE_BOTTOM) {
			//set horizontal offset of graphic
			if (pane->graphics[i].offset < 0)
				xoffset =
				    (rect->w / 2) -
				    (pane->graphics[i].width / 2);
			else
				xoffset =
				    pane->graphics[i].offset + GRAPHIC_BWIDTH;
		} else if (pane->graphics[i].side == INFOPANE_LEFT
			   || pane->graphics[i].side == INFOPANE_RIGHT) {
			//set vertical offset of graphic
			if (pane->graphics[i].offset < 0)
				yoffset =
				    (rect->h / 2) -
				    (pane->graphics[i].height / 2);
			else
				yoffset =
				    pane->graphics[i].offset + GRAPHIC_BWIDTH;
		}
		...
		if (pane->graphics[i].side == INFOPANE_LEFT) {
			...
			if (pane->graphics[i].width > maxLeft)
				maxLeft =
				    pane->graphics[i].width + GRAPHIC_BWIDTH;
			...
			if (pane->graphics[i].height > maxTop)
				maxTop =
				    pane->graphics[i].height + GRAPHIC_BWIDTH;
		...
		//draw the graphic
		UI_DrawHandlePic(x + xoffset, y + yoffset, width, height,
				 graphic);
	}
...
static void UI_DrawSkill(rectDef_t * rect, float scale, vec4_t color,
			 int textStyle)
{
	int i;
	...
	Text_Paint(rect->x, rect->y, scale, color, skillLevels[i - 1], 0, 0,
		   textStyle);
}
...
{
	int i;
	i = UI_TeamIndexFromName(UI_Cvar_VariableString
				 ((blue) ? "ui_blueTeam" : "ui_redTeam"));
	if (i >= 0 && i < uiInfo.teamCount) {
		Text_Paint(rect->x, rect->y, scale, color,
			   va("%s: %s", (blue) ? "Blue" : "Red",
			      uiInfo.teamList[i].teamName), 0, 0, textStyle);
	}
}
			      ...
			      qboolean blue, int num, int textStyle)
{
	...
	// 2..NumCharacters - Bot
	int value =
	    trap_Cvar_VariableValue(va
				    (blue ? "ui_blueteam%i" : "ui_redteam%i",
				     num));
	const char *text;
	if (value <= 0) {
		text = "Closed";
				 ...
				 int textStyle)
{
	...
	if (ui_currentMap.integer < 0
	    || ui_currentMap.integer > uiInfo.mapCount) {
		ui_currentMap.integer = 0;
		trap_Cvar_Set("ui_currentMap", "0");
	}

	time =
	    uiInfo.mapList[ui_currentMap.integer].timeToBeat[uiInfo.
							     gameTypes
							     [ui_gameType.
							      integer].gtEnum];
	...
	Text_Paint(rect->x, rect->y, scale, color,
		   va("%02i:%02i", minutes, seconds), 0, 0, textStyle);
}
...
//    UI_RegisterClientModelname( &info, model, head, team);
	...
	UI_DrawPlayer(rect->x, rect->y, rect->w, rect->h, &info,
		      uiInfo.uiDC.realTime / 2);

}

static void UI_DrawNetSource(rectDef_t * rect, float scale, vec4_t color,
			     int textStyle)
{
	...
	Text_Paint(rect->x, rect->y, scale, color,
		   va("Source: %s", netSources[ui_netSource.integer]), 0, 0,
		   textStyle);
}
...
static void UI_DrawNetMapCinematic(rectDef_t * rect, float scale, vec4_t color)
{
	...
	if (uiInfo.serverStatus.currentServerCinematic >= 0) {
		trap_CIN_RunCinematic(uiInfo.serverStatus.
				      currentServerCinematic);
		trap_CIN_SetExtents(uiInfo.serverStatus.currentServerCinematic,
				    rect->x, rect->y, rect->w, rect->h);
		trap_CIN_DrawCinematic(uiInfo.serverStatus.
				       currentServerCinematic);
	} else {
...
static void UI_DrawTier(rectDef_t * rect, float scale, vec4_t color,
			int textStyle)
{
	int i;
	...
	Text_Paint(rect->x, rect->y, scale, color,
		   va("Tier: %s", uiInfo.tierList[i].tierName), 0, 0,
		   textStyle);
}
...
static void UI_DrawTierMapName(rectDef_t * rect, float scale, vec4_t color,
			       int textStyle)
{
	int i, j;
	i = trap_Cvar_VariableValue("ui_currentTier");
	...
	Text_Paint(rect->x, rect->y, scale, color,
		   UI_EnglishMapName(uiInfo.tierList[i].maps[j]), 0, 0,
		   textStyle);
}

static void UI_DrawTierGameType(rectDef_t * rect, float scale, vec4_t color,
				int textStyle)
{
	int i, j;
	i = trap_Cvar_VariableValue("ui_currentTier");
	...
	Text_Paint(rect->x, rect->y, scale, color,
		   uiInfo.gameTypes[uiInfo.tierList[i].gameTypes[j]].gameType,
		   0, 0, textStyle);
}
...
static void UI_DrawOpponent(rectDef_t * rect)
{
	...
	if (updateOpponentModel) {
		...
		UI_PlayerInfo_SetInfo(&info2, LEGS_IDLE, TORSO_STAND,
				      viewangles, vec3_origin, WP_MACHINEGUN,
				      qfalse);
		UI_RegisterClientModelname(&info2, model, headmodel, team);
		updateOpponentModel = qfalse;
	}

	UI_DrawPlayer(rect->x, rect->y, rect->w, rect->h, &info2,
		      uiInfo.uiDC.realTime / 2);
				    ...
				    int textStyle, qboolean net)
{
	...
	if (map >= 0 && map < uiInfo.mapCount) {
		Text_Paint(rect->x, rect->y, scale, color,
			   uiInfo.mapList[map].mapName, 0, 0, textStyle);
	}
}
...
static int UI_OwnerDrawWidth(int ownerDraw, float scale)
{
	...
	case UI_NETSOURCE:
		if (ui_netSource.integer < 0
		    || ui_netSource.integer > uiInfo.numJoinGameTypes) {
			ui_netSource.integer = 0;
		}
...
static void UI_DrawBotSkill(rectDef_t * rect, float scale, vec4_t color,
			    int textStyle)
{
	if (uiInfo.skillIndex >= 0 && uiInfo.skillIndex < numSkillLevels) {
		Text_Paint(rect->x, rect->y, scale, color,
			   skillLevels[uiInfo.skillIndex], 0, 0, textStyle);
	}
}
...
static void UI_BuildPlayerList(void)
{
	...
	for (n = 0; n < count; n++) {
		...
		if (info[0]) {
			BG_ClientListParse(&uiInfo.
					   ignoreList[uiInfo.playerCount],
			...
			if (team2 == team) {
				Q_strncpyz(uiInfo.
					   rawTeamNames[uiInfo.myTeamCount],
					   Info_ValueForKey(info, "n"),
					   MAX_NAME_LENGTH);
...
static void UI_DrawServerRefreshDate(rectDef_t * rect, float scale,
				     vec4_t color, int textStyle)
{
	if (uiInfo.serverStatus.refreshActive) {
		...
		LerpColor(color, lowLight, newColor,
			  0.5 +
			  0.5 * sin(uiInfo.uiDC.realTime / PULSE_DIVISOR));
		Text_Paint(rect->x, rect->y, scale, newColor,
			   va("Getting info for %d servers (ESC to cancel)",
			      trap_LAN_GetServerCount(ui_netSource.integer)), 0,
			   0, textStyle);
	} else {
		char buff[64];
		Q_strncpyz(buff,
			   UI_Cvar_VariableString(va
						  ("ui_lastServerRefresh_%i",
						   ui_netSource.integer)), 64);
		Text_Paint(rect->x, rect->y, scale, color,
			   va("Refresh Time: %s", buff), 0, 0, textStyle);
	}
}
...
static void UI_DrawServerMOTD(rectDef_t * rect, float scale, vec4_t color)
{
	if (uiInfo.serverStatus.motdLen) {
		...
		if (uiInfo.serverStatus.motdOffset >
		    uiInfo.serverStatus.motdLen) {
		...
		if (uiInfo.uiDC.realTime > uiInfo.serverStatus.motdTime) {
			uiInfo.serverStatus.motdTime =
			    uiInfo.uiDC.realTime + 10;
			if (uiInfo.serverStatus.motdPaintX <= rect->x + 2) {
				if (uiInfo.serverStatus.motdOffset <
				    uiInfo.serverStatus.motdLen) {
					uiInfo.serverStatus.motdPaintX +=
					    Text_Width(&uiInfo.serverStatus.
						       motd[uiInfo.serverStatus.
							    motdOffset], scale,
						       1) - 1;
					uiInfo.serverStatus.motdOffset++;
				} else {
					uiInfo.serverStatus.motdOffset = 0;
					if (uiInfo.serverStatus.motdPaintX2 >=
					    0) {
						uiInfo.serverStatus.motdPaintX =
						    uiInfo.serverStatus.
						    motdPaintX2;
		...
		Text_Paint_Limit(&maxX, uiInfo.serverStatus.motdPaintX,
				 rect->y + rect->h - 3, scale, color,
				 &uiInfo.serverStatus.motd[uiInfo.serverStatus.
							   motdOffset], 0, 0);
		if (uiInfo.serverStatus.motdPaintX2 >= 0) {
			float maxX2 = rect->x + rect->w - 2;
			Text_Paint_Limit(&maxX2,
					 uiInfo.serverStatus.motdPaintX2,
					 rect->y + rect->h - 3, scale, color,
					 uiInfo.serverStatus.motd, 0,
					 uiInfo.serverStatus.motdOffset);
		}
...
//  int ofs = 0; TTimo: unused
	if (Display_KeyBindPending()) {
		Text_Paint(rect->x, rect->y, scale, color,
			   "Waiting for new key... Press ESCAPE to cancel", 0,
			   0, textStyle);
	} else {
		Text_Paint(rect->x, rect->y, scale, color,
...
static void UI_DrawGLInfo(rectDef_t * rect, float scale, vec4_t color,
			  int textStyle)
{
	char *eptr;
	char buff[1024];
	...
	Text_Paint(rect->x + 2, rect->y, scale, color,
	...
	Text_Paint(rect->x + 2, rect->y + 15, scale, color,
		   va("VERSION: %s: %s", uiInfo.uiDC.glconfig.version_string,
		      uiInfo.uiDC.glconfig.renderer_string), 0, 30, textStyle);
	Text_Paint(rect->x + 2, rect->y + 30, scale, color,
		   va("PIXELFORMAT: color(%d-bits) Z(%d-bits) stencil(%d-bits)",
		      ...
		      uiInfo.uiDC.glconfig.stencilBits), 0, 30, textStyle);
	...
	i = 0;
	while (i < numLines) {
		Text_Paint(rect->x + 2, y, scale, color, lines[i++], 0, 20,
			   textStyle);
		if (i < numLines) {
			Text_Paint(rect->x + rect->w / 2, y, scale, color,
				   lines[i++], 0, 20, textStyle);
		}
		y += 10;
			 ...
			 int textStyle)
{
	...
	case UI_TEAMINFOPANE:
		if ((pane = uiInfo.tremTeamList[uiInfo.tremTeamIndex].infopane))
			UI_DrawInfoPane(pane, &rect, text_x, text_y, scale,
					color, textStyle);
		break;

	case UI_ACLASSINFOPANE:
		if ((pane =
		     uiInfo.tremAlienClassList[uiInfo.tremAlienClassIndex].
		     infopane))
			UI_DrawInfoPane(pane, &rect, text_x, text_y, scale,
					color, textStyle);
		break;
	...
	case UI_AUPGRADEINFOPANE:
		if ((pane =
		     uiInfo.tremAlienUpgradeList[uiInfo.tremAlienUpgradeIndex].
		     infopane))
			UI_DrawInfoPane(pane, &rect, text_x, text_y, scale,
					color, textStyle);
		break;
	...
	case UI_HITEMINFOPANE:
		if ((pane =
		     uiInfo.tremHumanItemList[uiInfo.tremHumanItemIndex].
		     infopane))
			UI_DrawInfoPane(pane, &rect, text_x, text_y, scale,
					color, textStyle);
		break;
	...
	case UI_HBUYINFOPANE:
		if ((pane =
		     uiInfo.tremHumanArmouryBuyList[uiInfo.
						    tremHumanArmouryBuyIndex].
		     infopane))
			UI_DrawInfoPane(pane, &rect, text_x, text_y, scale,
					color, textStyle);
		break;
	...
	case UI_HSELLINFOPANE:
		if ((pane =
		     uiInfo.tremHumanArmourySellList[uiInfo.
						     tremHumanArmourySellIndex].
		     infopane))
			UI_DrawInfoPane(pane, &rect, text_x, text_y, scale,
					color, textStyle);
		break;
	...
	case UI_ABUILDINFOPANE:
		if ((pane =
		     uiInfo.tremAlienBuildList[uiInfo.tremAlienBuildIndex].
		     infopane))
			UI_DrawInfoPane(pane, &rect, text_x, text_y, scale,
					color, textStyle);
		break;
	...
	case UI_HBUILDINFOPANE:
		if ((pane =
		     uiInfo.tremHumanBuildList[uiInfo.tremHumanBuildIndex].
		     infopane))
			UI_DrawInfoPane(pane, &rect, text_x, text_y, scale,
					color, textStyle);
		break;
	...
	while (flags) {
		...
		if (flags & UI_SHOW_TEAMVOTEACTIVE) {
			if (team == PTE_ALIENS) {
				if (!trap_Cvar_VariableValue
				    ("ui_alienTeamVoteActive"))
					vis = qfalse;
			} else if (team == PTE_HUMANS) {
				if (!trap_Cvar_VariableValue
				    ("ui_humanTeamVoteActive"))
					vis = qfalse;
			}
		...
		if (flags & UI_SHOW_LEADER) {
			...
			if (!uiInfo.teamLeader) {
				...
				// if showing yourself
				if (ui_selectedPlayer.integer <
				    uiInfo.myTeamCount
				    && uiInfo.teamClientNums[ui_selectedPlayer.
							     integer] ==
				    uiInfo.playerNumber) {
					vis = qfalse;
				}
				      ...
				      qboolean resetMap)
{
	if (key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER
		...
		UI_LoadBestScores(uiInfo.mapList[ui_currentMap.integer].
				  mapLoadName,
...
static qboolean UI_NetGameType_HandleKey(int flags, float *special, int key)
{
	if (key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER
		...
		trap_Cvar_Set("ui_netGameType",
			      va("%d", ui_netGameType.integer));
...
static qboolean UI_JoinGameType_HandleKey(int flags, float *special, int key)
{
	if (key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER
		...
		trap_Cvar_Set("ui_joinGameType",
			      va("%d", ui_joinGameType.integer));
...
static void UI_LoadTremAlienClasses(void)
{
	...
	if (BG_ClassIsAllowed(PCL_ALIEN_BUILDER0_UPG) &&
	    BG_FindStagesForClass(PCL_ALIEN_BUILDER0_UPG,
				  UI_GetCurrentAlienStage()))
...
static void UI_ParseCarriageList(int *weapons, int *upgrades)
{
	...
	trap_Cvar_VariableStringBuffer("ui_carriage", carriageCvar,
				       sizeof(carriageCvar));
...
static void UI_LoadTremHumanArmourySells(void)
{
	...
	for (i = WP_NONE + 1; i < WP_NUM_WEAPONS; i++) {
		if (weapons & (1 << i)) {
			uiInfo.tremHumanArmourySellList[j].text =
			    String_Alloc(BG_FindHumanNameForWeapon(i));
			uiInfo.tremHumanArmourySellList[j].cmd =
			    String_Alloc(va
					 ("cmd sell %s retrigger\n",
					  BG_FindNameForWeapon(i)));
...
static void UI_LoadTremAlienUpgrades(void)
{
	...
	for (i = PCL_NONE + 1; i < PCL_NUM_CLASSES; i++) {
		if (BG_ClassCanEvolveFromTo(class, i, credits, 0) >= 0 &&
		    BG_FindStagesForClass(i, stage) && BG_ClassIsAllowed(i)) {
			uiInfo.tremAlienUpgradeList[j].text =
			    String_Alloc(BG_FindHumanNameForClassNum(i));
...
static void UI_LoadMods(void)
{
	...
	for (i = 0; i < numdirs; i++) {
		...
		uiInfo.modList[uiInfo.modCount].modDescr =
		    String_Alloc(descptr);
...
static void UI_LoadMovies(void)
{
	...
	uiInfo.movieCount =
	    trap_FS_GetFileList("video", "roq", movielist, 4096);
...
static void UI_LoadDemos(void)
{
	...
	Com_sprintf(demoExt, sizeof(demoExt), "dm_%d",
		    (int)trap_Cvar_VariableValue("protocol"));

	uiInfo.demoCount =
	    trap_FS_GetFileList("demos", demoExt, demolist, 4096);
	...
	if (uiInfo.demoCount) {
		...
		for (i = 0; i < uiInfo.demoCount; i++) {
			len = strlen(demoname);
			if (!Q_stricmp
			    (demoname + len - strlen(demoExt), demoExt)) {
...
static qboolean UI_SetNextMap(int actual, int index)
{
	...
	for (i = actual + 1; i < uiInfo.mapCount; i++) {
		if (uiInfo.mapList[i].active) {
			Menu_SetFeederSelection(NULL, FEEDER_MAPS, index + 1,
						"skirmish");
			return qtrue;
		}
	}
...
static void UI_StartSkirmish(qboolean next)
{
	...
	if (next) {
		...
		if (UI_SetNextMap(actual, index)) {
			...
			UI_MapCountByGameType(qtrue);
			Menu_SetFeederSelection(NULL, FEEDER_MAPS, 0,
						"skirmish");
		}
	}
	...
	trap_Cmd_ExecuteText(EXEC_APPEND,
			     va("wait ; wait ; map %s\n",
				uiInfo.mapList[ui_currentMap.integer].
				mapLoadName));
	skill = trap_Cvar_VariableValue("g_spSkill");
	trap_Cvar_Set("ui_scoreMap",
		      uiInfo.mapList[ui_currentMap.integer].mapName);
...

	{
		...
		for (i = 0;
		     i < uiInfo.mapList[ui_currentMap.integer].teamMembers;
		     i++) {
			Com_sprintf(buff, sizeof(buff),
				    "addbot %s %f %s %i %s\n",
				    UI_AIFromName(uiInfo.teamList[k].
						  teamMembers[i]), skill, "",
	...
	if (String_Parse(args, &name)) {
		if (Q_stricmp(name, "StartServer") == 0) {
			...
			trap_Cvar_SetValue("dedicated",
					   Com_Clamp(0, 2,
						     ui_dedicated.integer));
			trap_Cvar_SetValue("g_gametype",
					   Com_Clamp(0, 8,
						     uiInfo.
						     gameTypes[ui_netGameType.
							       integer].
						     gtEnum));
			trap_Cvar_Set("g_redTeam",
				      UI_Cvar_VariableString("ui_teamName"));
			trap_Cvar_Set("g_blueTeam",
				      UI_Cvar_VariableString
				      ("ui_opponentName"));
			trap_Cmd_ExecuteText(EXEC_APPEND,
					     va("wait ; wait ; map %s\n",
						uiInfo.mapList[ui_currentNetMap.
							       integer].
						mapLoadName));
			skill = trap_Cvar_VariableValue("g_spSkill");
			...
			UI_MapCountByGameType(qtrue);
			ui_mapIndex.integer =
			    UI_GetIndexFromSelection(ui_currentMap.integer);
			trap_Cvar_Set("ui_mapIndex",
				      va("%d", ui_mapIndex.integer));
			Menu_SetFeederSelection(NULL, FEEDER_MAPS,
						ui_mapIndex.integer,
						"skirmish");
			UI_GameType_HandleKey(0, NULL, K_MOUSE1, qfalse);
			...
			UI_LoadArenas();
			UI_MapCountByGameType(qfalse);
			Menu_SetFeederSelection(NULL, FEEDER_ALLMAPS, 0,
						"createserver");
      ...
      UI_LoadBestScores(uiInfo.mapList[ui_currentMap.integer].mapLoadName, uiInfo.gameTypes[ui_gameType.integer].gtEnum);*/
			...
			if (uiInfo.demoAvailable) {
				trap_Cmd_ExecuteText(EXEC_APPEND,
						     va("demo %s_%i\n",
							uiInfo.
							mapList[ui_currentMap.
								integer].
							mapLoadName,
							uiInfo.
							gameTypes[ui_gameType.
								  integer].
							gtEnum));
			}
...
//TA: tremulous menus
			...
			if ((cmd =
			     uiInfo.tremAlienUpgradeList[uiInfo.
							 tremAlienUpgradeIndex].
			     cmd))
				trap_Cmd_ExecuteText(EXEC_APPEND, cmd);
			...
			if ((cmd =
			     uiInfo.tremAlienBuildList[uiInfo.
						       tremAlienBuildIndex].
			     cmd))
				trap_Cmd_ExecuteText(EXEC_APPEND, cmd);
			...
			if ((cmd =
			     uiInfo.tremHumanBuildList[uiInfo.
						       tremHumanBuildIndex].
			     cmd))
				trap_Cmd_ExecuteText(EXEC_APPEND, cmd);
		} else if (Q_stricmp(name, "Say") == 0) {
			char buffer[MAX_CVAR_VALUE_STRING];
			trap_Cvar_VariableStringBuffer("ui_sayBuffer", buffer,
						       sizeof(buffer));
			...
			if (len > 0 && (len < sizeof(text) - 1)) {
				...
				Com_sprintf(command, 32, "ptrcrestore %s",
					    text);
...
//TA: tremulous menus
			...
			UI_BuildServerStatus(qtrue);
			Menu_SetFeederSelection(NULL, FEEDER_FINDPLAYER, 0,
						NULL);
			...
			if (uiInfo.serverStatus.currentServer >= 0
				...
				trap_LAN_GetServerAddressString(ui_netSource.
								integer,
								uiInfo.
								serverStatus.
								displayServers
			...
			trap_Key_SetCatcher(trap_Key_GetCatcher() &
					    ~KEYCATCH_UI);
			...
			if (ui_netSource.integer != AS_FAVORITES) {
				...
				name[0] = addr[0] = '\0';
				Q_strncpyz(name,
					   Info_ValueForKey(buff, "hostname"),
					   MAX_NAME_LENGTH);
				Q_strncpyz(addr, Info_ValueForKey(buff, "addr"),
					   MAX_NAME_LENGTH);
				if (strlen(name) > 0 && strlen(addr) > 0) {
					res =
					    trap_LAN_AddServer(AS_FAVORITES,
							       name, addr);
					if (res == 0) {
			...
			if (ui_netSource.integer == AS_FAVORITES) {
				...
				Q_strncpyz(addr, Info_ValueForKey(buff, "addr"),
					   MAX_NAME_LENGTH);
				if (strlen(addr) > 0) {
					trap_LAN_RemoveServer(AS_FAVORITES,
							      addr);
				}
			}
			...
			if (uiInfo.ignoreIndex >= 0
			    && uiInfo.ignoreIndex < uiInfo.playerCount) {
				if (BG_ClientListTest
				    (&uiInfo.ignoreList[uiInfo.myPlayerIndex],
				     uiInfo.clientNums[uiInfo.ignoreIndex])) {
					BG_ClientListRemove(&uiInfo.
							    ignoreList[uiInfo.
								       myPlayerIndex],
							    uiInfo.
							    clientNums[uiInfo.
...
static int UI_MapCountByGameType(qboolean singlePlayer)
{
	...
	c = 0;
	game =
	    singlePlayer ? uiInfo.gameTypes[ui_gameType.integer].
...
qboolean UI_hasSkinForBase(const char *base, const char *team)
{
	...
	Com_sprintf(test, sizeof(test),
		    "models/players/%s/%s/lower_default.skin", base, team);
	...
	if (!init) {
		for (i = 0; i < uiInfo.characterCount; i++) {
			...
			for (j = 0; j < uiInfo.teamCount; j++) {
				if (UI_hasSkinForBase
				    (uiInfo.characterList[i].base,
				     uiInfo.teamList[j].teamName)) {
					uiInfo.characterList[i].reference |=
					    (1 << j);
				}
			}
		}
	...
	for (j = 0; j < TEAM_MEMBERS; j++) {
		for (k = 0; k < uiInfo.aliasCount; k++) {
			if (uiInfo.aliasList[k].name != NULL) {
				if (Q_stricmp
				    (uiInfo.teamList[tIndex].teamMembers[j],
				     uiInfo.aliasList[k].name) == 0) {
					for (i = 0; i < uiInfo.characterCount;
					     i++) {
						if (uiInfo.characterList[i].
						    headImage != -1
						    && uiInfo.characterList[i].
						    reference & (1 << tIndex)
						    && Q_stricmp(uiInfo.
								 aliasList[k].
								 ai,
...
static void UI_RemoveServerFromDisplayList(int num)
{
	...
	for (i = 0; i < uiInfo.serverStatus.numDisplayServers; i++) {
		if (uiInfo.serverStatus.displayServers[i] == num) {
			uiInfo.serverStatus.numDisplayServers--;
			for (j = i; j < uiInfo.serverStatus.numDisplayServers;
			     j++) {
...
static void UI_BinaryServerInsertion(int num)
{
	...
	while (mid > 0) {
		...
		//
		res =
		    trap_LAN_CompareServers(ui_netSource.integer,
					    uiInfo.serverStatus.sortKey,
					    uiInfo.serverStatus.sortDir, num,
					    uiInfo.serverStatus.
					    displayServers[offset + mid]);
		// if equal
		if (res == 0) {
...
//  qboolean startRefresh = qtrue; TTimo: unused
	...
	if (!
	    (force
	     ...
	     uiInfo.serverStatus.nextDisplayRefresh)) {
		return;
	}
	...
	trap_Cvar_VariableStringBuffer("cl_motdString",
				       uiInfo.serverStatus.motd,
	...
	for (i = 0; i < count; i++) {
		...
		if (ping > 0 || ui_netSource.integer == AS_FAVORITES) {
			...
			if (uiInfo.joinGameTypes[ui_joinGameType.integer].
			    gtEnum != -1) {
				game = atoi(Info_ValueForKey(info, "gametype"));
				if (game !=
				    uiInfo.joinGameTypes[ui_joinGameType.
							 integer].gtEnum) {
					trap_LAN_MarkServerVisible(ui_netSource.
								   integer, i,
								   qfalse);
			...
			if (ping > 0) {
				trap_LAN_MarkServerVisible(ui_netSource.integer,
							   i, qfalse);
				numinvisible++;
			}
		}
...
static void UI_SortServerStatusInfo(serverStatusInfo_t * info)
{
	...
	for (i = 0; serverStatusCvars[i].name; i++) {
		for (j = 0; j < info->numLines; j++) {
			...
			if (!Q_stricmp
			    (serverStatusCvars[i].name, info->lines[j][0])) {
				// swap lines
				  ...
				  serverStatusInfo_t * info)
{
	...
	if (trap_LAN_ServerStatus
	    (serverAddress, info->text, sizeof(info->text))) {
...
static void UI_BuildFindPlayerList(qboolean force)
{
	...
	if (!force) {
		...
		memset(&uiInfo.pendingServerStatus, 0,
		       sizeof(uiInfo.pendingServerStatus));
		...
		Com_sprintf(uiInfo.
			    foundPlayerServerNames[uiInfo.
						   numFoundPlayerServers - 1],
			    sizeof(uiInfo.
				   foundPlayerServerNames[uiInfo.
	...
	for (i = 0; i < MAX_SERVERSTATUSREQUESTS; i++) {
		...
		if (!uiInfo.pendingServerStatus.server[i].valid ||
			...
			UI_GetServerStatusInfo(uiInfo.pendingServerStatus.
					       server[i].adrstr, NULL);
			// reuse pending slot
			...
			if (uiInfo.pendingServerStatus.num <
				...
				trap_LAN_GetServerInfo(ui_netSource.integer,
						       ...
						       infoString,
						       sizeof(infoString));
				Q_strncpyz(uiInfo.pendingServerStatus.server[i].
					   name, Info_ValueForKey(infoString,
								  "hostname"),
					   sizeof(uiInfo.pendingServerStatus.
						  server[0].name));
				uiInfo.pendingServerStatus.server[i].valid =
	...
	if (i < MAX_SERVERSTATUSREQUESTS) {
		...
		UI_FeederSelection(FEEDER_FINDPLAYER,
				   uiInfo.currentFoundPlayerServer);
	}
}
...
static void UI_BuildServerStatus(qboolean force)
{
	...
	if (UI_GetServerStatusInfo
	    (uiInfo.serverStatusAddress, &uiInfo.serverStatusInfo)) {
		uiInfo.nextServerStatusRefresh = 0;
...
static int UI_FeederCount(float feederID)
{
	...
	if (feederID == FEEDER_HEADS) {
		...
		return UI_MapCountByGameType(feederID ==
					     FEEDER_MAPS ? qtrue : qfalse);
	...
	if (feederID == FEEDER_HEADS) {
		...
		if (index >= 0 && index < uiInfo.serverStatus.numDisplayServers) {
			...
			if (lastColumn != column
			    || lastTime > uiInfo.uiDC.realTime + 5000) {
				trap_LAN_GetServerInfo(ui_netSource.integer,
						       uiInfo.serverStatus.
						       displayServers[index],
		...
		if (index >= 0 && index < uiInfo.serverStatusInfo.numLines) {
			if (column >= 0 && column < 4) {
				return uiInfo.serverStatusInfo.
				    lines[index][column];
			}
		}
	} else if (feederID == FEEDER_FINDPLAYER) {
		...
		if (index >= 0 && index < uiInfo.playerCount) {
			...
			case 1:
				// am I ignoring him
				return (BG_ClientListTest
					(&uiInfo.
					 ignoreList[uiInfo.myPlayerIndex],
					 uiInfo.clientNums[index])) ? "X" : "";
			case 2:
				// is he ignoring me
				return (BG_ClientListTest
					(&uiInfo.ignoreList[index],
		...
		if (index >= 0 && index < uiInfo.characterCount) {
			if (uiInfo.characterList[index].headImage == -1) {
				uiInfo.characterList[index].headImage =
				    trap_R_RegisterShaderNoMip(uiInfo.
							       characterList
							       [index].
							       imageName);
			}
	...
	if (feederID == FEEDER_HEADS) {
		...
		if (index >= 0 && index < uiInfo.characterCount) {
			trap_Cvar_Set("team_model",
				      va("%s",
					 uiInfo.characterList[index].base));
			trap_Cvar_Set("team_headmodel",
				      va("*%s",
					 uiInfo.characterList[index].name));
			updateModel = qtrue;
		}
	} else if (feederID == FEEDER_Q3HEADS) {
		if (index >= 0 && index < uiInfo.q3HeadCount) {
		...
		map =
		    (feederID ==
		     FEEDER_ALLMAPS) ? ui_currentNetMap.integer : ui_currentMap.
		    integer;
		if (uiInfo.mapList[map].cinematic >= 0) {
		...
		if (feederID == FEEDER_MAPS) {
			...
			UI_LoadBestScores(uiInfo.mapList[ui_currentMap.integer].
					  mapLoadName,
					  uiInfo.gameTypes[ui_gameType.integer].
					  gtEnum);
		...
		uiInfo.serverStatus.currentServerPreview =
		    trap_R_RegisterShaderNoMip(va
						...
						Info_ValueForKey(info,
								 "mapname")));
		if (uiInfo.serverStatus.currentServerCinematic >= 0) {
			trap_CIN_StopCinematic(uiInfo.serverStatus.
					       currentServerCinematic);
		...
		if (mapName && *mapName) {
			uiInfo.serverStatus.currentServerCinematic =
			    trap_CIN_PlayCinematic(va("%s.roq", mapName), 0, 0,
						   0, 0,
						   (CIN_loop | CIN_silent));
		}
	} else if (feederID == FEEDER_SERVERSTATUS) {
		...
		if (index < uiInfo.numFoundPlayerServers - 1) {
			...
			Q_strncpyz(uiInfo.serverStatusAddress,
				   uiInfo.foundPlayerServerAddresses[uiInfo.
								     currentFoundPlayerServer],
				   sizeof(uiInfo.serverStatusAddress));
			Menu_SetFeederSelection(NULL, FEEDER_SERVERSTATUS, 0,
						NULL);
			UI_BuildServerStatus(qtrue);
		}
	} else if (feederID == FEEDER_PLAYER_LIST) {
...
static int UI_PlayCinematic(const char *name, float x, float y, float w,
			    float h)
{
	return trap_CIN_PlayCinematic(name, x, y, w, h,
				      (CIN_loop | CIN_silent));
}
...
static void UI_StopCinematic(int handle)
{
	if (handle >= 0) {
		...
		if (handle == UI_MAPCINEMATIC) {
			if (uiInfo.mapList[ui_currentMap.integer].cinematic >=
			    0) {
				trap_CIN_StopCinematic(uiInfo.
						       mapList[ui_currentMap.
							       integer].
						       cinematic);
				uiInfo.mapList[ui_currentMap.integer].
			...
			int i =
			    UI_TeamIndexFromName(UI_Cvar_VariableString
						 ("ui_teamName"));
			if (i >= 0 && i < uiInfo.teamCount) {
				if (uiInfo.teamList[i].cinematic >= 0) {
					trap_CIN_StopCinematic(uiInfo.
  ...
  UI_ParseGameInfo("gameinfo.txt");*/
	...
	if (uiInfo.uiDC.debug) {
		...
		for (i = 0; i < uiInfo.tremInfoPaneCount; i++) {
			...
			for (j = 0; j < uiInfo.tremInfoPanes[i].numGraphics;
			     j++)
				Com_Printf("graphic %d: %d %d %d %d\n", j,
					   uiInfo.tremInfoPanes[i].graphics[j].
					   side,
					   uiInfo.tremInfoPanes[i].graphics[j].
					   offset,
					   uiInfo.tremInfoPanes[i].graphics[j].
	...
	uiInfo.effectsColor =
	    gamecodetoui[(int)trap_Cvar_VariableValue("color1") - 1];
	uiInfo.currentCrosshair =
	    (int)trap_Cvar_VariableValue("cg_drawCrosshair");
...
void _UI_MouseEvent(int dx, int dy)
{
	...
	if (Menu_Count() > 0) {
		...
		Display_MouseMove(NULL, uiInfo.uiDC.cursorx,
				  uiInfo.uiDC.cursory);
	}
...
void _UI_SetActiveMenu(uiMenuCommand_t menu)
{
	...
	if (Menu_Count() > 0) {
		...
		case UIMENU_MAIN:
			...
			trap_Cvar_VariableStringBuffer("com_errorMessage", buf,
						       sizeof(buf));
			if (strlen(buf)) {
...
static void UI_ReadableSize(char *buf, int bufsize, int value)
{
	if (value > 1024 * 1024 * 1024) {	// gigs
		...
		Com_sprintf(buf + strlen(buf), bufsize - strlen(buf),
			    ".%02d GB",
			    (value % (1024 * 1024 * 1024)) * 100 / (1024 *
								    1024 *
								    1024));
...
static void UI_PrintTime(char *buf, int bufsize, int time)
{
	...
	if (time > 3600) {	// in the hours range
		...
		Com_sprintf(buf, bufsize, "%d min %d sec", time / 60,
			    time % 60);
	} else {		// secs
		      ...
		      const char *text, float adjust)
{
	...
	Text_Paint(x - len / 2, y, scale, color, text, 0, 0,
		   ITEM_TEXTSTYLE_SHADOWEDMORE);
}

void Text_PaintCenter_AutoWrapped(float x, float y, float xmax, float ystep,
			...
			*s2 = '\0';
			...
			if (c_bcp == '\0') {
				...
				if (*s2 != '\0')	// if we are printing an overflowing line we have s2 == s3
					Text_PaintCenter(x, y, scale, color, s2,
							 adjust);
				break;
			}
			s2++;
			...
			{
				Text_PaintCenter(x, y, scale, color, s1,
						 adjust);
				break;
			}
		}
...
static void UI_DisplayDownloadInfo(const char *downloadName, float centerPoint,
				   float yStart, float scale)
{
	...
	Text_PaintCenter(centerPoint, yStart + 112, scale, colorWhite, dlText,
			 0);
	Text_PaintCenter(centerPoint, yStart + 192, scale, colorWhite, etaText,
			 0);
	...
	if (downloadSize > 0) {
		s = va("%s (%d%%)", downloadName,
		       downloadCount * 100 / downloadSize);
	} else {
		s = downloadName;
	}
	...
	if (downloadCount < 4096 || !downloadTime) {
		Text_PaintCenter(leftWidth, yStart + 216, scale, colorWhite,
				 "estimating", 0);
		Text_PaintCenter(leftWidth, yStart + 160, scale, colorWhite,
				 va("(%s of %s copied)", dlSizeBuf,
				    totalSizeBuf), 0);
	} else {
		if ((uiInfo.uiDC.realTime - downloadTime) / 1000) {
			xferRate =
			    downloadCount /
			    ((uiInfo.uiDC.realTime - downloadTime) / 1000);
		} else {
			xferRate = 0;
		}
		...
		if (downloadSize && xferRate) {
			...
			UI_PrintTime(dlTimeBuf, sizeof dlTimeBuf,
				     (n -
				      (((downloadCount / 1024) * n) /
				       (downloadSize / 1024))) * 1000);

			Text_PaintCenter(leftWidth, yStart + 216, scale,
					 colorWhite, dlTimeBuf, 0);
			Text_PaintCenter(leftWidth, yStart + 160, scale,
					 colorWhite, va("(%s of %s copied)",
							dlSizeBuf,
							totalSizeBuf), 0);
		} else {
			Text_PaintCenter(leftWidth, yStart + 216, scale,
					 colorWhite, "estimating", 0);
			if (downloadSize) {
				Text_PaintCenter(leftWidth, yStart + 160, scale,
						 colorWhite,
						 va("(%s of %s copied)",
						    dlSizeBuf, totalSizeBuf),
						 0);
			} else {
				Text_PaintCenter(leftWidth, yStart + 160, scale,
						 colorWhite, va("(%s copied)",
								dlSizeBuf), 0);
			}
		}

		if (xferRate) {
			Text_PaintCenter(leftWidth, yStart + 272, scale,
					 colorWhite, va("%s/Sec", xferRateBuf),
					 0);
		}
	}
}
...
void UI_DrawConnectScreen(qboolean overlay)
{
	...
	if (trap_GetConfigString(CS_SERVERINFO, info, sizeof(info))) {
		Text_PaintCenter(centerPoint, yStart, scale, colorWhite,
				 va("Loading %s",
				    Info_ValueForKey(info, "mapname")), 0);
	}
	...
	if (!Q_stricmp(cstate.servername, "localhost")) {
		...
		Text_PaintCenter(centerPoint, yStart + 48, scale, colorWhite,
				 text, ITEM_TEXTSTYLE_SHADOWEDMORE);
	}

	// display global MOTD at bottom
	Text_PaintCenter(centerPoint, 600, scale, colorWhite,
			 Info_ValueForKey(cstate.updateInfoString, "motd"), 0);
	// print any server info (server full, bad version, etc)
	if (cstate.connState < CA_CONNECTED) {
		Text_PaintCenter_AutoWrapped(centerPoint, yStart + 176, 630, 20,
					     scale, colorWhite,
	...
	if (Q_stricmp(cstate.servername, "localhost")) {
		Text_PaintCenter(centerPoint, yStart + 80, scale, colorWhite, s,
				 0);
	}
...
static cvarTable_t cvarTable[] = {
	...
	{ &ui_lastServerRefresh_0, "ui_lastServerRefresh_0_time", "",
	 CVAR_ARCHIVE },
	{ &ui_lastServerRefresh_1, "ui_lastServerRefresh_1_time", "",
	 CVAR_ARCHIVE },
	{ &ui_lastServerRefresh_2, "ui_lastServerRefresh_2_time", "",
	 CVAR_ARCHIVE },
	{ &ui_lastServerRefresh_3, "ui_lastServerRefresh_3_time", "",
	 CVAR_ARCHIVE },
	...
	{ &ui_realCaptureLimit, "capturelimit", "8",
	 CVAR_SERVERINFO | CVAR_ARCHIVE | CVAR_NORESTART },
	{ &ui_serverStatusTimeOut, "ui_serverStatusTimeOut", "7000",
	 CVAR_ARCHIVE },
