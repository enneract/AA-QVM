...
void UI_SetBestScores(postGameInfo_t * newInfo, qboolean postGame)
{
	...
	trap_Cvar_Set("ui_scoreTeam",
		      va("%i to %i", newInfo->redScore, newInfo->blueScore));
	...
	trap_Cvar_Set("ui_scoreTime",
		      va("%02i:%02i", newInfo->time / 60, newInfo->time % 60));
	trap_Cvar_Set("ui_scoreCaptures", va("%i", newInfo->captures));
	if (postGame) {
		trap_Cvar_Set("ui_scoreAccuracy2",
			      va("%i%%", newInfo->accuracy));
		trap_Cvar_Set("ui_scoreImpressives2",
			      va("%i", newInfo->impressives));
		trap_Cvar_Set("ui_scoreExcellents2",
			      va("%i", newInfo->excellents));
		trap_Cvar_Set("ui_scoreDefends2", va("%i", newInfo->defends));
		trap_Cvar_Set("ui_scoreAssists2", va("%i", newInfo->assists));
		trap_Cvar_Set("ui_scoreGauntlets2",
			      va("%i", newInfo->gauntlets));
		trap_Cvar_Set("ui_scoreScore2", va("%i", newInfo->score));
		trap_Cvar_Set("ui_scorePerfect2", va("%i", newInfo->perfects));
		trap_Cvar_Set("ui_scoreTeam2",
			      va("%i to %i", newInfo->redScore,
				 newInfo->blueScore));
		trap_Cvar_Set("ui_scoreBase2", va("%i", newInfo->baseScore));
		trap_Cvar_Set("ui_scoreTimeBonus2",
			      va("%i", newInfo->timeBonus));
		trap_Cvar_Set("ui_scoreSkillBonus2",
			      va("%i", newInfo->skillBonus));
		trap_Cvar_Set("ui_scoreShutoutBonus2",
			      va("%i", newInfo->shutoutBonus));
		trap_Cvar_Set("ui_scoreTime2",
			      va("%02i:%02i", newInfo->time / 60,
				 newInfo->time % 60));
...
void UI_LoadBestScores(const char *map, int game)
{
	...
	Com_sprintf(fileName, MAX_QPATH, "demos/%s_%d.dm_%d", map, game,
...
void UI_ClearScores(void)
{
	...
	count =
	    trap_FS_GetFileList("games", "game", gameList, sizeof(gameList));
	...
	if (count > 0) {
		...
		for (i = 0; i < count; i++) {
			...
			if (trap_FS_FOpenFile
			    (va("games/%s", gameFile), &f, FS_WRITE) >= 0) {
...
static void UI_CalcPostGameStats(void)
{
	...
	newInfo.time =
	    (time - trap_Cvar_VariableValue("ui_matchStartTime")) / 1000;
	...
	newInfo.score =
	    newInfo.baseScore + newInfo.shutoutBonus + newInfo.timeBonus;
	newInfo.score *= newInfo.skillBonus;

	// see if the score is higher for this one
	newHigh = (newInfo.redScore > newInfo.blueScore
	...
	trap_Cvar_Set("capturelimit",
		      UI_Cvar_VariableString("ui_saveCaptureLimit"));
	...
	trap_Cvar_Set("g_friendlyFire",
		      UI_Cvar_VariableString("ui_friendlyFire"));
