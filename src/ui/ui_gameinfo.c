...
static void UI_LoadArenasFromFile(char *filename)
{
	...
	ui_numArenas +=
	    UI_ParseInfos(buf, MAX_ARENAS - ui_numArenas,
...
void UI_LoadArenas(void)
{
	...
	qsort(uiInfo.mapList, uiInfo.mapCount, sizeof(mapInfo),
	      UI_MapNameCompare);
}
...
static void UI_LoadBotsFromFile(char *filename)
{
	...
	ui_numBots +=
	    UI_ParseInfos(buf, MAX_BOTS - ui_numBots, &ui_botInfos[ui_numBots]);
}
...
void UI_ServerInfo(void)
{
	...
	if (trap_GetConfigString(CS_SERVERINFO, info, sizeof(info))) {
		...
		trap_Cvar_Set("ui_serverinfo_ff", Info_ValueForKey(info, "ff"));
	}
}
