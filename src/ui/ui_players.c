...
{
	pi->legsAnim =
	    ((pi->legsAnim & ANIM_TOGGLEBIT) ^ ANIM_TOGGLEBIT) | anim;
...
static void UI_LegsSequencing(playerInfo_t * pi)
{
	...
	if (pi->legsAnimationTimer > 0) {
		if (currentAnim == LEGS_JUMP) {
			jumpHeight =
			    JUMP_HEIGHT * sin(M_PI *
					      (UI_TIMER_JUMP -
					       pi->legsAnimationTimer) /
					      UI_TIMER_JUMP);
		}
		return;
	}
...
======================
*/
static void UI_PositionEntityOnTag(refEntity_t * entity,
				   ...
				   clipHandle_t parentModel, char *tagName)
{
	...
	MatrixMultiply(lerped.axis, ((refEntity_t *) parent)->axis,
		       entity->axis);
...
======================
*/
static void UI_PositionRotatedEntityOnTag(refEntity_t * entity,
					  const refEntity_t * parent,
					  clipHandle_t parentModel,
					  char *tagName)
{
	int i;
	orientation_t lerped;
...
===============
*/
static void UI_SetLerpFrameAnimation(playerInfo_t * ci, lerpFrame_t * lf,
				     int newAnimation)
{
	animation_t *anim;
...
===============
*/
static void UI_RunLerpFrame(playerInfo_t * ci, lerpFrame_t * lf,
			    int newAnimation)
{
	int f;
	animation_t *anim;
...
===============
*/
static void UI_PlayerAnimation(playerInfo_t * pi, int *legsOld, int *legs,
...
==================
*/
static void UI_SwingAngles(float destination, float swingTolerance,
...
===============
*/
static void UI_PlayerAngles(playerInfo_t * pi, vec3_t legs[3], vec3_t torso[3],
			    vec3_t head[3])
{
	...
	// torso
	UI_SwingAngles(torsoAngles[YAW], 25, 90, SWINGSPEED,
		       &pi->torso.yawAngle, &pi->torso.yawing);
...
===============
*/
static void UI_PlayerFloatSprite(playerInfo_t * pi, vec3_t origin,
				 qhandle_t shader)
{
	refEntity_t ent;
		   ...
		   int time)
{
	...
	if (pi->pendingWeapon != WP_NUM_WEAPONS
	...
	UI_PositionRotatedEntityOnTag(&torso, &legs, pi->legsModel,
				      "tag_torso");
	...
	UI_PositionRotatedEntityOnTag(&head, &torso, pi->torsoModel,
				      "tag_head");
...
==========================
*/
static qboolean UI_FindClientHeadFile(char *filename, int length,
				      const char *teamName,
	...
	while (1) {
		for (i = 0; i < 2; i++) {
			if (i == 0 && teamName && *teamName) {
				Com_sprintf(filename, length,
					    "models/players/%s%s/%s/%s%s_%s.%s",
					    headsFolder, headModelName,
					    headSkinName, teamName, base, team,
					    ext);
			} else {
				Com_sprintf(filename, length,
					    "models/players/%s%s/%s/%s_%s.%s",
					    headsFolder, headModelName,
					    headSkinName, base, team, ext);
			}
			...
			if (i == 0 && teamName && *teamName) {
				Com_sprintf(filename, length,
					    "models/players/%s%s/%s%s_%s.%s",
					    headsFolder, headModelName,
					    teamName, base, headSkinName, ext);
			} else {
				Com_sprintf(filename, length,
					    "models/players/%s%s/%s_%s.%s",
					    headsFolder, headModelName, base,
					    headSkinName, ext);
			}
			if (UI_FileExists(filename)) {
				return qtrue;
...
==========================
*/
static qboolean UI_RegisterClientSkin(playerInfo_t * pi, const char *modelName,
				      const char *skinName,
				      const char *headModelName,
				      const char *headSkinName,
				      const char *teamName)
{
	...
	if (UI_FindClientHeadFile
	    (filename, sizeof(filename), teamName, headModelName, headSkinName,
	     "head", "skin")) {
...
======================
*/
static qboolean UI_ParseAnimationFile(const char *filename,
				    ...
				    const char *teamName)
{
	...
	Com_sprintf(filename, sizeof(filename), "models/players/%s/lower.md3",
		    modelName);
	...
	Com_sprintf(filename, sizeof(filename), "models/players/%s/upper.md3",
		    modelName);
	...
	if (!UI_RegisterClientSkin
	    (pi, modelName, skinName, headModelName, headSkinName, teamName)) {
		if (!UI_RegisterClientSkin
		    (pi, modelName, "default", headModelName, "default",
		     teamName)) {
...
void UI_PlayerInfo_SetModel(playerInfo_t * pi, const char *model,
			    const char *headmodel, char *teamName)
{
			   ...
			   weapon_t weaponNumber, qboolean chat)
{
	...
	} else if ((currentAnim == TORSO_GESTURE || currentAnim == TORSO_ATTACK)
