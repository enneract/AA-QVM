...
void trap_R_AddLightToScene(const vec3_t org, float intensity, float r, float g,
			    float b)
{
	syscall(UI_R_ADDLIGHTTOSCENE, org, PASSFLOAT(intensity), PASSFLOAT(r),
...
{
	syscall(UI_R_DRAWSTRETCHPIC, PASSFLOAT(x), PASSFLOAT(y), PASSFLOAT(w),
...
int trap_CM_LerpTag(orientation_t * tag, clipHandle_t mod, int startFrame,
...
int trap_LAN_ServerStatus(const char *serverAddress, char *serverStatus,
			  int maxLen)
{
	return syscall(UI_LAN_SERVERSTATUS, serverAddress, serverStatus,
		       maxLen);
}
