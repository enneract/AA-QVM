...
static void G_ClientShove(gentity_t * ent, gentity_t * victim)
{
	...
	if (ent->client->pers.teamSelection == PTE_ALIENS) {
		...
		if (BG_InventoryContainsUpgrade
		    (UP_BATTLESUIT, ent->client->ps.stats))
			entMass *= 2;
	} else
		return;
	...
	if (victim->client->pers.teamSelection == PTE_ALIENS) {
		vicMass =
		    BG_FindHealthForClass(victim->client->pers.classSelection);
	} else
	    if (BG_InventoryContainsUpgrade
		(UP_BATTLESUIT, victim->client->ps.stats)) {
		vicMass *= 2;
	}
		...
		if (client->ps.pm_flags & PMF_QUEUED) {
			if (client->ps.stats[STAT_PTEAM] == PTE_ALIENS) {
				client->ps.persistant[PERS_QUEUEPOS] =
				    G_GetPosInSpawnQueue(&level.alienSpawnQueue,
							 client->ps.clientNum);
			} else if (client->ps.stats[STAT_PTEAM] == PTE_HUMANS) {
				client->ps.persistant[PERS_QUEUEPOS] =
				    G_GetPosInSpawnQueue(&level.humanSpawnQueue,
							 client->ps.clientNum);
			}
		}
	}
...
void ClientTimerActions(gentity_t * ent, int msec)
{
	...
	usercmd_t *ucmd;
	int aForward, aRight;
	qboolean walking = qfalse, stopped = qfalse, crouched = qfalse;
	...
	while (client->time100 >= 100) {
		...
		if ((client->ps.stats[STAT_STATE] & SS_SPEEDBOOST) && !crouched) {
			//subtract stamina
			if (BG_InventoryContainsUpgrade
			    (UP_LIGHTARMOUR, client->ps.stats))
				client->ps.stats[STAT_STAMINA] -=
				    STAMINA_LARMOUR_TAKE;
			else
				client->ps.stats[STAT_STAMINA] -=
				    STAMINA_SPRINT_TAKE;
		...
		if (client->ps.weapon == WP_ALEVEL3
			...
			if (client->ps.stats[STAT_MISC] < pounceSpeed
				...
				client->ps.stats[STAT_MISC] +=
				    (100.0f /
				     (float)LEVEL3_POUNCE_CHARGE_TIME) *
				    pounceSpeed;
			...
			if (client->ps.stats[STAT_MISC] < LEVEL4_CHARGE_TIME
				...
				if (ucmd->forwardmove > 0) {
					...
					client->ps.stats[STAT_MISC] +=
					    (int)(100 *
						  (float)
						  LEVEL4_CHARGE_CHARGE_RATIO);

					if (client->ps.stats[STAT_MISC] >
					    LEVEL4_CHARGE_TIME)
						client->ps.stats[STAT_MISC] =
						    LEVEL4_CHARGE_TIME;
				} else
		...
		case WP_HBUILD2:
			...
			if ((client->ps.
			     stats[STAT_BUILDABLE] & ~SB_VALID_TOGGLEBIT) >
			    BA_NONE) {
				int dist =
				...
				if (G_CanBuild
				    (ent,
				     client->ps.
				     stats[STAT_BUILDABLE] &
				     ~SB_VALID_TOGGLEBIT, dist,
				     dummy) == IBE_NONE)
					client->ps.stats[STAT_BUILDABLE] |=
					    SB_VALID_TOGGLEBIT;
				else
					client->ps.stats[STAT_BUILDABLE] &=
					    ~SB_VALID_TOGGLEBIT;
		...
		if (client->ps.stats[STAT_STATE] & SS_MEDKIT_ACTIVE) {
			int remainingStartupTime =
			    MEDKIT_STARTUP_TIME - (level.time -
	...
	while (client->time1000 >= 1000) {
		...
		if (client->ps.stats[STAT_STATE] & SS_POISONCLOUDED)
			G_Damage(ent, client->lastPoisonCloudedClient,
				 client->lastPoisonCloudedClient, NULL, NULL,
		...
		if (client->ps.stats[STAT_STATE] & SS_POISONED) {
			...
			if (BG_InventoryContainsUpgrade
			    (UP_BATTLESUIT, client->ps.stats))
				damage -= BSUIT_POISON_PROTECTION;
			if (BG_InventoryContainsUpgrade
			    (UP_HELMET, client->ps.stats))
				damage -= HELMET_POISON_PROTECTION;
			if (BG_InventoryContainsUpgrade
			    (UP_LIGHTARMOUR, client->ps.stats))
				damage -= LIGHTARMOUR_POISON_PROTECTION;

			G_Damage(ent, client->lastPoisonClient,
				 client->lastPoisonClient, NULL, 0, damage, 0,
				 MOD_POISON);
		}
		...
		if (client->ps.stats[STAT_PTEAM] == PTE_ALIENS &&
			...
			vec3_t range =
			    { LEVEL1_REGEN_RANGE, LEVEL1_REGEN_RANGE,
			 LEVEL1_REGEN_RANGE };
			vec3_t mins, maxs;
			int i, num;
			...
			for (i = 0; i < num; i++) {
				...
				if (boostEntity->client
				    && boostEntity->client->ps.
				    stats[STAT_PTEAM] == PTE_ALIENS
				    && boostEntity->client->ps.
				    stats[STAT_PCLASS] == PCL_ALIEN_LEVEL1_UPG
					...
					if (ent != boostEntity
						...
						G_AddCreditToClient
						    (boostEntity->client,
						     LEVEL1_REGEN_REWARD,
						     qtrue);
						boostEntity->client->pers.
						    statscounters.earned +=
						    LEVEL1_REGEN_REWARD;
					}
					...
					if (client->ps.stats[STAT_PCLASS] == PCL_ALIEN_LEVEL4) {
						modifier = BOOSTER_REGEN_MOD * 1.2f; // give rants a slightly faster healing
			...
			if (ent->health > 0
			    ...
			    level.time) {
				ent->health +=
				    BG_FindRegenRateForClass(client->ps.
							     stats[STAT_PCLASS])
				    * modifier;
		...
		if (ent->client->ps.stats[STAT_HEALTH] > 0
			...
			if (G_BuildableRange
			    (ent->client->ps.origin, 900, BA_A_OVERMIND)) {
				...
				client->pers.campPenalty +=
				    client->pers.campPenalty <
				    0 ? 1 : g_sdDefenderPenaltyIncrement.
				    integer;
			} else
				client->pers.campPenalty -=
				    client->pers.campPenalty <
			...
			if (BG_ClassHasAbility
			    (ent->client->ps.stats[STAT_PCLASS],
			     SCA_WALLCLIMBER)) {
			...
			if (G_BuildableRange
			    (ent->client->ps.origin, 900, BA_H_REACTOR)) {
				ent->client->pers.statscounters.timeinbase++;
				level.alienStatsCounters.timeinbase++;
				if (client->pers.campPenalty <
				    g_sdDefenderMaxPenalty.integer)
					client->pers.campPenalty +=
					    (client->pers.campPenalty <
					     0) ? 1 :
					    g_sdDefenderPenaltyIncrement.
					    integer;
			} else
				client->pers.campPenalty -=
				    (client->pers.campPenalty <
		...
		if (mod_jetpackFuel.value >= 10.0f) {
			...
			if (BG_InventoryContainsUpgrade
				...
				if (client->jetpackfuel <= 0.0f) {
					BG_DeactivateUpgrade(UP_JETPACK,
							     client->ps.stats);
				} else if (client->jetpackfuel < 10.0f
					   && client->jetpackfuel > 0.0f) {
					client->jetpackfuel =
					    client->jetpackfuel -
					    mod_jetpackConsume.value;
				...
				&& !BG_UpgradeIsActive(UP_JETPACK,
						       client->ps.stats)) {
				if (client->jetpackfuel >
				    (mod_jetpackFuel.value - 10.0f)
				    && client->jetpackfuel <=
				    mod_jetpackFuel.value) {
					client->jetpackfuel =
					    client->jetpackfuel +
					    mod_jetpackRegen.value;
					trap_SendServerCommand(client -
							       level.clients,
							       "cp \"^3Fuel Status: ^2Full!^7\n\"");
				} else if (client->jetpackfuel <
					   mod_jetpackFuel.value) {
		...
		if (client->damageOvertime > 0) {
			client->pers.campPenalty -=
			    (int)(client->damageOvertime / 15.0f);
			client->damageOvertime = 0;
		}
		if (client->pers.campPenalty < -g_sdDefenderForgiveness.integer)
			client->pers.campPenalty =
			    -g_sdDefenderForgiveness.integer;
		else if (client->pers.campPenalty >
			 g_sdDefenderMaxPenalty.integer)
			client->pers.campPenalty =
			    g_sdDefenderMaxPenalty.integer;
...
void ClientIntermissionThink(gclient_t * client)
{
	...
	if (client->
	    buttons & (BUTTON_ATTACK | BUTTON_USE_HOLDABLE) & (client->
							       oldbuttons ^
							       client->buttons))
...
void ClientEvents(gentity_t * ent, int oldEventSequence)
{
	...
	for (i = oldEventSequence; i < client->ps.eventSequence; i++) {
		...
		case EV_FALL_FAR:
			...
			fallDistance =
			    ((float)client->ps.stats[STAT_FALLDIST] -
			     MIN_FALL_DISTANCE) / (MAX_FALL_DISTANCE -
						   MIN_FALL_DISTANCE);
			...
			damage = (int)((float)BG_FindHealthForClass(class) *
				       BG_FindFallDamageForClass(class) *
				       fallDistance);

			VectorSet(dir, 0, 0, 1);
			BG_FindBBoxForClass(class, mins, NULL, NULL, NULL,
					    NULL);
			mins[0] = mins[1] = 0.0f;
			VectorAdd(client->ps.origin, mins, point);

			ent->pain_debounce_time = level.time + 200;	// no normal pain sound
			G_Damage(ent, NULL, NULL, dir, point, damage,
...
void G_UnlaggedCalc(int time, gentity_t * rewindEnt)
{
	...
	if (i == MAX_UNLAGGED_MARKERS) {
		...
		frameMsec =
		    level.unlaggedTimes[stopIndex] -
		    level.unlaggedTimes[startIndex];
		lerp =
		    (float)(time -
			    level.unlaggedTimes[startIndex]) / (float)frameMsec;
	}
...
void G_UnlaggedOff(void)
{
	...
	for (i = 0; i < level.maxclients; i++) {
		...
		VectorCopy(ent->client->unlaggedBackup.origin,
			   ent->r.currentOrigin);
...
void G_UnlaggedOn(gentity_t * attacker, vec3_t muzzle, float range)
{
	...
	for (i = 0; i < level.maxclients; i++) {
		...
		VectorCopy(ent->r.currentOrigin,
			   ent->client->unlaggedBackup.origin);
...
//    G_Printf("serverTime >>>>>\n" );
	...
	if (pmove_fixed.integer || client->pers.pmoveFixed) {
		ucmd->serverTime =
		    ((ucmd->serverTime + pmove_msec.integer -
	...
	if (BG_InventoryContainsUpgrade(UP_MEDKIT, client->ps.stats) &&
		...
		if (client->ps.stats[STAT_STATE] & SS_MEDKIT_ACTIVE ||
			...
			//remove anti toxin
			BG_DeactivateUpgrade(UP_MEDKIT, client->ps.stats);
			BG_RemoveUpgradeFromInventory(UP_MEDKIT,
						      client->ps.stats);

			client->ps.stats[STAT_STATE] &= ~SS_POISONED;
			client->poisonImmunityTime =
			    level.time + MEDKIT_POISON_IMMUNITY_TIME;
	...
	if (!(ucmd->buttons & BUTTON_TALK)
		...
		case WP_ALEVEL0:
			if (client->ps.weaponTime <= 0)
				pm.autoWeaponHit[client->ps.weapon] =
				    CheckVenomAttack(ent);
			break;
	...
	if (level.paused)
		client->ps.pm_type = real_pm_type;
	...
	if ((client->buttons & BUTTON_GETFLAG)
	    && !(client->oldbuttons & BUTTON_GETFLAG)
		...
		if (client->ps.stats[STAT_STATE] & SS_HOVELING) {
			...
			if (!AHovel_Blocked(hovel, ent, qtrue)) {
				...
				//hovel is empty
				G_SetBuildableAnim(hovel, BANIM_ATTACK2,
						   qfalse);
				hovel->active = qfalse;
			} else {
				//exit is blocked
				G_TriggerMenu(ent->client->ps.clientNum,
					      MN_A_HOVEL_BLOCKED);
			}
		} else {
#define USE_OBJECT_RANGE 64
			...
			int entityList[MAX_GENTITIES];
			vec3_t range =
			    { USE_OBJECT_RANGE, USE_OBJECT_RANGE,
			 USE_OBJECT_RANGE };
			vec3_t mins, maxs;
			int i, num;
			...
			VectorMA(client->ps.origin, USE_OBJECT_RANGE, view,
				 point);
			trap_Trace(&trace, client->ps.origin, NULL, NULL, point,
				   ent->s.number, MASK_SHOT);

			traceEnt = &g_entities[trace.entityNum];

			if (traceEnt
			    && traceEnt->biteam == client->ps.stats[STAT_PTEAM]
			    && traceEnt->use)
				...
				if (i == num
				    && client->ps.stats[STAT_PTEAM] ==
				    PTE_ALIENS) {
					if (BG_UpgradeClassAvailable
					    (&client->ps)) {
						//no nearby objects and alien - show class menu
						G_TriggerMenu(ent->client->ps.
							      clientNum,
							      MN_A_INFEST);
					} else {
						//flash frags
	...
	if (level.framenum > client->retriggerArmouryMenu
	    && client->retriggerArmouryMenu) {
	...
	if (client->pers.bubbleTime && client->pers.bubbleTime < level.time) {
		...
		bubble =
		    G_TempEntity(client->ps.origin, EV_PLAYER_TELEPORT_OUT);
		...
		if (clientNum >= 0) {
			...
			if (cl->pers.connected == CON_CONNECTED) {
				...
				} else	//we are stickyspec-spectating someone who is spectating someone else
				{
					...
					if (cl->pers.teamSelection ==
					    PTE_ALIENS)
						G_SelectAlienLockSpawnPoint
						    (spawn_origin,
						     spawn_angles);
					else if (cl->pers.teamSelection ==
						 PTE_HUMANS)
