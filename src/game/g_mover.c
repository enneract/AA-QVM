...
gentity_t *G_TestEntityPosition(gentity_t * ent)
{
	...
	if (ent->client)
		trap_Trace(&tr, ent->client->ps.origin, ent->r.mins,
			   ent->r.maxs, ent->client->ps.origin, ent->s.number,
			   mask);
	else
			    ...
			    vec3_t amove)
{
	...
	if (check->client)
		VectorSubtract(check->client->ps.origin,
			       pusher->r.currentOrigin, org);
	else
		VectorSubtract(check->s.pos.trBase, pusher->r.currentOrigin,
			       org);
	...
	if (check->client) {
		VectorAdd(check->client->ps.origin, move,
			  check->client->ps.origin);
		VectorAdd(check->client->ps.origin, move2,
			  check->client->ps.origin);
	...
	if (!block) {
		// pushed ok
		if (check->client)
			VectorCopy(check->client->ps.origin,
		     ...
		     gentity_t ** obstacle)
{
	...
	*obstacle = NULL;
	...
	listedEntities =
	    trap_EntitiesInBox(totalMins, totalMaxs, entityList, MAX_GENTITIES);
...
void G_MoverTeam(gentity_t * ent)
{
	...
	for (part = ent; part; part = part->teamchain) {
		...
		if (part->s.apos.trType == TR_LINEAR_STOP) {
			if (level.time >=
			    part->s.apos.trTime + part->s.apos.trDuration) {
				if (part->reached) {
					part->reached(part);
				}
			}
...
void G_RunMover(gentity_t * ent)
{
	...
	if ((ent->s.pos.trType != TR_STATIONARY || ent->s.apos.trType != TR_STATIONARY) && ent->moverState < MODEL_POS1)	//yuck yuck hack
		G_MoverTeam(ent);
...
void SetMoverState(gentity_t * ent, moverState_t moverState, int time)
{
	...
	if (moverState >= MOVER_POS1 && moverState <= MOVER_2TO1)
		BG_EvaluateTrajectory(&ent->s.pos, level.time,
				      ent->r.currentOrigin);
...
void Think_CloseModelDoor(gentity_t * ent)
{
	...
	numEntities =
	    trap_EntitiesInBox(clipBrush->r.absmin, clipBrush->r.absmax,
...
void Blocked_Door(gentity_t * ent, gentity_t * other)
{
	...
	if (ent->damage)
		G_Damage(other, ent, ent, NULL, NULL, ent->damage, 0,
			 MOD_CRUSH);
...
static void manualDoorTriggerSpectator(gentity_t * door, gentity_t * player)
{
	...
	if (door->moverState == MOVER_1TO2 ||
	    ...
	    door->moverState == MODEL_1TO2 || door->moverState == MODEL_POS2)
		return;
...
void manualTriggerSpectator(gentity_t * trigger, gentity_t * player)
{
	...
	while ((t = G_Find(t, FOFS(targetname), trigger->target)) != NULL) {
		...
		if (!trigger->inuse) {
			G_Printf
			    ("triggerity was removed while using targets\n");
			return;
		}
	}
	...
	if (i > 0) {
		...
		for (j = 0; j < i; j++) {
			float d =
			    Distance(player->r.currentOrigin,
...
void SpawnPlatTrigger(gentity_t * ent)
{
	...
	if (tmax[0] <= tmin[0]) {
		tmin[0] =
		    ent->pos1[0] + (ent->r.mins[0] + ent->r.maxs[0]) * 0.5;
		tmax[0] = tmin[0] + 1;
	}

	if (tmax[1] <= tmin[1]) {
		tmin[1] =
		    ent->pos1[1] + (ent->r.mins[1] + ent->r.maxs[1]) * 0.5;
		tmax[1] = tmin[1] + 1;
	}
...
void SP_func_button(gentity_t * ent)
{
	...
	VectorSubtract(ent->r.maxs, ent->r.mins, size);
	distance =
	    abs_movedir[0] * size[0] + abs_movedir[1] * size[1] +
...
void Blocked_Train(gentity_t * self, gentity_t * other)
{
	if (self->spawnflags & TRAIN_BLOCK_STOPS)
		...
		if (!other->client) {
			...
			//KILL!!1!!!
			G_Damage(other, self, self, NULL, NULL, 10000, 0,
				 MOD_CRUSH);
