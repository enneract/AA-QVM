...
===============
*/
void SP_use_particle_system(gentity_t * self, gentity_t * other,
			    gentity_t * activator)
{
...
===============
*/
void SP_use_anim_model(gentity_t * self, gentity_t * other,
		       gentity_t * activator)
{
	if (self->spawnflags & 1) {
...
===============
*/
void SP_use_light_flare(gentity_t * self, gentity_t * other,
			gentity_t * activator)
{
	self->s.eFlags ^= EF_NODRAW;
}
	...
	for (i = -1; i <= 1; i++) {
		for (j = -1; j <= 1; j++) {
			for (k = -1; k <= 1; k++) {
				...
				trap_Trace(&tr, test, NULL, NULL, test, -1,
					   MASK_SOLID);

				if (!tr.allsolid) {
					trap_Trace(&tr, test, NULL, NULL,
						   origin, -1, MASK_SOLID);
