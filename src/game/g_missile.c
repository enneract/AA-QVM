...
void G_BounceMissile(gentity_t * ent, trace_t * trace)
{
	...
	hitTime =
	    level.previousTime + (level.time -
				  level.previousTime) * trace->fraction;
...
void G_ExplodeMissile(gentity_t * ent)
{
	...
	if (ent->splashDamage)
		G_RadiusDamage(ent->r.currentOrigin, ent->parent,
			       ...
			       ent->dflags, ent->splashMethodOfDeath);
...
void G_MissileImpact(gentity_t * ent, trace_t * trace)
{
		...
		if (other->s.eType == ET_BUILDABLE
			...
			if (!ent->parent)
				G_Printf(S_COLOR_YELLOW
					 "WARNING: hive entity has no parent in G_MissileImpact\n");
			else
	...
	if (other->takedamage) {
		...
		if (ent->damage) {
			...
			G_Damage(other, ent, attacker, velocity, ent->s.origin,
				 ent->damage, ent->dflags, ent->methodOfDeath);
		}
	}
	...
	if (ent->splashDamage)
		G_RadiusDamage(trace->endpos, ent->parent, ent->splashDamage,
			       ent->splashRadius, other, ent->dflags,
...
void G_RunMissile(gentity_t * ent)
{
	...
	if (tr.startsolid || tr.allsolid) {
		...
		trap_Trace(&tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs,
			   ent->r.currentOrigin, passent, ent->clipmask);
		tr.fraction = 0;
	} else
			      ...
			      int damage, int radius)
{
	...
	int localDamage = (int)(ceil(((float)damage /
				      (float)LCANNON_TOTAL_CHARGE) *
				     (float)LCANNON_DAMAGE));
...
gentity_t *launch_grenade(gentity_t * self, vec3_t start, vec3_t dir)
{
	...
	bolt->splashRadius = GRENADE_RANGE * 3;
...
void AHive_ReturnToHive(gentity_t * self)
{
	...
	if (!self->parent) {
		G_Printf(S_COLOR_YELLOW
			 "WARNING: AHive_ReturnToHive called with no self->parent\n");
		return;
	}

	trap_UnlinkEntity(self->parent);
	trap_Trace(&tr, self->r.currentOrigin, self->r.mins, self->r.maxs,
		   self->parent->r.currentOrigin, self->r.ownerNum,
		   self->clipmask);
...
void AHive_SearchAndDestroy(gentity_t * self)
{
	...
	trap_Trace(&tr, self->r.currentOrigin, self->r.mins, self->r.maxs,
		   self->target_ent->r.currentOrigin, self->r.ownerNum,
		   self->clipmask);
	...
	if (tr.entityNum == ENTITYNUM_WORLD ||
	    Distance(self->r.currentOrigin,
		     self->parent->r.currentOrigin) > (HIVE_RANGE * 5)
