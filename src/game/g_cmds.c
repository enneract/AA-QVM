	...
	for (i = 0; i < numSorted; i++) {
		...
		if (cl->sess.sessionTeam != TEAM_SPECTATOR &&
		    (ent->client->pers.teamSelection == PTE_NONE
		     || cl->pers.teamSelection ==
		     ent->client->pers.teamSelection)) {
			weapon = cl->ps.weapon;

			if (BG_InventoryContainsUpgrade
			    (UP_BATTLESUIT, cl->ps.stats))
				upgrade = UP_BATTLESUIT;
			else if (BG_InventoryContainsUpgrade
				 (UP_JETPACK, cl->ps.stats))
				upgrade = UP_JETPACK;
			else if (BG_InventoryContainsUpgrade
				 (UP_BATTPACK, cl->ps.stats))
				upgrade = UP_BATTPACK;
			else if (BG_InventoryContainsUpgrade
				 (UP_HELMET, cl->ps.stats))
				upgrade = UP_HELMET;
			else if (BG_InventoryContainsUpgrade
				 (UP_LIGHTARMOUR, cl->ps.stats))
		...
		Com_sprintf(entry, sizeof(entry),
			    " %d %d %d %d %d %d", level.sortedClients[i],
			    cl->pers.score, ping,
			    (level.time - cl->pers.enterTime) / 60000, weapon,
			    upgrade);
...
qboolean G_Flood_Limited(gentity_t * ent)
{
	...
	if (millisSinceLastCommand < g_floodMinTime.integer)
		ent->client->pers.floodDemerits +=
		    (g_floodMinTime.integer - millisSinceLastCommand);
	else {
		ent->client->pers.floodDemerits -=
		    (millisSinceLastCommand - g_floodMinTime.integer);
	...
	if (!g_floodMaxDemerits.integer)
		maximumDemerits =
		    g_floodMinTime.integer * g_floodMinTime.integer / 1000;
	else
...
void Cmd_Team_f(gentity_t * ent)
{
	...
	if (ent->client->sess.invisible == qtrue) {
		trap_SendServerCommand(ent - g_entities,
				       va
				       ("print \"You cannot join a team while invisible\n\""));
		return;
	}
	...
	if (g_scrimMode.integer != 0
		...
		trap_SendServerCommand(ent - g_entities,
				       va
				       ("print \"You can't join a team when scrim mode is enabled\n\""));
		return;
	}
	...
	if (g_doWarmup.integer && g_warmupMode.integer == 1 &&
		...
		trap_SendServerCommand(ent - g_entities,
				       va("print \"team: you can't join"
					  " a team during warm up (%d seconds remaining)\n\"",
					  g_warmup.integer - (level.time -
							      level.startTime) /
					  1000));
		return;
	}
	...
	if (Q_stricmpn(s, "spec", 4)) {
		if (G_admin_level(ent) < g_minLevelToJoinTeam.integer) {
			trap_SendServerCommand(ent - g_entities,
					       "print \"Sorry, but your admin level is only permitted to spectate.\n\"");
			return;
		}
	}
	...
	if (!Q_stricmpn(s, "spec", 4)) {
		...
		trap_SendServerCommand(ent - g_entities,
					...
					(ent->client->pers.specExpires -
					 level.time) / 1000));
		return;
	} else if (!Q_stricmpn(s, "alien", 5)) {
		if (g_forceAutoSelect.integer
	...
	if (oldteam != PTE_NONE
		...
		trap_SendServerCommand(ent - g_entities,
				       va
				       ("print \"You cannot change teams until build timer expires\n\""));
		return;
	}
	...
	if (team != PTE_NONE) {
		...
		Q_strncpyz(namebuff, ent->client->pers.netname,
			   sizeof(namebuff));
		Q_CleanStr(namebuff);
		...
		if (!namebuff[0] || !Q_stricmp(namebuff, "UnnamedPlayer")) {
			trap_SendServerCommand(ent - g_entities,
					       va
					       ("print \"Please set your player name before joining a team. Press ESC and use the Options / Game menu  or use /name in the console\n\""));
			return;
		}
	}
	...
	if (team == PTE_ALIENS) {
		if (oldteam == PTE_HUMANS)
			Com_sprintf(buf, sizeof(buf),
				    "%s^7 abandoned humans and joined the aliens.",
				    ent->client->pers.netname);
		else
			Com_sprintf(buf, sizeof(buf), "%s^7 joined the aliens.",
				    ent->client->pers.netname);
	} else if (team == PTE_HUMANS) {
		if (oldteam == PTE_ALIENS)
			Com_sprintf(buf, sizeof(buf),
				    "%s^7 abandoned the aliens and joined the humans.",
				    ent->client->pers.netname);
		else
			Com_sprintf(buf, sizeof(buf), "%s^7 joined the humans.",
				    ent->client->pers.netname);
	} else if (team == PTE_NONE) {
		if (oldteam == PTE_HUMANS)
			Com_sprintf(buf, sizeof(buf), "%s^7 left the humans.",
				    ent->client->pers.netname);
		else
			Com_sprintf(buf, sizeof(buf), "%s^7 left the aliens.",
				    ent->client->pers.netname);
	}
...
static void G_SayTo(gentity_t * ent, gentity_t * other, int mode, int color,
		    const char *name, const char *message, const char *prefix)
{
	...
	if ((mode == SAY_TEAM || mode == SAY_ACTION_T)
	...
	if (mode == SAY_ADMINS &&
	    (!G_admin_permission(other, ADMF_ADMINCHAT)
	     || other->client->pers.ignoreAdminWarnings
	     || (g_scrimMode.integer != 0
		 && !G_admin_permission(ent, ADMF_NOSCRIMRESTRICTION))))
		return;

	if (mode == SAY_HADMINS &&
	    (!G_admin_permission(other, ADMF_HIGHADMINCHAT)
	     || other->client->pers.ignoreAdminWarnings))
		return;

	if (BG_ClientListTest
	    (&other->client->sess.ignoreList, ent - g_entities))
		ignore = qtrue;
...
void G_Say(gentity_t * ent, gentity_t * target, int mode, const char *chatText)
{
	...
	if (ent) {
		if (g_spamTime.integer) {
			if ((level.time - ent->client->pers.lastMessageTime) <
			    (g_spamTime.integer * 1000)
			    && !Q_stricmp(ent->client->pers.lastMessage,
					  chatText)
			    && !G_admin_permission(ent, ADMF_NOCENSORFLOOD)
			    && ent->client->pers.floodDemerits <=
			    g_floodMaxDemerits.integer) {
				trap_SendServerCommand(ent - g_entities,
						       "print \"Your message has been ignored to prevent spam\n\"");
				return;
			} else {
				ent->client->pers.lastMessageTime = level.time;
				Q_strncpyz(ent->client->pers.lastMessage,
					   chatText,
					   sizeof(ent->client->pers.
						  lastMessage));
			}
		}
	}
	...
	case SAY_ACTION:
		G_LogPrintf("action: %s^7: %s^7\n", ent->client->pers.netname,
			    chatText);
		Com_sprintf(name, sizeof(name), "^2%s^7%s%s%c%c" EC " ",
			    g_actionPrefix.string, prefix,
			    ent->client->pers.netname, Q_COLOR_ESCAPE,
			    COLOR_WHITE);
		color = COLOR_WHITE;
		break;
	...
	case SAY_ACTION_T:
		...
		if (Team_GetLocationMsg(ent, location, sizeof(location)))
			Com_sprintf(name, sizeof(name),
				    EC "^5%s^7%s%c%c" EC "(%s)" EC " ",
				    g_actionPrefix.string,
				    ent->client->pers.netname, Q_COLOR_ESCAPE,
				    COLOR_WHITE, location);
		else
			Com_sprintf(name, sizeof(name),
				    EC "^5%s^7%s%c%c" EC "" EC " ",
				    g_actionPrefix.string,
				    ent->client->pers.netname, Q_COLOR_ESCAPE,
				    COLOR_WHITE);
		color = COLOR_WHITE;
	...
	case SAY_ADMINS:
		if (G_admin_permission(ent, ADMF_ADMINCHAT))	//Differentiate between inter-admin chatter and user-admin alerts
		{
			G_LogPrintf("say_admins: ^7[^6ADMIN^7]%s^7: %s^7\n",
				    (ent) ? ent->client->pers.
				    netname : "console", chatText);
			Com_sprintf(name, sizeof(name), "%s^7[^6ADMIN^7]%s^7: ",
				    prefix,
				    (ent) ? ent->client->pers.
				    netname : "console");
			color = COLOR_MAGENTA;
	...
	case SAY_HADMINS:
		...
		{
			G_LogPrintf
			    ("say_hadmins: ^7[^QHIGH ADMIN^7]%s^7: %s^7\n",
			     (ent) ? ent->client->pers.netname : "console",
			     chatText);
			Com_sprintf(name, sizeof(name),
				    "%s^7[^QHIGH ADMIN^7]%s^7: ", prefix,
				    (ent) ? ent->client->pers.
				    netname : "console");
			color = COLOR_AMERICAN_ROSE;
		} else if (G_admin_permission(ent, ADMF_ADMINCHAT)) {
			G_LogPrintf
			    ("say_haadmins: ^7[^6LOWER ADMIN^7]^7%s^7: %s^7\n",
			     (ent) ? ent->client->pers.netname : "console",
			     chatText);
			Com_sprintf(name, sizeof(name),
				    "%s^7[^6LOWER ADMIN^7]^7%s^7: ", prefix,
				    (ent) ? ent->client->pers.
				    netname : "console");
			color = COLOR_AMERICAN_ROSE;
		} else {
			G_LogPrintf("say_hadmins: ^7[^ZPLAYER^7]%s^7: %s^7\n",
				    ent->client->pers.netname, chatText);
			Com_sprintf(name, sizeof(name),
				    "%s^7[^ZPLAYER^7]%s^7: ", prefix,
				    (ent) ? ent->client->pers.
	...
	if (mode != SAY_TEAM && ent && ent->client
		...
		trap_SendServerCommand(ent - g_entities,
				       va
				       ("print \"Sorry, but your admin level may only use teamchat while spectating.\n\""));
		return;
	}

	Com_sprintf(text, sizeof(text), "%s^7", chatText);

	if (ent && ent->client && g_aimbotAdvertBan.integer
	    &&
	    (Q_stricmp
	     (text,
	      "^1N^7ullify for ^1T^7remulous [beta] | Get it at CheatersUtopia.com^7")
	     == 0)) {
		trap_SendConsoleCommand(0,
					va("!ban %s %s %s\n",
					   ent->client->pers.ip,
					   (Q_stricmp
					    (g_aimbotAdvertBanTime.string,
					     "0") ==
					    1) ? g_aimbotAdvertBanTime.
					   string : "",
					   g_aimbotAdvertBanReason.string));
		Q_strncpyz(text,
			   "^7has been caught hacking and will be dealt with.",
			   sizeof(text));
	}
...
static void Cmd_SayArea_f(gentity_t * ent)
{
	...
	for (i = 0; i < num; i++)
		G_SayTo(ent, &g_entities[entityList[i]], SAY_TEAM, color, name,
			msg, prefix);
...
static void Cmd_Say_f(gentity_t * ent)
{
	...
	if (Q_stricmpn(args, "say_team ", 9) == 0)
		mode = SAY_TEAM;
	if (Q_stricmpn(args, "say_admins ", 11) == 0
	    || Q_stricmpn(args, "a ", 2) == 0)
		mode = SAY_ADMINS;
	if (Q_stricmpn(args, "say_hadmins ", 12) == 0
	    || Q_stricmpn(args, "ha ", 3) == 0)
		mode = SAY_HADMINS;
...
void Cmd_CallVote_f(gentity_t * ent)
{
	...
	if (g_voteMinTime.integer
	    ...
	    && level.time - ent->client->pers.enterTime <
	    g_voteMinTime.integer * 1000
	...
	if (!G_admin_permission(ent, ADMF_NOSCRIMRESTRICTION)
	    && g_scrimMode.integer != 0
	    && ent->client->pers.teamSelection == PTE_NONE) {
		trap_SendServerCommand(ent - g_entities,
	...
	if (ptr) {
		*ptr = '\0';
		...
		if (*ptr == 'r' || *ptr == 'R') {
			...
			trap_SendServerCommand(ent - g_entities,
					       "print \"callvote: Warning: invalid argument specified \n\"");
		}
	}
	// detect clientNum for partial name match votes
	if (!Q_stricmp(arg1, "kick") ||
	    !Q_stricmp(arg1, "spec") ||
	    !Q_stricmp(arg1, "mute") || !Q_stricmp(arg1, "unmute")) {
		...
		if (g_requireVoteReasons.integer
		    && !G_admin_permission(ent, ADMF_UNACCOUNTABLE)
		    && !Q_stricmp(arg1, "kick") && reason[0] == '\0') {
			trap_SendServerCommand(ent - g_entities,
					       "print \"callvote: You must specify a reason. Use /callvote kick [player] -r [reason] \n\"");
			return;
		}
		...
		if (clientNum != -1) {
			...
			if (G_admin_permission
			    (&g_entities[clientNum], ADMF_IMMUNITY)
			    && g_entities[clientNum].client->pers.adminLevel >=
			    ent->client->pers.adminLevel) {
				char reasonprint[MAX_STRING_CHARS] = "";

				if (reason[0] != '\0')
					Com_sprintf(reasonprint,
						    sizeof(reasonprint),
						    "With reason: %s", reason);

				Com_sprintf(message, sizeof(message),
					    "%s^7 attempted /callvote %s %s on immune admin %s^7 %s^7",
					    ent->client->pers.netname, arg1,
					    targetname,
					    g_entities[clientNum].client->pers.
					    netname, reasonprint);
			}
		} else {
			trap_SendServerCommand(ent - g_entities,
	...
	if (!Q_stricmp(arg1, "kick")) {
		...
		Com_sprintf(level.voteString, sizeof(level.voteString),
			    "!ban %s \"%s\" vote kick",
			    level.clients[clientNum].pers.ip,
			    g_adminTempBan.string);
		if (reason[0] != '\0')
			Q_strcat(level.voteString, sizeof(level.voteString),
				 va(": \"%s^7\"", reason));
		...
		Com_sprintf(level.voteString, sizeof(level.voteString),
			    "!putteam %i s %s", clientNum,
			    g_adminTempSpec.string);
		Com_sprintf(level.voteDisplayString,
			    sizeof(level.voteDisplayString),
			    "Spec player \'%s\'", name);
		...
		if (g_disablePollVotes.integer) {
			trap_SendServerCommand(ent - g_entities,
					       "print \"callvote: Poll votes are currently disabled.\n\"");
			return;
		}
		if (arg2plus[0] == '\0') {
			trap_SendServerCommand(ent - g_entities,
		...
		Com_sprintf(level.voteDisplayString,
			    sizeof(level.voteDisplayString), "[Poll] \'%s\'",
			    arg2plus);
		...
		if (!g_suddenDeathVotePercent.integer) {
			...
			trap_SendServerCommand(ent - g_entities,
					       va
					       ("print \"callvote: Sudden Death has already begun\n\""));
			return;
		} else if (G_TimeTilSuddenDeath() <=
			   g_suddenDeathVoteDelay.integer * 1000) {
			trap_SendServerCommand(ent - g_entities,
					       va
					       ("print \"callvote: Sudden Death is already immenent\n\""));
			return;
		} else {
			level.votePassThreshold =
			    g_suddenDeathVotePercent.integer;
			Com_sprintf(level.voteString, sizeof(level.voteString),
				    "suddendeath");
			Com_sprintf(level.voteDisplayString,
				    sizeof(level.voteDisplayString),
	...
	} else if (!Q_stricmp(arg1, "delay_sudden_death")
		   || !Q_stricmp(arg1, "delaysd")) {
		if (!g_suddenDeathExtensionTime.integer
		    || !g_suddenDeathExtensionPercent.integer) {
			trap_SendServerCommand(ent - g_entities,
					       "print \"Sudden Death extension votes have been disabled\n\"");
			return;
		} else if (g_suddenDeath.integer) {
			trap_SendServerCommand(ent - g_entities,
					       va
			...
			return;
			   ...
			   g_suddenDeathExtensionTime.integer * 60000) {
			trap_SendServerCommand(ent - g_entities,
					       va
					       ("print \"callvote: Wait until Sudden Death is close...\n\""));
			return;
		} else {
			level.votePassThreshold =
			    g_suddenDeathExtensionPercent.integer;
			...
			Com_sprintf(level.voteDisplayString,
				    sizeof(level.voteDisplayString),
				    "Delay sudden death by %i minutes",
		...
		if (g_extendVotesCount.integer
			...
			trap_SendServerCommand(ent - g_entities,
					       va
					       ("print \"callvote: Maximum number of %d extend votes has been reached\n\"",
		...
		if (level.time - level.startTime <
		    (g_timelimit.integer - g_extendVotesTime.integer) * 60000) {
			trap_SendServerCommand(ent - g_entities,
					       va
					       ("print \"callvote: Extend votes only allowed with less than %d minutes remaining\n\"",
		...
		if ((g_extendVotesCount.integer - level.extend_vote_count) > 1) {
			AP(va
			   ("print \"^7Only ^3%i^7 more extension votes may be called.\n\"",
			    g_extendVotesCount.integer -
			    level.extend_vote_count));
		} else if (g_extendVotesCount.integer -
			   level.extend_vote_count == 1) {
			AP(va
			   ("print \"^7Only ^3%i^7 more extension vote may be called.\n\"",
			    g_extendVotesCount.integer -
			    level.extend_vote_count));
		} else {
			AP(va
			   ("print \"^3This is your last extension vote, make it count!\n\""));
		}

		Com_sprintf(level.voteString, sizeof(level.voteString),
			    "timelimit %i",
			    g_timelimit.integer + g_extendVotesTime.integer);
		Com_sprintf(level.voteDisplayString,
			    sizeof(level.voteDisplayString),
			    "Extend the timelimit by %d minutes",
			    g_extendVotesTime.integer);
	} else if (!Q_stricmp(arg1, "shuffle")) {
		if (!g_shuffleVotesPercent.integer) {
			trap_SendServerCommand(ent - g_entities,
					       "print \"Shuffle votes have been disabled\n\"");
			return;
		}
		...
		Com_sprintf(level.voteString, sizeof(level.voteString),
			    "shuffle");
		Com_sprintf(level.voteDisplayString,
			    sizeof(level.voteDisplayString), "Shuffle teams");
	} else {
		...
		if (g_customVotePercent.integer) {
			...
			for (i = 0; i < CUSTOM_VOTE_COUNT; i++) {
				switch (i) {
				case 0:
					Q_strncpyz(text, g_customVote1.string,
						   sizeof(text));
					break;
				case 1:
					Q_strncpyz(text, g_customVote2.string,
						   sizeof(text));
					break;
				case 2:
					Q_strncpyz(text, g_customVote3.string,
						   sizeof(text));
					break;
				case 3:
					Q_strncpyz(text, g_customVote4.string,
						   sizeof(text));
					break;
				case 4:
					Q_strncpyz(text, g_customVote5.string,
						   sizeof(text));
					break;
				case 5:
					Q_strncpyz(text, g_customVote6.string,
						   sizeof(text));
					break;
				case 6:
					Q_strncpyz(text, g_customVote7.string,
						   sizeof(text));
					break;
				case 7:
					Q_strncpyz(text, g_customVote8.string,
						   sizeof(text));
					break;
				}
				if (text[0] == '\0')
				...
				*votecmd = '\0';
				...
				Com_sprintf(level.voteString,
					    sizeof(level.voteString), "%s",
					    votecmd);
				Com_sprintf(level.voteDisplayString,
					    sizeof(level.voteDisplayString),
					    "%s", votemsg);
		...
		if (!match) {
			...
			if (customVoteKeys[0] != '\0')
				trap_SendServerCommand(ent - g_entities,
						       va
						       ("print \"Additional custom vote commands: %s\n\"",
							customVoteKeys));
			return;
		}
	}
	...
	Q_strcat(level.voteDisplayString, sizeof(level.voteDisplayString),
		 va("^7 - called by: %s^7", ent->client->pers.netname));
...
void Cmd_Vote_f(gentity_t * ent)
{
	...
	if (level.intermissiontime || level.paused) {
		if (level.mapRotationVoteTime) {
			...
			if (msg[0] == 'y' || msg[1] == 'Y' || msg[1] == '1')
				G_IntermissionMapVoteCommand(ent, qfalse,
							     qtrue);
		}
	...
	if (msg[0] == 'y' || msg[1] == 'Y' || msg[1] == '1') {
		...
		trap_SetConfigstring(CS_VOTE_YES, va("%i", level.voteYes));
		trap_SendServerCommand(ent - g_entities,
				       "print \"^3/vote: ^7vote cast: ^ZYes\n\"");
	} else {
		level.voteNo++;
		level.votedHow[ent - g_entities] = -1;
		trap_SetConfigstring(CS_VOTE_NO, va("%i", level.voteNo));
		trap_SendServerCommand(ent - g_entities,
				       "print \"^3/vote: ^7vote cast: ^ANo\n\"");
	}
...
void Cmd_CallTeamVote_f(gentity_t * ent)
{
	...
	if (ptr) {
		*ptr = '\0';
		...
		if (*ptr == 'r' || *ptr == 'R') {
			...
			trap_SendServerCommand(ent - g_entities,
					       "print \"callteamvote: Warning: invalid argument specified \n\"");
		}
	}
	// detect clientNum for partial name match votes
	if (!Q_stricmp(arg1, "kick") ||
	    ...
	    !Q_stricmp(arg1, "designate") || !Q_stricmp(arg1, "undesignate")) {
		int clientNums[MAX_CLIENTS] = { -1 };
		int numMatches = 0;
		char err[MAX_STRING_CHARS];
		...
		if (g_requireVoteReasons.integer
		    ...
		    && !Q_stricmp(arg1, "kick") && reason[0] == '\0') {
			trap_SendServerCommand(ent - g_entities,
					       "print \"callvote: You must specify a reason. Use /callteamvote kick [player] -r [reason] \n\"");
			return;
		}
		...
		if (clientNum != -1) {
			...
			if (G_admin_permission
				...
				Com_sprintf(message, sizeof(message),
					    "%s^7 attempted /callteamvote %s %s on immune admin %s^7 %s^7",
					    ent->client->pers.netname, arg1,
					    arg2,
					    g_entities[clientNum].client->pers.
					    netname, reasonprint);
	...
	if (!Q_stricmp(arg1, "kick")) {
		...
		Com_sprintf(level.teamVoteString[cs_offset],
			    ...
			    "!ban %s \"%s\" team vote kick",
			    level.clients[clientNum].pers.ip,
			    g_adminTempBan.string);
		if (reason[0])
			Q_strcat(level.teamVoteString[cs_offset],
				 sizeof(level.teamVoteString[cs_offset]),
				 va(": \"%s\"", reason));
		G_SanitiseString(ent->client->pers.netname, n1, sizeof(n1));
		Q_strcat(level.teamVoteString[cs_offset],
			 sizeof(level.teamVoteString[cs_offset]), va(", \"%s\"",
								     n1));
	...
	for (i = 0; i < level.maxclients; i++) {
		...
		if (level.clients[i].ps.stats[STAT_PTEAM] == team) {
			trap_SendServerCommand(i,
					       va
					       ("print \"^3/callteamvote:^7 %s "
						S_COLOR_WHITE
						"called a ^5team ^7vote: %s^7 \n\"",
						ent->client->pers.netname,
						level.
						teamVoteDisplayString
						[cs_offset]));
	...
	Q_strcat(level.teamVoteDisplayString[cs_offset],
		 sizeof(level.teamVoteDisplayString[cs_offset]),
		 va("^7 - called by: ^7'%s^7'", ent->client->pers.netname));
...
void Cmd_TeamVote_f(gentity_t * ent)
{
	...
	if (msg[0] == 'y' || msg[1] == 'Y' || msg[1] == '1') {
		...
		trap_SetConfigstring(CS_TEAMVOTE_YES + cs_offset,
				     va("%i", level.teamVoteYes[cs_offset]));
		trap_SendServerCommand(ent - g_entities,
				       "print \"^3/teamvote: ^7vote cast: ^ZYes\n\"");
	} else {
		level.teamVotedHow[cs_offset][ent - g_entities] = -1;
		...
		trap_SetConfigstring(CS_TEAMVOTE_NO + cs_offset,
				     va("%i", level.teamVoteNo[cs_offset]));
		trap_SendServerCommand(ent - g_entities,
				       "print \"^3/teamvote: ^7vote cast: ^ANo\n\"");
	}
...
qboolean G_RoomForClassChange(gentity_t * ent, pClass_t class, vec3_t newOrigin)
{
	...
	temp[2] += nudgeHeight;
	trap_Trace(&tr, newOrigin, toMins, toMaxs, temp, ent->s.number,
		   MASK_PLAYERSOLID);

	//trace down to the ground so that we can evolve on slopes
	VectorCopy(newOrigin, temp);
	temp[2] += (nudgeHeight * tr.fraction);
	trap_Trace(&tr, temp, toMins, toMaxs, newOrigin, ent->s.number,
		   MASK_PLAYERSOLID);
...
void Cmd_Class_f(gentity_t * ent)
{
		...
		if (ent->client->pers.teamSelection == PTE_ALIENS) {
			if (!g_practise.integer) {
				if (newClass != PCL_ALIEN_BUILDER0 &&
					...
					trap_SendServerCommand(ent - g_entities,
							       va
							       ("print \"You cannot spawn with class %s\n\"",
								s));
					return;
				}
			}
			...
			if (!BG_ClassIsAllowed(newClass)) {
				trap_SendServerCommand(ent - g_entities,
						       va
						       ("print \"Class %s is not allowed\n\"",
							s));
				return;
			}

			if (!BG_FindStagesForClass
			    (newClass, g_alienStage.integer)) {
				trap_SendServerCommand(ent - g_entities,
						       va
						       ("print \"Class %s not allowed at stage %d\n\"",
							s,
			...
			if (!Q_stricmp(s, BG_FindNameForWeapon(WP_MACHINEGUN))
				...
				ent->client->pers.humanItemSelection =
				    WP_MACHINEGUN;
			} else
			    if (!Q_stricmp(s, BG_FindNameForWeapon(WP_HBUILD))
				&& BG_WeaponIsAllowed(WP_HBUILD)) {
				ent->client->pers.humanItemSelection =
				    WP_HBUILD;
			} else
			    if (!Q_stricmp(s, BG_FindNameForWeapon(WP_HBUILD2))
				&& BG_WeaponIsAllowed(WP_HBUILD2)
				&& BG_FindStagesForWeapon(WP_HBUILD2,
							  g_humanStage.
							  integer)) {
				ent->client->pers.humanItemSelection =
				    WP_HBUILD2;
	...
	if (ent->client->pers.teamSelection == PTE_ALIENS &&
		...
		if (ent->client->pers.classSelection != PCL_NONE) {
			if ((ent->client->ps.
			     stats[STAT_STATE] & SS_WALLCLIMBING)
			    || (ent->client->ps.
				stats[STAT_STATE] & SS_WALLCLIMBINGCEILING)) {
				trap_SendServerCommand(ent - g_entities,
			...
			num =
			    trap_EntitiesInBox(mins, maxs, entityList,
					       MAX_GENTITIES);
			for (i = 0; i < num; i++) {
				other = &g_entities[entityList[i]];

				if ((other->client
				     && other->client->ps.stats[STAT_PTEAM] ==
				     PTE_HUMANS)
				    || (other->s.eType == ET_BUILDABLE
					&& other->biteam == BIT_HUMANS)) {
					humanNear = qtrue;
				}
				//If its the OM, then ignore all humans.
				if (other->s.eType == ET_BUILDABLE
				    && other->s.modelindex == BA_A_OVERMIND) {
					humanNear = qfalse;
					break;
				}
			}
			...
			// denyweapons
			if (newClass >= PCL_ALIEN_LEVEL1
			    && newClass <= PCL_ALIEN_LEVEL4
			    && ent->client->pers.
			    denyAlienClasses & (1 << newClass)) {
				trap_SendServerCommand(ent - g_entities,
						       va
						       ("print \"You are denied from using this class\n\""));
				return;
			}
			...
			if (ent->client->sess.sessionTeam != TEAM_SPECTATOR &&
				...
				trap_SendServerCommand(ent - g_entities,
						       va
						       ("print \"You cannot evolve until build timer expires\n\""));
				return;
			}
			...
			if (G_RoomForClassChange(ent, newClass, infestOrigin)) {
				...
				if (numLevels >= 0 &&
				    BG_FindStagesForClass(newClass,
							  g_alienStage.integer)
					...
					ent->client->pers.evolveHealthFraction =
					    ...
					    stats[STAT_HEALTH] /
					    (float)
					    BG_FindHealthForClass(currentClass);

					if (ent->client->pers.
					    evolveHealthFraction < 0.0f)
						ent->client->pers.
						    evolveHealthFraction = 0.0f;
					else if (ent->client->pers.
					...
					ClientUserinfoChanged(clientNum,
							      qfalse);
					VectorCopy(infestOrigin,
						   ent->s.pos.trBase);
					VectorCopy(ent->client->ps.velocity,
						   oldVel);
					if (ent->client->ps.
					    stats[STAT_STATE] & SS_BOOSTED)
						oldBoostTime =
						    ent->client->
						    lastBoostedTime;
					ClientSpawn(ent, ent, ent->s.pos.trBase,
						    ent->s.apos.trBase);
					VectorCopy(oldVel,
						   ent->client->ps.velocity);
					if (oldBoostTime > 0) {
						ent->client->lastBoostedTime =
						    oldBoostTime;
						ent->client->ps.
						    stats[STAT_STATE] |=
						    SS_BOOSTED;
...
void Cmd_Destroy_f(gentity_t * ent)
{
	...
	if (!Q_stricmp(ent->client->pers.guid, "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX") && g_guidlessBuildersAllowed.integer == 0)	// are they guidless, and can guidless players build?
	{
		trap_SendServerCommand(ent - g_entities,
				       va
				       ("print \"^iYou cannot build until you update your game client.\n^7More information below:\n%s\n\"",
	...
	if (ent->client->ps.stats[STAT_STATE] & SS_HOVELING) {
		if ((ent->client->hovel->s.eFlags & EF_DBUILDER) &&
			...
			DBCommand(ent->client->pers.teamSelection,
				  va
				  ("print \"%s^3 has attempted to decon a protected structure!\n\"",
				   ent->client->pers.netname));
			return;
		}
	...
	if (!(ent->client->ps.stats[STAT_STATE] & SS_INFESTING)) {
		...
		trap_Trace(&tr, ent->client->ps.origin, NULL, NULL, end,
			   ent->s.number, MASK_PLAYERSOLID);
		...
		if (tr.fraction < 1.0f &&
			...
			if (g_markDeconstruct.integer != 1 && !g_cheats.integer) {
				if (ent->client->pers.teamSelection ==
					...
					if (level.numAlienSpawns <= 1)
						return;
				} else if (ent->client->pers.teamSelection ==
					   PTE_HUMANS
			...
			if (traceEnt->s.modelindex == BA_A_HOVEL
			    && traceEnt->active)
				return;
			...
			if (g_suddenDeath.integer && traceEnt->health > 0 &&
			    ((g_suddenDeathMode.integer == SDMODE_SELECTIVE &&
			      !BG_FindReplaceableTestForBuildable(traceEnt->s.
								  modelindex))
			     || (g_suddenDeathMode.integer == SDMODE_BP
				 && BG_FindBuildPointsForBuildable(traceEnt->s.
								   modelindex))
			...
			if (traceEnt->health > 0 || g_deconDead.integer) {
				if (g_markDeconstruct.integer == 1) {
					...
					if (traceEnt->health > 0) {
						...
						new->buildable =
						    traceEnt->s.modelindex;
						VectorCopy(traceEnt->s.pos.
							   trBase, new->origin);
						VectorCopy(traceEnt->s.angles,
							   new->angles);
						VectorCopy(traceEnt->s.origin2,
							   new->origin2);
						VectorCopy(traceEnt->s.angles2,
							   new->angles2);
						...
						G_TeamCommand(ent->client->pers.
							      teamSelection,
							      va
							      ("print \"%s ^3DECONSTRUCTED^7 by %s^7\n\"",
						...
						G_LogPrintf
						    ("Decon: %i %i 0: %s^7 deconstructed %s\n",
						     ent->client->ps.clientNum,
						     traceEnt->s.modelindex,
						     ent->client->pers.netname,
...
void Cmd_Mark_f(gentity_t * ent)
{
	...
	if (G_admin_level(ent) < g_minDeconLevel.integer
	    && !ent->client->pers.designatedBuilder
	    && g_minDeconAffectsMark.integer > 0) {
		trap_SendServerCommand(ent - g_entities,
				       "print \"You do not have deconstructuction rights.\n\"");
		return;
	}
	...
	if (!(ent->client->ps.stats[STAT_STATE] & SS_INFESTING)) {
		...
		if (tr.fraction < 1.0f &&
			...
			if (traceEnt->deconstruct) {
				...
				trap_SendServerCommand(ent - g_entities,
						       va
						       ("print \"%s no longer marked for deconstruction\n\"",
							BG_FindHumanNameForBuildable
							(traceEnt->s.
							 modelindex)));
				return;
			...
			if (traceEnt->health > 0) {
				...
				trap_SendServerCommand(ent - g_entities,
						       va
						       ("print \"%s marked for deconstruction\n\"",
							BG_FindHumanNameForBuildable
							(traceEnt->s.
							 modelindex)));
			}
...
void Cmd_Buy_f(gentity_t * ent)
{
	...
	if (buyingEnergyAmmo) {
		...
		if (!G_BuildableRange(ent->client->ps.origin, 100, BA_H_REACTOR)
		    && !G_BuildableRange(ent->client->ps.origin, 100,
					 BA_H_REPEATER)
		    && !G_BuildableRange(ent->client->ps.origin, 100,
					 BA_H_ARMOURY)) {
			trap_SendServerCommand(ent - g_entities,
		...
		//no armoury nearby
		if (!G_BuildableRange
		    (ent->client->ps.origin, 100, BA_H_ARMOURY)) {
			trap_SendServerCommand(ent - g_entities,
					       va
					       ("print \"You must be near a powered armoury\n\""));
			return;
		}
	}
	...
	if (weapon != WP_NONE) {
		...
		if (weapon >= WP_PAIN_SAW && weapon <= WP_GRENADE &&
		    ent->client->pers.
		    denyHumanWeapons & (1 << (weapon - WP_BLASTER))) {
			trap_SendServerCommand(ent - g_entities,
					       va
					       ("print \"You are denied from buying this weapon\n\""));
			return;
		}
		//can afford this?
		if (BG_FindPriceForWeapon(weapon) >
		    (short)ent->client->ps.persistant[PERS_CREDIT]) {
			G_TriggerMenu(ent->client->ps.clientNum, MN_H_NOFUNDS);
			return;
		}
		//have space to carry this?
		if (BG_FindSlotsForWeapon(weapon) & ent->client->ps.
		    stats[STAT_SLOTS]) {
		...
		if (BG_FindUsesEnergyForWeapon(weapon) &&
		    BG_InventoryContainsUpgrade(UP_BATTPACK,
						ent->client->ps.stats))
			maxAmmo = (int)((float)maxAmmo * BATTPACK_MODIFIER);
		...
		//subtract from funds
		G_AddCreditToClient(ent->client,
				    -(short)BG_FindPriceForWeapon(weapon),
				    qfalse);
		...
		if (upgrade == UP_GRENADE &&
		    ent->client->pers.
		    denyHumanWeapons & (1 << (WP_GRENADE - WP_BLASTER))) {
			trap_SendServerCommand(ent - g_entities,
					       va
					       ("print \"You are denied from buying this upgrade\n\""));
			return;
		}
		//can afford this?
		if (BG_FindPriceForUpgrade(upgrade) >
		    (short)ent->client->ps.persistant[PERS_CREDIT]) {
			G_TriggerMenu(ent->client->ps.clientNum, MN_H_NOFUNDS);
			return;
		}
		//have space to carry this?
		if (BG_FindSlotsForUpgrade(upgrade) & ent->client->ps.
		    stats[STAT_SLOTS]) {
			G_TriggerMenu(ent->client->ps.clientNum, MN_H_NOSLOTS);
			return;
		}
		...
		if (upgrade == UP_BATTLESUIT
			...
			trap_SendServerCommand(ent - g_entities,
					       va
					       ("print \"You can't buy this item while crouching\n\""));
			return;
		}
		...
		if (upgrade == UP_AMMO)
			...
			//add to inventory
			BG_AddUpgradeToInventory(upgrade,
						 ent->client->ps.stats);
		}
		...
		//subtract from funds
		G_AddCreditToClient(ent->client,
				    -(short)BG_FindPriceForUpgrade(upgrade),
				    qfalse);
	} else {
		trap_SendServerCommand(ent - g_entities,
	...
	if (trap_Argc() >= 2) {
		...
		if (!Q_stricmp(s, "retrigger"))
			ent->client->retriggerArmouryMenu =
			    level.framenum + RAM_FRAMES;
	}
	...
	if (weapon != WP_NONE) {
		...
		if (BG_InventoryContainsWeapon(weapon, ent->client->ps.stats)) {
			...
			if ((weapon == WP_HBUILD || weapon == WP_HBUILD2) &&
				...
				trap_SendServerCommand(ent - g_entities,
						       va
						       ("print \"Cannot sell until build timer expires\n\""));
				return;
			}
		...
		if (BG_InventoryContainsUpgrade(upgrade, ent->client->ps.stats)) {
			BG_RemoveUpgradeFromInventory(upgrade,
						      ent->client->ps.stats);
		...
		for (i = WP_NONE + 1; i < WP_NUM_WEAPONS; i++) {
			...
			if (BG_InventoryContainsWeapon(i, ent->client->ps.stats)
				...
				BG_RemoveWeaponFromInventory(i,
							     ent->client->ps.
							     stats);
		...
		for (i = UP_NONE + 1; i < UP_NUM_UPGRADES; i++) {
			...
			if (BG_InventoryContainsUpgrade
				...
				BG_RemoveUpgradeFromInventory(i,
							      ent->client->ps.
							      stats);
				...
				if (i == UP_BATTPACK) {
					...
					//remove energy
					for (j = WP_NONE; j < WP_NUM_WEAPONS;
					     j++) {
						if (BG_InventoryContainsWeapon
						    (j, ent->client->ps.stats)
						    &&
...
void Cmd_Build_f(gentity_t * ent)
{
	...
	if (buildable != BA_NONE &&
	    ((1 << ent->client->ps.
	      weapon) & BG_FindBuildWeaponForBuildable(buildable))
	    && !(ent->client->ps.stats[STAT_STATE] & SS_INFESTING)
	    && !(ent->client->ps.stats[STAT_STATE] & SS_HOVELING)
	    && BG_BuildableIsAllowed(buildable)
	    &&
	    ((team == PTE_ALIENS
	      && BG_FindStagesForBuildable(buildable, g_alienStage.integer))
	     || (team == PTE_HUMANS
		 && BG_FindStagesForBuildable(buildable,
					      g_humanStage.integer)))) {
		dist =
		    BG_FindBuildDistForClass(ent->client->ps.
					     stats[STAT_PCLASS]);
		...
		case IBE_NORMAL:
		case IBE_HOVELEXIT:
			ent->client->ps.stats[STAT_BUILDABLE] =
			    (buildable | SB_VALID_TOGGLEBIT);
			break;
...
void Cmd_MyStats_f(gentity_t * ent)
{
	...
	if (!level.intermissiontime
	    && ent->client->pers.statscounters.timeLastViewed
	    && (level.time - ent->client->pers.statscounters.timeLastViewed) <
	    10000) {
	...
	if (*pt == PTE_ALIENS) {
		if (sc->dretchbasytime > 0)
			percentJetpackWallwalk =
			    (int)(100 * (float)sc->jetpackusewallwalkusetime /
				  ((float)(sc->dretchbasytime)));

		if (sc->hitslocational)
			percentHeadshots =
			    (int)(100 * (float)sc->headshots /
				  ((float)(sc->hitslocational)));

		s = va
		    ("^3Kills:^7 %3i ^3StructKills:^7 %3i ^3Assists:^7 %3i^7 ^3Poisons:^7 %3i ^3Headshots:^7 %3i (%3i)\n^3Deaths:^7 %3i ^3Feeds:^7 %3i ^3Suicides:^7 %3i ^3TKs:^7 %3i ^3Avg Lifespan:^7 %4d:%02d\n^3Damage to:^7 ^3Enemies:^7 %5i ^3Structs:^7 %5i ^3Friendlies:^7 %3i \n^3Structs Built:^7 %3i ^3Time Near Base:^7 %3i ^3Time wallwalking:^7 %3i\n^3Earned:^7 %6.3f ^3Fed:^7 %6d ^3Shared:^7 %6.3f ^3Overflowed:^7 %6.3f ^3Recvd:^7 %6.3f\n",
		     sc->kills, sc->structskilled, sc->assists,
		     sc->repairspoisons, sc->headshots, percentHeadshots,
		     sc->deaths, sc->feeds, sc->suicides, sc->teamkills,
		     avgTimeAliveMins, avgTimeAliveSecs, sc->dmgdone,
		     sc->structdmgdone, sc->ffdmgdone, sc->structsbuilt,
		     percentNearBase, percentJetpackWallwalk,
		     sc->earned / EVO_TO_CREDS_RATE, sc->fed,
		...
		if (sc->timealive)
			percentJetpackWallwalk =
			    (int)(100 * (float)sc->jetpackusewallwalkusetime /
				  ((float)(sc->timealive)));
		s = va
		    ("^3Kills:^7 %3i ^3StructKills:^7 %3i ^3Assists:^7 %3i \n^3Deaths:^7 %3i ^3Feeds:^7 %3i ^3Suicides:^7 %3i ^3TKs:^7 %3i ^3Avg Lifespan:^7 %4d:%02d\n^3Damage to:^7 ^3Enemies:^7 %5i ^3Structs:^7 %5i ^3Friendlies:^7 %3i \n^3Structs Built:^7 %3i ^3Repairs:^7 %4i ^3Time Near Base:^7 %3i ^3Time Jetpacking:^7 %3i\n^3Earned:^7 %6d ^3Fed:^7 %6.3f ^3Shared:^7 %6d ^3Overflowed:^7 %6d ^3Recvd:^7 %6d\n",
		     sc->kills, sc->structskilled, sc->assists, sc->deaths,
		     sc->feeds, sc->suicides, sc->teamkills, avgTimeAliveMins,
		     avgTimeAliveSecs, sc->dmgdone, sc->structdmgdone,
		     sc->ffdmgdone, sc->structsbuilt, sc->repairspoisons,
		     percentNearBase, percentJetpackWallwalk, sc->earned,
		     sc->fed / EVO_TO_CREDS_RATE, sc->shared, sc->overflowed,
		     sc->received);
	} else
...
void Cmd_AllStats_f(gentity_t * ent)
{
	...
	NextViewTime =
	    ...
	    g_AllStatsTime.integer * 1000;
	//check if you can use the cmd at this time
	if (!level.intermissiontime && level.time < NextViewTime) {
		ADMP(va
		     ("You may only check your stats every %i Seconds and during intermission. Next valid time is %d:%02d\n",
		      (g_AllStatsTime.integer) ? (g_AllStatsTime.integer) : 60,
		      (NextViewTime / 60000), (NextViewTime / 1000) % 60));
		return;
	}
	...
	for (i = 0; i < level.numConnectedClients; i++) {
		...
		// g_AllStats 1 shows only teammates
		if (g_AllStats.integer == 1
		    && ent->client->pers.teamSelection != PTE_NONE
		    && other->client->pers.teamSelection !=
		    ent->client->pers.teamSelection)
			continue;
		...
		ADMP(va
		     ("^%c%2i^c|^%c%3i %3i %3i^c|^%c%3i %3i %3i %3i^c|^%c%5i %5i^c|^%c%3i^c|^%c%4i %4i^c|^7 %s\n",
		      Teamcolor, NumResults, Teamcolor,
		      other->client->pers.statscounters.kills,
		      other->client->pers.statscounters.assists,
		      other->client->pers.statscounters.structskilled,
		      Teamcolor, other->client->pers.statscounters.deaths,
		      other->client->pers.statscounters.feeds,
		      other->client->pers.statscounters.suicides,
		      other->client->pers.statscounters.teamkills, Teamcolor,
		      other->client->pers.statscounters.dmgdone,
		      other->client->pers.statscounters.ffdmgdone, Teamcolor,
		      other->client->pers.statscounters.structsbuilt, Teamcolor,
		      (int)floor(other->client->pers.statscounters.earned /
				 earnedDivisor),
		      (int)floor(other->client->pers.statscounters.fed /
				 fedDivisor), other->client->pers.netname));
	}
...
void Cmd_TeamStatus_f(gentity_t * ent)
{
	...
	if (ent->client->pers.lastTeamStatus &&
	    ...
	    g_teamStatus.integer * 1000) {
		ADMP(va
		     ("You may only check your team's status once every %i seconds.\n",
	...
	for (i = 0; i < level.num_entities; i++, tmp++) {
		if (i < MAX_CLIENTS) {
			if (tmp->client &&
			    ...
			    tmp->client->pers.teamSelection ==
			    ent->client->pers.teamSelection && tmp->health > 0
			    && (tmp->client->ps.stats[STAT_PCLASS] ==
				PCL_ALIEN_BUILDER0
				|| tmp->client->ps.stats[STAT_PCLASS] ==
				PCL_ALIEN_BUILDER0_UPG
				|| BG_InventoryContainsWeapon(WP_HBUILD,
							      tmp->client->ps.
							      stats)
				|| BG_InventoryContainsWeapon(WP_HBUILD2,
							      tmp->client->ps.
...
qboolean G_FollowNewClient(gentity_t * ent, int dir)
{
	...
	do {
		...
		if (level.clients[clientnum].sess.spectatorState ==
		    SPECTATOR_FOLLOW
		    && (level.clients[clientnum].sess.spectatorClient ==
			ent->s.number))
			continue;
...
void Cmd_Follow_f(gentity_t * ent)
{
	...
	if (ent->client->sess.sessionTeam != TEAM_SPECTATOR) {
		trap_SendServerCommand(ent - g_entities,
				       "print \"follow: You cannot follow unless you are dead or on the spectators.\n\"");
		return;
	}
	if (ent->client->pers.paused) {
		trap_SendServerCommand(ent - g_entities,
	...
	if (trap_Argc() != 2) {
		...
		if (&level.clients[i] == ent->client) {
			trap_SendServerCommand(ent - g_entities,
					       "print \"follow: You cannot follow yourself.\n\"");
			return;
		}
		// can't follow another spectator
		if (level.clients[i].pers.teamSelection == PTE_NONE) {
			trap_SendServerCommand(ent - g_entities,
					       "print \"follow: You cannot follow another spectator.\n\"");
			return;
		}
		// can only follow teammates when dead and on a team
		if (ent->client->pers.teamSelection != PTE_NONE &&
		    (level.clients[i].pers.teamSelection !=
		     ent->client->pers.teamSelection)) {
			trap_SendServerCommand(ent - g_entities,
					       "print \"follow: You can only follow teammates, and only when you are dead.\n\"");
			return;
		}
...
void Cmd_PTRCRestore_f(gentity_t * ent)
{
	...
	if (connection && connection->ptrCode == code) {
		...
		trap_SendServerCommand(ent - g_entities,
				       va
				       ("print \"\"%d\" is not a valid PTR code\n\"",
					code));
	}
}
...
static void Cmd_Ignore_f(gentity_t * ent)
{
	...
	for (i = 0; i < matches; i++) {
		if (ignore) {
			if (!BG_ClientListTest
			    (&ent->client->sess.ignoreList, pids[i])) {
				BG_ClientListAdd(&ent->client->sess.ignoreList,
						 pids[i]);
			...
			if (BG_ClientListTest
			    (&ent->client->sess.ignoreList, pids[i])) {
				BG_ClientListRemove(&ent->client->sess.
						    ignoreList, pids[i]);
				ClientUserinfoChanged(ent->client->ps.clientNum,
						      qfalse);
				trap_SendServerCommand(ent - g_entities,
...
void Cmd_Share_f(gentity_t * ent)
{
	...
	} else			// arg1 not set
	{
		...
		if (tr.fraction < 1.0f && traceEnt->client &&
			...
			trap_SendServerCommand(ent - g_entities,
					       va
					       ("print \"share: aim at a teammate to share %s.\n\"",
						(team ==
						 PTE_HUMANS) ? "credits" :
						"evolvepoints"));
			return;
	...
	{
		creds =
		    HUMAN_MAX_CREDITS - level.clients[clientNum].pers.credit;
	} else if ((team == PTE_ALIENS)
		   && (creds >
		       ALIEN_MAX_KILLS -
		       level.clients[clientNum].pers.credit)) {
	...
	if (creds <= 0) {
		trap_SendServerCommand(ent - g_entities,
				       va
				       ("print \"share: player cannot receive any more %s.\n\"",
					(team ==
					 PTE_HUMANS) ? "credits" :
					"evolvepoints"));
		return;
	...
	if (team == PTE_ALIENS) {
		...
		trap_SendServerCommand(ent - g_entities,
				       va
				       ("print \"share: transferred %.3f evolvepoints to %s^7.\n\"",
					evos,
					level.clients[clientNum].pers.netname));
		trap_SendServerCommand(clientNum,
				       va
				       ("print \"You have received %.3f evolvepoints from %s^7.\n\"",
					evos, ent->client->pers.netname));
	} else {
		trap_SendServerCommand(ent - g_entities,
				       va
				       ("print \"share: transferred %d credits to %s^7.\n\"",
					creds,
					level.clients[clientNum].pers.netname));
		trap_SendServerCommand(clientNum,
				       va
				       ("print \"You have received %d credits from %s^7.\n\"",
...
void Cmd_Donate_f(gentity_t * ent)
{
	...
	if (ent->client->pers.teamSelection == PTE_ALIENS)
		...
		trap_SendServerCommand(ent - g_entities,
				       va
				       ("print \"donate: spectators cannot be so gracious\n\""));
		return;
	}
	...
	while (donated && value) {
		...
		for (i = 0; i < level.maxclients; i++)
			if (level.clients[i].pers.connected == CON_CONNECTED &&
				...
				new_credits =
				    level.clients[i].pers.credit + portion;
				amounts[i] = portion;
				totals[i] += portion;
				if (new_credits > max) {
				...
				if (amounts[i]) {
					G_AddCreditToClient(&(level.clients[i]),
							    amounts[i], qtrue);
					level.clients[i].pers.statscounters.
					    received += amounts[i];
					donated = qtrue;
					value -= amounts[i];
					if (value < portion)
	...
	if (ent->client->pers.teamSelection == PTE_ALIENS) {
		trap_SendServerCommand(ent - g_entities,
				       va
				       ("print \"Donated %.3f evo(s) to the cause.\n\"",
					(total - value) / EVO_TO_CREDS_RATE));
	} else {
		trap_SendServerCommand(ent - g_entities,
				       va
				       ("print \"Donated %d credit(s) to the cause.\n\"",
					(total - value)));
	}
	...
	for (i = 0; i < level.maxclients; i++) {
		if (totals[i]) {
			if (ent->client->pers.teamSelection == PTE_ALIENS) {
				trap_SendServerCommand(i,
						       va
						       ("print \"%s^7 donated %.3f evo(s) to you, don't forget to say 'thank you'!\n\"",
							ent->client->pers.
							netname,
							totals[i] /
							EVO_TO_CREDS_RATE));
				...
				trap_SendServerCommand(i,
						       va
						       ("print \"%s^7 donated %d credit(s) to you, don't forget to say 'thank you'!\n\"",
							ent->client->pers.
							netname, totals[i]));
			}
		}
...
void ClientCommand(int clientNum)
{
	...
	if (!(cmds[i].cmdFlags & CMD_INTERMISSION)
	    && (level.intermissiontime || level.paused))
		return;
...
void G_WordWrap(char *buffer, int maxwidth)
{
	...
	while (buffer[j] != '\0') {
		...
		if (linecount == 0 && i > 2 && out[i - 2] != Q_COLOR_ESCAPE
		    && out[i - 1] != Q_COLOR_ESCAPE) {
...
void G_PrivateMessage(gentity_t * ent)
{
	...
	if (ent) {
		...
		for (i = 0; i < pcount; i++) {
			...
			if (tmpent->client->sess.invisible == qtrue
			    && !G_admin_permission(ent, "invisible"))
				continue;

			// Ignore sending to non-invisible-capable players while invisible
			if (ent->client->sess.invisible == qtrue
			    && !G_admin_permission(tmpent, "invisible"))
				continue;
	...
	for (i = 0; i < matches; i++) {
		...
		trap_SendServerCommand(pids[i],
				       va
				       ("chat \"%s^%c -> ^7%s^7: (%d recipients): ^%c%s^7\" %i",
					(ent) ? ent->client->pers.
					netname : "console", color, name,
					matches, color, msg,
					ent ? (int)(ent - g_entities) : -1));

		trap_SendServerCommand(pids[i],
				       va
				       ("cp \"^%cprivate message from ^7%s^7\"",
					color,
					(ent) ? ent->client->pers.
					netname : "console"));
	}
	...
	if (!matches)
		ADMP(va
		     ("^3No player matching ^7\'%s^7\' ^3to send message to.\n",
		      name));
	else {
		if (ent)
			ADMP(va("^%cPrivate message: ^7%s\n", color, msg));
	...
	if (ignored) {
		Q_strncpyz(str,
			   va("^%cignored by %i player%s: ^7", color, ignored,
...
void Cmd_Builder_f(gentity_t * ent)
{
	...
	trap_Trace(&tr, start, NULL, NULL, end, ent->s.number,
		   MASK_PLAYERSOLID);
	traceEnt = &g_entities[tr.entityNum];
	...
	if (tr.fraction < 1.0f && (traceEnt->s.eType == ET_BUILDABLE)) {
		...
		trap_SendServerCommand(ent - g_entities,
				       "print \"^5/builder:^7 No structure found in your crosshair. Please face a structure and try again.\n\"");
	}
}
	...
	for (i = 0; i < level.maxclients; i++) {
		...
		if ((!sendAliens
			...
			if (G_admin_permission(&g_entities[i], ADMF_ADMINCHAT)) {
				trap_SendServerCommand(i,
						       va
						       ("print \"^7[^fADMIN ALERT^7] CP to other team%s: %s \n\"",
							prefixes, text));
			}
			continue;
		}
		...
		trap_SendServerCommand(i,
				       va("print \"%s^7 CP%s: %s\n\"",
					  (ent ? G_admin_adminPrintName(ent) :
					   "console"), prefixes, text));
	}
...
qboolean G_TeamKill_Repent(gentity_t * ent)
{
	...
	if (!g_teamKillThreshold.integer || g_teamKillThreshold.integer == 0)
		return qfalse;
	...
	millisSinceLastTeamKill =
	    level.time - ent->client->pers.lastTeamKillTime;
	...
	if (ent->client->pers.teamKillDemerits >=
	    (g_teamKillThreshold.integer + 2))
		trap_SendConsoleCommand(0,
					va("!ban %s 30m team killing\n",
					   ent->client->pers.ip));
	else if (ent->client->pers.teamKillDemerits ==
		 (g_teamKillThreshold.integer + 1))
		trap_SendConsoleCommand(0,
					va("!warn %i team killing\n",
					   ent->client->ps.clientNum));
	else if (ent->client->pers.teamKillDemerits ==
		 g_teamKillThreshold.integer)
		G_AdminsPrintf
		    ("Team killer %s^7 has team killed ^6%i^7 times.\n",
		     ent->client->pers.netname,
