...
g_admin_schachts_t g_admin_schachts[] = {
	{ "adjustban",
	 "%s^7, bans are issued as-is with with no warranty or accidental damage cover.",
	 NULL },
	{ "adminlog",
	 "sorry, %s^7, but only the NSA have access to this feature.", NULL },
	{ "admintest", "you have no power here, %s^7.", NULL },
	{ "allready",
	 "%s^7 must gather their party before venturing forth from this place.",
	 NULL },
	{ "buildlog",
	 "perhaps %s^7 is looking for a compiler, have they tried clang?",
	 NULL },
	{ "ban", "wir fahren, fahren, fahren, auf die Autoban - %s^7.", NULL },
	{ "cp", "I didn't you were into ^1that, ^7%s^7. I'm calling the cops.",
	 NULL },
	{ "devmap",
	 "%s^7 should be using ^3!setdevmode^7 in the middle of a pub, just like a true Zittriger would.",
	 NULL },
	{ "cancelvote", "this is a democracy, and %s^7 cannot change that.",
	 NULL },
	{ "decon", "sorry, %s^7! no refunds, no returns!", NULL },
	{ "demo", "did you mean OBS, %s^7?", NULL },
	{ "designate", "the team shall decide the designation, not you, %s^7.",
	 NULL },
	{ "denybuild", "denywhat? sorry %s^7, never heard of that here.",
	 NULL },
	{ "denyweapon", "%s^7 should be using callvote!", NULL },
	{ "drop", "%s^7 is trying to hide something by not using ^3!kick^7.",
	 NULL },
	{ "flag", "%s^7 wants to put up the white flag and surrender.", NULL },
	{ "flaglist",
	 "sorry, %s^7, i can't even shitpost country flags because this game doesn't support emoticons :(",
	 NULL },
	...
	{ "layoutsave",
	 "it's 2007 again and %s^7 forgot to use microsoft visio", NULL },
	{ "listadmins",
	 "uh, there's a bunch of people on the list, but %s^7 isn't one of them.",
	 NULL },
	{ "listlayouts", "*DEFAULT*, *DEFAULT*, *DEFAULT*, ...", NULL },
	{ "listplayers",
	 "some NPCs, campers, stackers, a few overseers, and you, %s^7.",
	 NULL },
	{ "listmaps",
	 "might as well contain atcs, utcs, nano and niveus, which are %s^7's favourite maps, right?",
	 NULL },
	...
	{ "mute",
	 "this is a democracy, where people like %s^7 should be using /callvote",
	 NULL },
	{ "namelog",
	 "sorry, %s^7, but only the NSA have access to this feature.", NULL },
	{ "nextmap",
	 "%s^7 must gather their party before venturing forth from this place.",
	 NULL },
	{ "nobuild", "it's not like %s^7 would know how to use it anyway.",
	 NULL },
	{ "passvote", "this is a democracy, and %s^7 cannot change that.",
	 NULL },
	...
	{ "rename",
	 "sorry, %s^7, but only the NSA have access to this feature.", NULL },
	{ "restart", "sorry, %s^7! no refunds, no returns!", NULL },
	{ "revert", "sorry, %s^7! no refunds, no returns!", NULL },
	{ "rotation",
	 "%s^7, is a circular movement of an object around a center (or point) of rotation. (from Wikipedia, the free encyclopedia)",
	 NULL },
	{ "seen", "seen who, %s^7? ( o).( o)", NULL },
	{ "setlevel", "the Overmind needs spawns, %s^7, do you have some?",
	 NULL },
	{ "showbans",
	 "%s^7, bans are issued as-is with with no warranty or accidental damage cover (and you cannot view the Policy right now).",
	 NULL },
	{ "slap", "%s^7 is trying to get kinky!", NULL },
	{ "spec999",
	 "999ing is still a thing in 2020? %s^7 should up their game...",
	 NULL },
	...
	{ "undesignate",
	 "the team shall decide the designation, not you, %s^7.", NULL },
	{ "unflag", "%s^7 wants to put up the white flag and surrender.",
	 NULL },
	{ "unmute",
	 "this is a democracy, where people like %s^7 should be using /callvote",
	 NULL },
	{ "unpause", "there's no stopping now, %s^7.", NULL },
	{ "warn",
	 "it's not like they would have taken any notice of %s^7's warning anyway.",
	 NULL },

	{ "setdevmode",
	 "%s^7 should be using ^3!devmap^7 to start a new game with cheats on!",
	 NULL },
	...
	{ "give",
	 "in accordance with the Zittrig Code of Conduct, %s^7 is not permitted to ^3!give^7 credits or evos.",
	 NULL },
	{ "setrotation",
	 "the laws of physics set the rotation, not mere mortals like you, %s^7.",
	 NULL },
	...
	{ "shuffle",
	 "%s^7: ^2As I lay there listening to the strange night sounds, I hear the shuffle of someone creeping by outside in the grass.",
	 NULL },
};
...
static int adminNumSchachts =
    sizeof(g_admin_schachts) / sizeof(g_admin_schachts[0]);
...
qboolean G_admin_permission_guid(const char *guid, const char *flag)
{
	...
	for (i = 0; i < MAX_ADMIN_ADMINS && g_admin_admins[i]; i++) {
		if (!Q_stricmp(guid, g_admin_admins[i]->guid)) {
			if (admin_permission
			    (g_admin_admins[i]->flags, flag, &perm))
				return perm;
	...
	for (i = 0; i < MAX_ADMIN_LEVELS && g_admin_levels[i]; i++) {
		if (g_admin_levels[i]->level == l)
			return admin_permission(g_admin_levels[i]->flags, flag,
						&perm) && perm;
	}
	return qfalse;
}
...
qboolean G_admin_name_check(gentity_t * ent, char *name, char *err, int len)
{
	...
	for (i = 0; i < level.maxclients; i++) {
		...
		G_SanitiseString(client->pers.netname, testName,
				 sizeof(testName));
		if (!Q_stricmp(name2, testName)) {
			Q_strncpyz(err,
				   va("The name '%s^7' is already in use",
				      name), len);
			return qfalse;
		}
	}
	...
	if (Q_isdigit(name2[0]) || name2[0] == '-') {
		Q_strncpyz(err,
			   "Names cannot begin with a number or with a dash. Please choose another.",
			   len);
		return qfalse;
	}
	...
	for (i = 0; name2[i] != '\0'; i++) {
		...
		if (name2[i] == ' ') {
			if (name2[i + 1] == '-') {
				Q_strncpyz(err,
					   "Names cannot contain a - preceded by a space. Please choose another.",
					   len);
				return qfalse;
			}
		}
	...
	if (alphaCount == 0) {
		Q_strncpyz(err,
			   va
			   ("The name '%s^7' does not include at least one letter. Please choose another.",
			    name), len);
		return qfalse;
	}
	...
	for (i = 0; i < MAX_ADMIN_ADMINS && g_admin_admins[i]; i++) {
		if (g_admin_admins[i]->level < 1)
			continue;
		G_SanitiseString(g_admin_admins[i]->name, testName,
				 sizeof(testName));
		if (!Q_stricmp(name2, testName)
		    && Q_stricmp(ent->client->pers.guid,
				 g_admin_admins[i]->guid)) {
			Q_strncpyz(err,
				   va("The name '%s^7' belongs to an admin. "
...
static qboolean admin_higher_guid(char *admin_guid, char *victim_guid)
{
	...
	for (i = 0; i < MAX_ADMIN_ADMINS && g_admin_admins[i]; i++) {
		if (!Q_stricmp(admin_guid, g_admin_admins[i]->guid)) {
			...
			Q_strncpyz(aguid, g_admin_admins[i]->guid,
				   sizeof(aguid));
	...
	for (i = 0; i < MAX_ADMIN_ADMINS && g_admin_admins[i]; i++) {
		if (!Q_stricmp(victim_guid, g_admin_admins[i]->guid)) {
			...
			return (!admin_permission
				(g_admin_admins[i]->flags, ADMF_IMMUTABLE,
				 &perm) || !perm);
		}
	}
	return qtrue;
...
void admin_writeconfig(void)
{
	...
	if (len < 0) {
		G_Printf
		    ("admin_writeconfig: could not open g_admin file \"%s\"\n",
		     g_admin.string);
		return;
	}
	for (i = 0; i < MAX_ADMIN_LEVELS && g_admin_levels[i]; i++) {
	...
	for (i = 0; i < MAX_ADMIN_ADMINS && g_admin_admins[i]; i++) {
		...
		if (expiretime > 0) {
			...
			if (t - expiretime > g_admin_admins[i]->seen
...
static void admin_default_levels(void)
{
	...
	Q_strncpyz(g_admin_levels[0]->flags,
		   "listplayers admintest help specme time", sizeof(l->flags));
...
void G_admin_set_adminname(gentity_t * ent)
{
	...
	if (found) {
		Q_strncpyz(ent->client->pers.adminName, g_admin_admins[i]->name,
			   sizeof(ent->client->pers.adminName));
	} else {
		Q_strncpyz(ent->client->pers.adminName, "",
			   sizeof(ent->client->pers.adminName));
	}
}
	...
	*flags = '\0';
	if (admin) {
		for (i = 0; i < MAX_ADMIN_ADMINS && g_admin_admins[i]; i++) {
			if (!Q_stricmp
				...
				for (j = 0;
				     j < MAX_ADMIN_LEVELS && g_admin_levels[j];
				     j++) {
					if (g_admin_levels[j]->level ==
					    a->level) {
						l = g_admin_levels[j];
	...
	if (victim && Q_stricmp(cmd, "attempted")) {
		...
		Com_sprintf(string, sizeof(string),
			    ...
			    (admin) ? admin->client->pers.netname : "console",
			    (admin) ? admin->client->pers.adminName : "console",
			    flags, cmd, G_SayConcatArgs(1 + skiparg));
	}
			    ...
			    int minlevel)
{
	...
	for (i = 0; i < level.maxclients && search[0]; i++) {
		...
		for (j = 0; j < MAX_ADMIN_LEVELS && g_admin_levels[j]; j++) {
			if (g_admin_levels[j]->level == l) {
				...
				Com_sprintf(lname_fmt, sizeof(lname_fmt),
					    "%%%is",
					    (int)(admin_level_maxname +
						  strlen(g_admin_levels[j]->
							 name)
						  - strlen(lname)));
	...
	for (i = 0; i < MAX_ADMIN_ADMINS && g_admin_admins[i]
		...
		if (g_admin_admins[i]->level >= minlevel) {
			...
			if (search[0]) {
				G_SanitiseString(g_admin_admins[i]->name, name,
						 sizeof(name));
				if (!strstr(name, search))
					continue;
				...
				for (j = 0; j < level.maxclients; j++) {
					vic = &g_entities[j];
					if (!vic->client
					    || vic->client->pers.connected !=
					    CON_CONNECTED)
						continue;
					G_SanitiseString(vic->client->pers.
							 netname, name2,
							 sizeof(name2));
					if (!Q_stricmp
			...
			lname[0] = '\0';
			Q_strncpyz(lname_fmt, "%s", sizeof(lname_fmt));
			for (j = 0; j < MAX_ADMIN_LEVELS && g_admin_levels[j];
			     j++) {
				if (g_admin_levels[j]->level ==
				    g_admin_admins[i]->level) {
					G_DecolorString(g_admin_levels[j]->name,
							lname);
					Com_sprintf(lname_fmt,
						    sizeof(lname_fmt), "%%%is",
						    (int)(admin_level_maxname +
							  strlen(g_admin_levels
								 [j]->name)
							  - strlen(lname)));
					Com_sprintf(lname, sizeof(lname),
						    lname_fmt,
						    g_admin_levels[j]->name);
					break;
				}
			...
			ADMBP(va("%4i %4i %s^7 (*%s) %s^7\n",
				 (i + MAX_CLIENTS),
				 g_admin_admins[i]->level,
				 lname, guid_stub, g_admin_admins[i]->name));
			drawn++;
		}
	ADMBP_end();
	return drawn;
	...
	*reason = '\0';
	...
	for (i = 0; i < MAX_ADMIN_BANS && g_admin_bans[i]; i++) {
		...
		if (!ignoreIP) {
			...
			memset(IP, 0, sizeof(IP));
			ipscanfcount =
			    sscanf(g_admin_bans[i]->ip, "%d.%d.%d.%d/%d",
			...
			if (intIP == tempIP || mask == 0) {
				...
				if (t - lastConnectTime >= 300 ||
					...
					lastConnectTime = t;
					Q_strncpyz(lastConnectIP, ip,
						   sizeof(lastConnectIP));

					G_WarningsPrintf("ban",
		...
		if (*guid && !Q_stricmp(g_admin_bans[i]->guid, guid)) {
			...
			Com_sprintf(reason,
				    ...
				    g_admin_bans[i]->banner,
				    g_admin_bans[i]->reason, duration);
			G_Printf
			    ("Banned player tried to connect with GUID %s\n",
			     guid);
			return qtrue;
		}
	}
	if (*guid) {
		...
		if (!valid || count != 32) {
			...
			G_Printf
			    ("Player with invalid GUID [%s] connect from IP %s\n",
			     guid, ip);
			return qtrue;
		}
	}
...
void G_admin_namelog_update(gclient_t * client, qboolean disconnect)
{
	...
	for (i = 0; i < MAX_ADMIN_NAMELOGS && g_admin_namelog[i]; i++) {
		...
		if (!Q_stricmp(client->pers.ip, g_admin_namelog[i]->ip)
			...
			for (j = 0; j < MAX_ADMIN_NAMELOG_NAMES
				...
				G_SanitiseString(g_admin_namelog[i]->name[j],
						 n2, sizeof(n2));
				if (!Q_stricmp(n1, n2))
					break;
			}
			...
			Q_strncpyz(g_admin_namelog[i]->name[j],
				   client->pers.netname,
				   sizeof(g_admin_namelog[i]->name[j]));
			g_admin_namelog[i]->slot =
			    (disconnect) ? -1 : clientNum;
			...
			if (!disconnect) {
				if (g_admin_namelog[i]->muted) {
					...
					client->pers.muteExpires =
					    g_admin_namelog[i]->muteExpires;
					G_AdminsPrintf
					    ("^7%s^7's mute has been restored\n",
					     client->pers.netname);
				...
				if (g_admin_namelog[i]->denyBuild) {
					...
					G_AdminsPrintf
					    ("^7%s^7's Denybuild has been restored\n",
					     client->pers.netname);
					g_admin_namelog[i]->denyBuild = qfalse;
				}
				if (g_admin_namelog[i]->denyHumanWeapons > 0
				    || g_admin_namelog[i]->denyAlienClasses >
				    0) {
					if (g_admin_namelog[i]->
					    denyHumanWeapons > 0)
						client->pers.denyHumanWeapons =
						    g_admin_namelog[i]->
						    denyHumanWeapons;
					if (g_admin_namelog[i]->
					    denyAlienClasses > 0)
						client->pers.denyAlienClasses =
						    g_admin_namelog[i]->
						    denyAlienClasses;

					G_AdminsPrintf
					    ("^7%s^7's Denyweapon has been restored\n",
					     client->pers.netname);
					g_admin_namelog[i]->denyHumanWeapons =
					    0;
					g_admin_namelog[i]->denyAlienClasses =
					    0;
				}
				if (g_admin_namelog[i]->specExpires > 0) {
					client->pers.specExpires =
					    g_admin_namelog[i]->specExpires;
					G_AdminsPrintf
					    ("^7%s^7's Putteam spectator has been restored\n",
					     client->pers.netname);
					g_admin_namelog[i]->specExpires = 0;
				}
				if (g_admin_namelog[i]->voteCount > 0) {
					client->pers.voteCount =
					    g_admin_namelog[i]->voteCount;
	...
	if (i >= MAX_ADMIN_NAMELOGS) {
		G_Printf
		    ("G_admin_namelog_update: warning, g_admin_namelogs overflow\n");
		return;
	}
...
qboolean G_admin_readconfig(gentity_t * ent, int skiparg)
{
	...
	if (!g_admin.string[0]) {
		ADMP("^3!readconfig: g_admin is not set, not loading configuration " "from a file\n");
	...
	if (len < 0) {
		ADMP(va
		     ("^3!readconfig: ^7could not open admin config file %s\n",
		      g_admin.string));
		admin_default_levels();
		return qfalse;
	}
	...
	*(cnf + len) = '\0';
	...
	while (*t) {
		if (!Q_stricmp(t, "[level]") ||
		    !Q_stricmp(t, "[admin]") ||
		    !Q_stricmp(t, "[ban]") || !Q_stricmp(t, "[command]")) {
		...
		if (level_open) {
			if (!Q_stricmp(t, "level")) {
				...
				admin_readconfig_string(&cnf, l->name,
							sizeof(l->name));
			} else if (!Q_stricmp(t, "flags")) {
				admin_readconfig_string(&cnf, l->flags,
							sizeof(l->flags));
			} else {
				ADMP(va
				     ("^3!readconfig: ^7[level] parse error near %s on line %d\n",
				      t, COM_GetCurrentParseLine()));
			}
		} else if (admin_open) {
			if (!Q_stricmp(t, "name")) {
				admin_readconfig_string(&cnf, a->name,
							sizeof(a->name));
			} else if (!Q_stricmp(t, "guid")) {
				admin_readconfig_string(&cnf, a->guid,
							sizeof(a->guid));
			} else if (!Q_stricmp(t, "level")) {
				admin_readconfig_int(&cnf, &a->level);
			} else if (!Q_stricmp(t, "flags")) {
				admin_readconfig_string(&cnf, a->flags,
							sizeof(a->flags));
			} else if (!Q_stricmp(t, "seen")) {
				admin_readconfig_int(&cnf, &a->seen);
			} else {
				ADMP(va
				     ("^3!readconfig: ^7[admin] parse error near %s on line %d\n",
				      t, COM_GetCurrentParseLine()));
			}

		} else if (ban_open) {
			if (!Q_stricmp(t, "name")) {
				admin_readconfig_string(&cnf, b->name,
							sizeof(b->name));
			} else if (!Q_stricmp(t, "guid")) {
				admin_readconfig_string(&cnf, b->guid,
							sizeof(b->guid));
			} else if (!Q_stricmp(t, "ip")) {
				admin_readconfig_string(&cnf, b->ip,
							sizeof(b->ip));
			} else if (!Q_stricmp(t, "reason")) {
				admin_readconfig_string(&cnf, b->reason,
							sizeof(b->reason));
			} else if (!Q_stricmp(t, "made")) {
				admin_readconfig_string(&cnf, b->made,
							sizeof(b->made));
				...
				admin_readconfig_string(&cnf, b->suspendby,
							sizeof(b->suspendby));
			} else if (!Q_stricmp(t, "banner")) {
				admin_readconfig_string(&cnf, b->banner,
							sizeof(b->banner));
			} else if (!Q_stricmp(t, "blevel")) {
				admin_readconfig_int(&cnf, &b->bannerlevel);
			} else {
				ADMP(va
				     ("^3!readconfig: ^7[ban] parse error near %s on line %d\n",
				      t, COM_GetCurrentParseLine()));
			}
		} else if (command_open) {
			if (!Q_stricmp(t, "command")) {
				admin_readconfig_string(&cnf, c->command,
							sizeof(c->command));
			} else if (!Q_stricmp(t, "exec")) {
				admin_readconfig_string(&cnf, c->exec,
							sizeof(c->exec));
			} else if (!Q_stricmp(t, "desc")) {
				admin_readconfig_string(&cnf, c->desc,
							sizeof(c->desc));
			} else if (!Q_stricmp(t, "flag")) {
				admin_readconfig_string(&cnf, c->flag,
							sizeof(c->flag));
			} else {
				ADMP(va
				     ("^3!readconfig: ^7[command] parse error near %s on line %d\n",
				      t, COM_GetCurrentParseLine()));
			}
		}
	...
	for (i = 0; i < level.maxclients; i++)
		if (level.clients[i].pers.connected != CON_DISCONNECTED)
			level.clients[i].pers.adminLevel =
			    G_admin_level(&g_entities[i]);
	return qtrue;
}
...
qboolean G_admin_time(gentity_t * ent, int skiparg)
{
	...
	if (mins == 0) {
		ADMP(va
		     ("^3!time: ^7this match has been going on for ^3%i ^7seconds.\n",
		      seconds));
	} else {
		ADMP(va
		     ("^3!time: ^7this match has been going on for ^3%i ^7minute%s and ^3%i ^7second%s\n",
		      mins, pluralM, seconds, pluralS));
	}

	if (g_suddenDeathTime.integer > 0 && !g_suddenDeath.integer) {
		ADMP(va
		     ("^3!time: ^7sudden death will be at ^3%02i:00^7, in about ^3%i ^7minutes\n",
		      level.suddenDeathBeginTime / 60000,
		      G_TimeTilSuddenDeath() / 60000));
...
static int G_admin_find_slot(gentity_t * ent, char *namearg,
			     const char *command)
{
	char name[MAX_NAME_LENGTH];
	char testname[MAX_NAME_LENGTH];
	...
	if (numeric) {
		...
		if (id >= 0 && id < level.maxclients) {
			...
			vic = &g_entities[id];
			if (vic && vic->client
			    && vic->client->pers.connected != CON_DISCONNECTED)
				return id;

			ADMP(va
			     ("^3!%s:^7 no one connected by that slot number\n",
			      command));
			return -1;
		}
		...
		ADMP(va
		     ("^3!%s:^7 no match for slot or admin number %d\n",
		      command, id));
		return -1;
	}

	for (i = 0; i < MAX_ADMIN_ADMINS && g_admin_admins[i] && matches < 2;
	     i++) {
		G_SanitiseString(g_admin_admins[i]->name, testname,
				 sizeof(testname));
	...
	for (i = 0; i < level.maxclients && matches < 2; i++) {
		...
		if (matches && guid
		    && !Q_stricmp(level.clients[i].pers.guid, guid))
			continue;
	...
	ADMP(va
	     ("^3!%s:^7 multiple matches, use the admin number instead:\n",
	      command));
...
static int G_admin_find_admin_slot(gentity_t * ent, char *namearg,
				   char *command, char *nick, int nick_len)
{
	gentity_t *vic;
	char *guid;
	int id;
	...
	if (id < MAX_CLIENTS) {
		...
		if (id < MAX_CLIENTS) {
			ADMP(va
			     ("^3!%s:^7 player is not !registered\n", command));
			return -1;
		}
	}
...
qboolean G_admin_setlevel(gentity_t * ent, int skiparg)
{
	...
	if (ent && l > ent->client->pers.adminLevel) {
		ADMP("^3!setlevel: ^7you may not use !setlevel to set a level higher " "than your current level\n");
		return qfalse;
	}
	...
	if (id >= 0 && id < MAX_CLIENTS) {
		vic = &g_entities[id];
		Q_strncpyz(adminname, vic->client->pers.netname,
			   sizeof(adminname));
	...
	if (ent && !admin_higher_guid(ent->client->pers.guid, guid)) {
		ADMP("^3!setlevel: ^7sorry, but your intended victim has a higher" " admin level than you\n");
		return qfalse;
	}
	...
	AP(va
	   ("print \"^3!setlevel: ^7%s^7 was given level %d admin rights by %s\n\"",
	    adminname, l, G_admin_adminPrintName(ent)));
	if (vic) {
	...
	if (!g_admin.string[0])
		ADMP("^3!setlevel: ^7WARNING g_admin not set, not saving admin record " "to a file\n");
	else
		admin_writeconfig();
	return qtrue;
}
...
const char *G_admin_user_flag(char *oldflags, char *flag, qboolean add,
			      qboolean clear, char *newflags, int size)
{
	char *token, *token_p;
	char *key;
	...
	if (!flagset && !clear) {
		if (!strcmp(flag, ADMF_ALLFLAGS)) {
			...
			if (head_count < MAX_USER_FLAGS) {
				Com_sprintf(head_flags[head_count],
					    sizeof(head_flags[head_count]),
					    "%s%s", (add) ? "" : "-", flag);
				head_count++;
			}
			...
			if (tail_count < MAX_USER_FLAGS) {
				Com_sprintf(tail_flags[tail_count],
					    sizeof(tail_flags[tail_count]),
					    "%s%s", (add) ? "+" : "-", flag);
				tail_count++;
			}
	...
	if (allflag[0]) {
		...
		for (i = 0; i < tail_count; i++) {
			Q_strcat(newflags, size, va(" %s", tail_flags[i]));
		}
	}
...
static AdminFlagListEntry_t adminFlagList[] = {
	...
	{ ADMF_SEESINCOGNITO,
	 "sees registered name of players flagged with INCOGNITO" },
	...
	{ ADMF_NOSCRIMRESTRICTION,
	 "team joining, vote and chat restrictions during scrims do not apply"
	 },
...
qboolean G_admin_flaglist(gentity_t * ent, int skiparg)
{
	...
	for (i = 0; i < adminNumFlags; i++) {
		ADMBP(va("  %s%-20s ^7%s\n",
			 ...
			 adminFlagList[i].flag, adminFlagList[i].description));
	}
...
qboolean G_admin_flag(gentity_t * ent, int skiparg)
{
	...
	if (name[0] == '*') {
		if (ent) {
			ADMP(va
			     ("^3!%s: only console can change admin level flags\n",
			      cmd));
			return qfalse;
		}
		id = atoi(name + 1);
		...
		if (admin_level < 0) {
			ADMP(va
			     ("^3!%s: admin level %d does not exist\n", cmd,
			      id));
			return qfalse;
		}
		Com_sprintf(adminname, sizeof(adminname), "admin level %d", id);
	} else {
		id = G_admin_find_admin_slot(ent, name, cmd, adminname,
					     sizeof(adminname));
		if (id < 0)
			return qfalse;

		if (ent
		    && !admin_higher_guid(ent->client->pers.guid,
					  g_admin_admins[id]->guid)) {
			ADMP(va
			     ("^3%s:^7 your intended victim has a higher admin level than you\n",
			      cmd));
			return qfalse;
		}
	}
	...
	if (G_SayArgc() < 3 + skiparg) {
		...
		level = 0;
		if (admin_level < 0) {
			for (i = 0; i < MAX_ADMIN_LEVELS && g_admin_levels[i];
			     i++) {
				if (g_admin_admins[id]->level ==
	...
	if (ent && !Q_stricmp(ent->client->pers.guid, g_admin_admins[id]->guid)) {
		ADMP(va
		     ("^3%s:^7 you may not change your own flags (use rcon)\n",
		      cmd));
		return qfalse;
	}
	if (flag[0] != '.' && !G_admin_permission(ent, flag)) {
		ADMP(va
		     ("^3%s:^7 you can only change flags that you also have\n",
		      cmd));
		return qfalse;
	}
	...
	if (admin_level < 0) {
		result =
		    G_admin_user_flag(g_admin_admins[id]->flags, flag, add,
				      clear, g_admin_admins[id]->flags,
				      sizeof(g_admin_admins[id]->flags));
	} else {
		result =
		    G_admin_user_flag(g_admin_levels[admin_level]->flags, flag,
				      add, clear,
				      g_admin_levels[admin_level]->flags,
				      sizeof(g_admin_levels[admin_level]->
					     flags));
	}
	if (result) {
		ADMP(va
		     ("^3!flag: ^7an error occured setting flag '^3%s^7', %s\n",
		      flag, result));
		return qfalse;
	}
	...
	if (!Q_stricmp(cmd, "flag")) {
		G_AdminsPrintf("^3!%s: ^7%s^7 was %s admin flag '%s' by %s\n",
			       cmd, adminname,
			       (add) ? "given" : "denied",
			       flag, G_admin_adminPrintName(ent));
	} else {
		G_AdminsPrintf
		    ("^3!%s: ^7admin flag '%s' for %s^7 cleared by %s\n", cmd,
		     flag, adminname, G_admin_adminPrintName(ent));
	}

	if (!g_admin.string[0])
		ADMP(va
		     ("^3!%s: ^7WARNING g_admin not set, not saving admin record "
		      "to a file\n", cmd));
	else
		admin_writeconfig();
...
static qboolean admin_create_ban(gentity_t * ent,
				 char *netname,
				 char *guid,
				 char *ip, int seconds, char *reason)
{
	g_admin_ban_t *b = NULL;
	qtime_t qt;
	int t;
...
qboolean G_admin_kick(gentity_t * ent, int skiparg)
{
	...
	if (!admin_higher(ent, &g_entities[pids[0]])) {
		ADMP("^3!kick: ^7sorry, but your intended victim has a higher admin" " level than you\n");
		return qfalse;
	}
	...
	trap_SendServerCommand(pids[0],
			       va
			       ("disconnect \"You have been kicked.\n%s^7\nreason:\n%s\n%s\"",
				(ent) ? va("admin:\n%s",
...
qboolean G_admin_ban(gentity_t * ent, int skiparg)
{
	...
	if (seconds <= 0) {
		if (g_adminMaxBan.integer
			...
			ADMP(va
			     ("^3!ban: ^7using your admin level's maximum ban length of %s\n",
		...
		if (g_adminMaxBan.integer &&
			...
			seconds = G_admin_parse_time(g_adminMaxBan.string);
			ADMP(va
			     ("^3!ban: ^7ban length limited to %s for your admin level\n",
			      g_adminMaxBan.string));
		}
	}
	...
	for (i = 0;
	     !exactmatch && i < MAX_ADMIN_NAMELOGS && g_admin_namelog[i]; i++) {
		// skip players in the namelog who have already been banned
		if (g_admin_namelog[i]->banned)
			continue;
		...
		for (j = 0; j < MAX_ADMIN_NAMELOG_NAMES
		     && g_admin_namelog[i]->name[j][0]; j++) {
			G_SanitiseString(g_admin_namelog[i]->name[j], n2,
					 sizeof(n2));
			if (strstr(n2, s2)) {
				if (logmatch != i)
					logmatches++;
	...
	if (!logmatches) {
		...
		ADMBP_begin();
		ADMBP
		    ("^3!ban: ^7multiple recent clients match name, use IP or slot#:\n");
		for (i = 0; i < MAX_ADMIN_NAMELOGS && g_admin_namelog[i]; i++) {
			for (j = 0; j < 8; j++)
	...
	if (ent && !admin_higher_guid(ent->client->pers.guid,
		...
		ADMP("^3!ban: ^7sorry, but your intended victim has a higher admin" " level than you\n");
		return qfalse;
	}
	...
	admin_create_ban(ent,
			 ...
			 g_admin_namelog[logmatch]->guid,
			 g_admin_namelog[logmatch]->ip, seconds, reason);
	...
	AP(va
	   ("print \"^3!ban: ^7%s ^7was banned by %s^7, duration: %s, reason: %s\n\"",
	    g_admin_namelog[logmatch]->name[0], G_admin_adminPrintName(ent),
	    duration, (*reason) ? reason : "banned by admin"));
...
qboolean G_admin_adjustban(gentity_t * ent, int skiparg)
{
	...
	if (length < 0)
		...
		if (length) {
			...
			if (mode == '+')
				expires =
				    g_admin_bans[bnum - 1]->expires + length;
			else if (mode == '-')
				expires =
				    g_admin_bans[bnum - 1]->expires - length;
			else
	...
	AP(va
	   ("print \"^3!adjustban: ^7ban #%d for %s^7 has been updated by %s^7 "
	    "%s%s%s%s%s\n\"", bnum, g_admin_bans[bnum - 1]->name,
	    G_admin_adminPrintName(ent), (length >= 0) ? "duration: " : "",
	    duration, (length >= 0
		       && *reason) ? ", " : "", (*reason) ? "reason: " : "",
	    reason));
	if (ent) {
		Q_strncpyz(g_admin_bans[bnum - 1]->banner,
			   G_admin_get_adminname(ent),
...
qboolean G_admin_subnetban(gentity_t * ent, int skiparg)
{
	...
	if (mask >= 0 && mask <= 32) {
		...
		memset(IP, 0, sizeof(IP));
		sscanf(g_admin_bans[bnum - 1]->ip, "%d.%d.%d.%d/%d", &IP[4],
		       &IP[3], &IP[2], &IP[1], &IP[0]);
		for (k = 4; k >= 1; k--) {
			if (IP[k])
				IPRlow |= IP[k] << 8 * (k - 1);
		...
		if (mask == 32) {
			Q_strncpyz(g_admin_bans[bnum - 1]->ip,
				   va("%i.%i.%i.%i", IP[4], IP[3], IP[2],
				      IP[1]), sizeof(g_admin_bans[bnum - 1]->ip)
			    );
		} else {
			Q_strncpyz(g_admin_bans[bnum - 1]->ip,
				   va("%i.%i.%i.%i/%i", IP[4], IP[3], IP[2],
				      IP[1], mask),
	...
	if (mask > 0) {
		Q_strncpyz(cIPRlow,
			   va("%u.%u.%u.%u", (IPRlow & (255 << 24)) >> 24,
			      (IPRlow & (255 << 16)) >> 16,
			      (IPRlow & (255 << 8)) >> 8, IPRlow & 255),
			   sizeof(cIPRlow)
		    );
		Q_strncpyz(cIPRhigh,
			   va("%u.%u.%u.%u", (IPRhigh & (255 << 24)) >> 24,
			      (IPRhigh & (255 << 16)) >> 16,
			      (IPRhigh & (255 << 8)) >> 8, IPRhigh & 255),
			   sizeof(cIPRhigh)
		    );
	} else {
	...
	AP(va
	   ("print \"^3!subnetban: ^7ban #%d for %s^7 has been updated by %s^7 "
	    "%s (%s - %s)\n\"", bnum, g_admin_bans[bnum - 1]->name,
	    G_admin_adminPrintName(ent), g_admin_bans[bnum - 1]->ip, cIPRlow,
	    cIPRhigh));
	if (ent)
		Q_strncpyz(g_admin_bans[bnum - 1]->banner,
			   G_admin_get_adminname(ent),
...
qboolean G_admin_suspendban(gentity_t * ent, int skiparg)
{
	...
	} else if (g_admin_bans[bnum - 1]->expires > 0
		   && length + timenow > g_admin_bans[bnum - 1]->expires) {
		length = g_admin_bans[bnum - 1]->expires - timenow;
		G_admin_duration(length, duration, sizeof(duration));
		ADMP(va
		     ("^3!suspendban: ^7Suspension Duration trimmed to Ban duration: %s\n",
		      duration));
	}
	...
	Q_strncpyz(g_admin_bans[bnum - 1]->suspendby,
		   G_admin_get_adminname(ent),
		   sizeof(g_admin_bans[bnum - 1]->suspendby));

	if (length > 0) {
		G_admin_duration(length, duration, sizeof(duration));
		AP(va
		   ("print \"^3!suspendban: ^7ban ^3#%d^7 for %s^7 suspended for ^3%s^7 by %s^7\n\"",
		    bnum, g_admin_bans[bnum - 1]->name, duration,
		    G_admin_adminPrintName(ent)));
	} else {
		AP(va
		   ("print \"^3!suspendban: ^7ban ^3#%d^7 for %s^7 suspension removed by %s^7\n\"",
		    bnum, g_admin_bans[bnum - 1]->name,
		    G_admin_adminPrintName(ent)));
	}
...
qboolean G_admin_unban(gentity_t * ent, int skiparg)
{
	...
	if (g_adminMaxBan.integer &&
	    ...
	    G_admin_parse_time(g_adminMaxBan.string)) {
		ADMP(va
		     ("^3!unban: ^7your admin level cannot remove bans longer than %s\n",
		      g_adminMaxBan.string));
		return qfalse;
	}
	g_admin_bans[bnum - 1]->expires = t;
	AP(va("print \"^3!unban: ^7ban #%d for %s^7 has been removed by %s\n\"",
	      bnum, g_admin_bans[bnum - 1]->name, G_admin_adminPrintName(ent)));
	if (g_admin.string[0])
		admin_writeconfig();
	return qtrue;
}
...
qboolean G_admin_putteam(gentity_t * ent, int skiparg)
{
	...
	if (!admin_higher(ent, &g_entities[pids[0]])) {
		ADMP("^3!putteam: ^7sorry, but your intended victim has a higher " " admin level than you\n");
		return qfalse;
	}
	vic = &g_entities[pids[0]];
	...
	if (vic->client->pers.teamSelection == teamnum && teamnum != PTE_NONE) {
		ADMP(va
		     ("^3!putteam: ^7%s ^7is already on the %s team\n",
	...
	for (i = 0; i < level.maxclients && count < 10; i++) {
		...
		if (i == id || (search[0] && strstr(name, search))) {
			if (vic->client->sess.invisible == qfalse) {
				ADMBP(va
				      ("^3%4d ^7%s^7 is currently playing\n", i,
				       vic->client->pers.netname));
				count++;
			}
		}
	}
	for (i = 0; i < MAX_ADMIN_ADMINS && g_admin_admins[i] && count < 10;
	     i++) {
		G_SanitiseString(g_admin_admins[i]->name, name, sizeof(name));
		if (i + MAX_CLIENTS == id
		    || (search[0] && strstr(name, search))) {
			ison = qfalse;
			for (j = 0; j < level.maxclients; j++) {
				vic = &g_entities[j];
				if (!vic->client
				    ...
				    CON_CONNECTED)
					continue;
				G_SanitiseString(vic->client->pers.netname,
						 name, sizeof(name));
				if (!Q_stricmp
				    (vic->client->pers.guid,
				     g_admin_admins[i]->guid)
				    && strstr(name, search)) {
					if (vic->client->sess.invisible ==
					    qfalse) {
						ison = qtrue;
						break;
					}
...
void G_admin_adminlog_log(gentity_t * ent, char *command, char *args,
			  int skiparg, qboolean success)
{
	...
	if (!Q_stricmp(command, "adminlog") ||
	    ...
	    !Q_stricmp(command, "seen") || !Q_stricmp(command, "time"))
		return;
	...
	if (args)
		...
		Q_strncpyz(adminlog->args, G_SayConcatArgs(1 + skiparg),
			   sizeof(adminlog->args));
	...
	if (ent) {
		...
		for (i = 0; i < MAX_ADMIN_ADMINS && g_admin_admins[i]; i++) {
			if (!Q_stricmp
			    (g_admin_admins[i]->guid, ent->client->pers.guid)) {
				Q_strncpyz(adminlog->name,
					   g_admin_admins[i]->name,
					   sizeof(adminlog->name));
				found = qtrue;
				break;
			}
		}
		if (!found)
			Q_strncpyz(adminlog->name, ent->client->pers.netname,
				   sizeof(adminlog->name));
...
qboolean G_admin_adminlog(gentity_t * ent, int skiparg)
{
	...
	if (G_SayArgc() > 1 + skiparg) {
		...
		if ((*argbuf >= '0' && *argbuf <= '9') || *argbuf == '-') {
			...
			for (i = 0; i < MAX_ADMIN_ADMINLOGS; i++) {
				if (g_admin_adminlog[i]
				    && g_admin_adminlog[i]->id == id) {
					index = i;
					break;
				}
			}
	...
	if (search_cmd || search_name) {
		...
		while (g_admin_adminlog[index] &&
		       checked < MAX_ADMIN_ADMINLOGS && result_index < 10) {
			match = qfalse;
			if (search_cmd) {
				if (!Q_stricmp
				    (search_cmd,
				     g_admin_adminlog[index]->command))
					match = qtrue;
			}
			if (search_name) {
				G_SanitiseString(g_admin_adminlog[index]->name,
						 n1, sizeof(n1));
			...
			if (match) {
				result_swap[result_index] =
				    g_admin_adminlog[index];
				result_index++;
			}
	...
	for (i = 0; results[i] && i < 10; i++) {
		...
		Com_sprintf(fmt_name, sizeof(fmt_name), "%%%ds",
			    (int)(name_length + strlen(results[i]->name) -
				  strlen(n1)));
		Com_sprintf(n1, sizeof(n1), fmt_name, results[i]->name);
		Com_sprintf(levelbuf, sizeof(levelbuf), "%2d",
			    results[i]->level);
		ADMBP(va
		      ("%s%3d %3d:%02d %2s ^7%s^7 %s!%s ^7%s^7\n",
		       (results[i]->success) ? "^7" : "^1", results[i]->id,
		       t / 60, t % 60,
		       (results[i]->level) < 10000 ? levelbuf : " -", n1,
		       (results[i]->success) ? "^3" : "^1", results[i]->command,
		       results[i]->args));
	}
	if (search_cmd || search_name) {
		...
		if (checked < MAX_ADMIN_ADMINLOGS && g_admin_adminlog[checked])
			ADMBP(va(" run '!adminlog %s^7 %d' to see more",
				 argbuf, skipped + result_index));
		ADMBP("\n");
	} else if (results[0]) {
		ADMBP(va("^3!adminlog:^7 Showing %d - %d of %d.\n",
			 results[0]->id,
			 results[result_index - 1]->id, max_id));
	} else {
...
qboolean G_admin_devmap(gentity_t * ent, int skiparg)
{
	...
	if (G_SayArgc() > 2 + skiparg) {
		...
		if (!Q_stricmp(layout, "*BUILTIN*") ||
			...
			ADMP(va
			     ("^3!devmap: ^7invalid layout name '%s'\n",
			      layout));
			return qfalse;
		}
	}
	...
	level.restarted = qtrue;
	AP(va
	   ("print \"^3!devmap: ^7map '%s' started by %s^7 with cheats %s\n\"",
	    map, G_admin_adminPrintName(ent),
	    (layout[0]) ? va("(forcing layout '%s')", layout) : ""));
...
qboolean G_admin_maplog(gentity_t * ent, int skiparg)
{
	...
	while (*ptr != '\0' && count < MAX_ADMIN_MAPLOG_LENGTH + 1) {
		...
		if (ptr[0] && ptr[1] == ';') {
			...
			case 'A':
				result =
				    "^1Alien win ^7/ Humans admitted defeat";
				break;
			case 'h':
				result = "^4Human win";
				break;
			case 'H':
				result =
				    "^4Human win ^7/ Aliens admitted defeat";
				break;
			case 'd':
				result = "^5draw vote";
				break;
...
qboolean G_admin_demo(gentity_t * ent, int skiparg)
{
	...
	ent->client->pers.ignoreAdminWarnings =
	    !(ent->client->pers.ignoreAdminWarnings);

	ADMP(va("^3!demo: ^7your visibility of admin chat is now %s\n",
		(ent->client->pers.
		 ignoreAdminWarnings) ? "^1disabled" : "^2enabled"));
...
qboolean G_admin_mute(gentity_t * ent, int skiparg)
{
	...
	if (G_SayArgc() < 2 + skiparg) {
		ADMP(va
		     ("^3!%s: ^7usage: !%s [name|slot#] (duration)\n", cmd,
		      cmd));
		return qfalse;
	}
	...
	if (!admin_higher(ent, &g_entities[pids[0]])) {
		ADMP(va
		     ("^3!%s: ^7sorry, but your intended victim has a higher admin"
		      " level than you\n", cmd));
		return qfalse;
	}
	...
	if (!Q_stricmp(cmd, "unmute")) {
		...
		if (G_SayArgc() > 2 + skiparg) {
			G_SayArgv(2 + skiparg, secs, sizeof(secs));
			seconds = G_admin_parse_time(secs);
			vic->client->pers.muteExpires =
			    level.time + (seconds * 1000);
		}
...
qboolean G_admin_denybuild(gentity_t * ent, int skiparg)
{
	...
	if (vic->client->pers.denyBuild) {
		...
		if (vic->client->ps.stats[STAT_PCLASS] == PCL_ALIEN_BUILDER0
		    || vic->client->ps.stats[STAT_PCLASS] ==
...
qboolean G_admin_denyweapon(gentity_t * ent, int skiparg)
{
	...
	if (G_SayArgc() < 3 + skiparg) {
		ADMP(va
		     ("^3!%s: ^7usage: !%s [name|slot#] [class|weapon]\n", cmd,
		      cmd));
		return qfalse;
	}
	G_SayArgv(1 + skiparg, name, sizeof(name));
	...
	if (vic == ent && !Q_stricmp(cmd, "denyweapon")) {
	...
	if (!Q_stricmp(buffer, "all") && !Q_stricmp(cmd, "allowweapon")) {
		if (vic->client->pers.denyHumanWeapons ||
			...
			CPx(pids[0],
			    "cp \"^1You've regained all weapon and class rights\"");
			AP(va
			   ("print \"^3!%s: ^7all weapon and class rights for ^7%s^7 restored by %s\n\"",
			    cmd, vic->client->pers.netname,
			    G_admin_adminPrintName(ent)));
		} else {
			ADMP(va
			     ("^3!%s: ^7player already has all rights\n", cmd));
		}
		return qtrue;
	}

	if (!Q_stricmp(buffer, "all") && !Q_stricmp(cmd, "denyweapon")) {
		all = qtrue;
		weapon = WP_NONE;
		class = PCL_NONE;
		...
		if (vic->client->pers.denyHumanWeapons == 0xFFFFFF &&
			...
			ADMP(va
			     ("^3!%s: ^7player already has no weapon or class rights\n",
			      cmd));
			return qtrue;
		}
	...
	if (class == PCL_NONE) {
		...
		if (!Q_stricmp(cmd, "denyweapon")) {
			if ((vic->client->pers.denyHumanWeapons & flag) && !all) {
				ADMP(va
				     ("^3!%s: ^7player already has no %s rights\n",
				      cmd, realname));
				return qtrue;
			}
			...
			if (vic->client->pers.teamSelection == PTE_HUMANS) {
				if ((weapon == WP_GRENADE || all) &&
				    BG_InventoryContainsUpgrade(UP_GRENADE,
								vic->client->ps.
								stats)) {
					BG_RemoveUpgradeFromInventory
					    (UP_GRENADE, vic->client->ps.stats);
				...
				if (BG_InventoryContainsWeapon
				    (weapon, vic->client->ps.stats)) {
					int maxAmmo, maxClips;

					BG_RemoveWeaponFromInventory(weapon,
								     vic->
								     client->ps.
								     stats);
					G_AddCreditToClient(vic->client,
					...
					BG_AddWeaponToInventory(WP_MACHINEGUN,
								vic->client->ps.
								stats);
					BG_FindAmmoForWeapon(WP_MACHINEGUN,
							     &maxAmmo,
							     &maxClips);
			...
			if (!(vic->client->pers.denyHumanWeapons & flag)) {
				ADMP(va
				     ("^3!%s: ^7player already has %s rights\n",
				      cmd, realname));
				return qtrue;
			}
			vic->client->pers.denyHumanWeapons &= ~flag;
		...
		if (!Q_stricmp(cmd, "denyweapon")) {
			...
			if (vic->client->pers.teamSelection == PTE_ALIENS &&
				...
				G_RoomForClassChange(vic, PCL_ALIEN_LEVEL0,
						     infestOrigin);

				vic->client->pers.evolveHealthFraction =
				    (float)vic->client->ps.stats[STAT_HEALTH] /
				    (float)BG_FindHealthForClass(class);
				if (vic->client->pers.evolveHealthFraction <
				    0.0f)
					vic->client->pers.evolveHealthFraction =
					    0.0f;
				else if (vic->client->pers.
					 evolveHealthFraction > 1.0f)
					vic->client->pers.evolveHealthFraction =
					    1.0f;

				vic->client->pers.classSelection =
				    PCL_ALIEN_LEVEL0;
				cost =
				    BG_ClassCanEvolveFromTo(PCL_ALIEN_LEVEL0,
							    class, 9, 0);
				if (cost < 0)
					cost = 0;
				...
				ClientSpawn(vic, vic, vic->s.pos.trBase,
					    vic->s.apos.trBase);
			}
		} else {
...
qboolean G_admin_listadmins(gentity_t * ent, int skiparg)
{
	...
	if (!found) {
		if (minlevel > 1) {
			ADMP(va
			     ("^3!listadmins: ^7no admins level %i or greater found\n",
			      minlevel));
		} else {
	...
	if (search[0]) {
		...
		ADMBP(va
		      ("^3!listadmins:^7 showing admins level %i or greater %d - %d of %d.  ",
		       minlevel, (found) ? (start + 1) : 0,
		       ((start + MAX_ADMIN_LISTITEMS) >
...
qboolean G_admin_listplayers(gentity_t * ent, int skiparg)
{
	...
	for (i = 0; i < level.maxclients; i++) {
		...
		if (!Q_stricmp(p->pers.guid, "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX")
		    || (G_admin_permission(&g_entities[i], ADMF_INCOGNITO)
			&& !admin_higher(ent, &g_entities[i])
		...
		for (j = 0; j < MAX_ADMIN_ADMINS && g_admin_admins[j]; j++) {
			if (!Q_stricmp(g_admin_admins[j]->guid, p->pers.guid)) {
				...
				if (ent
				    && G_admin_permission(&g_entities[i],
							  ADMF_INCOGNITO)
				    && !G_admin_permission(ent,
							   ADMF_SEESINCOGNITO)
				    && (&g_entities[i] != ent)) {
				...
				G_SanitiseString(g_admin_admins[j]->name, n3,
						 sizeof(n3));
				if (Q_stricmp(n2, n3)) {
					Q_strncpyz(n, g_admin_admins[j]->name,
						   sizeof(n));
				}
				break;
			}
		...
		for (j = 0; j < MAX_ADMIN_LEVELS && g_admin_levels[j]; j++) {
			if (g_admin_levels[j]->level == l) {
				Q_strncpyz(lname, g_admin_levels[j]->name,
					   sizeof(lname));
				if (*lname) {
					...
					Com_sprintf(lname_fmt,
						    ...
						    (admin_level_maxname +
						     (int)(strlen(lname) -
							   strlen(lname2))));
					Com_sprintf(lname2, sizeof(lname2),
						    lname_fmt, lname);
				}
				break;
			}
		...
		if (G_admin_permission(ent, ADMF_SEESFULLLISTPLAYERS)) {
			ADMBP(va
			      ("%2i %s%s^7 %-3i %s^7 ^1%1s%1s%1s%1s%1s^7 %s^7 %s%s^7%s\n",
			       i, c, t, l, (*lname) ? lname2 : "", immune,
			       muted, dbuilder, denied, guidless,
			       p->pers.netname, (*n) ? "(a.k.a. " : "", n,
			       (*n) ? ")" : ""));
		} else {
			ADMBP(va
			      ("%2i %s%s^7 %-3i %s^7 ^1%1s%1s%1s%1s^7 %s^7 %s%s^7%s\n",
			       i, c, t, l, (*lname) ? lname2 : "", immune,
			       muted, dbuilder, denied, p->pers.netname,
			       (*n) ? "(a.k.a. " : "", n, (*n) ? ")" : ""));
		}
	}
	ADMBP_end();
...
qboolean G_admin_listmaps(gentity_t * ent, int skiparg)
{
	...
	filePtr = fileList;
	for (i = 0; i < numFiles && count < MAX_LISTMAPS_MAPS;
	     i++, filePtr += fileLen + 1) {
	...
	if (search[0])
		ADMBP(va
		      ("^3!listmaps: ^7found %d maps matching '%s^7'.\n", count,
		       search));
	else
...
qboolean G_admin_listrotation(gentity_t * ent, int skiparg)
{
	...
	for (i = 0; i < mapRotations.numRotations; i++) {
		if (i == g_currentMapRotation.integer) {
			...
			for (j = 0; j < mapRotations.rotations[i].numMaps; j++) {
				Q_strncpyz(mapnames,
					   mapRotations.rotations[i].maps[j].
					   name, sizeof(mapnames));
				...
				if (!Q_stricmp
					...
					for (k = 0;
						...
						if (mapRotations.rotations[i].
						    maps[j].conditions[k].lhs !=
						    MCV_VOTE)
							continue;

						thisMap =
						    mapRotations.rotations[i].
						    maps[j].conditions[k].dest;
	...
	if (g_nextMap.string[0]) {
		ADMP(va
		     ("^5 Next map overriden by g_nextMap to: %s\n",
		      g_nextMap.string));
	}
...
qboolean G_admin_showbans(gentity_t * ent, int skiparg)
{
	...
	if (G_SayArgc() >= 2 + skiparg) {
		...
		if (!numeric) {
			if (filter[0] != '-') {
				G_SanitiseString(filter, name_match,
						 sizeof(name_match));
				...
				if (!Q_strncmp(filter, "-sub", 4)) {
					...
					ADMP(va
					     ("^3!showbans: ^7invalid argument %s\n",
					      filter));
					return qfalse;
				}
			}
	...
	for (i = start; i < MAX_ADMIN_BANS && g_admin_bans[i]
		...
		if (!numeric) {
			if (!subnetfilter) {
				G_SanitiseString(g_admin_bans[i]->name, n1,
						 sizeof(n1));
				...
				int scanflen = 0;
				scanflen =
				    sscanf(g_admin_bans[i]->ip,
					   "%d.%d.%d.%d/%d", &dummy, &dummy,
					   &dummy, &dummy, &mask);
		...
		if ((match) || !ip_match
		    || Q_strncmp(ip_match, g_admin_bans[i]->ip,
				 ip_match_len) == 0) {
	...
	for (i = start; i < MAX_ADMIN_BANS && g_admin_bans[i]
		...
		{
			G_admin_duration(g_admin_bans[i]->length, length,
					 sizeof(length));
			if (secs > 1) {
				G_admin_duration(secs, duration,
						 sizeof(duration));
				Com_sprintf(duration, sizeof(duration),
					    " ^7(^3%s^7 remaining)", duration);
			}
		}

		if (g_admin_bans[i]->expires != 0 && (g_admin_bans[i]->expires - t) < 1)	// are they unbanned?
		{
			Com_sprintf(status, sizeof(status),
				    " ^7(^2expired or unbanned^7)");
		}

		if ((g_admin_bans[i]->suspend > t)
		    && (!g_admin_bans[i]->expires
			|| g_admin_bans[i]->expires - t > 0)) {
			G_admin_duration(g_admin_bans[i]->suspend - t,
					 sduration, sizeof(sduration));
			Com_sprintf(status, sizeof(status),
				    " ^7(^3suspended^7 for ^3%s^7 by %s^7)",
				    sduration, g_admin_bans[i]->suspendby);
		}
...
qboolean G_admin_help(gentity_t * ent, int skiparg)
{
	...
	if (G_SayArgc() < 2 + skiparg) {
		...
		ADMBP
		    ("run !help [^3command^7] for help with a specific command.\n");
		ADMBP
		    ("The following non-standard /commands may also be available to you: \n");
		count = 1;

		if (ent && g_AllStats.integer) {
			if (count > commandsPerLine
			    && (count % commandsPerLine) == 1)
				ADMBP("\n");
		...
		if (ent && g_allowVote.integer
		    && G_admin_permission(ent, ADMF_NO_VOTE)) {
			if (count > commandsPerLine
			    && (count % commandsPerLine) == 1)
				ADMBP("\n");
		...
		for (i = 0; i < adminNumCmds; i++) {
			if (!Q_stricmp(cmd, g_admin_cmds[i].keyword)) {
				if (!G_admin_permission
				    (ent, g_admin_cmds[i].flag)) {
					ADMBP(va
...
qboolean G_admin_pause(gentity_t * ent, int skiparg)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (vic->client->pers.paused) {
			...
			if (count == 1)
				AP(va
				   ("print \"^3!unpause: ^7%s^7 unpaused by %s\n\"",
				    vic->client->pers.netname,
				    G_admin_adminPrintName(ent)));
		} else {
			...
			if (count == 1)
				AP(va
				   ("print \"^3!pause: ^7%s^7 paused by %s\n\"",
				    vic->client->pers.netname,
				    G_admin_adminPrintName(ent)));
		}
...
qboolean G_admin_spec999(gentity_t * ent, int skiparg)
{
	...
	for (i = 0; i < level.maxclients; i++) {
		...
		if (vic->client->ps.ping == 999) {
			...
			AP(va
			   ("print \"^3!spec999: ^7%s^7 moved ^7%s^7 to spectators\n\"",
			    G_admin_adminPrintName(ent),
			    vic->client->pers.netname));
		}
	}
...
qboolean G_admin_register(gentity_t * ent, int skiparg)
{
	...
	if (!Q_stricmp
	    (ent->client->pers.guid, "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX")) {
	...
	if (!Q_stricmp(Q_CleanStr(name), "UnnamedPlayer")) {
		ADMP(va
		     ("^3!register:^7 You cannot register 'UnnamedPlayer'.\n"));
		return qfalse;
	}

	if (g_newbieNumbering.integer && g_newbieNamePrefix.string[0]
	    && !Q_stricmpn(ent->client->pers.netname, g_newbieNamePrefix.string,
	...
	trap_SendConsoleCommand(EXEC_APPEND,
				va("!setlevel %d %d;", (int)(ent - g_entities),
				   level));

	AP(va
	   ("print \"^3!register: ^7%s^7 is now a protected nickname.\n\"",
	    ent->client->pers.netname));
...
qboolean G_admin_rename(gentity_t * ent, int skiparg)
{
	...
	if (!admin_higher(ent, victim)) {
		ADMP("^3!rename: ^7sorry, but your intended victim has a higher admin" " level than you\n");
		return qfalse;
	}
...
qboolean G_admin_restart(gentity_t * ent, int skiparg)
{
	...
	if (G_SayArgc() > 1 + skiparg) {
		...
		if (Q_stricmp(layout, "keepteams")
			...
			if (!Q_stricmp(layout, "*BUILTIN*")
			    ||
			    trap_FS_FOpenFile(va
					      ("layouts/%s/%s.dat", map,
					       layout), NULL, FS_READ) > 0) {
				trap_Cvar_Set("g_layouts", layout);
			} else {
				ADMP(va
				     ("^3!restart: ^7layout '%s' does not exist\n",
				      layout));
				return qfalse;
			}
		} else {
	...
	if (!Q_stricmp(teampref, "keepteams")
	...
	} else if (!Q_stricmp(teampref, "switchteams")
		   || !Q_stricmp(teampref, "switchteamslock")) {
		for (i = 0; i < g_maxclients.integer; i++) {
			cl = level.clients + i;
			if (cl->pers.connected != CON_CONNECTED)
				continue;
...
qboolean G_admin_nobuild(gentity_t * ent, int skiparg)
{
	...
	if (!Q_stricmp(buffer, "log") || !Q_stricmp(buffer, "Log")) {
		...
		for (i = 0; i < MAX_GENTITIES; i++) {
			...
			ADMBP(va
			      ("^7#%i at origin (^5%f %f %f^7)^7\n", i + 1,
			       level.nbMarkers[i].Origin[0],
			       level.nbMarkers[i].Origin[1],
			       level.nbMarkers[i].Origin[2]));
			tmp++;
		}
	...
	if (!Q_stricmp(buffer, "save") || !Q_stricmp(buffer, "Save")) {
		...
		ADMP(va
		     ("^3!nobuild:^7 %i nobuild markers have been saved\n",
		      tmp));
		return qtrue;
	}
	return qfalse;
...
qboolean G_admin_nextmap(gentity_t * ent, int skiparg)
{
	...
	LogExit(va("nextmap was run by %s", G_admin_adminPrintName(ent)));
	G_admin_maplog_result("N");
	return qtrue;
}

static const char *displaySchachtmeisterJudgement(const
						  schachtmeisterJudgement_t * j,
						  g_admin_namelog_t * namelog)
{
	...
	if (G_admin_permission_guid(namelog->guid, ADMF_INCOGNITO)) {
		...
		Com_sprintf(buffer, sizeof(buffer), "^%i%+5i", color,
			    j->rating);
	}
...
qboolean G_admin_namelog(gentity_t * ent, int skiparg)
{
	...
	for (i = 0; i < MAX_ADMIN_NAMELOGS && g_admin_namelog[i]; i++) {
		...
		for (j = 0; j < 8; j++) {
			if (ent
			    && G_admin_permission_guid(g_admin_namelog[i]->guid,
						       ADMF_INCOGNITO)
			    && !admin_higher_guid(ent->client->pers.guid,
						  g_admin_namelog[i]->guid)
			    && !G_admin_permission(ent, ADMF_SEESINCOGNITO))
				guid_stub[j] = 'X';
			else
				guid_stub[j] = g_admin_namelog[i]->guid[j + 24];
		...
		ADMBP(va("%-2s (*%s) %15s %s^7",
			 ...
			 displaySchachtmeisterJudgement(&g_admin_namelog[i]->
							smj,
							g_admin_namelog[i])));
		for (j = 0;
...
qboolean G_admin_designate(gentity_t * ent, int skiparg)
{
	...
	if (!admin_higher(ent, &g_entities[pids[0]]) &&
		...
		ADMP("^3!designate: ^7sorry, but your intended victim has a higher admin" " level than you\n");
		return qfalse;
	}
	vic = &g_entities[pids[0]];
	if (vic->client->pers.designatedBuilder == qtrue) {
		...
		AP(va
		   ("print \"^3!designate: ^7%s^7 has been designated by ^7%s\n\"",
		    vic->client->pers.netname, G_admin_adminPrintName(ent)));
	}
	return qtrue;
}
...
{				//mostly copy and paste with the proper lines altered from !mute and !kick
	...
	if (!admin_higher(ent, &g_entities[pids[0]])) {
		ADMP("^3!warn: ^7sorry, but your intended victim has a higher admin" " level than you.\n");
		return qfalse;
	}
	...
	AP(va("print \"^3!warn: ^7%s^7 has been warned to cease and desist: %s by %s \n\"", vic->client->pers.netname, (*reason) ? reason : "his current activity", G_admin_adminPrintName(ent)));	//console announcement
	return qtrue;
}
...
qboolean G_admin_putmespec(gentity_t * ent, int skiparg)
{
	...
	if (g_preventRagequit.integer > 0
		...
		AP(va
		   ("print \"^3!specme: ^7%s^7 must gather their party before venturing forth from this place. "
	...
	if (ent->client->pers.teamSelection != PTE_NONE
		...
		|| BG_InventoryContainsWeapon(WP_HBUILD2,
					      ent->client->ps.stats))
	...
	AP(va
	   ("print \"^3!specme: ^7%s^7 decided to join the spectators\n\"",
	    ent->client->pers.netname));
	return qtrue;
}
...
qboolean G_admin_slap(gentity_t * ent, int skiparg)
{
	...
	if (!admin_higher(ent, vic)) {
		ADMP("^3!slap: ^7sorry, but your intended victim has a higher admin" " level than you\n");
		return qfalse;
	}
	if (vic->client->pers.teamSelection == PTE_NONE ||
	...
	if (g_slapDamage.integer > 0) {
		...
		if (G_SayArgc() > 2 + skiparg) {
			...
			if (g_slapDamage.integer > 100)
				g_slapDamage.integer = 100;
			damage =
			    BG_FindHealthForClass(vic->client->ps.
						  stats[STAT_PCLASS]) *
			    g_slapDamage.integer / 100;
			if (damage < 1)
				damage = 1;
...
qboolean G_admin_drop(gentity_t * ent, int skiparg)
{
	...
	if (!admin_higher(ent, &g_entities[pids[0]])) {
		ADMP("^3!drop: ^7sorry, but your intended victim has a higher admin" " level than you\n");
		return qfalse;
	}
	// victim's message
	if (G_SayArgc() > 2 + skiparg)
		trap_SendServerCommand(pids[0],
				       va
				       ("disconnect \"You have been dropped.\n%s^7\n\"",
					G_SayConcatArgs(2 + skiparg)));
	else
...
#define LOG_DISPLAY_LENGTH 10
	...
	int skip = 0, start = 0, lastID = -1, firstID = -1, i, len, matchlen =
	    0;
	...
	if (G_SayArgc() >= 2 + skiparg) {
		for (i = 1; i + skiparg < G_SayArgc(); i++) {
			...
			case '#':
				...
				break;
			case '-':
				if (G_ClientNumbersFromString(argbuf + 1, pids)
				    != 1) {
					G_MatchOnePlayer(pids, err,
							 sizeof(err));
	...
	*message = '\0';
	...
	for (; i + LOG_DISPLAY_LENGTH > len - skip && i > 0;
	     i--, ptr = ptr->next) {
		if (!ptr)
		...
		*markstring = '\0';	// reinit markstring
		...
		if (ptr->marked) {
			...
			for (j = BA_NUM_BUILDABLES; j >= 0; j--) {
				...
				// use an appropriate article
					...
					if (BG_FindUniqueTestForBuildable(j))
						article = "the";	// if only one
						 ...
						 ("aeiouAEIOU", *buildablename))
						article = "an";	// if first char is vowel
					else
						article = "a";
					Com_sprintf(bnames, 32, "%s %s",
						    article, buildablename);
				} else
				...
				Com_sprintf(markstring, sizeof(markstring),
					    "%s%s %s%s", bnames,
					    (*markstring) ? "," : "",
					    (and-- == 1) ? "and " : "",
					    markstring);
		...
		Com_sprintf(message, MAX_STRING_CHARS,
			    "%3d %s^7 %s^7 %s%s %s%s%s\n%s", ptr->ID, teamchar,
			    name, action,
	...
	if (matchlen)
		ADMP(va
		     ("%s^3!buildlog: showing log entries %d - %d of %d\n",
...
qboolean G_admin_revert(gentity_t * ent, int skiparg)
{
	...
	for (i = 0, ptr = prev = level.buildHistory; repeat > 0;
	     repeat--, j = 0) {
		if (!ptr)
		...
		case BF_BUILT:
			...
			for (j = MAX_CLIENTS, targ = g_entities + j;
				...
				if (targ->s.modelindex != ptr->buildable)
					continue;
				VectorSubtract(targ->s.pos.trBase, ptr->origin,
					       dist);
#define FIND_BUILDABLE_TOLERANCE 5
				if (VectorLength(dist) >
				    FIND_BUILDABLE_TOLERANCE)
			...
			if (targ->s.modelindex == BA_A_HOVEL && targ->active) {
				...
				VectorMA(newOrigin, 1.0f, targ->s.origin2,
					 newOrigin);
				...
				VectorCopy(newOrigin,
					   builder->client->ps.origin);
				G_SetClientViewAngle(builder, newAngles);

				//client leaves hovel
				builder->client->ps.stats[STAT_STATE] &=
				    ~SS_HOVELING;
			}
		...
		case BF_DESTROYED:
			...
			if (!force && !G_RevertCanFit(ptr)) {
				Com_sprintf(argbuf, sizeof argbuf,
					    "%s%s%s%s%s%s%s!",
					    (repeat > 1) ? "x" : "",
					    (repeat > 1) ? va("%d ",
							      repeat) : "",
		...
		if (j == level.num_entities) {
			ADMP(va
			     ("^3!revert: ^7could not find logged buildable #%d\n",
			      ptr->ID));
			prev = ptr;
			ptr = ptr->next;
			continue;
		...
		if (BG_FindUniqueTestForBuildable(ptr->buildable))
			...
			article = "a";
		AP(va
		   ("print \"^3!revert: ^7%s^7 reverted %s^7'%s %s of %s %s\n\"",
		    G_admin_adminPrintName(ent), name, strchr("Ss",
							      name[strlen(name)
								   -
								   1]) ? "" :
		    "s", action, article, bname));
		matchlen++;
	...
	if (!matchlen) {
		...
		ADMP(va
		     ("^3!revert: ^7reverted %d buildlog events\n", matchlen));
	}
...
#define MAX_INFO_PARSE_LOOPS 100
		for (i = 0; i < MAX_INFO_PARSE_LOOPS &&
		     G_StringReplaceCvars(message, message, sizeof(message));
		     i++) ;
		G_Unescape(message, message, sizeof(message));
		if (i == MAX_INFO_PARSE_LOOPS)
			G_Printf(S_COLOR_YELLOW
				 "WARNING: %s exceeds MAX_INFO_PARSE_LOOPS\n",
				 filename);
		ADMP(va("%s\n", message));
		return qtrue;
	}
...
{
	if (ent)
		trap_SendServerCommand(ent - level.gentities,
				       va("print \"%s\"", m));
	else {
...
qboolean G_admin_decon(gentity_t * ent, int skiparg)
{
	...
	char arg[64], err[MAX_STRING_CHARS], *name, *bname, *action, *article,
	    *reason;
	len = G_CountBuildLog();
	...
	if (builder->client->sess.invisible == qtrue) {
		ADMP(va
		     ("^3!decon: ^7no connected player by the name or slot #\n"));
		return qfalse;
	}

	if (!admin_higher(ent, builder)) {
		ADMP("^3!decon: ^7sorry, but your intended victim has a higher admin" "level than you\n");
		return qfalse;
	}
	...
	admin_create_ban(ent,
			 ...
			 builder->client->pers.ip,
			 G_admin_parse_time(g_deconBanTime.string),
...
qboolean G_admin_setdevmode(gentity_t * ent, int skiparg)
{
	...
	if (!Q_stricmp(str, "on")) {
		...
		AP(va
		   ("print \"^3!setdevmode: ^7%s ^7has switched developer mode on\n\"",
		    G_admin_adminPrintName(ent)));
		...
		trap_Cvar_Update(&g_cheats);
		AP(va
		   ("print \"^3!setdevmode: ^7%s ^7has switched developer mode off\n\"",
		    G_admin_adminPrintName(ent)));
	} else {
		ADMP("^3!setdevmode: ^7usage: !setdevmode [on|off]\n");
		return qfalse;
...
qboolean G_admin_hstage(gentity_t * ent, int skiparg)
{
	...
	AP(va("print \"^3!hstage: ^7%s^7 gave humans stage ^3%i^7\n\"",
	      G_admin_adminPrintName(ent), lvl));
...
qboolean G_admin_bubble(gentity_t * ent, int skiparg)
{
	...
	if (g_Bubbles.integer) {
		...
		if (vic->client->sess.invisible == qtrue) {
			ADMP(va
			     ("^3!bubble: ^7no connected player by that name or slot #\n"));
			return qfalse;
		}
		if (!admin_higher(ent, &g_entities[pids[0]])) {
			ADMP("^3!bubble: ^7sorry, but your intended victim has a higher admin" " level than you\n");
			return qfalse;
		}
...
qboolean G_admin_scrim(gentity_t * ent, int skiparg)
{
	...
	if (!Q_stricmp(state, "on")) {
		...
		AP(va
		   ("print \"^3!scrim: ^7%s ^7turned scrim mode ^2on^7\n\"",
		    G_admin_adminPrintName(ent)));
		...
		AP(va
		   ("print \"^3!scrim: ^7%s ^7turned scrim mode ^1off^7\n\"",
		    G_admin_adminPrintName(ent)));
...
too_big:
		...
		AP(va
		   ("print \"^3!give: ^7%s^7 was given %.3f evo%s by ^7%s^7\n\"",
	...
	case PTE_HUMANS:
		...
		AP(va
		   ("print \"^3!give: ^7%s^7 was given %d credit%s by ^7%s^7\n\"",
		    target->client->pers.netname, amount,
		    (abs(amount) != 1 ? "s" : ""),
		    G_admin_adminPrintName(ent)));
		break;
...
qboolean G_admin_setrotation(gentity_t * ent, int skiparg)
{
	...
	for (i = 0; i < mapRotations.numRotations; i++) {
		if (Q_stricmp(mapRotations.rotations[i].name, new_rotation) ==
		    0) {
			G_StartMapRotation(new_rotation, qfalse);
			trap_SendServerCommand(-1,
					       va
					       ("print \"^3!setrotation: ^7rotation ^3%s ^7was started by %s\n",
						new_rotation,
						G_admin_adminPrintName(ent)));
			return qtrue;
		}
...
rotationlist:
	{
		...
		ADMP(va
		     ("Number of available rotations: ^3%d\n",
...
static int calc_ff_pct(statsCounters_t * sc)
{
	if (sc->dmgdone + sc->structdmgdone <= 0) {
		...
		return round((float)sc->ffdmgdone /
			     (sc->ffdmgdone + sc->dmgdone +
			      sc->structdmgdone) * 100);
	}
}
...
qboolean G_admin_showff(gentity_t * ent, int skiparg)
{
	...
	if (G_SayArgc() == 1 + skiparg) {
		...
		ADMBP_begin();
		ADMBP
		    ("^3!showff:^7 friendly fire damage percentage for all connected players\n");
		...
		ADMBP
		    ("^7for detailed information, use ^3!showff player|slot^7\n");
		ADMBP_end();
		return qtrue;
	}
	...
	ADMP(va
	     ("^7damage to: Enemies: ^1%d^7, structures: ^1%d^7, friendlies: ^1%d\n",
...
void G_admin_tklog_log(gentity_t * attacker, gentity_t * victim,
		       int meansOfDeath)
{
	...
	Q_strncpyz(tklog->name, attacker->client->pers.netname,
		   sizeof(tklog->name));

	if (victim) {
		Q_strncpyz(tklog->victim, victim->client->pers.netname,
			   sizeof(tklog->victim));
		...
		tklog->damage =
		    attacker->client->pers.statscounters.spreebleeds;
...
qboolean G_admin_tklog(gentity_t * ent, int skiparg)
{
	...
	if (search_name) {
		...
		while (g_admin_tklog[index] &&
		       checked < MAX_ADMIN_TKLOGS && result_index < 10) {
			match = qfalse;

			G_SanitiseString(g_admin_tklog[index]->name, n1,
					 sizeof(n1));
			if (strstr(n1, search_name))
				match = qtrue;
	...
	for (i = 0; results[i] && i < 10; i++) {
		...
		Com_sprintf(fmt_name, sizeof(fmt_name), "%%%ds",
			    (name_length +
			     (int)(strlen(results[i]->name) - strlen(n1))));
		Com_sprintf(n1, sizeof(n1), fmt_name, results[i]->name);

		if (results[i]->team == PTE_HUMANS)
			weaponName =
			    BG_FindHumanNameForWeapon(results[i]->weapon);
		else
			weaponName =
			    BG_FindHumanNameForClassNum(results[i]->weapon);
		...
		ADMBP(va("^3%3d %3d:%02d ^7%s^7 tk'd %s^7\n"
			 ...
			 n1,
			 results[i]->victim,
			 weaponName, results[i]->damage, results[i]->value));
	}
	if (search_name) {
...
usage:
	...
	if (!Q_stricmp(feature, "ipa")) {
		...
		if (sscanf
		    (ipa, "%i.%i.%i.%i", &parts[0], &parts[1], &parts[2],
		     &parts[3]) != 4 || parts[0] < 0 || parts[0] > 255
		    || parts[1] < 0 || parts[1] > 255 || parts[2] < 0
		    || parts[2] > 255 || parts[3] < 0 || parts[3] > 255) {
			ADMP("^3!sm: ^7invalid IP address\n");
			return qfalse;
		}

		Com_sprintf(ipa, sizeof(ipa), "%i.%i.%i.%i", parts[0], parts[1],
			    parts[2], parts[3]);

		if (rand() % 2 /* FIXME cache hit */ ) {
			const char *answer = "interesting";
			ADMP(va
			     ("^3!sm: ^7IP address %s is: %s\n", ipa, answer));
			return qtrue;
		}
...
qboolean G_admin_switch(gentity_t * ent, int skiparg)
{
	...
	trap_SendServerCommand(vic - g_entities,
			       va("print \"^7%s^7 switched with you\n\"",
				  ent->client->pers.netname));
	trap_SendServerCommand(ent - g_entities,
			       va("print \"^7you switched with ^7%s^7\n\"",
				  vic->client->pers.netname));
...
qboolean G_admin_range(gentity_t * ent, int skiparg)
{
	...
	AP(va
	   ("print \"^3!range: ^7range for %s ^7was multipled by ^2%s ^7by %s^7\n\"",
	    vic->client->pers.netname, rangeInt, G_admin_adminPrintName(ent)));
...
qboolean G_admin_practise(gentity_t * ent, int skiparg)
{
	...
	AP(va("print \"^3!practise: ^7practise mode was %s by %s^7\n\"",
	      arg, G_admin_adminPrintName(ent)));
...
qboolean G_admin_spawn(gentity_t * ent, int skiparg)
{
	...
	ADMP(va
	     ("^3!spawn: ^7%s at ^3%.0f %.0f %.0f\n", arg, entityOrigin[0],
	      entityOrigin[1], entityOrigin[2]));
	AP(va
	   ("print \"^3!spawn: ^7buildable ^3%s^7 spawned by %s^7 at location ^3%.0f %.0f %.0f^7\n",
	    BG_FindHumanNameForBuildable(buildable),
	    G_admin_adminPrintName(ent), entityOrigin[0], entityOrigin[1],
	    entityOrigin[2]));
