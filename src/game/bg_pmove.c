...
static void PM_StartTorsoAnim(int anim)
{
	...
	pm->ps->torsoAnim =
	    ((pm->ps->torsoAnim & ANIM_TOGGLEBIT) ^ ANIM_TOGGLEBIT)
	    | anim;
}
...
static void PM_StartLegsAnim(int anim)
{
	...
	pm->ps->legsAnim =
	    ((pm->ps->legsAnim & ANIM_TOGGLEBIT) ^ ANIM_TOGGLEBIT)
	    | anim;
}
...
static void PM_Friction(void)
{
	...
	if (pm->waterlevel <= 1) {
		if ((pml.walking || pml.ladder)
			...
			if (!(pm->ps->pm_flags & PMF_TIME_KNOCKBACK)) {
				float stopSpeed =
				    BG_FindStopSpeedForClass(pm->ps->
							     stats
							     [STAT_PCLASS]);
	...
	if (pm->waterlevel)
		drop +=
		    speed * pm_waterfriction * pm->waterlevel * pml.frametime;
...
static float PM_CmdScale(usercmd_t * cmd)
{
	...
	if (pm->ps->weapon == WP_ALEVEL4 && pm->ps->pm_flags & PMF_CHARGE)
		modifier *=
		    (1.0f +
		     (pm->ps->stats[STAT_MISC] / (float)LEVEL4_CHARGE_TIME) *
	...
	if (pm->ps->pm_type != PM_SPECTATOR && pm->ps->pm_type != PM_NOCLIP) {
		if (BG_FindJumpMagnitudeForClass(pm->ps->stats[STAT_PCLASS]) ==
		    0.0f)
			cmd->upmove = 0;

		//prevent speed distortions for non ducking classes
		if (!(pm->ps->pm_flags & PMF_DUCKED)
		    && pm->ps->pm_type != PM_JETPACK && cmd->upmove < 0)
			cmd->upmove = 0;
	}
...
static qboolean PM_CheckPounce(void)
{
	...
	VectorMA(pm->ps->velocity, pm->ps->stats[STAT_MISC], pml.forward,
		 pm->ps->velocity);
...
static qboolean PM_CheckWallJump(void)
{
	...
	if (pm->ps->grapplePoint[2] > 0
	    && pm->ps->grapplePoint[2] < LEVEL2_WALLJUMP_MAXNRM) {
	...
	VectorMA(pm->ps->velocity,
		 BG_FindJumpMagnitudeForClass(pm->ps->stats[STAT_PCLASS]), dir,
		 dircp);
	storedVert = dircp[2];
	dircp[2] = 0;
	...
	if (speedModulus > LEVEL2_WALLJUMP_MAXSPEED
	    && DotProduct(pm->ps->velocity, dircp) > 0) {
		float speedFac =
		    0.001f + pow(LEVEL2_WALLJUMP_OVERCAPEXP,
				 (LEVEL2_WALLJUMP_MAXSPEED -
				  speedModulus) / LEVEL2_WALLJUMP_OVERCAPSCL);
...
static qboolean PM_CheckJump(void)
{
	...
	if ((pm->ps->weapon == WP_ALEVEL3 ||
	     pm->ps->weapon == WP_ALEVEL3_UPG) && pm->ps->stats[STAT_MISC] > 0)
		return qfalse;
	...
	if (pm->ps->stats[STAT_STATE] & SS_WALLCLIMBING) {
		...
		VectorMA(pm->ps->velocity,
			 BG_FindJumpMagnitudeForClass(pm->ps->
						      stats[STAT_PCLASS]),
...
static void PM_WaterMove(void)
{
	...
	if (!scale) {
		...
		for (i = 0; i < 3; i++)
			wishvel[i] =
			    scale * pml.forward[i] * pm->cmd.forwardmove +
...
static void PM_AirMove(void)
{
	...
	PM_Accelerate(wishdir, wishspeed,
		      BG_FindAirAccelerationForClass(pm->ps->
						     stats[STAT_PCLASS]));
...
//  wishvel[2] = 0;
	...
	if ((pml.groundTrace.surfaceFlags & SURF_SLICK)
		...
		accelerate =
		    BG_FindAccelerationForClass(pm->ps->stats[STAT_PCLASS]);
...
static void PM_LadderMove(void)
{
	...
	//slanty ladders
	if (pml.groundPlane
	    && DotProduct(pm->ps->velocity,
			  pml.groundTrace.plane.normal) < 0.0f) {
		vel = VectorLength(pm->ps->velocity);
...
static void PM_NoclipMove(void)
{
	...
	// move
	VectorMA(pm->ps->origin, pml.frametime, pm->ps->velocity,
		 pm->ps->origin);
}
...
static void PM_GroundTraceMissed(void)
{
	...
	if (pm->ps->groundEntityNum != ENTITYNUM_NONE) {
		...
		pm->trace(&trace, pm->ps->origin, NULL, NULL, point,
			  pm->ps->clientNum, pm->tracemask);
		if (trace.fraction == 1.0f) {
			if (pm->cmd.forwardmove >= 0) {
				if (!
				    (pm->ps->
				     persistant[PERS_STATE] & PS_NONSEGMODEL))
					PM_ForceLegsAnim(LEGS_JUMP);
				else
					PM_ForceLegsAnim(NSPA_JUMP);
...
static void PM_GroundClimbTrace(void)
{
	...
	for (i = 0; i <= 4; i++) {
		...
		case 2:
			if (pml.groundPlane != qfalse && PM_PredictStepMove()) {
				//step down
				VectorMA(pm->ps->origin, -STEPSIZE, surfNormal,
					 point);
				pm->trace(&trace, pm->ps->origin, pm->mins,
					  pm->maxs, point, pm->ps->clientNum,
					  pm->tracemask);
			} else
				continue;
			break;
		...
		case 3:
			...
			if (pml.groundPlane != qfalse) {
				VectorMA(pm->ps->origin, -16.0f, surfNormal,
					 point);
		...
		if (trace.fraction < 1.0f
			...
			VectorSubtract(trace.plane.normal, ceilingNormal,
				       normalDelta);
			...
			if (i == 2 || i == 3) {
				if (i == 2)
					PM_StepEvent(pm->ps->origin,
						     trace.endpos, surfNormal);

				VectorCopy(trace.endpos, pm->ps->origin);
			}
			//calculate a bunch of stuff...
			CrossProduct(trace.plane.normal, surfNormal,
				     traceCROSSsurf);
			VectorNormalize(traceCROSSsurf);

			CrossProduct(trace.plane.normal, refNormal,
				     traceCROSSref);
			...
			traceDOTsurf =
			    DotProduct(trace.plane.normal, surfNormal);
			...
			if (!VectorCompare(trace.plane.normal, surfNormal)) {
				...
				if (!VectorCompare
				    (trace.plane.normal, refNormal)
				    && !VectorCompare(surfNormal, refNormal)
				    && !VectorCompare(trace.plane.normal,
						      ceilingNormal)
				    && !VectorCompare(surfNormal,
						      ceilingNormal)) {
					...
					RotatePointAroundVector(refTOtrace,
								traceCROSSref,
								horizontal,
								-traceANGref);

					//calculate reference rotated through to surf plane then to trace plane
					RotatePointAroundVector(tempVec,
								surfCROSSref,
								horizontal,
								-surfANGref);
					RotatePointAroundVector
					    (refTOsurfTOtrace, traceCROSSsurf,
					     tempVec, -traceANGsurf);

					//calculate angle between refTOtrace and refTOsurfTOtrace
					rTtDOTrTsTt =
					    DotProduct(refTOtrace,
						       refTOsurfTOtrace);
					rTtANGrTsTt =
					    ANGLE2SHORT(RAD2DEG
							(acos(rTtDOTrTsTt)));
					...
					CrossProduct(refTOtrace,
						     refTOsurfTOtrace, tempVec);
				...
				if ((abc[0] * point[0] + abc[1] * point[1] +
				     abc[2] * point[2] - d) > 0)
				...
				if ((ldDOTtCs =
					...
					ldDOTtCs =
					    DotProduct(lookdir, traceCROSSsurf);
				}
				...
				if (VectorCompare(surfNormal, ceilingNormal)) {
					vectoangles(trace.plane.normal,
						    toAngles);
					vectoangles(pm->ps->grapplePoint,
						    surfAngles);

					pm->ps->delta_angles[1] -=
					    ANGLE2SHORT(((surfAngles[1] -
							  toAngles[1]) * 2) -
							180.0f);
				}
			}
	...
	if (trace.fraction >= 1.0f) {
		...
		if (pm->ps->stats[STAT_STATE] & SS_WALLCLIMBINGCEILING) {
			...
			RotatePointAroundVector(rotated, pm->ps->grapplePoint,
						forward, 180.0f);
			vectoangles(rotated, angles);

			pm->ps->delta_angles[YAW] -=
			    ANGLE2SHORT(angles[YAW] - pm->ps->viewangles[YAW]);
		}
...
static void PM_GroundTrace(void)
{
	...
	// if the trace didn't hit anything, we are in free fall
	if (trace.fraction == 1.0f
	    || trace.plane.normal[2] < LEVEL2_WALLJUMP_MAXNRM) {
		...
		if (pml.groundPlane != qfalse && PM_PredictStepMove()) {
			...
			//if we hit something
			if (trace.fraction < 1.0f) {
				PM_StepEvent(pm->ps->origin, trace.endpos,
					     refNormal);
				VectorCopy(trace.endpos, pm->ps->origin);
				steppedDown = qtrue;
			}
		...
		if (!steppedDown) {
			...
			if (BG_ClassHasAbility
			    (pm->ps->stats[STAT_PCLASS], SCA_WALLJUMPER)) {
				ProjectPointOnPlane(movedir, pml.forward,
						    refNormal);
				VectorNormalize(movedir);
				...
				if (trace.fraction < 1.0f
				    && !(trace.
					 surfaceFlags & (SURF_SKY | SURF_SLICK))
	...
	if (pm->ps->groundEntityNum == ENTITYNUM_NONE) {
		...
		if (BG_ClassHasAbility
		    (pm->ps->stats[STAT_PCLASS], SCA_TAKESFALLDAMAGE))
			PM_CrashLand();
...
static void PM_CheckDuck(void)
{
	...
	BG_FindBBoxForClass(pm->ps->stats[STAT_PCLASS], PCmins, PCmaxs, PCcmaxs,
			    NULL, NULL);
...
static void PM_Weapon(void)
{
	...
	if (pm->ps->weaponTime <= 0 || pm->ps->weaponstate != WEAPON_FIRING) {
		...
		if (pm->ps->pm_flags & PMF_WEAPON_SWITCH) {
			...
			PM_BeginWeaponChange(pm->ps->
					     persistant[PERS_NEWWEAPON]);
		}
	}
	...
	if (!BG_FindInfinteAmmoForWeapon(pm->ps->weapon)) {
		...
		if (pm->ps->weapon == WP_LUCIFER_CANNON && attack1 && !attack2) {
			ammo -=
			    (int)(ceil
				  (((float)pm->ps->stats[STAT_MISC] /
				    (float)LCANNON_TOTAL_CHARGE) * 10.0f));
	...
	if (pm->ps->weapon == WP_CHAINGUN) {
		if (pm->ps->pm_flags & PMF_DUCKED ||
		    BG_InventoryContainsUpgrade(UP_BATTLESUIT, pm->ps->stats)) {
			pm->ps->delta_angles[PITCH] -=
			    ANGLE2SHORT(((random() * 0.5) -
					 0.125) * (30 / (float)addTime));
			pm->ps->delta_angles[YAW] -=
			    ANGLE2SHORT(((random() * 0.5) -
					 0.25) * (30.0 / (float)addTime));
		} else {
			pm->ps->delta_angles[PITCH] -=
			    ANGLE2SHORT(((random() * 8) -
					 2) * (30.0 / (float)addTime));
			pm->ps->delta_angles[YAW] -=
			    ANGLE2SHORT(((random() * 8) -
					 4) * (30.0 / (float)addTime));
		}
	}
...
void PM_UpdateViewAngles(playerState_t * ps, const usercmd_t * cmd)
{
	...
	if (ps->pm_type == PM_GRABBED
		...
		for (i = 0; i < 3; i++) {
			float diff =
			    AngleSubtract(ps->viewangles[i], angles[i]);
...
void PmoveSingle(pmove_t * pmove)
{
	...
	if (pm->ps->pm_type == PM_JETPACK)
		...
		if (BG_ClassHasAbility
		    (pm->ps->stats[STAT_PCLASS], SCA_WALLCLIMBER)
		    && (pm->ps->stats[STAT_STATE] & SS_WALLCLIMBING))
...
void Pmove(pmove_t * pmove)
{
	...
	pmove->ps->pmove_framecount =
	    (pmove->ps->pmove_framecount + 1) & ((1 << PS_PMOVEFRAMECOUNTBITS) -
						 1);
