...
void G_OverflowCredits(gclient_t * doner, int credits)
{
	...
	if (g_creditOverflow.integer == 1) {
		...
		while (credits > 0 && i < level.maxclients) {
			...
			if (vic->client->ps.stats[STAT_PTEAM] !=
			    doner->ps.stats[STAT_PTEAM]
			...
			if (vic->client->ps.persistant[PERS_CREDIT] + credits >
			    maxCredits) {
				int amount;
				...
				vic->client->pers.statscounters.received +=
				    amount;
			} else {
				vic->client->ps.persistant[PERS_CREDIT] +=
				    credits;
		...
		for (i = 0; i < level.numPlayingClients && credits > 0; i++) {
			...
			if (cl->ps.stats[STAT_PTEAM] !=
			    doner->ps.stats[STAT_PTEAM]
			    || cl->ps.persistant[PERS_CREDIT] >= maxCredits)
				continue;

			if (cl->ps.persistant[PERS_CREDIT] + credits >
			    maxCredits) {
				int amount;

				amount =
				    maxCredits - cl->ps.persistant[PERS_CREDIT];
				credits -= amount;
...
============
*/
gentity_t *G_SelectRandomFurthestSpawnPoint(vec3_t avoidPoint, vec3_t origin,
					    vec3_t angles)
{
	gentity_t *spot;
	vec3_t delta;
...
============
*/
gentity_t *G_SelectTremulousSpawnPoint(pTeam_t team, vec3_t preference,
				       vec3_t origin, vec3_t angles)
{
	...
	if (team == PTE_ALIENS)
		G_CheckSpawnPoint(spot->s.number, spot->s.origin,
				  spot->s.origin2, BA_A_SPAWN, origin);
	else if (team == PTE_HUMANS)
		G_CheckSpawnPoint(spot->s.number, spot->s.origin,
				  spot->s.origin2, BA_H_SPAWN, origin);
...
void SpawnCorpse(gentity_t * ent)
{
	...
	BG_FindBBoxForClass(ent->client->ps.stats[STAT_PCLASS], NULL, NULL,
			    NULL, body->r.mins, body->r.maxs);
	...
	trap_Trace(&tr, origin, body->r.mins, body->r.maxs, dest,
		   body->s.number, body->clipmask);
...
char *G_NextNewbieName(gentity_t * ent)
{
	...
	if (g_newbieNamePrefix.string[0])
		Q_strncpyz(namePrefix, g_newbieNamePrefix.string,
			   sizeof(namePrefix));
	else
...
void ClientUserinfoChanged(int clientNum, qboolean forceName)
{
	...
	if (strcmp(oldname, newname)) {
		...
		if (!G_admin_permission(ent, ADMF_SPECIALNAME))
			ClientCleanName(va("%s", client->pers.netname), oldname,
					sizeof(oldname), qfalse);
		else
			ClientCleanName(va("%s", client->pers.netname), oldname,
					sizeof(oldname), qtrue);

		if (g_newbieNumbering.integer) {
			if (!strcmp(newname, "UnnamedPlayer"))
				Q_strncpyz(newname, G_NextNewbieName(ent),
					   sizeof(newname));
			if (!strcmp(oldname, "UnnamedPlayer"))
				Q_strncpyz(oldname, G_NextNewbieName(ent),
					   sizeof(oldname));
		}
	...
	// set model
	if (client->ps.stats[STAT_PCLASS] == PCL_HUMAN
	    && BG_InventoryContainsUpgrade(UP_BATTLESUIT, client->ps.stats)) {
		Com_sprintf(buffer, MAX_QPATH, "%s/%s",
			    BG_FindModelNameForClass(PCL_HUMAN_BSUIT),
		...
		Com_sprintf(buffer, MAX_QPATH, "%s/%s",
			    BG_FindModelNameForClass(client->pers.
						     classSelection),
			    BG_FindSkinNameForClass(client->pers.
						    classSelection));
	}
		   ...
		   qboolean onConnect)
{
	...
	if (g_adminAutobahnNotify.integer)
		G_AdminsPrintf
		    ("Autobahn %s '%s^7' with rating %+i, connecting from %s.\n",
		     verb, name, rating, ip);
}
...
const char *ClientConnect(int clientNum, qboolean firstTime)
{
	...
	if (strlen(ip) < 7
	    && strcmp(Info_ValueForKey(userinfo, "ip"), "localhost")) {
		G_AdminsPrintf
	...
	if (g_maxGhosts.integer > 1) {
		...
		for (i = 0; i < level.maxclients; i++) {
			...
			if (other &&
			    (other->pers.connected == CON_CONNECTED
			     || other->pers.connected == CON_CONNECTING)
		...
		if (count + 1 > g_maxGhosts.integer) {
			G_AdminsPrintf
			    ("Connect from client exceeds %d maximum connections per IP: '%s' NAME: '%s^7'\n",
			     g_maxGhosts.integer, ip, Info_ValueForKey(userinfo,
								       "name"));
	...
	if (g_decolourLogfiles.integer) {
		char decoloured[MAX_STRING_CHARS] = "";
		if (g_decolourLogfiles.integer == 1) {
			Com_sprintf(decoloured, sizeof(decoloured),
				    " (\"%s^7\")", client->pers.netname);
			G_DecolorString(decoloured, decoloured);
			G_LogPrintfColoured
			...
			G_LogPrintf("ClientConnect: %i [%s] (%s) \"%s^7\"%s\n",
				    clientNum, client->pers.ip,
				    client->pers.guid, client->pers.netname,
				    decoloured);
		}
	} else {
	...
	if (client->pers.adminLevel) {
		G_LogPrintf
		    ("ClientAuth: %i [%s] \"%s^7\" authenticated to admin level %i using GUID %s (%s^7)\n",
		     clientNum, client->pers.ip, client->pers.netname,
		     client->pers.adminLevel, client->pers.guid,
...
============
*/
void ClientSpawn(gentity_t * ent, gentity_t * spawn, vec3_t origin,
		 vec3_t angles)
{
	int index;
	...
	if (client->sess.sessionTeam == TEAM_SPECTATOR) {
		if (teamLocal == PTE_NONE)
			spawnPoint =
			    G_SelectSpectatorSpawnPoint(spawn_origin,
							spawn_angles);
		else if (teamLocal == PTE_ALIENS)
			spawnPoint =
			    G_SelectAlienLockSpawnPoint(spawn_origin,
							spawn_angles);
	...
	BG_FindBBoxForClass(ent->client->pers.classSelection, ent->r.mins,
			    ent->r.maxs, NULL, NULL, NULL);
	...
	if (ent->client->pers.classSelection == PCL_HUMAN) {
		...
		weapon =
		    BG_FindStartWeaponForClass(ent->client->pers.
					       classSelection);
	else
		weapon = WP_NONE;
...
void ClientDisconnect(int clientNum)
{
	...
	for (ptr = level.buildHistory; ptr; ptr = ptr->next) {
		if (ptr->ent == ent) {
			ptr->ent = NULL;
			Q_strncpyz(ptr->name, ent->client->pers.netname,
				   MAX_NETNAME);
		}
	}
	...
	for (i = 0; i < level.maxclients; i++) {
		...
		BG_ClientListRemove(&level.clients[i].sess.ignoreList,
				    clientNum);
	}
	...
	if (ent->client->pers.connected == CON_CONNECTED &&
		...
		tent =
		    G_TempEntity(ent->client->ps.origin,
	...
	G_LogPrintf("ClientDisconnect: %i [%s] (%s) \"%s^7\"\n", clientNum,
		    ent->client->pers.ip, ent->client->pers.guid,
