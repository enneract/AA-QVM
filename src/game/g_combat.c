...
void LookAtKiller(gentity_t * self, gentity_t * inflictor, gentity_t * attacker)
{
	...
	if (attacker && attacker != self)
		...
		VectorSubtract(inflictor->s.pos.trBase, self->s.pos.trBase,
			       dir);
	else {
		...
		int damage, int meansOfDeath)
{
	...
	if (attacker) {
		...
		if (attacker->client) {
			...
			if (attacker != self
			    && attacker->client->ps.stats[STAT_PTEAM] ==
			    self->client->ps.stats[STAT_PTEAM]) {
				attacker->client->pers.statscounters.
				    teamkills++;
				if (attacker->client->pers.teamSelection ==
				    PTE_ALIENS) {
					level.alienStatsCounters.teamkills++;
				} else if (attacker->client->pers.
	...
	if (meansOfDeath == MOD_SLAP) {
		trap_SendServerCommand(-1,
				       va
				       ("print \"%s^7 felt %s^7's authority\n\"",
	...
	if (attacker && attacker->client) {
		...
		if (g_killerHP.integer ||
			...
			trap_SendServerCommand(self - g_entities,
					       va
					       ("print \"Your killer, %s^7, had %3i HP.\n\"",
		...
		if (attacker == self || OnSameTeam(self, attacker)) {
			...
			if (!g_retribution.integer) {
				if (attacker->client->ps.stats[STAT_PTEAM] ==
				    PTE_ALIENS) {
					G_AddCreditToClient(attacker->client,
							    -FREEKILL_ALIEN,
							    qtrue);
					attacker->client->pers.statscounters.
					    earned -= FREEKILL_ALIEN;
				} else if (attacker->client->ps.
					   stats[STAT_PTEAM] == PTE_HUMANS) {
					G_AddCreditToClient(attacker->client,
							    -FREEKILL_HUMAN,
							    qtrue);
					attacker->client->pers.statscounters.
					    earned -= FREEKILL_HUMAN;
				}
			}
		} else {
			AddScore(attacker, 1);
	...
	if (g_retribution.integer && totalTK) {
		...
		if (self->client->ps.persistant[PERS_CREDIT] + totalPrice > max)
			totalPrice =
			    max - self->client->ps.persistant[PERS_CREDIT];
		...
		if (totalPrice > 0) {
			...
			if (self->client->ps.stats[STAT_PTEAM] == PTE_HUMANS) {
				for (i = 0; i < MAX_CLIENTS; i++) {
					...
					if (g_entities[i].client->ps.
					    persistant[PERS_CREDIT] < price)
						price =
						    g_entities[i].client->ps.
						    persistant[PERS_CREDIT];
					if (price) {
						G_AddCreditToClient(self->
								    client,
								    price,
								    qtrue);
						G_AddCreditToClient(g_entities
								    [i].client,
								    -price,
								    qtrue);

						self->client->pers.
						    statscounters.received +=
						    price;
						g_entities[i].client->pers.
						    statscounters.shared +=
						    price;

						trap_SendServerCommand(self->
								       client->
								       ps.
								       clientNum,
								       va
								       ("print \"Received ^3%d credits ^7from %s ^7in retribution.\n\"",
									price,
									g_entities
									[i].
									client->
						...
						trap_SendServerCommand
						    (g_entities[i].client->ps.
						     clientNum,
						     va
						     ("print \"Transfered ^3%d credits ^7to %s ^7in retribution.\n\"",
						      price,
						      self->client->pers.
						      netname));
					}
				...
				for (i = 0; i < MAX_CLIENTS; i++) {
					...
					G_AddCreditToClient(self->client,
							    toPay[i], qtrue);
					G_AddCreditToClient(g_entities[i].
							    client, -toPay[i],
							    qtrue);

					self->client->pers.statscounters.
					    received += toPay[i];
					g_entities[i].client->pers.
					    statscounters.shared += toPay[i];

					trap_SendServerCommand(self->client->ps.
							       clientNum,
							       va
							       ("print \"Received ^3%.3f ^7evos from %s ^7in retribution.\n\"",
								toPay[i] /
								EVO_TO_CREDS_RATE,
								g_entities[i].
								client->pers.
								netname));
					trap_SendServerCommand(g_entities[i].
							       client->ps.
							       clientNum,
							       va
							       ("print \"Transfered ^3%.3f ^7evos to %s ^7in retribution.\n\"",
								toPay[i] /
								EVO_TO_CREDS_RATE,
								self->client->
								pers.netname));
	...
	if (!OnSameTeam(self, attacker)
		...
		if (self->client->ps.stats[STAT_PTEAM] == PTE_HUMANS) {
			trap_Cvar_Set("g_alienKills",
				      va("%d", g_alienKills.integer + 1));
			...
			trap_Cvar_Set("g_humanKills",
				      va("%d", g_humanKills.integer + 1));
	...
	if (totalDamage > 0.0f) {
		if (self->client->ps.stats[STAT_PTEAM] == PTE_ALIENS) {
			...
			float classValue =
			    BG_FindValueOfClass(self->client->ps.
						stats[STAT_PCLASS]);
			float factor = G_RewardFactor(self, attacker);

			if (g_debugRewards.integer)
				Com_Printf("DebugRewards: %d x %.2f = ",
					   (int)classValue, factor);
			classValue *= factor;
			if (g_debugRewards.integer)
			...
			for (i = 0; i < MAX_CLIENTS; i++) {
				...
				percentDamage =
				    (float)self->credits[i] / totalDamage;
				if (percentDamage > 0 && percentDamage < 1) {
					player->client->pers.statscounters.
					    assists++;
					level.humanStatsCounters.assists++;
				}
				//add credit
				amount = classValue * percentDamage;
				G_AddCreditToClient(player->client, amount,
						    qtrue);
				player->client->pers.statscounters.earned +=
				    amount;
				self->client->pers.statscounters.fed += amount;

				if (g_debugRewards.integer)
					Com_Printf
					    (", %d credit(s) to #%d for %.1f%% of damage",
					     amount, (int)(player - g_entities),
					     100 * percentDamage);
			}
			...
			float humanValue =
			    BG_GetValueOfHuman(&self->client->ps), factor;
			int frags;
			int unclaimedFrags;
			...
			for (i = 0; i < MAX_CLIENTS; i++) {
				...
				if (frags > 0) {
					...
					player->client->pers.statscounters.
					    earned += frags;
					self->client->pers.statscounters.fed +=
					    frags;

					if (g_debugRewards.integer)
						Com_Printf
						    (", %d/%d evo(s) to #%d for %.1f%% of damage",
						     frags,
						     (int)EVO_TO_CREDS_RATE,
			...
			if (unclaimedFrags) {
				...
				for (i = 0; i < unclaimedFrags; i++) {
					...
					if (maximum > 0) {
						...
						//add kills
						G_AddCreditToClient(player->
								    client, 1,
								    qtrue);
						player->client->pers.
						    statscounters.earned += 1;
						self->client->pers.
						    statscounters.fed += 1;
...
finish_dying:			// from MOD_SLAP
...

	{
		...
		self->client->ps.legsAnim =
		    ((self->client->ps.
		      legsAnim & ANIM_TOGGLEBIT) ^ ANIM_TOGGLEBIT) | anim;
...
void G_ParseArmourScript(char *buf, int upgrade)
{
	...
	while (1) {
		...
		if (count == MAX_ARMOUR_REGIONS) {
			G_Printf
			    ("Max armour regions exceeded in locdamage file\n");
			break;
		}
		//default
		...
		while (1) {
			...
			if (!Q_stricmp(token, "}")) {
				...
				g_armourRegions[upgrade][count].minHeight =
				    atof(token);
				...
				g_armourRegions[upgrade][count].maxHeight =
				    atof(token);
				...
				g_armourRegions[upgrade][count].minAngle =
				    atoi(token);
				...
				g_armourRegions[upgrade][count].maxAngle =
				    atoi(token);
				...
				g_armourRegions[upgrade][count].modifier =
				    atof(token);
...
void G_ParseDmgScript(char *buf, int class)
{
	...
	while (1) {
		...
		if (count == MAX_LOCDAMAGE_REGIONS) {
			G_Printf
			    ("Max damage regions exceeded in locdamage file\n");
			break;
		}
		//default
		...
		while (1) {
			...
			if (!Q_stricmp(token, "}")) {
				...
				g_damageRegions[class][count].minHeight =
				    atof(token);
			} else if (!strcmp(token, "maxHeight")) {
				token = COM_ParseExt(&buf, qfalse);
				...
				g_damageRegions[class][count].maxHeight =
				    atof(token);
			} else if (!strcmp(token, "minAngle")) {
				token = COM_ParseExt(&buf, qfalse);
				...
				g_damageRegions[class][count].minAngle =
				    atoi(token);
			} else if (!strcmp(token, "maxAngle")) {
				token = COM_ParseExt(&buf, qfalse);
				...
				g_damageRegions[class][count].maxAngle =
				    atoi(token);
			} else if (!strcmp(token, "modifier")) {
				token = COM_ParseExt(&buf, qfalse);
				...
				g_damageRegions[class][count].modifier =
				    atof(token);
			} else if (!strcmp(token, "crouch")) {
...
============
*/
static float G_CalcDamageModifier(vec3_t point, gentity_t * targ,
				  gentity_t * attacker, int class, int dflags)
{
	vec3_t targOrigin;
	vec3_t bulletPath;
	vec3_t bulletAngle;
	...
	if (g_unlagged.integer && targ->client
	    && targ->client->unlaggedCalc.used)
	...
	if (dflags & DAMAGE_NO_LOCDAMAGE) {
		for (i = UP_NONE + 1; i < UP_NUM_UPGRADES; i++) {
			...
			if (BG_InventoryContainsUpgrade
			    (i, targ->client->ps.stats)) {
				for (j = 0; j < g_numArmourRegions[i]; j++)
					totalModifier +=
					    g_armourRegions[i][j].modifier;
		...
		for (i = 0; i < g_numDamageRegions[class]; i++) {
			...
			if (g_damageRegions[class][i].minAngle >
				...
				rotationBound =
				    (hitRotation >=
				     g_damageRegions[class][i].minAngle
		...
		for (i = UP_NONE + 1; i < UP_NUM_UPGRADES; i++) {
			if (BG_InventoryContainsUpgrade
				...
				for (j = 0; j < g_numArmourRegions[i]; j++) {
					...
					if (rotationBound &&
					    hitRatio >=
					    g_armourRegions[i][j].minHeight
					    && hitRatio <=
					    g_armourRegions[i][j].maxHeight
...
void G_InitDamageLocations(void)
{
		...
		if (len >= MAX_LOCDAMAGE_TEXT) {
			G_Printf(va
				 (S_COLOR_RED
				  ...
				  filename, len, MAX_LOCDAMAGE_TEXT));
	...
	for (i = UP_NONE + 1; i < UP_NUM_UPGRADES; i++) {
		...
		Com_sprintf(filename, sizeof(filename), "armour/%s.armour",
			    modelName);
...
void G_SelectiveDamage(gentity_t * targ, gentity_t * inflictor,
		       ...
		       int damage, int dflags, int mod, int team)
{
	if (targ->client && (team != targ->client->ps.stats[STAT_PTEAM]))
		G_Damage(targ, inflictor, attacker, dir, point, damage, dflags,
			 mod);
}
	      ...
	      vec3_t dir, vec3_t point, int damage, int dflags, int mod)
{
	...
	if (dflags & DAMAGE_KNOCKBACK) {
		...
		if (targ->client) {
			knockback = (int)((float)knockback *
					  BG_FindKnockbackScaleForClass(targ->
									client->
									ps.
									stats
									[STAT_PCLASS]));
	...
	if (knockback && targ->client) {
		...
		VectorScale(dir, g_knockback.value * (float)knockback / mass,
			    kvel);
		VectorAdd(targ->client->ps.velocity, kvel,
	...
	if (!(dflags & DAMAGE_NO_PROTECTION)) {
		...
		if (targ != attacker && OnSameTeam(targ, attacker)) {
			if (g_dretchPunt.integer &&
				...
				VectorSubtract(targ->r.currentOrigin,
					       attacker->r.currentOrigin, dir);
				VectorNormalizeFast(dir);
				VectorScale(dir, (damage * 10.0f), push);
				push[2] = 64.0f;
				VectorAdd(targ->client->ps.velocity, push,
					  targ->client->ps.velocity);
				return;
			} else if (mod == MOD_LEVEL4_CHARGE || mod == MOD_LEVEL3_POUNCE) {	// don't do friendly fire on movement attacks
				if (g_friendlyFireMovementAttacks.value <= 0
				    || (g_friendlyFire.value <= 0
					&& g_friendlyFireAliens.value <= 0))
					return;
				else if (g_friendlyFireMovementAttacks.value > 0
				...
				if (targ->client->ps.stats[STAT_PTEAM] ==
				    PTE_HUMANS) {
					if (g_friendlyFireHumans.value <= 0)
						return;
					else if (g_friendlyFireHumans.value > 0
						 && g_friendlyFireHumans.value <
						 1)
						damage =
						    (int)(0.5 +
							  g_friendlyFireHumans.
							  value *
							  (float)damage);
				}
				if (targ->client->ps.stats[STAT_PTEAM] ==
					...
					if (g_friendlyFireAliens.value == 0)
						return;
					else if (g_friendlyFireAliens.value > 0
						 && g_friendlyFireAliens.value <
						 1)
						damage =
						    (int)(0.5 +
							  g_friendlyFireAliens.
							  value *
							  (float)damage);
				}
			} else if (g_friendlyFire.value > 0
				   && g_friendlyFire.value < 1) {
				damage =
				    (int)(0.5 +
					  g_friendlyFire.value * (float)damage);
			}
		}
		...
		if (targ->s.eType == ET_BUILDABLE && attacker->client &&
			...
			if (g_friendlyBuildableFire.value <= 0) {
				return;
			} else if (g_friendlyBuildableFire.value > 0
				   && g_friendlyBuildableFire.value < 1) {
				damage =
				    (int)(0.5 +
					  g_friendlyBuildableFire.value *
					  (float)damage);
			}
		}
		// check for godmode
	...
	if (client) {
		...
		damagemodifier =
		    G_CalcDamageModifier(point, targ, attacker,
		...
		if (attacker->client
			...
			if (targ->client->ps.stats[STAT_PTEAM] == PTE_HUMANS
				...
				targ->client->ps.stats[STAT_STATE] |=
				    SS_POISONED;
	...
	if (take) {
		...
		if (attacker && attacker->client) {
			if (targ->biteam == attacker->client->pers.teamSelection
				...
				attacker->client->pers.statscounters.
				    ffdmgdone += takeNoOverkill;
				if (attacker->client->pers.teamSelection ==
				    PTE_ALIENS) {
					level.alienStatsCounters.ffdmgdone +=
					    takeNoOverkill;
				} else if (attacker->client->pers.
					   teamSelection == PTE_HUMANS) {
					level.humanStatsCounters.ffdmgdone +=
					    takeNoOverkill;
				}
			} else if (targ->s.eType == ET_BUILDABLE) {
				attacker->client->pers.statscounters.
				    structdmgdone += takeNoOverkill;

				if (attacker->client->pers.teamSelection ==
				    PTE_ALIENS) {
					level.alienStatsCounters.
					    structdmgdone += takeNoOverkill;
				} else if (attacker->client->pers.
					   teamSelection == PTE_HUMANS) {
					level.humanStatsCounters.
					    structdmgdone += takeNoOverkill;
				}
				...
				if (targ->health > 0
					...
					attacker->client->pers.statscounters.
					    structskilled++;
					if (attacker->client->pers.
					    teamSelection == PTE_ALIENS) {
						level.alienStatsCounters.
						    structskilled++;
					} else if (attacker->client->pers.
						   teamSelection ==
				...
				attacker->client->pers.statscounters.dmgdone +=
				    takeNoOverkill;
				attacker->client->pers.statscounters.hits++;
				if (attacker->client->pers.teamSelection ==
				    PTE_ALIENS) {
					level.alienStatsCounters.dmgdone +=
					    takeNoOverkill;
				} else if (attacker->client->pers.
					   teamSelection == PTE_HUMANS) {
					level.humanStatsCounters.dmgdone +=
					    takeNoOverkill;
				}
			}
			...
			if (attacker->client) {
				...
				if ((aclient->ps.stats[STAT_PTEAM] == PTE_ALIENS
				     &&
				     !(G_BuildableRange
				       (aclient->ps.origin, 1100,
					BA_A_OVERMIND)))
				    || (aclient->ps.stats[STAT_PTEAM] ==
					PTE_HUMANS
					&&
					!(G_BuildableRange
					  (aclient->ps.origin, 1100,
		...
		if (targ->client && attacker->client) {
			if (attacker != targ && !OnSameTeam(targ, attacker))
				targ->credits[attacker->client->ps.clientNum] +=
				    take;
			else if (attacker != targ && OnSameTeam(targ, attacker))
				targ->client->tkcredits[attacker->client->ps.
							clientNum] +=
				    takeNoOverkill;
		}
...
============
*/
qboolean G_SelectiveRadiusDamage(vec3_t origin, gentity_t * attacker,
				 float damage, float radius, gentity_t * ignore,
				 int mod, int team)
{
	float points, dist;
	gentity_t *ent;
	...
	numListedEntities =
	    trap_EntitiesInBox(mins, maxs, entityList, MAX_GENTITIES);
	...
	for (e = 0; e < numListedEntities; e++) {
		...
		if (CanDamage(ent, origin)) {
			...
			G_SelectiveDamage(ent, NULL, attacker, dir, origin,
					  (int)points,
					  DAMAGE_RADIUS | DAMAGE_NO_LOCDAMAGE,
					  mod, team);
		}
	}
	...
	for (e = 0; e < numListedEntities; e++) {
		...
		if (CanDamage(ent, origin)) {
			...
			G_Damage(ent, NULL, attacker, dir, origin,
				 (int)points,
				 DAMAGE_RADIUS | DAMAGE_NO_LOCDAMAGE | dflags,
				 mod);
		}
	}
