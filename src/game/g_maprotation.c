...
static qboolean G_RotationExists(char *name)
{
	...
	for (i = 0; i < mapRotations.numRotations; i++) {
		if (Q_strncmp(mapRotations.rotations[i].name, name, MAX_QPATH)
		    == 0)
			return qtrue;
	}
...
===============
*/
static qboolean G_ParseMapCommandSection(mapRotationEntry_t * mre,
					 char **text_p)
{
	char *token;
	...
	while (1) {
		...
		while (token && token[0] != 0) {
			Q_strcat(mre->postCmds[mre->numCmds],
				 sizeof(mre->postCmds[0]), token);
			Q_strcat(mre->postCmds[mre->numCmds],
				 sizeof(mre->postCmds[0]), " ");
			token = COM_ParseExt(text_p, qfalse);
		}
	...
	while (1) {
		...
		if (!Q_stricmp(token, "{")) {
			...
			if (!Q_stricmp(token, "{")) {
					...
					if (!Q_stricmp(token, "}")) {
						...
						if (mre->numConditions <
							...
							G_Printf(S_COLOR_YELLOW
								 "WARNING: maximum number of maps for one vote (%d) reached\n",
								 MAX_MAP_ROTATION_CONDS);
						}
					}
				}
				if (!mre->numConditions) {
					G_Printf(S_COLOR_YELLOW
						 "WARNING: no maps in *VOTE* section\n");
				} else {
					mr->numMaps++;
					mnSet = qtrue;
				}
			...
			if (!Q_stricmp(token, "{")) {
				while (1) {
					...
					if (!Q_stricmp(token, "}")) {
						...
						if (mre->numConditions <
							...
							G_Printf(S_COLOR_YELLOW
								 "WARNING: maximum number of maps for one Random Slot (%d) reached\n",
								 MAX_MAP_ROTATION_CONDS);
						}
					}
				}
				if (!mre->numConditions) {
					G_Printf(S_COLOR_YELLOW
						 "WARNING: no maps in *RANDOM* section\n");
				} else {
					mr->numMaps++;
					mnSet = qtrue;
				}
	...
	while (1) {
		...
		if (!Q_stricmp(token, "{")) {
			if (mrNameSet) {
				...
				Q_strncpyz(mapRotations.
					   rotations[mapRotations.numRotations].
					   name, mrName, MAX_QPATH);
		...
		if (!mrNameSet) {
			...
			G_Printf(S_COLOR_RED
				 "ERROR: map rotation already named\n");
			return qfalse;
		}
	}
		...
		for (j = 0; j < mapRotations.rotations[i].numMaps; j++) {
			if (Q_stricmp
			    (mapRotations.rotations[i].maps[j].name,
			     "*VOTE*") != 0
			    && Q_stricmp(mapRotations.rotations[i].maps[j].name,
					 "*RANDOM*") != 0
			    && !G_MapExists(mapRotations.rotations[i].maps[j].
					    name)) {
				G_Printf(S_COLOR_RED
			...
			for (k = 0;
			     k <
			     mapRotations.rotations[i].maps[j].numConditions;
			     k++) {
				if (!G_MapExists
				    (mapRotations.rotations[i].maps[j].
				     conditions[k].dest)
				    && !G_RotationExists(mapRotations.
							 rotations[i].maps[j].
							 conditions[k].dest)) {
					G_Printf(S_COLOR_RED
...
void G_PrintRotations(void)
{
	...
	for (i = 0; i < mapRotations.numRotations; i++) {
		...
		for (j = 0; j < mapRotations.rotations[i].numMaps; j++) {
			...
			for (k = 0;
			     k < mapRotations.rotations[i].maps[j].numCmds;
			     k++) {
				G_Printf("    command: %s\n",
					 mapRotations.rotations[i].maps[j].
					 postCmds[k]);
...
static void G_IssueMapChange(int rotation)
{
	...
	Q_strncpyz(newmap, mapRotations.rotations[rotation].maps[map].name,
		   sizeof(newmap));
	...
	if (!Q_stricmp(newmap, "*VOTE*")) {
		...
		if (trap_FS_FOpenFile(va("maps/%s.bsp", newmap), &f, FS_READ) >
		    0) {
			trap_FS_FCloseFile(f);
		} else {
	...
	if (!g_layouts.string[0] &&
		...
		trap_Cvar_Set("g_layouts",
			      mapRotations.rotations[rotation].maps[map].
			      layouts);
	}
...
static mapConditionType_t G_ResolveConditionDestination(int *n, char *name)
{
	...
	for (i = 0;
	     i < mapRotations.rotations[g_currentMapRotation.integer].numMaps;
	     i++) {
		if (!Q_stricmp
		    (mapRotations.rotations[g_currentMapRotation.integer].
		     maps[i].name, name)) {
			*n = i;
			return MCT_MAP;
		}
...
static int G_GetMapVoteWinner(int *winvotes, int *totalvotes, int *resultorder)
{
	...
	qsort(results, MAX_MAP_ROTATION_CONDS, sizeof(results[0]),
	      SortMapVoteResults);
...
static void G_IntermissionMapVoteMessageReal(gentity_t * ent, int winner,
					     int winvotes, int totalvotes,
					     int *ranklist)
{
	int clientNum;
	...
	Q_strncpyz(string,
		   "^7Attack = down ^0/^7 Repair = up ^0/^7 F1 = vote\n\n"
		   "^2Map Vote Menu\n" "^7+------------------+\n",
		   sizeof(string));
...
void G_IntermissionMapVoteMessageAll(void)
{
	...
	for (i = 0; i < level.maxclients; i++) {
		if (level.clients[i].pers.connected == CON_CONNECTED)
			G_IntermissionMapVoteMessageReal(g_entities + i, winner,
							 winvotes, totalvotes,
							 ranklist);
	}
}
...
void G_IntermissionMapVoteCommand(gentity_t * ent, qboolean next,
				  qboolean choose)
{
	int clientNum;
	int n;
	...
	if (choose) {
		rotationVoteClientSelection[clientNum] =
		    rotationVoteClientPosition[clientNum];
	} else {
